...
TL;DR: 提出 TAPe+ML v3 紧凑视觉系统，基于 TAPe 结构化表示（编码感知元素间关系，先于识别阶段），用共享表示与模块化架构统一支持图像分类、目标检测与实例分割，参数量不足 10 万，在 COCO 检测/分割及 ImageNet-Real 分类等任务上取得较强性能。


<details>
  <summary>Details</summary>
Motivation: 传统计算机视觉直接对像素张量建模，模型容量与计算/内存开销大，且需大量数据。作者希望将部分建模负担从网络参数转移到结构化输入表示上，以降低数据、内存与算力需求，同时支持多任务。

Method: 构建 TAPe（Theory of Active Perception）共享结构化表示，在识别前编码感知元素间关系；系统不直接处理像素张量。TAPe+ML v3 采用模块化识别架构，包含背景与轮廓处理、局部目标定位、基于原型的分类，以及协调各专用子模型的协调器，统一用于分类、检测与分割。

Result: 参数量少于 100,000。COCO 目标检测：84.7 mAP50、65.3 mAP50-95；COCO 实例分割：80.7 mask mAP50、58.4 mask mAP50-95。分类：相同训练条件下 Imagenette 验证准确率 92%（对比原始像素基线），ImageNet-Real Top-1 89.9%。另在视频场景检测紧凑性与工业试点分布偏移适应中进行了评估。

Conclusion: 将部分建模负担由网络参数转移至结构化输入表示，可支撑紧凑的多任务视觉系统，并降低数据、内存与计算需求。
...
TL;DR: 提出MemeTAG双目标框架，通过关键词感知的语义引导机制进行有害网络模因分类，在三数据集上取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 有害网络模因泛滥构成社会威胁，但其内容具有细微的多模态特性，自动化分类难度大。

Method: 两部分语义引导机制：①用预训练视觉-语言模型生成描述性关键词以捕获高层语义；②提出基于注意力的聚合标签推理网络（ATIN），将关键词蒸馏为单一语义嵌入，并作为新型辅助重构损失的目标，促使视觉与文本特征深度对齐；配合高效三阶段训练策略。

Result: 在HarMeme、Hateful Memes Challenge（HMC）和PrideMM数据集上均达到新的最优性能，显著超越现有SOTA方法。

Conclusion: 关键词感知语义引导与辅助重构损失的结合有效提升了多模态有害模因分类性能。
...
TL;DR: 该研究开发了一种基于对比面板图像特征与机器学习的方法，用于估算商业牛饲养场中PM10浓度，XGBoost模型取得R²=0.792、中位绝对误差103 μg/m⁻³的预测性能，验证了图像法在远超城市研究的PM浓度范围内的可行性。


<details>
  <summary>Details</summary>
Motivation: 牛饲养场行业对低成本粉尘监测存在迫切需求；尽管基于摄像头的PM估算在城市空气质量领域研究日益增多，但尚未在集约化畜牧作业特有的高浓度范围（小时均值250–1,000 μg/m⁻³、瞬时5,000–20,000 μg/m⁻³）下得到评估。

Method: 在商业牛饲养场傍晚粉尘峰值时段采集灰度图像，提取对比面板对比度、黑白面板像素值、整体图像亮度等特征，并引入前序图像的历史值与太阳天顶角作为预测因子，采用多种候选模型（含XGBoost）进行PM10估算并开展特征重要性分析。

Result: XGBoost预测性能最佳，R²=0.792，中位绝对误差103 μg/m⁻³；特征重要性显示距相机最远的面板贡献最大，且黑色面板像素值比白色面板对PM10变化更敏感；日落过渡期（与傍晚粉尘峰值起始重合）的预测精度仍有待改进。

Conclusion: 研究证明了图像法PM10估算在显著超过以往城市研究的浓度范围内的可行性，并为饲养场环境的未来部署提供了实用指南，同时指出日落过渡期精度提升为后续研究方向。
...
TL;DR: 该论文研究壁画碎片（fresco fragment）的艺术风格分类，提出渐进式 Transformer 框架：前景引导掩码、基于修复的几何正则化、基于 KL 相似度的分布级监督对比目标，并用简单可学习 logit 集成融合各分支。在 CLEOPATRA 与 POMPAAF 上超过 ViT 基线，并指出复杂图融合变体不值得其额外复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有艺术风格分类通常针对完整艺术品，模型可利用全局构图、空间组织与图像志结构；但在考古场景中艺术品常仅以碎片形式存留，识别只能依赖不完整、不规则且缺乏上下文的视觉证据。因此需要针对碎片化证据的建模方法。

Method: 以 ViT-B/16 为基线，构建渐进式分支：(1) 前景引导掩码，抑制纯背景 token；(2) 基于修复（inpainting）的几何正则化，使不规则碎片支撑区域对齐 ViT patch 网格；(3) 在预测分布上通过 KL 相似度实现的监督对比目标。各分支用刻意简单的可学习 logit 集成融合；另评估更复杂的图融合变体。

Result: CLEOPATRA 上准确率从 0.604 提升至 0.656，macro-F1 从 0.596 提升至 0.648；POMPAAF 上在 6 种碎片设置中的 4 种优于最佳单分支。图融合变体在 POMPAAF 上与简单集成持平，在 CLEOPATRA 上仅有小幅且依赖数据集的增益。

Conclusion: 碎片感知建模优于标准 ViT 基线，简单 logit 集成有效。主要贡献有二：一是分布级对比目标能稳定提升单分支识别性能；二是可解释性分析验证模型确实利用真实绘画证据，同时量化出修复分支的部分归因来自合成周边区域。
...
TL;DR: 提出免训练的后处理压缩器 MAGIC，通过最优传输对齐补丁使用分布，缓解多向量文档检索在激进压缩下的性能退化。


<details>
  <summary>Details</summary>
Motivation: ColPali 等多向量视觉文档检索依赖 patch 级嵌入实现细粒度匹配，但带来巨大的索引存储和 MaxSim 打分开销。后处理合并可在不重训练检索器的前提下降低成本，但其均匀重建目标与 late-interaction 检索导致的稀疏、非均匀 patch 使用不匹配，激进压缩下会保留少用 patch 而将检索活动集中于过少的保留代表。

Method: MAGIC：面向冻结多向量嵌入的免训练后处理压缩器。推导 MaxSim 诱导的压缩代理目标，并用双边缘熵最优传输优化：检索需求源边缘优先高使用率 patch，平衡目标边缘正则化保留 facet 的使用。

Result: 在 ViDoRe 基准、不同保留率和检索骨干上，MAGIC 持续优于强后处理压缩器，激进压缩场景增益尤为显著；组件消融验证了两个边缘的互补作用。

Conclusion: MAGIC 以最优传输对齐检索需求，实现高效视觉文档检索的无训练压缩，代码已开源。
...
TL;DR: 提出MarsFM：一种图像条件潜空间流匹配模型，用单波段HiRISE RED正射影像估计火星局部地形起伏，结合生成先验、立体派生几何监督与可微Lunar-Lambert着色目标。


<details>
  <summary>Details</summary>
Motivation: 火星高分辨率地形（relief）恢复缺乏物理参考，需将学习到地形先验与影像约束结合；同时探索少步推理的可行性。

Method: 图像条件潜空间流匹配，融合预训练生成先验、立体派生几何监督、可微Lunar-Lambert着色目标；约束项包括relief、normal、gradient、curvature、ordinal及正仿射不变的影像比较。

Result: 固定训练步数下评估2024个patch记录；1至20步Euler采样在归一化signed-log relief空间获得0.0935–0.0957的仿射对齐RMSE（对VAE重建参考、正参考支撑），区间窄支持低步推理。空间/微分/谱诊断显示整体地形吻合，但存在平滑、振幅压缩与边界失配。

Conclusion: MarsFM为“学习地形先验+影像约束”提供框架；提升物理地形分辨率仍需匹配基线与独立高分辨率参考数据。
...
TL;DR: 提出基于视频扩散模型的迭代细化框架，通过稠密点云初始化和视频修复伪监督，显著提升稀疏视角动态4D场景重建质量。


<details>
  <summary>Details</summary>
Motivation: 稀疏视角视频下的动态场景合成存在观测不足与场景信息缺失导致的病态问题；同时稀疏视角4DGS的几何初始化质量差，COLMAP仅能重建稀疏不完整的点云，大量区域缺乏足够高斯支持，后续优化难以恢复。现有几何先验、自适应优化或密度控制策略无法从根本上解决该问题。

Method: 提出基于视频扩散模型的迭代细化框架：1）先估计多视角深度图并融合为稠密点云，为动态4DGS表示提供更完整的几何初始化；2）利用预训练视频修复模型，对新视角轨迹在不同时间步渲染的序列进行细化；3）将修复后的序列作为伪监督，对4DGS表示进行正则化与迭代细化。

Result: 在广泛使用的基准数据集上，方法大幅超越现有基线，相较此前最佳方法取得近2 dB的PSNR提升。

Conclusion: 该工作表明，结合稠密几何初始化与视频扩散/修复模型的伪监督迭代细化，可有效缓解稀疏视角4DGS的病态性与初始化缺陷，提升动态场景的完整性与一致性。
...
TL;DR: 本文提出 OnomatoBridge，一种面向漫画视觉拟声词翻译的过滤流水线，实现将日文拟声词干净替换为英文拟声词并保持视觉风格，在 Manga109 数据集上相较基线图像编辑模型提升英文文本正确性约 10–25 个点，并相对减少约 20–50% 的日文残留。


<details>
  <summary>Details</summary>
Motivation: 漫画中的拟声词以独特视觉风格传达声音、动作与情感，其翻译需在替换为目标语言拟声词的同时保留原有视觉风格。现有方法在去除日文拟声词与渲染风格化英文拟声词时，常产生残留伪影或风格不一致问题。

Method: 提出 OnomatoBridge，一种用于视觉拟声词翻译的过滤流水线（filtering pipeline），针对日文到英文的翻译任务，在 Manga109 拟声词数据集上与基线图像编辑模型进行对比评估。

Result: 实验表明，经 OnomatoBridge 过滤后的输出优于传统方法：英文文本正确性提升约 10–25 个点，日文残留文本相对减少约 20–50%。

Conclusion: OnomatoBridge 能有效缓解拟声词替换中的残留伪影与风格不一致问题，在漫画视觉拟声词翻译任务上取得优于基线的表现。
...
TL;DR: HACT：基于预测单手事件的过渡模型，用角色保持历史与带标记时序点过程计算语义/时间意外度，经监督证据头与双状态滤波输出单手异常后验，在双手电动工具流程上取得最优AUPRC/F1与最少恢复误报，并可跨装配顺序零重训迁移。


<details>
  <summary>Details</summary>
Motivation: 双手装配中的程序性异常检测需将每只手的动作与已执行过程对照判断：纠正性动作单独看似异常，视觉合理动作却可能违反工序顺序，因此不能仅依赖单帧外观或孤立动作。

Method: 提出HACT过渡模型，建模预测的单手事件序列；角色保持历史保留双手并发职责；带标记时序点过程为每次观测过渡分配语义意外度与时间意外度；监督证据头与双状态滤波器将其转换为单手异常后验；评估采用基于预测事件的恢复感知协议与参与者不相交折，并在验证参与者上选点。

Result: 在两个双手电动工具流程上，HACT的AUPRC与F1均高于对比方法，恢复报警最少；不重训直接应用于同一产品的不同装配顺序时，仍保持最高AUPRC与F1。

Conclusion: 结果表明基于预测事件过渡与双重意外度建模能有效判别程序性异常并抑制恢复动作误报，且具备跨装配顺序泛化能力；代码已开源。
...
TL;DR: 提出MegaEvent事件相机视觉位置识别系统，通过I2E将大规模帧数据集转为合成事件流微调预训练事件ViT，实现视角鲁棒定位，并发布Springfield-Event-VPR新数据集。


<details>
  <summary>Details</summary>
Motivation: 机器人定位需对视角变化鲁棒，事件相机受关注但现有事件定位器对视角变化研究不足，且缺乏强调视角变化的事件数据集。

Method: 将5个大规模地理标注帧数据集经Image-to-Event(I2E)转为合成事件流，用多损失函数微调预训练的事件视觉Transformer骨干，得到MegaEvent，学习视角鲁棒的位置识别特征。

Result: 在3个现有事件定位数据集上平均Recall@1达82%，领先次优事件方法20个召回点，领先直接用于事件帧的帧式VPR模型8–26个召回点；新数据集Springfield-Event-VPR（3.7km步行路线、三种相机朝向、共11.1km）上领先最强基线9个召回点。

Conclusion: MegaEvent在视角变化下实现鲁棒的事件视觉位置识别，显著超越事件与帧式基线，并提供了具有视角变化挑战的新数据集，代码已开源。
...
TL;DR: VGGT-CAD：将预训练3D几何先验迁移至参数化CAD重建，支持单/多视角输入，并构建VideoCAD多视角视频基准。


<details>
  <summary>Details</summary>
Motivation: 从视觉观测恢复参数化CAD需同时获得精确几何与可编辑建模操作，在视角有限且模糊的情况下极具挑战；现有方法主要依赖2D外观线索，缺乏强多视角几何先验。

Method: 将预训练3D几何先验迁移至CAD重建：把相机参数编码为条件token并与图像token联合建模；提出可变视角跨视角上下文聚合模块以自适应融合多视角特征；设计免训练、几何感知的视角选择策略在推理时挑选互补可靠帧；用非自回归解码器将表示解码为CAD命令序列；并通过多视角重渲染构建大规模多视角视频基准VideoCAD。

Result: 大量实验证明VGGT-CAD在不同观测配置下的视觉CAD重建中均有效。

Conclusion: 几何感知框架结合多视角几何先验与灵活视角融合，可提升单/多视角参数化CAD重建的鲁棒性与有效性。
...
TL;DR: 本文提出Context-Aware CCPDA，通过语义约束与源-目标上下文匹配改进野火图像分割的数据增强，在分割性能与上下文真实性上优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 野火图像分割的精确标注成本高、劳动力密集，且动态火场数据难以收集标注，导致标注数据集稀缺。作者前作CCPDA通过随机粘贴火簇生成样本，但随机放置会产生不符合语境的场景（如火烧在沥青上）。

Method: 提出上下文感知策略：将火灾粘贴限制在语义有效的目标区域，并选择Ash-Vegetation组成与源上下文最匹配的位置；同时保留目标图像中已有的火区域，避免不真实放置。通过数值分析及与其他增强方法的对比，采用加权和的多目标优化（MOO）进行评估。

Result: 上下文感知的数据增强策略带来更好的分割性能和上下文真实性，优于其他增强方法。

Conclusion: 上下文感知的CCPDA能提升小样本多类野火数据集的质量与真实感，是一种有效的增强方案。
...
TL;DR: 提出免除训练框架 SafeStyle，通过校准风格残差注入解耦风格与参考内容，实现风格保真与内容抑制的平衡。


<details>
  <summary>Details</summary>
Motivation: 参考图条件化易将可迁移风格线索与参考特有内容纠缠，导致风格保真与内容泄漏的固有权衡；且纹理主导与几何主导风格的空间组织差异进一步加剧该问题。

Method: 从紧凑校准集估计风格支持子空间与内容关联子空间，保留有用重叠并抑制无用内容变化；随后以自适应空间粒度迁移纯化后的风格证据，并通过显式残差范数预算约束其有效影响；全程在冻结扩散模型上进行、无需训练。

Result: 在纹理与几何主导风格上取得 0.432 的 DINO 风格相似度并保持竞争力的文本对齐；在语义不相交的泄漏压力基准上取得 0.474 的 DINO 风格相似度，语义泄漏仅 0.8%。

Conclusion: SafeStyle 有效平衡风格保真与参考内容抑制，为参考引导的扩散风格化提供免训练且可控的解决方案。
...
TL;DR: 论文提出曲率自适应管状修正（CAT），一种免训练插件，通过噪声相关几何预算同时调控指导梯度的法向一阶位移与切向二阶（曲率诱导）位移，在七个逆问题及黑洞重建上稳定扩散指导采样并提升感知指标。


<details>
  <summary>Details</summary>
Motivation: 梯度引导扩散采样器为逆问题和条件生成提供灵活先验，但强引导会把采样轨迹推入学习分数支撑不足的区域。现有切空间投影策略只限制相对等密度面的一阶偏离，丢弃了可能有用的法向运动，也忽视了弯曲表面上切向运动带来的二阶偏离。

Method: CAT将引导梯度分解为法向与切向分量，法向位移按一阶计费、切向位移按方向曲率计费，并在共享的噪声相关几何预算内通过一维对偶方程求解二者的联合最优幅度；采用Armijo回溯校准有限步长以匹配真实引导目标，并用无矩阵方向导数避免构造完整分数雅可比。

Result: 在FFHQ和ImageNet的七个逆问题上，CAT改进了像素空间与隐空间宿主采样器，感知指标增益尤其稳定；在InverseBench上改善黑洞重建；在所有测试的无分类器引导尺度下取得对比方法中最低FID，并保持稳定的饱和度与对比度。

Conclusion: 理论上建立了管状近似的局部保证、修正的唯一性以及目标函数的充分下降；实验表明曲率感知的管状控制是一种可复用的扩散指导稳定机制。
...
TL;DR: Edit-VAR是首个基于预训练视觉自回归视频模型、免训练且免反演的文本引导视频编辑框架，通过概率引导的条件token替换、注意力引导的token级与尺度感知调制，以及尺度解耦生成和残差引导的token剪枝，在编辑保真度、源保持、时序一致性和推理效率上整体优于现有免训练视频编辑方法。


<details>
  <summary>Details</summary>
Motivation: 文本引导视频编辑需在修改目标内容的同时保持未编辑区域的外观与时序一致性。基于训练的方法控制力强但依赖大量数据和算力；免训练方法分为免反演与基于反演两类：免反演方法虽避免轨迹恢复，但其源保持引导会限制编辑强度、导致语义修改不完整；基于反演的方法需先恢复隐轨迹再生成，近似误差会累积，引发源内容漂移与时序不一致。因此需要一种既免训练、免反演，又能兼顾编辑强度与源保持的视频编辑方案。

Method: Edit-VAR基于预训练视觉自回归视频模型，直接将源视频编码为多尺度离散token，并执行概率引导的条件token替换以实现源保持。通过注意力引导的token级与尺度感知调制，在编辑相关位置与生成阶段选择性放松源约束。采用尺度解耦生成（以“后期尺度约束释放”实现），重生成运动一致的细节并减少纹理破碎。此外，利用残差引导的token剪枝，在最后两个高分辨率尺度上挖掘冗余以降低推理开销。

Result: 大量实验与盲用户研究表明，Edit-VAR在编辑保真度、源保持、时序一致性以及推理效率四个方面整体优于现有免训练视频编辑方法。

Conclusion: Edit-VAR首次将免训练、免反演范式引入基于自回归视频模型的文本引导视频编辑，验证了通过离散token的概率引导替换与多尺度约束调制，可在不训练、不反演的前提下同时提升编辑质量与效率，为视频编辑提供了一条兼顾性能与成本的新路径。
...
TL;DR: 本文提出面向跨域PCBA视觉问答的多模态推理框架，通过统一指令格式构建、任务感知GRPO奖励设计与鲁棒推理策略，在官方PCBA Standard-to-Real挑战赛榜单上取得83.24的总体得分。


<details>
  <summary>Details</summary>
Motivation: PCBA自动检测中，基于标准的决策需要系统联合推理细粒度视觉线索、元件语义与制造知识。尽管大型视觉语言模型（VLM）提供了有前景的基础，但其部署受到两方面阻碍：一是标准衍生样本与真实产线图像之间的域偏移，二是涵盖选择型与数值计数型任务的异构输出空间。因此需要一套能够跨域、兼容异构任务的方案。

Method: 提出跨域PCBA视觉问答的多模态推理框架。数据层面，将标准衍生数据、真实世界数据与辅助PCB领域数据转换为统一指令格式，并构建与视觉证据、问题语义、候选选项及标准答案对齐的经校验推理轨迹。训练层面，提出任务感知的组相对策略优化（GRPO），不再依赖精确匹配监督，而是整合面向选择题的多组分语义奖励、面向计数题的距距离感知奖励，以及用于保证输出有效的辅助格式奖励。推理层面，结合答案—选项语义一致性校正、自一致性投票与多模型仲裁以提升预测鲁棒性。

Result: 在官方PCBA Standard-to-Real Grand Challenge榜单上取得83.24的总体得分（Overall Score），验证了任务感知奖励设计与鲁棒推理对跨域PCBA视觉问答的有效性。

Conclusion: 任务感知奖励设计与鲁棒推理策略能够有效缓解域偏移与异构输出空间带来的困难，为跨域PCBA视觉问答提供了可行且有效的解决路径。
...
TL;DR: 提出一种几何感知的后检测框架，利用RGB无人机影像中植株部件的质心与径向交点（RIP）几何特征，通过K-means与高斯混合模型判断冠层区域为单株或两株重叠，实现重叠植株实例的分离。


<details>
  <summary>Details</summary>
Motivation: 密集作物冠层中相邻植株常发生重叠，被检测器识别为单一对象，导致植株级测量可靠性下降；而现有方法通常依赖深度传感器、像素级分割、三维重建或重新训练检测器，成本高且不易集成。

Method: 将目标检测与植株部件的几何聚类相结合：对每个植株级区域内的叶片或枝条，提取两类互补几何特征——部件质心与由检测到的植株结构导出的径向交点（RIP）；使用K-means和高斯混合模型判别该区域包含单株还是两株重叠植株；引入密度过滤抑制虚假径向交点，并通过后处理集成策略融合空间与方向几何信息。

Result: 在田间茄子与番茄的无人机影像上评估：基于质心的聚类在茄子上取得F1=0.89；质心-RIP组合方法在番茄上表现最佳，采用K-means时准确率0.80、精确率1.00、F1=0.75；密度过滤显著提升了番茄的RIP聚类效果。

Conclusion: 该框架是一种轻量、模块化的工程方案，无需额外深度传感器、像素级分割、三维重建或重新训练主检测器即可接入现有RGB无人机监测流程，表明几何推理可补充深度学习检测，改善密集农业冠层中的植株级解译。
...
TL;DR: 提出S3VD视频去雨框架，通过多尺度语义融合与时空扫描融合两个模块，缓解Mamba在雨天视频中语义丢失与时空建模不足的问题，取得0.84 dB平均PSNR提升。


<details>
  <summary>Details</summary>
Motivation: 暴雨会破坏视频高频细节并引入运动模糊，严重影响下游视觉任务。Mamba等状态空间模型虽具线性复杂度与长程依赖建模优势，但其1D展平操作破坏2D空间语义，且在退化视频中难以有效建模3D时空关联。

Method: S3VD包含两个模块：(1) 多尺度语义融合模块（MSSF），引入DINOv2的时序语义先验指导特征表示，弥补Mamba一维展平导致的局部语义上下文丢失；(2) 时空扫描融合模块（STSF），设计时空扫描机制与解耦门控Mamba层（DG-Mamba），用两个独立门控分别自适应控制输入片段中前序与后续上下文信息，优化帧内与帧间关联建模。

Result: 在视频去雨基准上达到SOTA性能，相较Mamba类基线平均PSNR提升0.84 dB。

Conclusion: S3VD通过语义引导的时空扫描框架有效克服Mamba在退化雨天视频中的局限，验证了多尺度语义先验与解耦门控时空建模对视频去雨的有效性。
...
TL;DR: 本文提出 VeriFuse，一个面向车路协同 3D 检测的受限仲裁框架：将 VLM 的语义判断限定在候选池内部的选择/精修/拒绝三动作中，最终 3D 几何由确定性约束决定。


<details>
  <summary>Details</summary>
Motivation: VLM 在场景理解与语义判断上能力强，但在协同感知中的恰当角色尚不清晰。直接让 VLM 回归 3D 检测不可靠且计算昂贵；而仅用其选择单一来源的输出又会丢弃其他智能体的有用信息。因此需要一个既利用 VLM 语义能力、又不让其承担几何回归的框架。

Method: 各智能体（车辆与路侧）先独立产生检测。围绕每个车辆与路侧 proposal，生成源条件式的几何候选，将原始检测、其扰动以及跨源假设统一汇入候选池。冻结的 VLM 仅在三种可行动作中选择：SELECT（选取足够好的候选）、REFINE（目标有支撑但所有候选几何上不足时精修已有 anchor）、REJECT（拒绝无支撑的仅源自路侧的 proposal）。

Result: 在 DAIR-V2X 数据集上，VeriFuse 达到 0.494/0.357 的协同 3D AP50/AP70，并在 300 ms 延迟下将车辆侧 BEV AP50 的相对下降限制在 1.7%。

Conclusion: VeriFuse 为 VLM 在协同感知中赋予清晰且受限的角色：由语义推理解决跨智能体假设间的歧义，而最终 3D 几何由确定性约束确定，兼顾了性能与鲁棒性。
...
TL;DR: Cube-Splat是首个全景GS-SLAM框架，将360°帧分解为四虚拟针孔视图的立方体贴图，通过伴随映射实现多面观测协同更新单一位姿状态，并提出SynPano数据集，在两公开基准与新数据集上取得SOTA跟踪精度与重建保真度。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS SLAM多面向针孔相机，缺乏针对全景影像的管线设计；同时全景SLAM缺乏多样且具挑战性条件的严格评测基准。

Method: 将每帧360°影像因子化为四个固定朝向、共享单一光心的虚拟针孔视图立方体贴图；以前面为主位姿状态，用伴随映射聚合所有面的梯度，使多面观测协同更新单一状态并保持跨视图几何一致性；建图模块利用聚合的立方体射线加密并优化各向异性高斯以稠密重建；同时提出SynPano合成数据集，含参数化复杂轨迹与多模态真值。

Result: 在PALVIO、OmniBlender两公开基准及SynPano（覆盖室内外场景）上，跟踪精度与重建保真度均达SOTA。

Conclusion: Cube-Splat为全景GS-SLAM提供了统一几何一致的多面协同框架，并配套可扩展评测数据集，代码与数据已开源。
...
TL;DR: 提出多VLM框架PrismAlign，通过整合多样化视觉视角并对齐表格逻辑先验来缓解表格抽取中的结构错误与语义幻觉，在多个基准上取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 表格抽取任务普遍存在结构性错误与语义幻觉问题，现有方法难以消解歧义、缺乏对输出合理性的评估机制。

Method: PrismAlign：一个多VLM框架，对齐多样化的视觉视角以消解歧义；整合表格逻辑先验以评估输出合理性，将结构对齐与单元格内容对齐解耦；采用贝叶斯决策策略，利用抽取错误与可计算规则违规之间的相关性来最大化对齐准确率。

Result: 在开源与自研VLM上进行评估，PrismAlign降低了幻觉，并在OmniDocBench 1.5以及CC-OCR、PureDocBench的表格类别上达到最优性能。

Conclusion: 基于规则违规与错误的统计关联进行贝叶斯决策对齐，可有效提升多VLM表格抽取的准确性与可靠性。
...
TL;DR: 提出将异步事件流转换为常规视频格式，结合立体配置，实现对野外小型快速移动生物的三维连续低延迟跟踪。


<details>
  <summary>Details</summary>
Motivation: 传统帧式成像在追踪小型、快速移动生物时受低时间分辨率导致运动模糊及存储受限的制约，亟需更具适应性的成像方案。

Method: 利用事件相机在像素级捕捉亮度变化，将异步事件转换为常规视频格式，从而复用成熟的图像分析流程；并将该转换与立体（双目）配置耦合。

Result: 在野外条件下实现对快速移动目标的连续、低延迟三维跟踪，显著抑制运动伪影，更准确地表征动物运动。

Conclusion: 该方法使事件成像更易应用于自然野外场景，可支撑动物行为与生态研究、农业管理及其他需高保真野外目标跟踪的领域。
...
<details>
  <summary>Details</summary>
Motivation: MLRM能通过建筑风格、植被、光照等细微视觉线索对随意分享的照片进行结构化推理，精确推断用户地理位置，构成严重且尚未充分探讨的隐私威胁；现有基于拒绝的防护可被精心设计的越狱提示绕过（响应率达100%），而像素空间优化的扰动防御存在黑盒迁移性差和视觉伪影等结构性缺陷。

Method: 提出基于扩散模型的主动防御框架：在反向采样过程中将扰动注入扩散模型的潜在空间，直接作用于高层语义表示；并以与GPS坐标显式对齐的GeoCLIP作为代理模型来定位和破坏MLRM所利用的地理信号。

Result: 越狱提示可使拒绝式防护的响应率升至100%；所提方法在保持感知图像质量的同时，显著提升了黑盒迁移性，克服了效果—效用瓶颈。

Conclusion: 基于潜在空间语义破坏的扩散防御框架可有效抵御MLRM驱动的地理定位隐私泄露，且质量损失小，便于在社交媒体平台上无缝集成。
...
TL;DR: 本文提出 RobotEQ-Video，将社交主动智能（SPI）研究从静态图像推进到动态视频，并基于层级化世界状态分类体系构建了大规模视频基准。


<details>
  <summary>Details</summary>
Motivation: 现有 SPI 研究存在两大局限：一是仅关注静态图像，无法利用视频中推断人类状态与需求的关键动态线索；二是依赖自由形式的数据采集流程，难以保证场景覆盖的全面性。

Method: 构建层级化世界状态分类体系，采用四级由粗到细结构，包含 6 个领域、20 个维度、142 个一级属性和 816 个二级属性；据此采集视频并标注，形成 2K+ 视频、100K+ 人工标注及 16K+ 行为恰当性标签的基准，并探索世界模型在该任务中的作用。

Result: 基准评测显示，现有系统表现不可靠，且低于人类水平。

Conclusion: 该工作将 SPI 研究从静态图像拓展至动态视频，并在基准测试中实现了更全面的场景覆盖。
...
TL;DR: 提出解耦式交通预测框架，冻结V-JEPA与Cosmos，仅训练轻量对齐模块，在AI City Challenge 2026 Track 5获75.1297分、排名第三。


<details>
  <summary>Details</summary>
Motivation: 准确交通预测需兼顾场景动态理解与真实未来观测合成。现有扩散视频生成方法虽能生成视觉合理的预测，但端到端训练代价高昂，且将场景理解与图像合成纠缠在一起，效率与可扩展性受限。

Method: 将未来表征学习与视频生成解耦：先用冻结的V-JEPA编码器从观测交通视频中提取预测性潜在表征，在语义潜空间中捕捉场景动态；再用轻量潜空间对齐模块将这些表征投影到冻结的Cosmos扩散模块的条件空间，实现未来视频合成而无需重训大生成模型。整个框架冻结全部基础模型，只训练轻量对齐模块。

Result: 在AI City Challenge 2026 Track 5基准上取得75.1297分，竞赛排名第三。表明V-JEPA学到的预测性世界表征能有效引导下游视频生成。

Conclusion: 该框架大幅降低优化复杂度，同时保持预测能力，为端到端扩散预测提供了一种实用且高效的替代方案。
...
TL;DR: 本文提出 AgentVidBench，一个面向多跳视频问答的基准，用于评估 MLLM 智能体在空间、时间与因果推理方面的能力，并提供逐步解题轨迹以支持过程评估。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准多局限于简单场景级查询或全局摘要，仅需单步推理；真实世界视频理解需要多跳多模态推理，但缺乏能严格评估此类智能体能力的视频基准。

Method: 构建 AgentVidBench 多跳视频问答基准，除标准问答对外还提供逐步解题轨迹以进行“轨迹评估”，判断智能体是否显式获取了支撑答案所需的证据；对 12 个专有与开源 MLLM 进行实验，并将其接入先进智能体工作流；同时提出一种简单有效的智能体策略作为基线。

Result: 单轮（single-turn）性能在 AgentVidBench 上仍有限；将模型集成到先进智能体工作流后，准确率与轨迹分数通常均有提升。

Conclusion: AgentVidBench 可作为智能体视频理解研究的整体性测试平台，所提简单策略构成有竞争力的基线，推动未来研究。
...
TL;DR: 提出 SceneLM，用单一视觉语言模型直接以结构化文本列表维护开放词汇 3D 场景地图，作为唯一持久记忆，无需传统感知/几何模块流水线。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇 3D 场景建图依赖人工设计的建图流水线进行观测关联、跨视角信息融合与一致性维护，并常存储嵌入或图像裁剪等特征表示，导致持久内存庞大且系统复杂。

Method: 将整个场景表示为结构化文本对象列表，作为模型唯一持久记忆；对每张输入图像，模型读取当前场景状态并通过添加、编辑、删除对象来更新地图；设计迭代式场景地图维护的监督任务，并提出无需人工标注、直接从图像生成训练数据的自动标注流水线。

Result: 在语言引导检索基准和定位基准上，SceneLM 生成的场景地图性能与由专用感知和几何模块构建的完整建图系统相当，而场景表示紧凑 6-12 倍；四足机器人实验表明模型可在边缘设备上在线运行。

Conclusion: 持续性的开放词汇 3D 场景地图可由单一视觉语言模型仅使用轻量文本表示直接维护。
...
TL;DR: 本文提出推理感知的手术器械分割任务RA-SIS，并构建SurgRS数据集与SIRA多模态框架，实现查询条件下的像素级语义定位。


<details>
  <summary>Details</summary>
Motivation: 现有手术器械分割方法多将分割视为类别驱动的定位问题，难以捕捉手术流程中的过程上下文与任务相关语义，缺乏推理能力。

Method: 提出RA-SIS任务形式，将分割建模为手术上下文下的查询条件推理；构建含41,000图像-文本对的SurgRS数据集，对齐实例级掩码与结构化问答监督；提出SIRA多模态框架，解耦目标级与查询级语义，通过查询锚定双对齐融合视觉特征，对齐查询语义与空间特征及分割提示。

Result: 在SurgRS数据集上的大量实验表明，SIRA优于现有推理感知基线方法。

Conclusion: 将手术器械分割重构为查询条件推理任务，配合专用数据集与多模态对齐框架，可有效增强语义-视觉一致性，推动推理感知分割研究。
...
TL;DR: 本文将量化去噪器的采样过程建模为在线估计问题，提出量化感知卡尔曼估计器QuAKE，利用量化去噪器输出历史以闭式递归方式恢复全精度输出，从而在多步ODE采样中补偿量化误差，在W4A4文本到图像扩散模型上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 量化可降低扩散模型的显存与计算开销，但激进压缩（如W4A4）会使量化输出严重偏离全精度结果。现有采样阶段校正方法主要依赖局部信息，未充分利用轨迹历史，难以纠正跨时间步传播的误差。

Method: 将量化去噪器采样形式化为在线估计问题：以量化去噪器输出历史为观测，恢复采样器所需的全精度输出。提出QuAKE（Quantization-Aware Kalman Estimator），结合平滑轨迹先验与条件高斯观测模型；每个采样步对输出窗口的后验进行闭式递归更新，并将其后验均值输入采样器。方法是轻量级即插即用校正器，无需修改量化网络，且天然支持任意高阶多步ODE采样器。

Result: 在W4A4量化的文本到图像扩散模型上，QuAKE在降低与全精度采样的分布差异方面持续优于现有方法。

Conclusion: QuAKE通过卡尔曼式递归利用采样轨迹历史，有效补偿量化去噪器的跨步误差传播，是一种通用、轻量且与高阶ODE采样器兼容的采样阶段校正方案。
...
TL;DR: 论文提出参数冻结的推理增强（PIE）替代持续测试时适应（CTTA）：用源训练分割器对解剖保持的尺度与翻转视图做预测、反映射到原位置并做概率平均，不更新权重与归一化统计。在M&Ms心脏MRI跨厂商序列流上，PIE平均Dice为0.7786，优于仅源推理（0.7680）与五种在线适应基线（0.7388–0.7416）。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割器在站点、扫描仪厂商或协议变化时性能下降。现有持续测试时适应（CTTA）无需目标标签，但在非平稳数据流上难以更新模型，且易引入大量错误，因此需要更合理、更有意义的替代方案。

Method: 提出参数冻结推理增强（PIE）：使用源域训练的分割器，构造解剖保持的尺度与翻转视图，将各视图预测映射回原始位置，并对其概率进行平均。全过程不修改模型权重，也不修改归一化统计量。并通过受控消融实验分析视图数量与多种后处理/加权策略的影响。

Result: 在M&Ms心脏MRI流（厂商A训练，依次在B、C、D评估）上：PIE平均Dice 0.7786，仅源推理为0.7680，五种在线适应基线为0.7388–0.7416。消融显示性能在28个视图时饱和；置信度加权、类别先验校正、连通域过滤、形态学细化与层间平滑无效果甚至导致负迁移。心脏MRI与眼底图像的定性结果一致，表明冻结集成能更好保留细薄与嵌套解剖结构。

Conclusion: 结果为医学CTTA提供了强而稳定的基线，并揭示一个重要失效模式：自适应与手工细化可能不如精心设计的推理可靠。
...
TL;DR: 本文提出P³-SAM框架，通过感知优化编码与并行提示生成两大策略赋能SAM，解决带钢表面缺陷小样本分割中的低对比度、光照不均与细粒度纹理难题，在三个基准上取得SOTA，Surface Defects-4i数据集mIoU提升12.00%。


<details>
  <summary>Details</summary>
Motivation: 带钢表面缺陷小样本分割（S³D）不同于自然场景：存在局部对比度低、光照不均、复杂细粒度纹理等工业特性。基于SAM的方法在自然图像FSS上表现良好，但直接迁移到工业缺陷场景时性能下降，因此需要针对工业特性改造SAM。

Method: 提出Perceptual Parallel Prompt（P³）框架，构建P³-SAM模型，包含两个核心策略：（1）Perceptual-Optimized Encoding（POE），增强局部对比度并保留关键纹理细节；（2）Parallel Prompt Generator（PPG），同时生成语义提示与空间提示，为SAM解码器提供跨图像的综合引导。

Result: 在三个小样本S³D基准上取得state-of-the-art性能，其中Surface Defects-4i数据集mIoU提升12.00%。

Conclusion: 针对工业缺陷图像特性对SAM进行感知优化与提示增强，可有效提升带钢表面缺陷小样本分割性能，验证了双策略框架的有效性。
...
TL;DR: 本文提出 LoG-VGGT，一种面向长序列三维重建的内存高效框架，通过局部时间建模与全局相机一致性相结合，在受限内存下实现跨窗口信息传播并缓解长期位姿漂移。


<details>
  <summary>Details</summary>
Motivation: 长序列三维重建中，全局注意力虽能保证相机一致性，但内存开销随序列长度急剧增长；而仅依赖局部建模又会导致长期位姿漂移。因此需要一种兼顾局部时间建模与全局相机一致性且内存可控的方法。

Method: 方法包含两部分：(1) 在少量 transformer 块中引入跨窗口注意力，实现相邻时间窗口间的信息传播，同时保持内存占用有界；(2) 设计全局相机一致性精化模块，令相机 token 与紧凑的 register token 通过交叉注意力交互，在整个序列范围内施加场景级约束，从而联合优化相机表示。

Result: 大量实验表明，LoG-VGGT 在多个长序列基准上提升了深度精度和相机位姿估计的鲁棒性，同时具备有竞争力的流式重建性能。

Conclusion: 通过在少量块中进行跨窗口注意力并结合全局相机一致性精化，可在不承担全序列注意力高昂代价的前提下提升长时程位姿稳定性与重建质量，为内存高效的长序列三维重建提供有效方案。
...
TL;DR: ME-Dex-1.0：统一视觉-触觉-动作的世界动作触觉模型，将触觉作为未来观测与视频联合建模，采用Mixture-of-Transformers三专家流匹配架构，并配套Agentic触觉数据引擎。


<details>
  <summary>Details</summary>
Motivation: 世界动作模型将视频模型的预测能力引入机器人动作生成，为未来视觉状态建模提供了基础；触觉感知可直接测量物理交互，是对该基础的有力补充。但现有方法多仅将触觉特征作为条件输入，未对未来触觉状态、视觉观测与动作进行联合预测。核心洞察是：触觉信号与视频一样，提供了世界状态演化的观测，应作为未来观测与视频一并建模。

Method: 提出ME-Dex-1.0（MachEmbodied-Dex-1.0），一个面向视觉、触觉与动作联合学习的统一世界动作触觉模型。架构采用Mixture-of-Transformers，包含Video Expert、Tactile Expert与Action Expert三个专家，统一以flow matching训练；通过中间层的共享注意力连接各专家，使动作生成在联合去噪过程中利用视觉与触觉动力学的表征。为支持多源异构触觉输入，设计Canonical Hand Model与Unified Tactile Autoencoder，将不同本体与传感布局的触觉观测映射到共享空间与隐空间。针对配对视觉-触觉-动作数据稀缺问题，开发Agentic Tactile Data Engine（基于智能体的数据生产平台），在仿真轨迹回放中直接由力传感器采集触觉数据，补充RoboTwin与DexJoCo数据集。

Result: 在RoboTwin、DexJoCo与ManiFeel仿真平台及真机评测上，配备触觉感知的夹爪与灵巧手均取得更优的操作性能。

Conclusion: 将触觉视作未来观测并与视频、动作联合建模，配合多专家流匹配架构与统一触觉表征，可有效提升含触觉的机器人操作性能；同时智能体数据引擎缓解了多模态配对数据不足的问题，验证了世界动作触觉模型范式的可行性。
...
TL;DR: 提出CompAdapt，一个物理一致的文本到视频框架，通过神经动力学建模扩展至复合物理行为，支持语言驱动的物理语义解析与少样本自适应，提升物理一致性同时保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散式T2V模型虽能生成真实且时序连贯的视频，但常违背基本物理动力学。已有的物理约束方法仅支持单一类型运动、依赖人工指定参数，且难以泛化到未见物理规律。

Method: 1) 将神经动力学建模从单类型运动扩展到复合物理行为，涵盖耦合运动、多阶段过渡与多物体碰撞；2) 将自然语言提示转化为结构化物理语义，端到端指定运动类型、时序关系和初始物理参数；3) 引入动力学感知先验匹配，实现无需重训练核心动力学模块的一次性（one-shot）自适应；4) 设计物理感知隐特征融合模块，提升快速复杂运动下的视觉保真度。

Result: 在面向物理的T2V基准上，CompAdapt相较通用T2V模型和物理约束基线提升了物理一致性，同时保持高视觉质量和对未见动力学的适应能力。

Conclusion: CompAdapt通过复合动力学建模、语言到物理语义的结构化映射及少样本先验匹配，实现了复杂真实场景下物理一致的T2V生成，并具备对未见物理规律的泛化能力。
...
TL;DR: 该论文研究点监督下的可穿戴传感器时序动作定位（TAL），提出渐进式传感器事件扩展（PSEE）方法，从单时间戳标注恢复伪片段以训练标准TAL检测器。


<details>
  <summary>Details</summary>
Motivation: 可穿戴传感器流中的时序动作定位需要动作类别与起止边界，但逐实例的起止标注成本高昂，因此希望用每实例仅一个时间戳及类别的点监督来降低标注负担。

Method: 提出PSEE，融合语义激活、传感器特有的过渡证据与自适应时间归属，从点监督中恢复伪片段；这些伪片段用于监督标准TAL检测器，且不改变其推理流程。

Result: 在四个惯性传感基准上的跨受试者实验表明，PSEE相较适配后的点监督基线提升了伪边界质量，兼容不同TAL检测器，并对点采样方式具有鲁棒性。

Conclusion: PSEE能有效缓解点监督TAL的边界标注缺失问题，生成高质量伪边界，具备检测器通用性与采样鲁棒性，代码已开源。
...
TL;DR: 本文提出SkillIR，一种以“退化为中心的动作证据”表示修复经验、在残余状态循环中逐步引导工具调用的技能引导框架，用于多模态智能体图像修复，提升修复质量与工具使用可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有修复智能体多从原始退化图像直接生成完整工具使用计划，或检索以往成功轨迹，难以为适应中间修复状态提供支持。作者发现已被采纳的工具执行会改变残余退化状态，从而影响后续工具的适用性，因此需要一种能随残余状态动态调整决策的方法。

Method: 提出SkillIR：将修复经验表示为以退化为中心的动作证据而非完整轨迹；把上下文相关的动作结果整合为场景感知的修复技能，刻画适用条件、预期效果与可归因失败案例。检索到的技能不规定完整计划，而是在经验证的残余状态循环中一次引导一个有界动作：每个工具输出先作为候选，经转移验证后才提交，随后重新评估当前残余退化。每次 rollout 后的证据用于创建、精炼或修补动态技能，使积累的修复经验改善后续输入的决策。

Result: 在合成与真实世界多退化数据集上的实验表明，SkillIR 提升了修复质量，并实现了更可靠、更有效的工具使用。

Conclusion: 以技能（退化为中心的动作证据）替代完整工具轨迹、采用有界动作与转移验证的残余状态循环，是提升智能体图像修复适应性与可靠性的有效途径。
...
TL;DR: MT-WAM在Fast-WAM基础上增加未来二维点轨迹与视觉特征的辅助监督，用轻量双流分支和结构化注意力掩码实现，推理时跳过未来视频预测，在LIBERO、LIBERO-Plus、RoboTwin 2.0及真实任务上显著提升成功率。


<details>
  <summary>Details</summary>
Motivation: Fast-WAM证明视频-动作协同训练可提升控制性能，且推理时无需生成未来视频，单次视频扩散Transformer前向的表征对动作生成至关重要。但仅用未来观测预测并未显式优先学习控制所需的未来动态与视觉结构，因此需要引入更有针对性的监督信号。

Method: 保留原始训练目标，增加未来二维点轨迹和视觉特征的互补监督。从视频骨干网络最后若干层复制出轻量双流分支，进行目标特定处理；采用结构化注意力掩码阻止跨流注意力。运动流token为动作专家提供额外动态条件；未来视觉特征预测在捕捉物体与空间结构的特征空间中提供监督，从而训练视频骨干在视觉条件变化下为动作生成提供更有信息的视觉上下文，且不将视觉特征流token加入动作条件。推理时使用每次重规划计算一次的视频与运动缓存，并跳过未来视频预测。

Result: 无需额外具身策略预训练：LIBERO成功率98.2%，LIBERO-Plus 73.7%，后者较Fast-WAM提升23.8个百分点；RoboTwin 2.0 Clean2Rand上Random成功率由6.30%升至19.40%；四个真实世界任务平均成功率由67.0%升至77.8%。

Conclusion: 显式引入未来运动轨迹与视觉特征监督，可在不增加推理时未来视频生成开销的前提下，有效提升视频表征对动作生成的支持能力，在仿真与真实任务上均取得一致且显著的性能增益。
...
TL;DR: 本文提出OpenSAL360，首个开源、可扩展、低成本的360°视频显著性采集平台。它仅需普通屏幕、鼠标和网络，无需VR头显，即可通过众包并行采集数据。作者用7个VR眼动数据集验证了协议，并做了参数消融实验，最终公开了一个含500个全景视频、2000+标注者的最大规模显著性数据集。


<details>
  <summary>Details</summary>
Motivation: 全向视频显著性预测对视口自适应流、压缩、注视点渲染、网格简化、感知质量评估等沉浸式应用很重要，但VR头显采集眼动数据成本高、流程复杂，导致大规模数据集难以扩展。

Method: 提出OpenSAL360开源平台：不依赖VR头显，仅需标准屏幕、鼠标和互联网连接，支持从普通众包评估者处并行采集显著性数据；并针对关键界面、预处理和后处理参数进行消融实验，同时与7个成熟的VR眼动数据集进行协议验证。

Result: 验证了采集协议的有效性，并完成关键参数消融；基于该平台采集并公开了一个包含500个全向视频、由2000+众包评估者标注的显著性数据集，据称是该领域目前最大的数据集。

Conclusion: OpenSAL360提供了一种低成本、可扩展的360°视频显著性采集方案，无需专用硬件即可实现大规模众包标注，并已开源发布平台与数据集，为全向视频显著性研究提供了重要数据基础。
...
TL;DR: VoxelTTO 提出一种前馈式 3DGS 框架，将图像特征聚合到全局体素表示并从中解码高斯，摆脱像素-高斯对应关系，并结合冻结 VFM 的 LoRA 测试时优化与随机实体体积渲染，提升几何保真度与位姿估计。


<details>
  <summary>Details</summary>
Motivation: 现有前馈 3DGS 方法通常回归像素对齐的高斯原语，导致高斯过度重叠与伪影；同时预测相机位姿不准确会在新视角合成中引起错位。因此需要一种摆脱像素对齐、并能利用已知相机参数提升几何精度与位姿估计的框架。

Method: 1) 将密集图像特征聚合为全局体素表示，并从体素特征解码高斯，打破像素到高斯的对应关系；2) 在冻结预训练视觉基础模型（VFM）参数的前提下，引入测试时优化（TTO），利用位姿监督自适应调整轻量 LoRA 模块；3) 训练与推理阶段用随机实体体积渲染替代原始 3DGS 光栅化；4) 训练仅更新体素对齐的高斯重建模块。

Result: 训练耗时 80 GPU 小时。在 Replica、Tanks and Temples 和 DTU 数据集上，相比先前方法在 RGB-D 新视角合成与相机位姿估计上均有提升。

Conclusion: VoxelTTO 通过体素化解码、LoRA 测试时优化与实体体积渲染，从任意数量图像及可选相机参数中重建几何精确的 3DGS 场景，验证了该框架在 NVS 与位姿估计任务上的有效性。
...
TL;DR: 提出以记忆为核心的3D基础模型，通过自适应世界记忆机制统一机器人定位、稠密重建与高斯渲染。


<details>
  <summary>Details</summary>
Motivation: 现有3D基础模型虽能从RGB图像进行可泛化的几何推理，但在持久记忆、可扩展性以及可渲染场景建模方面仍受限，难以支撑机器人在长序列、大场景中的持续世界建模。

Method: 核心为自适应世界记忆机制：结合基于Transformer的门控更新与测试时时空调节，通过学习门控控制循环记忆传播，并利用时序状态演化与空间观测-状态一致性调节长序列上token级的更新与遗忘；将记忆组织为局部子图以支持大尺度建图，融合渐进式建图与跟踪、回环检测以及基于SL(4)的全局优化以兼顾局部精度与全局一致性；并以高斯重建头将记忆增强特征解码为可渲染图元，在单一模型内统一相机位姿估计、稠密点云重建与逼真渲染。

Result: 在公开基准与多种机器人平台自采数据集上，轨迹精度、重建完整性和渲染质量均优于现有3D基础重建与SLAM基线。

Conclusion: 结果表明自适应记忆可作为持久机器人世界建模的基础；数据集与代码将开源。
...
TL;DR: 提出基于2D高斯泼溅的主动重建框架，用概率可靠性模型与香农互信息度量（2D GauSS-MI）实现兼顾视觉与几何质量的高效主动视角选择。


<details>
  <summary>Details</summary>
Motivation: 主动重建需在有限机载算力下高效选择视角以获得高质量重建。现有方法难以兼顾视觉与几何质量同实时计算效率之间的平衡。

Method: 基于2DGS构建增量RGB-D观测的在线建图流水线；提出刻画单个2D高斯泼溅视角相关重建质量的概率可靠性模型；在其上定义2D GauSS-MI——利用2DGS显式表面朝向的互信息度量，评估候选视角的期望信息增益，使视角选择同时考虑视觉与几何质量。

Result: 在8个Replica场景上与三种先进基线对比，方法在视觉与几何重建质量间取得良好平衡，计算开销显著降低，模型存储具有竞争力。

Conclusion: 以2DGS为表示、以互信息为准则的主动视角选择可在低算力条件下实现高质量主动重建，兼具质量与效率优势。
...
TL;DR: 该论文提出 VidOmni-Bench，一个通过让模型验证密集视频字幕中每个事件是否被视频支持来评估 Video-LLM 细粒度视频理解的基准，包含 500 个多复杂度、多时长视频，并揭示模型在字幕幻觉与事实验证上的薄弱环节。


<details>
  <summary>Details</summary>
Motivation: 现有 Video-LLM 评估基准多依赖问答或与参考字幕匹配，模型可能借助表面线索通过测试，且标注不完整，难以可靠衡量细粒度视频理解能力。

Method: 构建 VidOmni-Bench：收集覆盖五种复杂度类型、时长 4 秒至 90 分钟的 500 个视频；用多种 Video-LLM 生成密集字幕，并经人工核验得到句子级标签，其中含错误事件的句子作为硬负样本用于评估。

Result: 实验得到三点发现：(i) Video-LLM 在密集视频字幕中频繁产生幻觉描述；(ii) 作为验证器时也难以可靠识别看似合理但错误的事件描述；(iii) 模型弱点随视频复杂度与时长变化，暴露出多样且模型特定的瓶颈。

Conclusion: VidOmni-Bench 提供了更具挑战性的细粒度视频理解评测范式，表明当前 Video-LLM 在字幕生成与验证两方面均存在显著缺陷，且瓶颈因模型和视频特性而异。
...
TL;DR: 提出免训练框架RTF，通过通道级特征精炼与可靠性感知的多模态融合，提升小样本3D识别性能。


<details>
  <summary>Details</summary>
Motivation: 预训练基础模型为3D视觉任务提供丰富多模态先验，但在小样本场景下效果受限：高维特征存在通道冗余与任务无关噪声，且不同模态的可靠性随样本变化；直接聚合异质表征会忽视样本级模态可靠性，掩盖判别性线索。

Method: RTF为免训练框架：1）联合建模类间相似性与类内稳定性，识别判别性特征通道，从预训练模型缓存表征中解耦领域知识精炼；2）引入可靠性感知融合机制，由特征精炼引起的分布偏移估计样本级模态可靠性，实现多模态自适应聚合；3）构建整合实例级支持特征与类级原型的记忆缓存以推断查询标签。

Result: 在五个基准上，RTF持续优于单模态基线、部分融合变体及现有轻量适配方法，取得SOTA小样本3D识别性能，且无需梯度优化、额外训练数据、辅助训练或参数更新。

Conclusion: RTF以免训练方式有效解决小样本3D识别中的特征冗余与模态可靠性问题，验证了通道精炼与可靠性感知融合的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分析中的多标签小样本学习（MLFSL）尚存两大局限：其一，传统原型生成易混杂无关疾病信息，导致原型被污染、性能下降；其二，已有研究多在嵌入空间强制类间可分，却忽视疾病间的内在相关性。

Method: PPR先进行原型纯化：利用样本级共病评分突出疾病特异特征，生成更纯净的疾病原型；再在此基础上，引入疾病级共病统计自适应调控类间相似度，构建共病感知嵌入空间，从而依次捕获纯疾病特征与类间关系。

Result: 在四个胸部X光基准数据集（含跨域评估）上，PPR一致优于现有最优方法，显著提升疾病检测性能，并展现良好泛化性与临床适用性。

Conclusion: PPR通过原型纯化与共病感知的类间关系调控，为医学图像可靠的多标签小样本学习提供了有效方案。
...
TL;DR: 该论文研究通用视觉-语言模型（VLM）获得OCR能力时，是发展新阅读回路还是复用已有机制。作者以全序列OCR为设定，通过证据支撑的保留因果干预协议，在GLM-OCR、MinerU2.5、PaddleOCR-VL-1.6中识别出稀疏稳定的OCR头集合，并发现其与通用VLM中独立识别的文本检索/复制头高度重叠且因果相关。结果表明全序列OCR本质上是密集顺序多模态复制粘贴机制；当通用VLM专门化为OCR专家时，头部身份基本保留，仅重新分配功能与因果强度。


<details>
  <summary>Details</summary>
Motivation: 回答通用视觉-语言模型获得专门OCR能力时，究竟是形成新的阅读回路，还是复用已有机制这一核心问题，并区分全序列OCR与局部答案检索两种不同设定。

Method: 采用证据支撑的协议与留出因果干预，在GLM-OCR、MinerU2.5、PaddleOCR-VL-1.6中识别稀疏、稳定的OCR头集合；将其与两个通用VLM中独立识别的文本检索/复制头进行对比，计算未调优的top-20交集比例和全头Spearman相关性；并通过匹配的基座—专门化模型对比，考察OCR专门化过程中头身份与功能/因果强度的变化。

Result: 两个通用VLM中，视觉OCR头与独立识别的文本检索/复制头高度重叠，未调优top-20交集达73.3%，全头Spearman相关为0.677–0.886；因果干预进一步支持该重叠。基座到专门化的匹配对比显示，OCR专门化基本保持头身份，每任务top-20中保留17–20个头，全头秩相关为0.874–0.942，但功能与因果强度发生重新分配。

Conclusion: 全序列OCR并非发展出全新的阅读回路，而是复用并改造通用的文本检索/复制回路，表现为密集顺序的多模态复制粘贴；OCR专门化主要在保留原有头身份的基础上重新分配功能与因果强度。
...
TL;DR: GestureFAR：基于流匹配自回归的流式语音共语手势生成框架，用连续隐变量替代离散码本，并通过头部流蒸馏实现实时token因果生成。


<details>
  <summary>Details</summary>
Motivation: 具身对话智能体需要在用户尚未说完时即生成手势，因此流式（在线）共语手势生成至关重要。现有流式系统虽通过离散运动token自回归实现在线生成，但将高维连续运动压缩进有限码本，限制了生成手势的真实性与多样性。核心矛盾在于：既要保持因果性（causality，不依赖未来语音），又要保持连续表达力（continuous expressiveness）。

Method: 提出GestureFAR，一个flow-autoregressive流式共语手势生成框架，包含两部分：1）在因果连续运动隐变量（causal continuous motion latents）上自回归，用Transformer建模流式音频-运动上下文，并用逐token的flow-matching头从连续分布中采样下一个隐变量，从而避免离散化；2）头部流蒸馏（head-only flow distillation）：冻结因果主干，将多步逐token流头通过一致性（consistency）与分布匹配（distribution-matching）目标蒸馏为单次网络评估，在保持token因果性的同时消除实时交互的主要延迟瓶颈。

Result: 在BEAT2数据集上的实验表明，GestureFAR在具备流式能力的方法中显著改善了质量-延迟权衡（quality–latency trade-off），在保持较强手势质量的同时实现实时的token因果生成。

Conclusion: 以连续隐变量的流匹配自回归结合头部流蒸馏，可在不牺牲手势质量的前提下兼顾因果性与低延迟，为实时流式共语手势生成提供了有效方案。项目主页：https://andypinxinliu.github.io/GestureFAR
...
TL;DR: 本文介绍了4D-ImageNet，一个包含174,000个衍射图谱的4D-STEM数据集，涵盖145,000个实验数据与29,000个多切片模拟数据，并提供伪标签与基准测试。


<details>
  <summary>Details</summary>
Motivation: 4D-STEM在每个探针位置记录二维衍射图谱，产生大规模、异构且空间分辨的倒空间数据，但缺乏标准化数据集阻碍了表示学习、盘检测与模拟-实验对照等研究。

Method: 构建数据集：实验部分从29次采集（Ag、Au、Au-Ag混合、CoO、Pd、ZnO，多视场、扫描尺寸、相机长度与曝光时间）中筛选145,000个衍射图，每次采集贡献5,000个按质量排序的图谱并附扫描坐标与元数据；模拟部分含13种晶体结构、29,000个多切片模拟，包含欧拉旋转、倒空间采样及低指数束方向；采用集合预测检测器生成直接束与布拉格盘中心的模型伪标签及置信度，并提供分组混合域掩码重建基准以评估防泄漏加载与评价。

Result: 得到174,000个衍射图谱（145,000实验+29,000模拟），带采集级标签、质量排序、扫描坐标、采集元数据以及带置信度的束/盘中心伪标签，并具备防泄漏的分组混合域基准。

Conclusion: 该数据集适用于表示学习、盘检测、衍射图谱检索、取向分析及模拟到实验的研究。
...
TL;DR: HAT 是一个利用帧间运动在配准与融合前对 CAD 位姿假设进行选择的因果式单目 6-DoF 位姿估计框架，包含 Mega-HAT 与 Pico-HAT 两个版本。


<details>
  <summary>Details</summary>
Motivation: 非合作目标的单目 6-DoF 位姿估计对在轨服务与碎片清除很重要，但单帧估计器会混淆近对称航天器朝向，跟踪方法又可能锁定错误位姿。

Method: HAT 保留多个竞争朝向历史，利用帧间运动在配准与融合前选择假设，并以此锚定单目 SLAM 估计的相对轨迹；通过稀疏锚点与位姿融合在初始化后给出每帧估计，且不修改历史输出。方法仅需标定 RGB 序列、度量 CAD 模型与目标图像区域，预训练位姿与 SLAM 网络无需目标特定训练或微调。

Result: 在 SPARK-2024、SwissCube、SHIRT 及 YCB-Video 上评估：相对独立 MegaPose，Mega-HAT 平均位姿误差降低 9.4%、持续输入 FPS 提升 3.76 倍；相对独立 PicoPose，Pico-HAT 平均位姿误差降低 23.9%、FPS 提升 2.42 倍。Mega-HAT 在 SPARK 上的消融实验分析了各组件贡献及修改历史估计的影响。

Conclusion: 通过帧间运动驱动的假设锚定跟踪，HAT 在无需目标特定训练的前提下提升了精度与效率，并保持因果性，适用于空间域及域外场景。
...
TL;DR: 该论文提出一种轻量级情感调节模型（MobileNetV4+FiLM），单次前向传播（3.7 ms）即可预测图像情感强度的全局变换参数，替代此前每图80 s的迭代优化，并在用户研究（N=54）中降低了唤醒度且质量评分优于灰度滤镜，已集成至Android应用实现Instagram视频60 fps实时调节。


<details>
  <summary>Details</summary>
Motivation: 问题性网络使用影响日益扩大，而现有干预（时间限制、屏蔽、强制休息）具有强制性且易被规避；调整视觉内容的情感强度是一种更温和的替代方案，但先前基于优化的情感调控方法需对每张图像迭代优化（约80 s），无法满足实时部署需求。

Method: 以MobileNetV4为骨干网络，结合FiLM机制进行情感条件化，输出可微全局变换的参数；用单次前向传播直接预测该情感变换，取代此前的迭代优化。随后开展用户研究（N=54）评估唤醒度与感知质量，并将模型集成进Android应用，对Instagram视频进行实时处理。

Result: 单张处理时间由约80 s的迭代优化降至3.7 ms的单次前向传播；用户研究中，模型相对未编辑图像显著降低观看者自报唤醒度，效果与灰度健康滤镜相当，但感知质量评分更高；Android应用在三星Galaxy S23上处理Instagram视频可稳定维持60 fps。

Conclusion: 通过学习式单次前向预测替代逐图优化，可在保证情感调节效果的同时实现实时、低侵入的视觉内容情感强度自适应，为问题性网络使用提供了一种非强制性的干预途径，且具备移动端实时部署的可行性。
...
TL;DR: 在1422张全景片、42142个专家标注牙多边形上系统隔离输入分辨率、架构与解剖先验三大因素：分辨率主导（掩膜mAP50-95显著提升），架构在域内几乎无关，三项先验/表征干预均无效；边界精度而非检测是瓶颈。


<details>
  <summary>Details</summary>
Motivation: 自动牙齿分割与FDI编号是计算机辅助牙科诊断的基础，但何种因素决定其性能尚不明确。现有研究多聚焦架构创新，缺乏在统一评估协议下对各影响因素的受控对比，导致难以判断性能提升的真正来源。

Method: 构建含1422张全景片、42142个专家勾画牙多边形（32类FDI分类体系）的语料，由30名牙科从业者标注、另2名独立复核。在单一评估协议下开展三类实验：(1) 640/1024/1280输入分辨率消融，并以图像级配对bootstrap检验显著性；(2) 对比查询式Transformer（参数量为单阶段检测器的2.1倍）与单阶段检测器的域内、域外表现及CPU推理速度、ONNX可执行性；(3) 三项针对性干预：LoRA适配的自监督编码器、可提示基础分割模型、全局最优解剖标签分配。另进行零样本迁移至独立多中心队列，并按牙轴分解掩膜误差。

Result: 分辨率主导：mAP50-95由0.656升至0.710、0.717（p<0.001，p=0.024），mAP50稳定在约0.982且变化不显著，说明分辨率提升带来的是边界精度而非检测能力。域内架构近乎无关（95% CI [-0.0064, +0.0064]），查询式Transformer仅域外略优，CPU推理慢5.5倍且无法在标准ONNX运行时执行。三项干预均失败：LoRA自监督编码器性能下降，可提示基础分割模型掩膜退化39%，全局最优标签分配仅+0.0007（尽管纠正了域外预测中40%违反的约束）。零样本迁移损失62%的mAP50-95但仅损失18%的mAP50，复现该解离；按牙轴分解将残差定位至根尖三分之一。

Conclusion: 边界精度是约束性能的瓶颈。提升应优先投向输入分辨率与采集多样性，而非架构新颖性；解剖先验与表征干预在本任务中收益有限。
...
TL;DR: 提出通过线性分配恢复通道间patch对应关系的解耦注意力，使DC-ViT可结合标准掩码多通道训练，在六个基准上超越最强MC-ViT基线。


<details>
  <summary>Details</summary>
Motivation: 多通道成像各通道语义不同，MC-ViT将各通道token拼接后全局自注意力无约束，导致单通道特征被稀释；DC-ViT虽分离通道内与跨通道更新，但其公式按空间位置配对token，要求各通道可见token一致，无法直接用于独立逐通道掩码。

Method: 在DC-ViT中引入基于线性分配（linear assignment）的通道间patch对应求解：对各通道保留patch进行匹配，从而在独立逐通道掩码下恢复对应关系，使解耦注意力可与标准配置的掩码多通道训练结合，而非受限配置。

Result: 在荧光显微、成像质谱流式和卫星成像三类模态、三个分类与三个分割基准（含高通道数密集预测）上，该公式优于最强MC-ViT基线。

Conclusion: 线性分配匹配可有效桥接解耦注意力与独立逐通道掩码，扩展DC-ViT适用性并提升多通道成像任务的性能。
...
TL;DR: 该论文研究 Digital Green 面向小农户的农业咨询服务 Farmer.Chat，基于约116万张来自埃塞俄比亚、印度、肯尼亚和尼日利亚的田间照片，将任务拆分为质量门控（M0）、作物检测（M1）和病害/虫害检测（M2）三阶段，并比较单模型统一路线（微调Qwen3-VL-4B）与专用小模型路线（DaViT、YOLO26）。


<details>
  <summary>Details</summary>
Motivation: Farmer.Chat 的真实使用场景中，农民仅上传一张照片，不描述症状、不提供作物名称，照片多由低端手机在弱光、移动条件下拍摄。现有生产系统不可调整：缺乏照片拒绝阈值、无法新增作物与问题类别、没有置信度截断。同时生产质量门控拒绝了46.8%的图像，超过四分之一进入诊断的图像未返回作物名称，标注为"disease"的问题中35.8%实为可在未知作物情况下识别的虫害。这些缺陷促使作者重新设计可配置的多阶段系统。

Method: 研究分析约1.16百万张 Farmer.Chat 照片，将流程拆为三阶段：质量门控M0、作物检测M1、病害或虫害检测M2。设置两条路线：Route A 使用单一微调视觉语言模型 Qwen3-VL-4B，一次调用完成三阶段；Route B 每阶段使用小型专用模型（DaViT、YOLO26）。生产质量门控由 GPT-4o 替换为 MobileNetV3。并在同一测试集上以统一评分方式比较各系统，评估作物准确率、诊断表现与拒答行为。

Result: 小型 MobileNetV3 质量门控达到86.9% F1，推理耗时12 ms，替代生产环境 GPT-4o 门控。在同一评分测试集上，分层 DaViT-Base 作物准确率为95.41%，高于生产基线91.46%。该模型在诊断任务上也领先，且从不拒绝作答；相比之下，所有语言模型对比系统均留下大量无诊断行。微调 Qwen3-VL-4B 保留了专用模型不具备的两项能力：一次调用完成三阶段，以及在图像不足以支持回答时主动请求更好的照片。

Conclusion: 针对小农户单图咨询场景，专用小模型路线在质量门控效率、作物识别与诊断覆盖率上优于生产基线；统一微调视觉语言模型则在流程集成度与主动澄清（请求重拍）方面具有独特优势。三阶段拆分与可配置设计解决了原生产系统不可调整、无法拒绝低质图像以及作物与问题类别难以扩展的问题。
...
TL;DR: 本文提出 Dense Reasoning Trace (DRT) 范式，用紧凑的结构化推理轨迹替代自然语言 CoT，实现 5.5 倍 token 效率提升并带来 1.3 个百分点的准确率增益。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大模型（MLLMs）的思维链（CoT）范式局限于自然语言表达空间，导致过度的语言冗余，进而引发信息稀释与视觉基础（visual grounding）薄弱的问题。

Method: 1）Dense Trace Initialization：将 DRT 推理模式内化到模型中，在保留视觉证据的同时显著提升 token 效率；2）Trace-Grounded Reinforcement Learning 框架：通过三视角验证流程构建参考轨迹，并采用带结构化奖励的 Trace-Grounded GRPO，促使模型生成简洁的 DRT 式轨迹，降低幻觉、增强逻辑基础。

Result: 在具有挑战性的推理基准上，DRT 相比 Qwen3-VL 基线实现了 5.5 倍 token 效率提升，同时准确率提高 1.3 个百分点。

Conclusion: 复杂的多模态推理可能并不需要冗长的自然语言轨迹，这为下一代 MLLMs 开辟了更高效的路径。代码与数据已开源。
...
TL;DR: 提出基于扩散修复的肾脏肿瘤合成框架，无需额外标注，首次系统比较2D、2.5D与全3D（MAISI）合成策略；在低数据场景下，2.5D与3D增强将假阳性从约18–20%降至约3–6%，且2.5D在各项指标上匹配3D且计算成本显著更低。


<details>
  <summary>Details</summary>
Motivation: 深度学习肾脏肿瘤分割依赖大规模标注数据，但临床部署中目标部位通常仅有少量肿瘤阳性病例，数据稀缺且标注成本高，亟需无需额外标注的数据增强手段。

Method: 构建基于扩散模型的图像修复（inpainting）框架，在健康CT扫描中合成解剖上合理的肾脏肿瘤；在公开数据（KiTS23、KIRC）上训练扩散模型，在内部队列上以三种低数据设置评估nnU-Net分割；并首次系统对比2D、2.5D与全3D（MAISI）三种合成策略。

Result: 2.5D与3D增强显著降低假阳性（从约18–20%降至约3–6%），同时维持Dice；2D方法无一致收益；所提出的2.5D方法在所有指标上与全3D合成相当，但计算成本显著更低。

Conclusion: 仅需局部体积一致性即可实现有效的数据增强，2.5D扩散合成是数据与资源受限临床场景下高效、实用的肿瘤分割增强方案。
...
TL;DR: 提出统一的、基于姿态的无注释手语翻译与生成框架SignGPT，将身体、手部、面部动作的分部位层级表示集成进共享语言模型，实现手语作为输入与输出的双向建模，并在两个数据集上验证，探索手语对话应用潜力。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型对手语交互的支持有限；手语翻译（SLT）与手语生成（SLG）通常由分离的模型承担，在sign-text交互中需要频繁切换模型，缺乏统一框架。作者希望将SLT与SLG统一，使手语既能作为输入也能作为输出，从而支持手语对话。

Method: 提出SignGPT：一个统一的、基于姿态（pose-based）且无需gloss标注的SLT与SLG框架。技术上，将身体、手部、面部动作的分部位层级表示（part-aware hierarchical representations）集成到共享语言模型中，并采用非对称多token预测（asymmetric multi-token prediction）与渐进式训练（progressive training）进行双向建模。

Result: 在How2Sign（ASL）与Phoenix-2014T（DGS）两个数据集上，通过基准对比、定性分析与组件消融实验对SignGPT进行了评估。此外，一项由12名聋人ASL手语者参与的探索性研究评估了基于LLM中介的手语到手语响应流程（sign-to-sign response pipeline）。
...
TL;DR: ZYT-World：单一架构原生生成7路（4鱼眼+3针孔）驾驶世界模型，通过投影特定Plucker适配器、因果一致性蒸馏、RigCritic与隐式记忆模块，将40步双向教师蒸馏为一步流式生成器，速度提升107.7倍，同时保留>90%的PSNR/SSIM。


<details>
  <summary>Details</summary>
Motivation: 生成式世界模型虽能为端到端及VLA驾驶策略提供可控、可重复的闭环仿真，但实际部署面临三项未解需求：1）在原生分辨率下忠实复现鱼眼-针孔混合相机rig；2）在因果逐时间步交互与长时程稳定性、低延迟之间取得平衡；3）当同一地点被重访时保持场景身份一致性。

Method: 提出统一架构ZYT-World：原生生成4个FOV>180°的鱼眼视图与3个针孔视图；采用投影特定的Plucker适配器编码相机几何，ego-motion自适应层归一化实现全局运动控制，轻量像素对齐布局以实例级框、朝向与颜色条件化交通参与者与信号；异构训练结合全rig几何覆盖与高分辨率细节；通过teacher forcing、因果一致性蒸馏、自rollout分布匹配蒸馏及RigCritic将40步双向教师转为一步逐潜变量流式生成器；配合19M参数TinyVAE解码器、W8A8量化与推理引擎降低各环节开销；最后用真实采集的跨轨迹对训练即插即用隐式记忆模块以保留地点特定证据。

Result: 在内部多视图测试集上，一步模型保留教师模型>90%的PSNR与SSIM，FID、FVD、LPIPS均在教师模型11%以内；按图2仅生成器计时，比40步双向教师快107.7倍；TinyVAE解码比Wan快59.8倍；30秒rollout与跨轨迹重访实验验证了预期的长时程与记忆行为。

Conclusion: ZYT-World以单一架构统一解决了混合相机rig原生生成、因果流式低延迟与地点重访记忆保持三项部署需求，在显著加速的同时保持了接近教师模型的生成质量，并展现出长时程稳定与隐式记忆能力。
...
TL;DR: 针对不平衡二分类分割中熵最小化易崩溃的问题，提出平衡锚点提示适配（BAPA），含类平衡锚点（CBA）与动态提示适配（DPA），在四领域实验中取得最高平均Dice。


<details>
  <summary>Details</summary>
Motivation: 熵最小化是测试时适配（TTA）的标准目标，但在不平衡二分类分割中会失效。与图像分类不同，稠密分割会聚合数千个像素预测，导致占多数的预测类主导更新，把少数类预测拉向自身；随着预测饱和、其熵梯度消失，最终产生退化的分割掩码。

Method: 作者在共享偏移（shared-shift）模型下从理论上证明了上述崩溃现象。据此提出 BAPA，包含两个互补模块：类平衡锚点（CBA）分别从每个预测类中挑选高置信度锚点，并使前景与背景获得相等的总损失权重，避免较大区域主导更新；动态提示适配（DPA）在每次预测更新后刷新这些锚点，并仅优化文本侧提示残差，冻结视觉-语言编码器。该仅提示更新在不改变预训练稠密视觉表示的前提下细化前景-背景决策边界。

Result: 在来自四个领域的实验中，BAPA 在所评估方法中取得最高平均 Dice；因子化消融进一步验证了 CBA 与 DPA 的互补作用。

Conclusion: 平衡的提示适配可作为熵最小化的有效替代方案，用于测试时二分类分割。
...
TL;DR: 本文提出SFVO，一个对应关系驱动的双目视觉里程计框架，直接基于预训练的双目匹配和光流模型，将学习到的对应关系映射为几何约束并预测可信点，通过解耦的旋转/平移置信度图提升约束可靠性，实现鲁棒精确的位姿估计。


<details>
  <summary>Details</summary>
Motivation: 1. 现有深度学习VO多集中于单目方法，存在尺度模糊问题；
2. 双目VO天然提供真实度量尺度，但因计算成本高、建模复杂而研究不足；
3. 双目匹配与光流估计的进展使稠密视觉对应日益精确，但其互补的几何信息在VO中尚未被充分利用。

Method: 1. 提出SFVO——对应关系驱动的双目VO框架，直接构建于预训练的双目匹配与光流模型之上；
2. 利用预训练模型估计双目对应（立体匹配）与时间对应（光流）；
3. 不直接从图像学习位姿，而是将学习到的对应关系映射为几何约束，并预测哪些点值得信赖；
4. 引入针对旋转与平移的解耦置信度图（decoupled confidence maps），以更好匹配视觉对应与6自由度变换的特性，提升几何约束可靠性。

Result: 在室外与室内数据集上的大量实验表明，SFVO实现了鲁棒且精确的位姿估计，并具备强泛化能力。代码将开源。

Conclusion: SFVO证明，充分利用预训练匹配/光流模型提供的稠密对应关系，并以解耦置信度建模其可靠性，是构建高精度、高泛化双目VO的有效途径，弥补了深度学习双目VO研究的不足。
...
TL;DR: 该研究通过在4个数据集、12200份胸片记录上对3个医学视觉语言模型和1个通用模型进行多维度评估，发现模型排名、分数可靠性与筛查性能均无法在队列、提示词、负样本谱、患病率与阈值变化下稳健保持。


<details>
  <summary>Details</summary>
Motivation: 医学模型基准分数不能证明同一结论在其他评估设定下仍然成立；作者希望检验关于模型排名、分数可靠性与筛查性能的论断能否在队列、提示词、负样本谱、指定患病率与操作阈值变化下保持稳定。

Method: 审计BioMedCLIP、CheXficient、MedSigLIP三个医学视觉语言模型及通用域对照OpenCLIP，使用Montgomery、Shenzhen、TBX11K、VinDr-CXR四个数据集的12200份胸片记录；构建5个固定提示词族，生成244000个模型-图像-提示词分数；进行多重性控制的AUROC比较、替换负样本对照、阈值迁移测试，并以五种子监督源模型验证外部泛化。

Result: 无模型在所有队列与可靠性标准上领先；提示词族变化在48个多重性控制比较中有21个改变AUROC；将健康对照替换为患病非结核对照使四模型AUROC下降0.075–0.306；VinDr-CXR上三个医学模型对结核与无异常对照的区分显著优于对肺炎或肺肿瘤，对两种指定疾病的AUROC点估计均低于0.5；TBX11K训练集上按95%灵敏度选取的阈值在16个目标评估中仅4个按点估计保持该约束；监督源模型TBX11K验证AUROC为0.999，而两个外部队列均为0.629，保守剔除感知重叠候选可缩小但未消除差距。

Conclusion: 这些回顾性、单任务结果表明，判别能力、分数可靠性与阈值保持支持不同的可移植性论断；胸片结核筛查的证据应明确完整的评估规范，而非仅凭checkpoint归因临床可移植性。
...
TL;DR: 本文提出 XCalib——一种无监督的稠密热红外-可见光配准框架，将虚拟针孔相机参数化作为几何约束空间，结合单目度量深度优化相对位姿与内参，并引入归一化边缘相关性（NEC）度量，实现优于无约束稠密光流的对齐精度与时间稳定性。


<details>
  <summary>Details</summary>
Motivation: 在多模态感知任务（图像融合、目标检测、语义分割）中，图像配准是关键预处理步骤。ADAS 场景下可见光（RGB）与红外（IR）相机因光轴不重合、视场差异导致空间错位，产生非均匀视差与视觉重影。传统关键点方法受限于全局单应假设，在动态深度下失效；无约束稠密光流缺乏结构正则化，存在时间不稳定问题。因此需要一种兼具几何约束与稠密跨模态对齐能力的配准方法。

Method: 提出 XCalib 无监督稠密热红外-可见光配准框架。其核心是将虚拟针孔相机参数化严格作为几何约束空间而非绝对度量标定工具：通过优化有效相对位姿与相机内参，并结合预测的单目度量深度，将空间位移的搜索空间限制在物理有效的投影几何范围内，从而以相机参数化作为稠密跨模态形变的隐式正则化。同时提出归一化边缘相关性（NEC），一种面向跨光谱对齐的鲁棒结构相似性度量。

Result: 在多个公开 ADAS 数据集上进行了广泛的定量与定性评估，结果表明 XCalib 在时间稳定性与对齐精度上均优于无约束稠密光流基线方法。

Conclusion: XCalib 通过将相机参数化用作稠密跨模态形变的隐式正则化，弥合了全局单应方法与无约束稠密光流之间的鸿沟，配合 NEC 度量，为 ADAS 热红外-可见光配准提供了更稳定、更精确的无监督解决方案。
...
TL;DR: PointLAM 是一种高效的纯点云 3D 检测架构，通过 Laplacian Point Sampler (LPS) 和 Local Hadamard Aggregator (LHA) 两项创新，分别解决下采样瓶颈与局部建模延迟，结合双向 Mamba 构成 Local Attentive Mamba (LAM) 模块，在 nuScenes 与 Waymo 上以极低计算代价取得与体素方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: LiDAR 3D 检测存在根本性两难：体素方法效率高但牺牲几何量化精度，点云方法保真度高却受限于计算瓶颈。点云架构具体受制于缓慢的下采样策略（如 FPS）和高代价的动态近邻查询（如 k-NN）以及昂贵的连续交互，亟需系统性解决这些低效问题。

Method: 提出 PointLAM 架构，包含两项协同创新：1) Laplacian Point Sampler (LPS)，利用隐式离散拉普拉斯高通滤波与双排序采样 (Doubly Sorted Sampling)，实现快速、结构感知的前景保留；2) Local Hadamard Aggregator (LHA)，用瞬态网格将空间索引与特征表示解耦，并以 Hadamard Gating 机制替代复杂连续交互，实现拓扑感知的注意力调制。将局部门控与双向 Mamba (BDM) 层耦合进行全局序列建模，构成 Local Attentive Mamba (LAM) 模块。

Result: 在 nuScenes 和 Waymo 数据集上，PointLAM 在纯点云检测器中达到有竞争力的性能，可与高度优化的体素方法相媲美，同时仅需其一小部分计算量，并在小目标检测和极端稀疏场景下表现出明显优势。

Conclusion: PointLAM 通过 LPS 与 LHA 两项创新有效缓解了点云方法的下采样与局部建模瓶颈，结合 BDM 全局建模，验证了高效纯点云架构的可行性，在精度与效率间取得良好平衡，尤其擅长小实例检测与稀疏场景。
...
TL;DR: 本文将无监督异常检测(UAD)重构为贝叶斯逆问题,推断导致观测的最可能损坏,并以推断出的损坏参数能量作为概率性异常分数,统一了若干现有方法,并在MVTec AD与脑MRI基准上验证有效。


<details>
  <summary>Details</summary>
Motivation: 传统UAD方法只关注偏离正常分布的标志或定位,忽略了异常本身的生成机制;而异常的性质与其存在同样重要。

Method: 将UAD建模为贝叶斯逆问题,推断每次观测对应的最可能损坏(corruption);以所推断损坏参数的能量作为概率性异常分数,并作为开发新UAD算法的通用范式。通过不同建模选择,可导出多种现有方法为同一能量分数下的实例。

Result: 在受控环境下研究框架各组件;通过调整底层损坏模型,在MVTec AD数据集上目标类别AUROC提升2.3%;在脑MRI基准上取得较强检测性能,同时给出病理强度、偏置和几何的估计。

Conclusion: 贝叶斯逆问题框架为UAD提供了原则性范式,既能给出异常分数,又能刻画异常的生成机制,统一既有方法并具备实际改进与可迁移性。
...
TL;DR: VideoReloc 将长期室内视频重定位重新表述为对空间扩展视频查询的验证，通过对齐里程计生成自适应片段并提出假设优先的配准，仅需 100 kB 的类别标注框地图，在 RIO10 与 ReplicaCAD 上取得 73.5%/61.1% 的因果评估成功率。


<details>
  <summary>Details</summary>
Motivation: 长期室内场景中存在光照与家具变化，基于外观的视觉定位方法不可靠；而逐帧依靠物体类别与几何进行定位又导致证据稀疏、存在歧义。作者希望将判别依据从存储的外观转移到时间上下文中，从而支持极轻量级的语义地图。

Method: VideoReloc 包含三部分：(1) 自适应片段，利用里程计累积空间证据，直到满足物体与运动判据，使查询长度随场景自适应；(2) 片段级决策，在相连片段间累积证据，对冲突位姿进行复检，稳定轨迹；(3) 假设优先配准，由物体三元组提出候选位姿，并用片段范围的物体中心和框表面逐一验证；(4) 方向感知细化，利用框面、重力与墙面方向消解相机朝向歧义并精化完整位姿。

Result: 在 RIO10 和 ReplicaCAD 上，1 m/10° 的全帧定位成功率在因果评估下分别为 73.5% 和 61.1%，加入片段闭合后提升至 90.6% 和 74.8%。对比的逐帧场景坐标回归器仅达 47.6% 和 49.8%，且地图大小为 12.6–42 MB。

Conclusion: 该方法将稀疏地图重定位转化为对空间扩展视频查询的验证，用时间上下文替代存储外观作为判别支撑，在仅 100 kB 的类别标注框地图下显著优于逐帧场景坐标回归方法。
...
TL;DR: 论文指出目标检测基准的质量瓶颈在于标注不完整而非架构，重新标注四大数据集发现漏标显著，提出高召回+软标签的标注流程，并构建两个大规模基准，表明现有检测器与标签错误检测方法在真实标注误差下表现受限。


<details>
  <summary>Details</summary>
Motivation: 目标检测虽在架构与开放词汇模型上取得进展，但基准质量受标注不完整制约；现有标注多为确定性且遗漏大量小目标、遮挡与密集实例，导致评估与人类感知不一致。

Method: 对 COCO、Pascal VOC、Cityscapes、KITTI 四个常用数据集进行重新标注，量化漏标与标签错误来源；提出可扩展标注流程，强调高召回，并通过每目标至少 11 名标注者聚合的软标签刻画歧义；据此构建两个大规模基准：不确定性感知目标检测基准与基于真实标签错误的错误检测基准。

Result: 重新标注使标注目标数量显著增加（KITTI 最高 +60%，COCO +40%），主要来自此前未标注的小、遮挡或密集实例；缺失标注是所有数据集中标签错误的主要来源。基准性能对标注质量高度敏感，但模型排名大体稳定；当前检测器高度依赖标注质量且与人类感知不一致；现有标签错误检测方法虽在合成噪声上表现良好，但在真实标签错误上召回率与精确率较低。
...
TL;DR: 本文提出形态学感知的模糊性学习框架，将晶圆图缺陷识别从固定分类转为支持单类诊断、双类辅助诊断与全复核三种动作。


<details>
  <summary>Details</summary>
Motivation: 传统晶圆图缺陷识别把每片晶圆强制归为单一缺陷类别，但部分晶圆形态处于类别边界附近，单一预测信息量不足，需要提供合理的替代诊断选项。

Method: 利用训练晶圆图的径向、角度与几何特征构建类别级模糊矩阵，表示形态相似、可作为替代诊断的缺陷类别对；据此引导模型学习合理的替代类而非等价看待所有错误类；推理时由该矩阵判断不确定预测能否用有意义的两类诊断集合表示，否则升级为全复核。

Result: 在WM-811K上，该框架在缺陷识别与诊断决策支持方面优于常规方法，能给出有意义的两类替代方案，并将全复核保留给模糊性未解决的样本；示例性成本分析显示该路由策略具有潜在成本优势；诊断行为在不同骨干网络上保持一致。

Conclusion: 形态学感知的模糊性学习框架可有效支持分级诊断决策，兼顾识别性能与成本，且不依赖特定骨干架构。
...
TL;DR: 本文提出 Chronosphere，一种在时空环面 S^2×S^1 上自适应部署可学习站点并共享局部基函数的时空神经场，用于气候表征学习，在空间/时间任务及迁移场景下达到或超越现有位置编码器。


<details>
  <summary>Details</summary>
Motivation: 地理表征学习的核心难点在于环境过程的时空复杂度差异极大，而现有位置编码器普遍在所有位置固定单一细节层级：全球基（如球谐函数）在时空上均匀分配容量，局部基只能覆盖预定义区域，学习型镶嵌虽能自适应但对高频表征效率低。

Method: Chronosphere 统一上述思路，将时空环面 S^2×S^1 上的可学习站点自适应镶嵌与共享的局部基函数库相结合，使容量放置位置与各区域细节量均随数据在空间和时间上自适应调整，并以重建气候态为训练目标。

Result: 在空间与时间任务上，Chronosphere 达到或领先当前最先进的位置编码器，且在空间与时间迁移场景下增益最大。

Conclusion: 自适应时空镶嵌配合共享局部基函数能够有效解决固定细节层级编码器的容量分配问题，提升气候表征的精度与泛化能力。
...
TL;DR: 本文提出一个面向VLM人脸识别的基准框架，将"解释质量"作为核心评估维度，定义相关性（依赖身份稳定的面部特征）与忠实性（与可见图像内容一致、无幻觉特征）两项标准，并通过结构化解释格式实现自动化量化。对多个开放权重VLM家族同时评估人脸验证准确率与解释质量，揭示现有解释的不足。


<details>
  <summary>Details</summary>
Motivation: VLM可同时输出相似度分数与自然语言解释，被视为司法等取证场景人脸比对的有力工具，因为此类场景要求决策透明、可审计。但现有评估多聚焦识别准确率，对生成解释的有效性缺乏量化，无法判断解释是否真正支撑可审计性。

Method: 构建VLM人脸识别的基准测试框架，把解释质量作为核心评估轴。提出解释应满足的两项准则：相关性（依赖身份稳定的面部特征）与忠实性（与可见图像内容对齐、不含幻觉特征）。设计量化方法，通过将模型输出约束为结构化解释格式，支持自动化查询与审计。据此对多个开放权重VLM家族进行基准测试，联合评估人脸验证准确率与解释质量。

Result: 联合评估人脸验证准确率与解释质量后，结果暴露出所生成解释仍存在明显缺陷，说明仅看准确率无法完整刻画模型性能，解释质量指标具有必要性。

Conclusion: 所提出的基准与开源评估工具为可解释人脸识别系统的规范化基准测试及后续微调奠定了基础。
...
TL;DR: Catena是一个面向连接组学的开源、开发者中心型软件套件，通过集成分割、突触检测、微管追踪与神经递质推断等模块，并采用可组合的分块流水线与容器化部署，实现电子显微镜体数据的可复现、可扩展连接组重建。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜密集标注神经组织的体数据是连接组映射的金标准，但神经元树突/轴突重建、校对及突触标注依赖多个软件工具，这些工具往往碎片化、维护不一致或为专有软件，阻碍了可复现性与自动化。作者意在整合这些分散组件，提供统一的开源解决方案。

Method: 提出Catena软件套件，包含3D神经元与细胞器分割、突触检测、微管追踪、神经递质推断等模块；模块组织为可组合的分块（chunk-wise）处理流水线，设计完全文档化、可扩展、可适配；通过预训练机器学习模型降低计算与标注数据需求并支持微调；模块完全容器化以封装依赖，保证跨工作站与集群的一致执行。

Result: Catena整合了开放组件、可共享模型与容器化运行环境，提供从电子显微镜体数据映射细胞连接组的可复现且可扩展的方法；代码与文档见 https://github.com/Mohinta2892/catena.git。

Conclusion: Catena以开源、模块化、容器化和预训练模型为核心，弥补了现有连接组学工具碎片化、难复现的缺陷，为电子显微镜连接组重建提供了可复现、可扩展的统一软件框架。
...
TL;DR: 该论文挑战了现有单木枝叶分割方法为跨传感器兼容性而舍弃辐射特征的做法。通过在Heidelberg（TLS, 1550nm）训练、在加拿大安大略新数据集（RPA-LS, 905nm）测试的跨站点、跨平台实验证明：仅依赖几何特征（包括高密度LiDAR训练的SOTA深度学习模型）在稀疏、俯视的航空扫描几何下泛化失败（F1≤0.56）；引入辐射特征（强度、回波次数、回波数量）将F1提升至0.61，并将木质召回率从0.16提高到0.35（提升119%），同时保持更强的结构连通性，更适合QSM重建。


<details>
  <summary>Details</summary>
Motivation: 单木枝叶分割是定量结构模型（QSM）与非破坏性生物量估算的关键步骤。现有方法普遍剔除辐射特征（强度、回波数等）以追求跨传感器兼容性。本文质疑这一设计选择，旨在验证辐射特征在跨站点、跨平台泛化中的实际价值，以及几何特征因视角依赖而易过拟合扫描模式的局限性。

Method: 采用跨站点、跨平台泛化评估：在公开Heidelberg数据集（地基TLS，1550nm，高密度）上训练，在一份来自加拿大安大略省的新数据集（RPA-LS，905nm，稀疏、俯视的航空扫描几何）上测试。对比仅几何特征方法与引入辐射特征（intensity、return number、number of returns）的方法，并分析分割结果的结构连通性与碎片化程度。

Result: 仅几何特征方法（含高密度LiDAR训练的SOTA深度学习模型）无法泛化到稀疏、俯视的航空扫描几何，F1≤0.56；加入辐射特征后F1提升至0.61；更关键的是木质召回率由0.16提升至0.35，增幅119%。此外，仅几何方法常导致茎干与枝条组件破碎，而辐射特征能保持更强的结构连通性，得到更连贯的架构，更利于QSM重建。

Conclusion: 几何模式具有视角依赖性，易过拟合扫描模式；而辐射特征编码了物理材质属性，能够在不同传感器与环境间泛化。因此，在单木枝叶分割及QSM重建中应重新纳入辐射特征，而非为跨传感器兼容性而舍弃它们。
...
TL;DR: 提出 Info3R，一种信息自适应的在线 3D 重建测试时训练方法，通过信息感知状态更新与动态状态重置缓解长序列性能退化。


<details>
  <summary>Details</summary>
Motivation: 现有基于 Transformer 的在线 3D 重建方法在处理长图像流时忽略两个关键信号：每个输入帧的重要性，以及模型内部状态的信息饱和问题，导致长序列性能退化。

Method: 引入信息感知状态更新，依据输入帧的冗余度与信息量调节状态更新强度；提出动态状态重置，由状态更新的累积幅度与模型预测置信度触发，并配合锚点到世界坐标的对齐。

Result: 在相机位姿估计、视频深度估计和 3D 重建上取得一致提升，显著缓解长序列评估中的性能退化；在 KITTI Odometry 上，ATE 平均比 LongStream 低 1.68 倍。

Conclusion: Info3R 通过信息自适应测试时训练有效提升在线 3D 重建在长序列上的鲁棒性，尤其适用于扩展的户外序列场景。
...
TL;DR: 该论文提出PRIME，一种学习式反馈机制，通过情境记忆将导航目标与推理信息注入感知查询，实现意图驱动的感知注意力，参数增量极小（0.41%），在Bench2Drive闭环基准上取得SOTA驾驶得分82.47与60.00%成功率。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶VLA模型主要采用感知—推理—规划的前馈推理范式。虽然现代架构在感知模块内保留时序递归，但早期感知对下游推理与导航目标"视而不见"，处理视觉输入时缺乏目标导向，无法优先关注由先前决策所提示的关键线索，形成感知与决策目标之间的割裂。

Method: 提出PRIME——一种学习式反馈机制。构建"情境记忆"（Situational Memory），通过交叉注意力聚合过去L步窗口内感知、推理、导航目标与预测行为的潜在表示，并以此条件化VLA的感知查询，从而以极小计算开销实现意图驱动的感知注意力。额外参数最多29.7M，仅占7.3B基础模型的0.41%。

Result: 在Bench2Drive闭环基准上，驾驶得分（Driving Score）达82.47，较ORION提升4.73；成功率（Success Rate）达60.00%，提升5.38个百分点。该得分为已发表、基于Think2Drive示范训练的VLA中最高。

Conclusion: PRIME通过情境记忆将下游推理与导航意图反馈至早期感知，有效弥合前馈VLA中感知与决策目标间的鸿沟，以极低的参数与计算代价显著提升闭环驾驶性能，验证了意图驱动感知注意力的有效性与高效性。
...
TL;DR: 基于YOLOv2的交通标志检测与分类系统，通过分支预测层实现早期退出以降低计算量，并引入无监督贝叶斯分割得到的几何信息以减少相似标志的分类错误。


<details>
  <summary>Details</summary>
Motivation: 交通标志识别需同时完成标志定位与语义分类，且要求高效；现有方法在易样本上计算冗余，且视觉相似标志易被误分类。

Method: 以YOLOv2为基干，做两类改进：(1) 增加中间预测层构成分支架构，支持整图与按单元格分支策略，易例可提前终止推理；(2) 用无监督贝叶斯图像分割生成二值表示，在YOLOv2边界框内与类别几何模板比对，可在推理阶段使用或作为训练附加信号。数据由GTSDB与GTSRB经无缝克隆与受控图像变换合成。

Result: 10类交通标志、3000训练/300测试样本。选定分支架构运行时0.647 s、mAP 0.680；基线YOLOv2为0.6607 s、mAP 0.680。推理阶段几何验证将mAP提升至0.713；几何特征训练变体mAP 0.697，运行时0.6608 s。

Conclusion: 分支架构可在不损失精度下缩短推理时间，几何信息（尤其推理阶段验证）能显著提升mAP，但训练变体收益较小且略增开销。
...
TL;DR: 本文提出OmniVBench基准与Omni-R2V数据集，以评估和训练全能参考到视频（omni R2V）生成模型。


<details>
  <summary>Details</summary>
Motivation: 现有R2V基准测试的参考类型与组合有限，评估协议多针对整体参考一致性，忽视了参考因子是否被正确保留、解耦与路由；同时全能R2V训练数据构建成本高、资源稀缺。

Method: 构建OmniVBench，覆盖7个任务族、18个细粒度任务，涵盖内容、运动、风格、结构、叙事与多参考设置，并引入基于因子的评估，含12,172条案例特定清单项；同时构建Omni-R2V数据集，基于大规模专业视频语料，含340K处理后的训练样本，并提供任务特定的参考-目标配对构建流程。

Result: 对先进开源与闭源R2V模型的广泛评估显示，各任务族与评估维度上存在明显性能差距，揭示当前模型的局限。

Conclusion: OmniVBench与Omni-R2V数据集为全能R2V生成提供了更全面的评估与可扩展的训练资源，推动该领域发展。
...
TL;DR: MintAct 是一系列 2B/4B/8B 规模的视觉-语言模型，统一了 UI grounding、跨移动/桌面/Web 的多步导航与视觉工具使用，性能匹敌各领域专用模型，并在 OSWorld-Verified 上取得 48.9 的 SOTA 成绩。


<details>
  <summary>Details</summary>
Motivation: 现有 UI 智能体能力通常由各领域专用模型分别实现，缺乏统一的跨移动、桌面、Web 的多能力模型；同时训练此类模型需要可扩展的环境与 RL 基础设施来支撑大规模轨迹采集与在线训练。

Method: 统一训练 UI grounding、多步导航、视觉工具使用三类能力，覆盖 2B/4B/8B 规模；精心设计环境、数据与训练配方；构建可扩展的环境与 RL 基础设施：托管跨异构领域后端的数百个并发实例，用于轨迹采集和在线 RL；采用异步 RL 框架，显式控制跨域训练数据分布，并在噪声环境反馈与 off-policy 漂移下保持稳定。

Result: MintAct 在可比模型规模下于广泛基准测试中达到 SOTA，其中 OSWorld-Verified 得分为 48.9，且在各能力上匹配领域专用模型的性能。

Conclusion: 通过统一的环境、数据与训练配方以及可扩展的异步 RL 基础设施，单一模型家族可同时胜任 UI grounding、跨平台多步导航与视觉工具使用，无需为每个领域单独训练专用模型。
...
TL;DR: 提出训练自适应的卷积稀疏编码框架，用FISTA展开并将稀疏系数作为可微变量联合学习，从信息瓶颈视角平衡信息保留与压缩，并引入无标签后训练策略，在CIFAR与ImageNet上实现竞争性干净精度与显著提升的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视觉信号需要紧凑但充分的表示以支撑鲁棒的下游预测。卷积稀疏编码（CSC）能显式抑制冗余并保留信号内容，但其稀疏系数通常固定且需人工设定，限制了自适应性与鲁棒性。

Method: 用快速迭代收缩阈值算法（FISTA）展开CSC优化；将稀疏系数视为可微变量，与网络参数联合学习。从信息瓶颈视角，该系数控制信息保留与压缩的权衡：稀疏项促进紧凑表示，重构项与任务损失共同保留任务相关信息。此外引入无标签后训练策略，在固定主网络参数的情况下，针对受扰输入调整压缩强度。

Result: 在CIFAR和ImageNet数据集上的实验表明，该方法在干净数据识别上具有竞争性表现，并在不同输入扰动下大幅提升鲁棒性。

Conclusion: 通过将稀疏系数纳入可学习框架并结合信息瓶颈与无标签后训练，实现了训练自适应的卷积稀疏编码，兼顾紧凑表示与任务相关性，在精度与鲁棒性之间取得良好平衡。
...
TL;DR: 该论文提出AnyviewMeter，一个几何条件化的机器人奖励模型适配框架，通过将相机几何（Plücker射线）与同步块注意力结合低秩微调，实现单视图与多视图奖励预测，在仿真与真实任务中显著降低进度预测误差。


<details>
  <summary>Details</summary>
Motivation: 机器人奖励模型从视觉观测评估任务执行，但其预测会随相机视角和遮挡变化而改变，即便任务状态本身未变。因此，将预训练奖励模型适配到特定任务时，必须考虑该任务的观测方式（相机几何）。

Method: 提出AnyviewMeter，将任务进度表示为标量奖励信号；结合低秩微调（LoRA）与令牌对齐的Plücker射线及同步块注意力：射线条件将相机几何注入视觉特征以及注意力的查询与键，块注意力则在预训练解码器内融合同步视图。框架支持单视图奖励预测与联合多视图评估，通过参数高效方式适配预训练的Robometer模型。

Result: 在PickCube上，单视图适配在每个相机组均提升进度预测，视场变化下MAE相比RGB微调降低约21%。在仿真操作任务中，联合多视图预测相比平均单视图RGB预测降低进度误差41-69%，并在约88%的任务-相机组中改善时间排序。在固定相机与腕部相机的真实任务中，MAE相比平均RGB微调降低约21%。
...
<details>
  <summary>Details</summary>
Motivation: 现有放射报告生成模型通常仅基于单次检查或最近一次既往检查，难以进行有效的纵向对比和细微间隔变化检测；已有尝试虽引入多次既往检查，但多采用固定长度历史聚合，未在融合前显式建模各既往检查的角色依赖相关性。

Method: TALON（Temporally Aware LONgitudinal RRG）框架，其核心为双通道时序融合模块（DCTFM）：通过互补的相似性通道与变化通道，将当前检查与每个既往检查进行对比，分别捕捉持续征象与间隔变化；并设计通道特定注意力估计各既往检查的相关性，配合可学习的既往特异性门控，自适应整合有信息的纵向证据并抑制冗余。

Result: 在MIMIC-CXR数据集上，TALON在多项临床效能指标与基于图的指标上优于当前最先进方法；当可用既往检查增多时，其性能进一步提升。

Conclusion: TALON的DCTFM在建模纵向RRG时能有效处理比现有方法更长、更复杂的患者病史，验证了自适应变长历史融合的优越性。
...
### [78] [Curriculum-Based Noise Adaptation for Phoneme-to-Text Reconstruction in Visual Speech Recognition](https://arxiv.org/abs/2609.20839)
*Matthew Kit Khinn Teng,Haibo Zhang,Takeshi Saitoh*

Main category: cs.CL

TL;DR: 提出渐进式错误课程训练（PECT），通过逐步引入愈发真实的音素预测错误来增强音素到文本重建模型的鲁棒性，从而提升音素中心视觉语音识别性能。


<details>
  <summary>Details</summary>
Motivation: 音素中心视觉语音识别的整体性能高度依赖音素到文本重建模型的鲁棒性。现有重建方法多基于干净音素序列或人工合成噪声训练，导致训练条件与推理时真实音素预测错误之间存在失配。

Method: 提出渐进式错误课程训练（PECT）课程学习框架，基于NLLB构建音素到文本重建模型，依次使用合成音素扰动、多域伪标签以及由视觉语音识别器生成的目标域伪标签，渐进式地让重建模型接触越来越真实的音素预测错误。

Result: 在LRS2与LRS3基准上，PECT对多种音素前端（V-ASR、PV-ASR、HP-VSR）均带来一致的性能提升：HP-VSR-FiLMFuse（L4）在LRS2上WER由23.3%降至22.2%；HP-VSR-ResFiLM在LRS3上WER由30.3%降至29.7%。消融实验与定性分析进一步验证了方法的有效性。

Conclusion: PECT为音素中心视觉语音识别中的音素到文本重建提供了一种有效且可泛化的课程学习策略，通过渐进适应真实音素预测错误，在保持句子重建精度的同时提升鲁棒性。
...
### [79] [MME-Safety: A Fine-grained Benchmark for Safety Evaluation of MLLMs](https://arxiv.org/abs/2609.20850)
...
TL;DR: 本文提出 MME-Safety，一个具有四维标注体系（风险场景、危害严重度、模态特定隐蔽性等）的多模态大模型安全基准，并配套分层评估框架，对17个先进MLLM进行零样本评测，揭示跨模态输入配置与思维链推理的安全影响。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的跨模态能力带来易绕过单模态过滤器的复合脆弱性；现有基准缺乏细粒度的意图相关标注，且依赖单一维度指标，难以全面评估鲁棒性。

Method: 提出 MME-Safety 基准，包含经严格验证的四维标注范式，对风险场景、危害严重度及模态特定隐蔽水平进行分类；同时提出分层评估框架，从基础响应可靠性、实际风险暴露和防御行为结构性完整性三个层面进行评价。

Result: 对17个当前最先进 MLLM 进行广泛零样本评测，得到多模态系统安全画像；系统分析跨模态输入配置，并揭示与思维链（CoT）推理相关的安全影响。

Conclusion: 多维发现表明多模态场景亟需具备推理感知能力的安全对齐机制。
...
### [80] [Omni Demand Understanding: A Benchmark for Contextual User-Intent Inference in Multimodal Interaction](https://arxiv.org/abs/2609.21392)
...
TL;DR: 本文提出 Omni Demand Understanding (ODU) 这一多模态上下文推理任务，并构建 ODU-Bench 基准，从五个维度评估模型能否从语音、视觉与对话历史中推断用户潜在需求及判断需求是否存在。评测 14 个原生 MLLM 显示存在系统性能力缺口。


<details>
  <summary>Details</summary>
Motivation: 现有交互能力基准主要关注回答质量，忽视了更根本的问题：模型能否从复杂的多模态交互中正确推断用户潜在需求。真实需求常在语音中表述不完整，需借助多模态线索与对话历史推断，并受表达歧义、不流畅及噪声环境影响；同时请求式语音未必构成真实需求，易导致误触发。该能力此前未被系统评估。

Method: 将 ODU 定义为独立的多模态上下文推理问题：给定交互流，模型需检测是否存在用户需求，并从多模态与对话上下文推断意图。从五个维度进行评估，覆盖单轮与多轮交互。通过挑战驱动的分类法、分类法引导的智能体视频生成及真人录制交互构建 ODU-Bench，并进行媒体对齐标注与人工验证。评测 14 个原生 MLLM。

Result: 最强模型 Gemini 3.1 Pro 仅能恢复 44.7% 需从视觉、声学或对话上下文推断的关键信息；14 个模型中有 11 个在非需求场景下的误触发率超过 50%。

Conclusion: 结果揭示当前 MLLM 在推断上下文用户需求方面存在系统性能力缺口。作者希望 ODU 能建立对这一多模态交互中此前未被充分探索却至关重要的能力的评估，即在生成恰当回复之前先正确理解用户需求。
...
### [81] [Evaluating In-Context Learning and Retrieval Strategies for Devanagari Post-OCR Correction](https://arxiv.org/abs/2609.21595)
*Abhishek Bhandari,Gaurav Harit*

Main category: cs.CL

TL;DR: 本文首次系统评估了3B-32B规模LLM在印地语和马拉地语（天城文）上的免训练OCR后校正，提出基于字符n-gram BM25相似度的CharBM25示例检索策略，在2万句基准上验证其显著优于域随机与稠密检索。


<details>
  <summary>Details</summary>
Motivation: 利用LLM的上下文学习实现免训练OCR后校正颇具前景，但其在天城文（Devanagari）上的有效性尚未被探索；同时，少样本示例的检索策略对校正质量的影响缺乏系统研究，且缺乏相应基准。

Method: 构建覆盖5个新闻领域的2万句基准；对3B-32B的多个LLM在印地语和马拉地语上评测；对比三种上下文示例检索策略：域随机选择、稠密语义检索，以及提出的CharBM25（在OCR输入上以字符n-gram BM25相似度检索示例，以匹配与测试句共享的错误模式）；并比较不同n-gram阶数（1/2/3）。

Result: 检索策略是校正质量的决定性因素：CharBM25以字符三元组（优于二元、一元）相对域随机选择在印地语上绝对WER降低2.8-4.0pp、马拉地语上2.9-3.8pp；模型规模主导性能，Gemma-3-27B在CharBM25-5下WER相对下降印地语55.0%、马拉地语33.3%；少样本增益受容量门控，小于8B的模型无法可靠优于OCR基线，马拉地语上最小的3B模型使更多句子变差；马拉地语因形态复杂度更高而始终比印地语更难校正。

Conclusion: CharBM25是一种有效且无需GPU的检索策略，以可忽略的计算成本达到或超过稠密检索；与12B+参数的通用LLM结合，可在无需任务特定微调的情况下实现可靠的天城文免训练OCR后校正。数据集已公开。
...
### [82] [Enhancing Audio Reasoning via Semantic Summary Prediction](https://arxiv.org/abs/2609.20849)
*Francesco Bonzi,Pooneh Mousavi,Cem Subakan,Mirco Ravanelli*

Main category: cs.CL

TL;DR: 该论文提出SPARE方法，通过一个与结论对齐的register token在推理前为模型注入目标语义，缓解大型音频语言模型在显式CoT下推理性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 大型音频语言模型虽在复杂问答上表现良好，但存在推理缺口：显式思维链（CoT）反而比直接作答准确率更低。作者假设原因是长推理序列使得注意力偏离音频输入。

Method: 提出SPARE（Semantic Prediction for Audio REasoning）。引入一个register token，用Sentence-BERT嵌入配合余弦相似度损失，使其与最终结论对齐，从而在推理开始前就条件化模型的潜在空间，注入目标语义。

Result: 在MMAU与MMAR基准上，使用SALMONN模型进行实验，零样本推理性能提升，且对音频的早期注意力更强，同时不增加额外推理开销。

Conclusion: 通过对齐结论的register token预先引导语义目标，可有效改善音频语言模型在CoT下的推理表现与音频关注度，且无额外推理成本。
...
### [83] [Per-Aetiology Contrastive Severity Embeddings with Phonological Pseudo-Labelling for Multilingual Dysarthric Speech](https://arxiv.org/abs/2609.21789)
*Bernard Muller,Antonio Armando Ortiz Barrañón,LaVonne Roberts*

Main category: cs.CL

TL;DR: 该研究用四个匹配的HuBERT-base对比嵌入模型进行受控比较，发现按病因分别建模优于混合病因基线，且伪标签数据可进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有多语言构音障碍严重度系统要么仅在单一"病因-语言"对上训练，要么将异质病因合并到一个标签空间。作者通过受控实验检验"合并病因"这一假设是否成立。

Method: 在共享骨干、训练配方、语料注册表和留出评估下，训练四个匹配的HuBERT-base对比嵌入模型：一个混合病因基线，及脑瘫（CP）、帕金森病（PD）、肌萎缩侧索硬化（ALS）三个病因专属模型。训练结合临床标注语音与来自免训练音系画像方法的序数伪标签。

Result: 在说话人不相交、去泄漏的留出子集上，各病因专属模型全面优于混合基线：CP（宏F1 0.829 vs 0.676，相对+22.6%）、PD（0.715 vs 0.511，+40.0%）、ALS（0.788 vs 0.596，+32.3%）。CP上加入144名SAP与44名CDSD伪标签说话人后，宏F1由0.786升至0.829（+4.3个百分点）。各病因训练数据覆盖三至七种语言。

Conclusion: 本研究将结果定位为对标签空间设计选择的受控比较，并指出伪标签校准、划分卫生（split hygiene）和基于置信度阈值的部署是未来工作的重要局限。
...
TL;DR: 该论文提出COPES数据集与三轴评估框架，评估LLM在心理健康同伴支持中与社区视角的对齐程度，发现后训练（SFT、DPO）能显著提升策略与情感语调和情绪对齐，但改进异构且引发策略分布偏移。


<details>
  <summary>Details</summary>
Motivation: 专业心理医疗资源有限，许多人转向Reddit等在线平台寻求基于真实生活经验的同伴支持，但大量求助帖无人回应。LLM虽有潜力填补这一空白，其在临床基准上表现良好，却缺乏对"基于生活经验、契合社区规范"的同伴支持生成能力的探索。

Method: 构建COmmunity-centered Peer Engaged Support（COPES）数据集，并提出三轴评估框架，用以衡量LLM在面对心理健康求助时与社区视角的对齐程度。评估对象包括零样本模型以及经SFT和DPO后训练的模型。

Result: 在COPES上后训练可使通用模型策略对齐提升50%以上，并改善情感与语调对齐；但改进呈异构性，不同子版块（subreddit）与不同应对策略上的对齐提升差异显著。此外，后训练引入分布偏移，过度偏向问题导向型建议而抑制情绪导向型策略。

Conclusion: 基于社区数据精选可提升LLM响应的对齐性，但模型表现在不同子社区与具体心理健康需求间仍不均衡，未来需针对多元社区与需求进一步优化。
...
TL;DR: 构建12000条双选项道德困境数据集（含印、阿、西、中四种翻译），评测LLM跨语言价值偏好，并借助任务向量正交化方法分离特定价值偏好方向。


<details>
  <summary>Details</summary>
Motivation: LLM在价值冲突场景中表现出隐性偏见与跨语言指令遵循脆弱性，需要可探测其价值观取向及跨语言一致性的基准与方法。

Method: 构建覆盖三组两两价值冲突（诚实vs公正、公正vs自主、自主vs诚实）的12000条双选项困境数据集，并翻译为印地语、阿拉伯语、西班牙语、中文；在GPT-5-mini与Llama-3.2-1/3B上基准测试；对Llama采用普通微调与DPO微调；提出任务向量迁移实验，将价值偏好方向的任务向量对通用指令遵循向量做正交化处理。

Result: GPT-5-mini在五种语言下无策略给定均一致偏好诚实高于自主；Llama-3.2-1/3B存在明显首选项偏见，普通微调与DPO微调均可消除该偏见，准确率提升至98%以上；正交化方法能有效分离出特定价值偏好方向，并可通过任务算术得到立场相反的模型。

Conclusion: 该工作提供了跨语言道德困境基准，揭示模型价值偏好的一致性偏见，并证明任务向量正交化可解耦数据相关性与抽象价值，实现价值取向的可控编辑。
...
TL;DR: 本文提出以"产出质量/交互成本"为核心的人机协作生产力评估框架，发现同等质量会话的交互成本可相差70倍，且质量—成本关系因任务而异，主观评分无法替代生产力度量。


<details>
  <summary>Details</summary>
Motivation: 现有AI生产力度量（任务完成时间、经济价值、结果质量提升）将协作视为黑箱，只关注产出什么，而忽略了产出过程中的交互成本，导致"高成本的成功"与"高生产力的成功"被混为一谈。

Method: 借鉴经济学文献，构建以结果质量相对于交互成本的生产力导向框架；在两个数据集、四项任务上对会话进行质量与交互成本联合分析，并对成功与失败会话的对话行为（如智能体探询时机、用户修复投入）进行对比。

Result: （1）质量评分相同的会话，交互成本差异可达70倍；（2）质量—成本关系随任务变化，部分任务受益于延长交互，部分任务偏好快速收敛；（3）主观用户评分不能可靠替代生产力指标；（4）高生产力会话中智能体更早进行探询，用户耗费更少精力修复交互。

Conclusion: 通过区分"高生产力的成功"与"高成本的成功"，该框架使交互成本可见，并表明对话分析可为AI系统的评估与设计提供依据。
...
### [87] [Hallucination-R1: Robustness-Oriented Paraphrase Generation for Factual Consistency](https://arxiv.org/abs/2609.21227)
*Wenhan Yu,Wenxin Wu,Hao Wang,Lei Sha*

Main category: cs.CL

TL;DR: 提出HALLUCINATION-R1框架，通过生成语义保真但能暴露模型事实不稳定的改写句，研究改写诱导的幻觉问题，并在多个数据集上验证其一致性与多样性权衡及鲁棒性训练价值。


<details>
  <summary>Details</summary>
Motivation: 模型对同一事实问题在原句下回答正确，但语义等价的改写句下却回答错误，这种不一致揭示了语义不变条件下的潜在事实不稳定。而通用改写句作为鲁棒性监督信号不足：近似抄袭的改写信号弱，过度多样的改写又可能破坏语义等价。

Method: 提出HALLUCINATION-R1，一个面向鲁棒性的改写生成框架。采用两阶段优化：首先稳定语义保真且多样的改写生成，然后奖励那些能在下游问答模型中暴露事实一致性退化的改写句。

Result: 在SimpleQuestions、PopQA和TruthfulQA上，HALLUCINATION-R1实现了一致性与多样性之间的良好权衡，并在多种模型家族和数据集上暴露出鲁棒性失败。分析表明这些失败并非表层伪影或语义漂移所致，而是语义保持变化下的事实不稳定。轻量微调研究显示，其生成数据能在改写变化下提升鲁棒准确率。

Conclusion: HALLUCINATION-R1生成的改写句能有效暴露并缓解模型在语义等价变化下的事实不稳定，对鲁棒性导向训练具有实用价值，代码与模型已公开。
...
### [88] [From Memory to Behavior: A Behavior-Aware Role-Playing Framework for Social Media Influencers](https://arxiv.org/abs/2609.21349)
*Ji-Lun Peng,Yi-Zhen Zhang,Chun-Nan Chou,Yun-Nung Chen*

Main category: cs.CL

TL;DR: 本文提出"情境-内部状态-行为（SIB）人格"方法，将情境依赖的行为策略纳入角色扮演，并设计配套评测协议，在真实个体社媒回复生成任务与虚构角色基准上均优于现有ICL基线。


<details>
  <summary>Details</summary>
Motivation: LLM作为真实个体的角色扮演代理潜力巨大，但忠实模仿仍具挑战：现有基于上下文学习（ICL）的方法无法刻画个体在不同情境下的反应差异；同时针对冷门/不知名个体的LLM自动评测也缺乏可靠依据。

Method: 提出情境-内部状态-行为人格（Situation–Internal state–Behavior Persona）方法，显式建模情境依赖的行为策略；并设计评测协议，为LLM评测者提供关于被模仿个体的参考信息；同时构建了面向社交媒体回复生成任务的新数据集。

Result: 在新建数据集上，所提方法优于当前最先进的ICL基线；评测协议与人工判断达到中等程度相关性；在虚构角色基准上的实验表明该方法可迁移至社交媒体之外的场景。

Conclusion: 引入行为信息能广泛提升真实个体（社交媒体场景）及虚构角色角色扮演的保真度。
...
### [89] [Consistent Relexicalization of Clinical Documents using Graph-Based Approach](https://arxiv.org/abs/2609.21387)
*Dipankar Das,Atri Mandal,Sandeep Singh,Tushar Shandhilya*

Main category: cs.CL

TL;DR: G-RELIC 结合大语言模型与图结构，通过图映射机制保证实体一一对应、并用确定性时间重定位算法维持时间一致性，显著提升临床文本重词化数据的关系完整性与时间连贯性。


<details>
  <summary>Details</summary>
Motivation: 临床 NLP 中重词化（relexicalization）可对敏感信息进行掩蔽并合成高保真数据集，但现有方法多采用独立实体替换，导致纵向病历中出现临床不一致，破坏结构完整性、关系连贯性与时间一致性，降低数据下游分析价值。

Method: 提出 G-RELIC（Graph Based Contextual Relexicalization with Improved Consistency），将 LLM 与图结构结合：(1) 基于图的映射机制，优化原始实体与替代实体之间的一一对应关系；(2) 确定性时间重定位算法，保持时间一致性。

Result: 在多样化真实临床数据集上显著优于现有最优基线：关系完整性提升 30.4 个百分点（62.1%→92.5%），时间连贯性提升 45.9 个百分点（46%→91.9%），且未损害临床数据集的隐私基准。

Conclusion: G-RELIC 在保持隐私保护水平的同时最大化重词化数据集的分析效用，兼顾再识别风险的最小化，为临床 NLP 数据合成提供更可靠的方案。
...
### [90] [Talking Past the Machine: Morality, Politeness, and Alignment in Human-AI Dialogue](https://arxiv.org/abs/2609.21401)
*Marina Mitiaeva,Lu Xiao*

Main category: cs.CL

TL;DR: 该研究对比15,881段人-ChatGPT与10,784段人人多轮对话，发现AI只复制了合作性交流的表层特征，却缺乏支撑人际适应的社会架构，且部分合作机制在AI情境下发生方向逆转。


<details>
  <summary>Details</summary>
Motivation: 对话式AI能生成流畅、社交得体的回应，但其究竟是在进行真正的合作性交流，还是仅模拟其表层形式尚不明确；这一问题直接关系到此类系统如何被评估、信任与设计。

Method: 基于15,881段人-ChatGPT与10,784段人人多轮对话，聚焦道德、礼貌与对齐三个合作性对话核心维度，采用混合效应模型识别预测逐轮对齐（turn-to-turn alignment）的特征。

Result: 观察到一致性的分离：AI产出合作交流的表层特征却缺少底层社会架构；道德输出似为预配置而非协商生成；温暖感在缺乏面子敏感性的情况下被生成；语言趋同持续下降。合作机制本身发生方向逆转：人际间促进顺从的模糊限制与软化措辞，在AI产出时反而与对齐降低相关；人际间导致分歧的纯洁性框架，在AI情境下却伴随用户向AI趋同。能动性（给予用户塑造交流的空间）是两类互动中对齐最稳定的预测因子；较新模型道德断言性降低并未带来更好的合作。

Conclusion: AI复制了合作的表层，却缺少支撑人际合作的相互适应；更令人意外的是，维系人际顺从的机制在AI情境下可能反向运行，说明仅从逐轮视角可能不足以判断互动层面的成功。
...
### [91] [Steering LLMs Responses Towards Moral Foundations on the Norwegian MFQ-30](https://arxiv.org/abs/2609.21636)
*Hans Andersen,David Dichas*

Main category: cs.CL

TL;DR: 将挪威语MFQ-30施测于6个开源LLM并与1282名挪威人比较：半数模型真正参与作答，半数输出趋中而无项目敏感性；无分布信息的中性北欧人设使参与模型向人类均值靠近44-77%（Mahalanobis d²），单对ActAdd仅压平剖面，且人设可同时诱发原本缺失的作答参与。


<details>
  <summary>Details</summary>
Motivation: 近期研究用人类心理测量问卷为LLM elicits道德与价值画像，但尚不清楚这些工具是否测量模型中任何稳定的东西，也不清楚所得画像能否被推向目标人类群体分布；本文旨在检验其测量稳定性与可引导性。

Method: 对6个open-weight LLM施测挪威语版MFQ-30，与N=1282挪威受访者样本对比基础画像；设置attention check判定模型是否真正参与；实施两类steering干预：prompt层面的persona steering与activation层面的ActAdd（固定中层、单对向量）。

Result: 半数模型通过attention check并参与问卷，另半数默认输出flat/central-tendency结果，均值上近似人类但不追踪item内容；不含任何人类样本分布信息的中性Nordic-respondent persona使参与型模型在Mahalanobis d²上向挪威均值靠近44-77%；固定中层单对ActAdd并未steering单个foundation，而是flatten整体foundation profile；至少一个模型中同一persona在推移画像的同时诱发了基线时缺失的engagement。

Conclusion: LLM的心理测量画像部分依赖于伪作答与分布拟合，而非稳定特质；persona steering可在无分布信息下有效对齐群体均值，但也会同时诱发engagement变化，构成Peereboom et al. (2025)所警示的cognitive phantoms的具体实例，提示此类问卷在模型上并非纯粹的特质测量工具。
...
### [92] [Chinese Competitive Debating Dataset and Benchmark](https://arxiv.org/abs/2609.21637)
*Zongrui Yang,Haoyuan Li,Zhongsheng Wang,Zhirui Zeng,Pengqian Han,Yi Zhou,Yuting Wang,Jiamou Liu*

Main category: cs.CL

TL;DR: 该论文构建了一个中文竞技辩论数据集与基准，涵盖比赛、阶段、辩手三个层级，用于评估大语言模型对互动式论证的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有数据集很少将细粒度辩论转写文本与真实比赛中在统一评分量表下收集的专业评判相结合，而辩论裁决需要对论证在互动中如何发展进行追踪。

Method: 组织182场比赛并招募120名专业评委，每场比赛由三名评委依据预设量表独立裁决；剔除记录不完整比赛后，数据集包含148场比赛、2698个阶段和20542个交流单元，并保留人工核验的转写与切分、原始阶段分数、比赛投票、最佳辩手票及裁决理由；定义胜方倾向预测、阶段分数预测和最佳辩手预测三项任务，并对多个大语言模型进行零样本评估。

Result: 零样本评估中，最高胜方预测准确率为66.2%，模型阶段分数与人类平均评分的最高皮尔逊相关系数为0.250，最佳辩手预测最高准确率为56.8%。

Conclusion: 该数据集与基准为研究大语言模型对互动式论证的理解及其与专业评委的一致性提供了测试平台。
...
### [93] [When Steering Fails in Latent Reasoning: A Latent-to-Language Transition Gap](https://arxiv.org/abs/2609.21662)
*Gaoxiang Huang,Lei Qi*

Main category: cs.CL

TL;DR: 激活引导在显式CoT有效，但对潜空间连续思维（latent CoT）效果显著更弱，即使表征位移量相当；作者提出"潜空间到语言转换鸿沟"假说并给出两项证据。


<details>
  <summary>Details</summary>
Motivation: 激活引导已成为控制语言模型显式链式推理（CoT）的常用手段，研究者希望将其扩展到潜空间CoT。然而作者发现，对连续思维进行引导时，其对后续语言生成的影响远弱于对显式CoT的引导，这一现象需要解释。

Method: 首先验证任务信息在连续思维中依然可被识别；据此提出"latent-to-language transition gap"假说，即潜空间中的干预效果无法传递到语言生成。随后通过两项分析检验该假说：(1) 在转换边界处输出分布发生突变；(2) 任务相关方向在latent CoT中的双向控制能力远弱于显式CoT。

Result: 任务信息在连续思维中仍可识别；输出分布在潜空间到语言的转换边界处呈现突跃变化；任务相关方向对latent CoT的双向控制效力明显低于显式CoT，即便隐表征位移幅度相当。

Conclusion: 这些结果支持"潜空间到语言转换鸿沟"假说，表明转换界面是评估与设计未来潜空间引导方法的核心目标。
...
### [94] [CIBuzzBench: A Benchmark for Cross-Lingual Understanding of Chinese Internet Buzzwords](https://arxiv.org/abs/2609.21722)
*Yifan Wang,Junyu Lu,Qifan Wang,Shun Zhang,Chaozhuo Li,Jiahao Liu,Zhijun Cao,Lingbin Bu,Fanliang Bu*

Main category: cs.CL

TL;DR: 本文提出首个中英跨语言中文网络流行语理解基准CIBuzzBench（3,001条，含英文释义、英文对应词、类别与有害性标注），设计三个评测任务，发现现有LLM在跨语言理解上仍存在明显困难。


<details>
  <summary>Details</summary>
Motivation: 中文社交媒体网络流行语含义多非字面，且深植于本土文化与语用语境。既有研究多局限于中文语境下的解读，LLM能否将这类文化负载知识跨语言迁移并准确以英文传达其本意尚不清楚；该跨语言能力对安全也至关重要，因为有害表达可能借助谐音、委婉、反讽或编码语言掩盖其攻击性。

Method: 构建CIBuzzBench基准，包含3,001条中文网络流行语，标注英文释义、英文对应词、类别标签与有害性标签；基于标注设计三个任务：含义解释、跨语言对应词匹配、文化根植的有害性检测；在英文与中文两种提示设置下评测代表性先进闭源LLM与中文LLM。

Result: LLM在中文网络流行语的跨语言理解上仍普遍表现不佳，尤其体现在细粒度非字面释义、选项扰动下对应词匹配的稳健性，以及有害性检测的校准性方面。

Conclusion: 研究揭示了文化根植语言现象对多语言LLM及安全导向评测的持续挑战。数据集与代码已开源于 https://github.com/SuperYFan/CIBuzzBench。
...
### [95] [Do Personality-Tuned LLMs Make Better Social Agents?](https://arxiv.org/abs/2609.21857)
*Tim Krabbe,Xiaodan Shi*

Main category: cs.CL

TL;DR: 该研究探究人格感知微调能否缩小LLM在社交仿真中与人类的"异质感"，结果发现微调模型在人格角色扮演上并不优于基线模型。


<details>
  <summary>Details</summary>
Motivation: LLM在社交仿真中虽能模仿人类行为，但仍存在持续的"异质感"。作者希望通过人格感知微调提升人格条件化对话生成的一致性与可控性，从而缩小这一差距，并与仅使用指令提示的方法进行对比。

Method: 基于融合人格标注社交媒体帖子与对话的语料，对两个小型开放权重LLM（Qwen2.5-7B-Instruct与Ministral-8B-Instruct）进行微调，构建面向社交仿真的人格化对话引擎。在多个人际互动场景中，使用三个独立LLM评审评估人格保真度并提供基于证据的行为解读，同时量化评审者间一致性与生成对话的词汇特征。

Result: 微调模型在扮演不同人格方面并不优于各自的基线模型；但评审者间一致度较低，限制了结论的可信度。在生成文本质量上，微调模型与基线大体相当，微调仅提升了Qwen模型的语言多样性。基线模型整体表现最佳。

Conclusion: 结果整体可用，基线模型表现最优。未来研究应更加重视训练数据的质量与领域对齐，以实现准确的人格角色扮演。
...
### [96] [NemotronLabs VoiceChat: An Open Full-duplex Speech-to-Speech Model with Tool Calling Capabilities](https://arxiv.org/abs/2609.21967)
...
TL;DR: 提出开源全双工语音到语音模型 NemotronLabs VoiceChat，原生支持工具调用，在多项全双工对话与工具选择基准上取得领先结果。


<details>
  <summary>Details</summary>
Motivation: 现有语音对话系统难以在统一架构中同时实现全双工交互、语音识别与生成、通用语言能力及外部工具调用，且往往牺牲实时对话所需的时序行为。

Method: 将流式语音编码器与仅解码器语言模型结合，配置面向智能体文本与结构化函数调用的并行专用输出流、用于增量用户转写的辅助 RNN-T 分支，以及流式 TTS 解码器，形成统一的流式架构。

Result: 在 Full-Duplex-Bench 1.0 上取得所评估开源权重系统中最低的暂停处理接管率、用户打断后 100% 接管率及 4.33/5 的打断后回复质量分；在 Full-Duplex-Bench 1.5 上用户反馈后 93% 恢复应答；VoiceBench 归一化平均分 55.1；FDB 3.0 工具选择 F1 达 82.5%，但参数准确率与端到端工具执行仍有提升空间。

Conclusion: 全双工交互、语音识别与生成、通用语言能力和外部工具使用可集成于单一开源语音到语音模型中，且不牺牲实时对话行为。
...
### [97] [DiaVLo: Diagnosing Behaviours of Vision-Language Models](https://arxiv.org/abs/2609.22008)
*Lorenzo Corti,Jie Yang*

Main category: cs.CL

TL;DR: DiaVLo是一个诊断框架，利用人工策划与VLM生成能力构建期望与观察行为规范，并因果估计影响VLM行为的关键概念。


<details>
  <summary>Details</summary>
Motivation: VLM依赖子组件间存储与传递信息，验证其表现期望行为并避免有害行为对可靠部署至关重要，但识别VLM行为的方法仍很稀缺。

Method: DiaVLo结合人工策划与VLM生成能力，构建期望与观察到的VLM行为规范以揭示潜在错位，并提供因果估计识别最具影响力、驱动VLM行为的概念。

Result: 在多个开源VLM上于分类和生成条件下评估，DiaVLo生成的行为标签与模型性能相关并为性能提供背景；揭示了对齐与错位行为，以及VLM感知、组织和优先处理概念的模式。

Conclusion: DiaVLo能够有效诊断VLM行为，识别对齐与错位现象及概念组织模式，为VLM可靠部署提供行为层面的可解释依据。
...
### [98] [Moral Entropy: Auditing Bias and Uncertainty in Moral Judgment](https://arxiv.org/abs/2609.21992)
*Maciej Skorski*

Main category: cs.CL

TL;DR: 论文提出Moral Entropy贝叶斯框架，将道德标注分歧建模为可学习的不确定性，并分解为偶然与认识两类不确定性，用于审计常见聚合规则的偏差。


<details>
  <summary>Details</summary>
Motivation: 现有计算伦理研究通常把标注者对道德内容的分歧视作噪声，通过多数投票或任一标注者标记的宽松规则直接消除，忽略了其中蕴含的不确定性信息。

Method: 构建Moral Entropy:在真实标签上保留完整后验分布，将熵分解为偶然不确定性(道德内容本身的不可约分歧)与认识不确定性(标注不足或噪声导致);并借助交叉熵/KL、Brier分数和期望校准误差等熵方法，将任意启发式共识规则与校准后的真实标签进行对比审计。

Result: 在三个语料库、十五个话语领域上审计标准聚合规则:任一标注者规则与校准后验在约30%条目上不一致，合并后几乎全为假阳性，但在基础层面错误发生反转(MFTC上平均FPR/FNR为19.9%/38.9%);更严格的多数投票与两票规则则漏掉63-83%的真阳性。

Conclusion: 现有聚合规则存在当前流程未报告的偏差，将道德标注分歧建模为不确定性并加以学习，可为共识规则提供校准的审计基准，优于简单投票消除分歧的做法。
...
### [99] [HERMES: Contrast-Aware Knowledge Graph Reasoning from Clinical Notes for Patient Outcome Prediction](https://arxiv.org/abs/2609.20825)
*Gia-Bach Nguyen,Hoang-Ha Nguyen,Tuan-Cuong Vuong,Trang Mai Xuan,Duy Quoc Ngo,Tien-Cuong Nguyen,Huan Vu,Thien Van Luong*

Main category: cs.CL

TL;DR: HERMES是一个仅基于临床文本构建个性化知识图谱的图框架，通过大语言模型引导抽取与对比逻辑建模保留临床关系和时间动态，并用图注意力网络生成患者表征，在MIMIC-III/IV的院内死亡与30天再入院预测上优于强文本基线。


<details>
  <summary>Details</summary>
Motivation: 临床预测模型多依赖结构化电子健康记录数据（如时间序列、手术编码）。近期研究虽开始利用非结构化临床笔记，但通常将其编码为扁平序列，可能丢失临床叙述中显式的关系结构与时间结构。

Method: 提出HERMES图框架，仅作用于临床文本并保留临床关系。基于两个关键设计：(1) 通过大语言模型引导的抽取，从临床笔记构建个性化知识图谱，并采用对比逻辑建模显式捕捉时间动态、治疗失败及结局变化；(2) 使用图注意力网络在知识图谱上进行图学习，合成患者表征。

Result: 在MIMIC-III与MIMIC-IV上针对院内死亡率和30天再入院预测的实验表明，HERMES持续优于强文本-only基线。

Conclusion: 研究结果表明，借助对比逻辑建模进行显式关系建模可显著提升预测性能。
...
### [100] [Beyond WER: Entity and Disfluency Recall in Accented Conversational ASR](https://arxiv.org/abs/2609.20828)
*Fiza Husain,Ankit Pandey,Yash Singh*

Main category: cs.CL

TL;DR: 面向印度、印尼与拉美口音的对话英语，提出三阶段ASR流水线，显著提升命名实体与填充词召回，同时保持低WER。


<details>
  <summary>Details</summary>
Motivation: 以WER为优化目标的ASR系统，在带口音的对话英语中常漏识别命名实体与填充词，而这两者对语言学习反馈至关重要。

Method: 三阶段流水线：(1) 启发式SQL过滤，构建实体密度为随机采样2.8倍的训练数据；(2) 区域LoRA适配器，在Qwen2.5-Omni-3B上微调，单次前向同时输出逐字与纠正转写；(3) 六类错误分类体系，由LLM评判器验证（83.8%一致率，210条人工标注样本）。

Result: 6千条测试语音上，实体召回从53-55%提升至80-85%，填充词召回从<5%提升至76-86%，WER为6-10%；实体召回优于Whisper与某商用ASR，并以1/10参数量匹配零样本30B模型；配对bootstrap检验显示，仅数据策展即贡献2.8-4.2个百分点的实体召回增益（p<0.0001）。

Conclusion: 数据策展与轻量区域适配相结合，可在低参数量下兼顾实体/填充词召回与整体识别精度，满足口音对话英语的语言学习反馈需求。
...
### [101] [Recursive Language Models Generalize Out of Domain](https://arxiv.org/abs/2609.20831)
*Chenxiao Yang,Zhiyuan Li,David McAllester,Nathan Srebro*

Main category: cs.CL

TL;DR: 限制语言模型可见上下文（递归式隔离）在分布外场景下优于标准CoT，因为CoT会依赖跨子任务的捷径，而递归隔离排除了这种失败模式。


<details>
  <summary>Details</summary>
Motivation: 研究"限制模型可见信息"何时能提升学习效果，对比标准CoT（读取完整轨迹的通用学习器）与递归语言模型（在隔离上下文中求解每个子任务），探讨泛化与真正推理之间的关系。

Method: 理论分析：将标准CoT视为更通用的学习器，递归语言模型视为带上下文隔离约束的学习器，比较两者在分布内（IID）与分布外（OOD）的泛化保证，并结合简单性偏置（simplicity bias）分析假设选择行为。

Result: 分布内：CoT可高效模拟递归规则，IID泛化保证仅差常数因子，递归无显著优势。分布外：CoT依赖当前子任务之外的上下文作为捷径，一旦这些token变化即失效；递归上下文隔离排除了该失败模式。尽管CoT的假设类仍覆盖正确规则，简单性偏置会优先选择捷径而非真实规则。

Conclusion: 要超越分布精度并实现真正推理，仅"覆盖正确规则"是不够的，这挑战了经典学习理论中"假设类包含正确规则即可"的观点。
...
### [102] [PhysioBench: A Unified Benchmark for Physiological Signal Question Answering](https://arxiv.org/abs/2609.20836)
*Mengxuan Li,Junfa Chen,Jinze Xia,Yundan Chen,Lixin Fan,Ke Liu,Keyue Shi,Haishuai Wang*

Main category: cs.CL

TL;DR: 本文提出PhysioBench，一个统一的生理信号问答基准，将22个公开数据集整合为3000万+任务中的6140万个问题，评测21个模型在三种设定下的表现，发现现有模型在跨模态、跨任务上均无法稳定取得强性能。


<details>
  <summary>Details</summary>
Motivation: 生理信号基础模型通常需要针对每项任务进行特定适配，缺乏统一性。自然语言可作为指定不同预测目标的通用接口，但现有模型在生理信号模态间遵循此类指令的能力尚未得到充分评估，故需构建统一基准来填补这一空白。

Method: 构建PhysioBench统一基准：将22个公开数据集的标注统一为30个任务、6140万个问题，每个问答对均基于信号片段并可溯源至原始标注；在三种互补设定下评测21个代表性模型，涵盖大语言模型、视觉语言模型、时间序列语言模型及生理信号基础模型。

Result: 没有任何被评测模型能在生理信号模态与任务上持续保持强性能；引入自然语言支持跨任务统一预测，但性能对问题表述方式较为敏感。

Conclusion: PhysioBench为生理信号理解的细粒度分析与未来研究提供了可扩展平台，代码已开源。
...
### [103] [VISPATH: Visual-Intent-Guided Path Reasoning for Multimodal Knowledge Graph Question Answering](https://arxiv.org/abs/2609.20843)
*Jinke Wu,Zhengpin Li,Mengzhe Jia,Yang Li,Wentao Zhang*

Main category: cs.CL

TL;DR: 本文提出VISPATH，一个面向多模态知识图谱问答的视觉意图引导路径推理框架，通过在多跳推理的每一步重新计算多模态意图来指导路径扩展，并构建了VISPATH-Bench基准，实验证明其在多个基准上优于强基线。


<details>
  <summary>Details</summary>
Motivation: 现有多模态KGQA方法仅将多模态信息用于起始实体锚定或证据检索，之后的多跳推理退化为纯文本图搜索，无法利用在中间跳才变得重要的多模态线索。

Method: VISPATH首先结合多模态锚定与图结构线索识别可靠的起始实体；然后进行意图引导的路径发现，根据输入、问题和当前部分路径重新计算各跳特定的多模态意图，使每次扩展由当前推理状态引导；再通过推理链剪枝对候选路径作为完整证据链进行评估，基于其与问题、推理草图和各跳意图的一致性进行筛选；最后检查所选证据是否足以生成答案。此外构建了VISPATH-Bench基准，涵盖需要2至4跳KG路径的问题。

Result: 在VISPATH-Bench及另外三个多模态问答基准上的大量实验表明，VISPATH持续优于强基线；以GPT-4o为骨干时，在VISPATH-Bench上超过GPT-5.4，平均准确率相对提升10.6%，2跳推理提升13.1%。

Conclusion: VISPATH通过在多跳推理中引入逐跳多模态意图引导与推理链剪枝，有效解决了现有方法中间跳多模态线索利用不足的问题，并验证了其性能优势。
...
### [104] [Reading Less While Writing: A Closed-Form Bandwidth Dial for Streaming Multimodal Decoders](https://arxiv.org/abs/2609.20845)
*Yasir Mehmood,Kashif Javed*

Main category: cs.CL

TL;DR: 提出 ZENDAYA 调度器，用单一连续参数 γ 将离线解码器与流式解码器统一为一个家族，通过闭式函数依输入预测长度控制可见源前缀，实现可调延迟预算。


<details>
  <summary>Details</summary>
Motivation: 传统解码器需读完整输入才输出文本，离线场景冗余、实时场景不可行；现有流式方案采用固定 wait-k 规则，不考虑输入长度与节奏，在低延迟下性能崩塌。

Method: 用 ZENDAYA 替代固定偏移：可见源前缀为生成进度与输入预测长度的闭式函数，γ 同时决定每输出词的平均源消耗比例 Ē(γ)≈1/(1+γ)。证明结构性依赖定理：任何事先固定的非递减调度下，已输出 token 不能依赖未到达的输入；该保证对已训练/未训练权重均成立，并可扩展到异步到达的无界流。

Result: 在两种模态、三个公开语料（Charades-STA、ActivityNet Captions、LibriHeavy）上从零训练 29M 参数解码器，性能匹配或超过固定调度，且读取更少源；最低延迟处增益最显著。三个语料上流式 METEOR 提升均具统计显著性。

Conclusion: 固定调度被连续参数化调度取代，离线与流式成为同一家族的两个端点；少看源反而提升文本质量（源稀释注意力），γ 兼作延迟旋钮与可解释预算。
...
### [105] [Rewarding Efficient Reasoning Improves Abstention on Underspecified Tasks in Reasoning Models](https://arxiv.org/abs/2609.20846)
...
### [106] [How Many Humans Is a Judge Panel Worth?](https://arxiv.org/abs/2609.21277)
*Chao Li,Yingying Yu,Yunfeng Li*

Main category: cs.CL

TL;DR: 该论文提出"有效规模"（effective size）是目标依赖的测量：通过谱残差多样性（nu_H）与分布均方误差匹配（nu_MSE）两种口径审计语言模型评审团对类别人群标签分布的还原能力，发现同一32评审团在ChaosNLI三个任务上nu_H为4.24–6.50而nu_MSE仅为2.30–3.75，二者不可互换。


<details>
  <summary>Details</summary>
Motivation: 现有研究常把语言模型评审团视为人类判断的替代品，"有多少人类判断"这一问题的答案取决于匹配什么量。二值化相对单一金标签的错误会掩盖标注分歧，因此需要用保留分歧的分布级指标重新审计评审团质量，明确谱多样性与分布还原度是否可互换。

Method: 构建谱残差多样性指标nu_H：对归一化残差Gram矩阵匹配参与比（participation ratio）至条件独立的人类参考抽样；另以分布平方误差匹配得到nu_MSE。在ChaosNLI三个任务上对同一32评审团进行审计，并用谱恒等式分解决定误差的特征值、成员能量与平均方向权重；再利用可实现硬标签评审团构造对照，检验等成员能量、非负相关条件下的多样性与分布还原关系。

Result: 同一32评审团在三个ChaosNLI任务上nu_H=4.24–6.50，而nu_MSE仅2.30–3.75，两者显著分离。可实现硬标签评审团显示，在成员能量相等、相关性非负时，更高的谱多样性可能与更差的分布还原并存。观察到的评审团中，同规模排名一致性随任务剧烈变化，部分成员增补产生跨两个条目半集持续存在的冲突变化。中心化残差方差中共识方向占比gamma_co在MNLI-m为43.8%、SNLI为33.7%，量化了平均化保留的共享变异。

Conclusion: 有效规模是目标特定的量：谱多样性与分布还原度不应被视为可互换的评审团质量指标，也不应被当作通用的人类替代率。研究提供了对齐投票数据与分析协议，以支持对上述区分的审计。
...
### [107] [Prediction Dynamics in Depth-Recurrent Language Models](https://arxiv.org/abs/2609.21383)
*Xinyue Luo,Fei Yu*

Main category: cs.CL

TL;DR: 论文提出一种紧致的margin刻画，将幅度界（magnitude bound）的保守性分解为公共平移、相对获胜者的方向以及各竞争者更新与其分数差距的配对三部分，用于解释深度循环语言模型中中间答案与终点一致但分数持续变化的现象。


<details>
  <summary>Details</summary>
Motivation: 深度循环语言模型通过重复的潜在更新精炼预测，但存在一个反直觉现象：中间答案与终点答案一致，其分数却持续变化。作者希望刻画这种分数变化的几何与组成结构，解释为何在有限深度下答案能够保持稳定，并厘清幅度界为何过于保守。

Method: 推导紧致的margin刻画，将幅度界的保守性分解为：公共平移、相对获胜者的方向、以及每个竞争者更新与其分数差距的配对。在Huginn-3.5B与Ouro-1.4B上，基于完整答案文本评分（回顾性使用已完成轨迹）与标签评分进行验证；对共享预测分布，正交分离公共运动与对比运动，并用候选集质量与集合内集中度表达公共分量。

Result: 在全答案文本评分下，计入更新方向与竞争者配对后，平均最早合格深度相比仅去除平移的情形进一步下降，占总深度的22.5%-34.4%；标签评分下也有显著贡献。公共与对比运动能量可以以不同速率衰减，使得偏好变化占比增长与绝对更新收缩同时存在。

Conclusion: 通过观察到的分数变化的几何与组成结构，可解释有限深度下的答案保持现象；幅度界的保守性主要来自平移、方向与竞争者配对三方面，公共与对比运动的差异化衰减是答案稳定而分数变化的关键机制。
...
### [108] [Analysing the Linearity of Linguistic Relations in Language Model Embedding Spaces](https://arxiv.org/abs/2609.21655)
*Vasudevan Nedumpozhimana,Fathima Thekkekara,John Kelleher*

Main category: cs.CL

TL;DR: 提出一种通过受约束线性近似来量化语言关系在词嵌入空间中线性编码强度的框架，并在扩展BATS数据集上测试GloVe、RoBERTa与ModernBERT。


<details>
  <summary>Details</summary>
Motivation: 探究不同语言关系（屈折、派生、词典、百科）在语言模型嵌入空间中被线性编码的程度，以揭示哪些关系结构最易于线性访问。

Method: 形式化线性编码为在相关与不相关词对上施加约束的线性近似，并应用于覆盖屈折、派生、词典和百科关系的扩展BATS数据集，比较GloVe、RoBERTa和ModernBERT。

Result: 屈折和派生关系呈现近乎完美的线性编码，词典和百科关系误差显著更高，尤其是一对多和多对多关联；RoBERTa和ModernBERT总体上比GloVe更线性地编码关系。

Conclusion: 该框架能揭示嵌入中哪些关系结构最具线性可访问性，为探测和比较不同模型的关系几何提供紧凑工具。
...
### [109] [RheoSampling: Resolving the One-Hot Dilemma in Stochastic Dynamic-Tree Speculative Decoding](https://arxiv.org/abs/2609.21827)
*Qiao Hu,Yepeng Weng,Bo Zhang,Takehisa Yairi*

Main category: cs.CL

TL;DR: 本文提出 RheoSampling，通过解耦"树构建"与"验证"两种概率角色，首次实现兼具上下文感知 top-K 构建与随机采样的动态树推测解码，并保持无损性。


<details>
  <summary>Details</summary>
Motivation: 动态树方法（如 EAGLE-3）在贪心解码下表现优异，但在随机解码（T>0）时，其确定性 top-K 扩展与全局剪枝会把草稿分布坍缩为 one-hot 概率，导致接受率骤降。由此形成两难：动态树牺牲随机采样以保留上下文感知拓扑，静态树保留随机采样却使用上下文无关结构。根源在于同一概率分布被同时用于构建树和验证 token 这两个相互冲突的任务，这种耦合使直接注入随机性变得困难。

Method: 核心思路是解耦两种角色：RheoSampling 为从草稿分布采样的 token 分配一个代理概率用于树的扩展与剪枝，同时保留其真实采样概率用于验证。具体地，将采样 token 注入确定性 top-K 槽位中，并在构建和验证阶段使用不同概率处理。为建立无损保证，作者通过等价类分析将随机树空间压缩为可处理的类别；并引入基于最优传输（OT）的验证策略与稀疏草稿机制，以保证理论收益转化为实际效率。

Result: 在多个 LLM 与基准测试上，RheoSampling 在贪心解码下实现无损加速。

Conclusion: RheoSampling 是首个同时具备上下文感知 top-K 构建与随机采样且保持无损性的动态树方法，其解耦框架可为分析随机树结构提供模板。
...
### [110] [Do small language models know what they don't know?](https://arxiv.org/abs/2609.20824)
*Prashant Mudgal*

Main category: cs.CL

TL;DR: 该论文研究基于熵的置信信号能否提升3B以下参数的小语言模型（SLM）准确率。结论是词元级熵在SLM中近乎失效（91%的组合均值近零），而语义熵可恢复有效置信信号，并用于不确定性感知路由，跨家族路由平均提升+22.0%，最高可达+50个百分点。


<details>
  <summary>Details</summary>
Motivation: SLM可在消费级硬件本地运行，但准确率有限；现有基于熵的置信估计多针对大模型，其在SLM规模下的有效性未知。作者希望明确熵类置信信号在SLM中的可用性，并探索以更低成本提升SLM准确率的途径（如按需调用更大专家模型）。

Method: 在7组模型对（含SmolLM 360M、Phi-3.5-mini等）与5个标准NLU基准上系统评估7种方法：词元级熵早停、语义熵估计、不确定性感知路由至更大专家模型等。语义熵通过多次采样生成、按语义聚类答案、度量分布不确定性计算；据此将高不确定查询路由给更大专家模型。

Result: 词元级熵在SLM中近乎失效：91%的数据集-模型组合中平均词元熵接近零且与答案正确性无关。语义熵可恢复可行置信信号。基于语义熵的选择性路由最高带来+50个百分点准确率提升；跨家族路由平均提升+22.0%，显著优于同家族路由的+6.8%。

Conclusion: 在SLM中，熵类方法的核心价值不是计算节省，而是智能算力分配：把更多词元预算花在最需要的地方。专家模型质量比架构兼容性更重要；语义熵比词元级熵更适合作为SLM的不确定性度量与路由依据。
...
### [111] [From Discharge Notes to Patient Understanding: Persona-Grounded, Open-Ended Simulation of LLMs as Discharge Educators](https://arxiv.org/abs/2609.20827)
*Won Seok Jang,Zonghai Yao,Hong Yu*

Main category: cs.CL

TL;DR: 本文提出 DischargeBench，一个基于人格设定的多轮对话仿真基准，用于评估 LLM 在医院出院教育中提升患者理解的能力，并构建了包含 477 个病例的数据集 MIMIC-IV-Ext-DischargeBench。


<details>
  <summary>Details</summary>
Motivation: 出院教育是互动式教学任务，医生需根据患者的识字水平、记忆能力与人格特征调整出院计划；但现有 LLM 评测多针对静态任务或文本生成任务，无法在开放式对话中衡量患者理解程度，导致评测与实际临床教学目标脱节。

Method: 提出 DischargeBench：候选 LLM 教育者与虚拟患者进行多轮会话，教育监测智能体（Education Monitor Agent）在修改教育者的前提下调节患者真实感以保护评估信号；整理 MIMIC-IV-Ext-DischargeBench，含 24 个 ICD 章节的 477 个病例，并设置人格、教育水平、健康素养、病史回忆等人格轴用于分层分析；每次模拟由经医生标注对齐的 LLM-as-a-Judge 从对话质量、主题清单、理解度、事实一致性四个维度打分。

Result: 在闭源与开源 LLM 上，总体分数掩盖了跨 ICD 章节与患者人格的临床相关差异；困难人格暴露出覆盖失败、理解缺口以及源答案一致性下降的问题。

Conclusion: 面向出院教育的 LLM 评测应以患者理解为核心，而不能仅关注文本质量或答案准确性；总体聚合指标会掩盖重要差异，需按章节与人格分层评估。
...
### [112] [SAGE: Schema-Guided LLMs for Grant Review](https://arxiv.org/abs/2609.20829)
*Erik Varapaev,Andrei Chetvergov,Stepan Ukolov,Timofei Sivoraksha,Alexander Evseev,Sergey Bolovtsov*

Main category: cs.CL

TL;DR: SAGE 将资助评审标准转化为结构化检查项，并把评审判断链接到申请材料中的证据，在35份非营利资助申请上进行两阶段评估，辅助评审时与专家评分一致性显著提升（kappa 0.58），并提供可审计的证据溯源。


<details>
  <summary>Details</summary>
Motivation: 资助评审人需依据细化标准审阅申请书、预算及支撑材料，并产出可供同行核查的评估意见。此过程工作量大、标准执行不一，现有自动化方法难以将 rubric 落地为可核查的判断。作者希望构建一个能操作化评审方法、产出证据链接且可审计草稿的系统，供专家修正。

Method: 提出 SAGE（Schema-Guided Aspect-Based Grant Evaluation）：将资助 rubric 翻译为结构化检查项（schema-guided checks），并基于 aspect-based 方式，把每项判断链接到申请包中的证据。评估分两阶段，共35份非营利资助申请。阶段一：与原始竞赛的105份评审进行事后比较。阶段二：基金会在查看 SAGE 后开展 criterion-level 复审，产生202份评估，并与 one-prompt-per-criterion 基线对比。另做 claim-level 审计，区分结构化草稿中被确认、有争议和未涉及的部分。

Result: 阶段一事后比较达到 fair ordinal agreement（kappa = 0.29）。阶段二辅助评审中，SAGE 达到 kappa = 0.58，优于 one-prompt-per-criterion 基线（共同子集 kappa = 0.33），且排序相关性更高、误差更低。claim-level 审计可识别结构化草稿中被确认、有争议和未涉及的论断部分。

Conclusion: SAGE 通过生成详细、证据链接且可审计的评审草稿，将评审方法论操作化，服务于专家修正流程；在专家辅助场景下显著提升与人类评审的一致性，并具备可追溯的证据审计能力。
...
### [113] [Reviser: Revision-Capable Text Generation via Autoregressive Cursor Actions](https://arxiv.org/abs/2609.20830)
*Sean Diab*

Main category: cs.CL

TL;DR: Reviser 是一种仅解码器 Transformer，将文本生成建模为在可变画布上的光标相对动作序列（INSERT/MOVE/STOP），实现对编辑历史的下一动作自回归，从而在不进行重复序列级计算的情况下实现真正的非单调生成。


<details>
  <summary>Details</summary>
Motivation: 具备修订能力的生成很有吸引力，因为可以插入或修改先前内容，但许多非自回归与基于编辑的方法需要通过反复的序列级计算才能获得这种灵活性。因此作者希望在保留简单下一动作接口的同时实现真正的非单调生成，并降低推理计算量。

Method: 提出 Reviser：一个仅解码器 Transformer，将响应生成为可变画布上的一系列光标相对动作。每一步只预测一个动作 token：INSERT(token)、MOVE(Δ) 或 STOP；模型自回归的对象是编辑历史动作而非最终文本顺序。

Result: 在续写基准上，Reviser 在竞技场评估中显著优于 SEDD 和 MDLM；轨迹统计确认模型频繁进行向后移动与画布中部插入，而非仅模拟末尾追加式解码。与规模匹配的自回归基线相比，Reviser 在 100M 与 300M 规模上均具竞争力；在统一 FLOPs 约定下，其推理计算量显著低于代表性的多轮精修与扩散式基线。

Conclusion: Reviser 通过光标相对动作序列建模实现了真正的非单调生成与修订能力，同时保持简单的下一动作接口，并在质量与推理效率上展现出优势。
...
### [114] [TatBLiMP: A Benchmark of Linguistic Minimal Pairs for Tatar](https://arxiv.org/abs/2609.20832)
*Ilshat Saetov,Dmitry Gaynullin*

Main category: cs.CL

TL;DR: TatBLiMP 是首个针对鞑靼语（Qypchaq 突厥语族，西里尔文）的语言学最小对立对基准，含 16 类形态句法现象、1248 对句子，仅通过模型已有概率评分（无需生成、无需句法分析器）。实验表明基准性能主要反映针对鞑靼语的定向训练程度，而非参数规模。


<details>
  <summary>Details</summary>
Motivation: 鞑靼语此前缺乏任何语法性评测资源，甚至 101 语种的 MultiBLiMP 也未涵盖该语言，因此需要构建专门的最小对立对基准以评估语言模型的鞑靼语语法知识。

Method: 将 TurBLiMP 的现象清单与单语素破坏操作迁移至鞑靼语，并新增一条鞑靼语特有现象（数词/量词后光杆名词的数）。每对仅差一个语素，语法句取自鞑靼语文学散文真实语料，非语法句由 apertium-tat 转换器确定性单语素扰动生成；每对均经母语者审核，并遵循合理性原则使非语法句为可信的真实错误。

Result: 在从头训练的鞑靼语模型、跨语言适配模型与前沿多语种大模型上，基准反映的是鞑靼语定向训练而非参数规模：478M 从头训练模型与 125M 单语模型接近 0.97，7B 适配模型落后，30–120B 前沿大模型仅 0.80–0.92，轻度微调的多语模型最弱。模型通过一对的标准是对语法成员赋予更高概率。

Conclusion: TatBLiMP 提供了首个鞑靼语语法性评测，证明小规模定向训练可优于大规模通用模型。主要局限在于沿用的分类体系遗漏了母语者最敏感的形态音系现象（元音和谐与辅音同化），作者据此提出构建本土化第二层评测的设想。
...
### [115] [Transsion's Speaker-Attributed Multilingual ASR System for the MLC-SLM 2026 Challenge](https://arxiv.org/abs/2609.20833)
*Zhecheng Ren,Xuanji He,Xiaoxiao Li,Zhichen Han,Gaoyang Dong,Gaosheng Zhang,Minchuan Chen,Fengjie Zhu*

Main category: cs.CL

TL;DR: 本文介绍 Transsion 语音团队参加 MLC-SLM 2026 挑战赛 Task 1 的方案，提出由说话人日志、长音频多语种 ASR 和说话人-转写融合三模块构成的级联框架，在官方评测集上取得 tcpMER 15.41%、排名第二的成绩。


<details>
  <summary>Details</summary>
Motivation: 任务目标为多语种对话语音的"说话人属性转写"（speaker-attributed transcription），即在多语种、长时对话场景下同时完成说话人分离与内容转写，并将二者对齐输出结构化结果。该任务需要解决长音频建模、多语种识别以及时间戳精度等问题，因此需要设计一个能够协同日志与识别能力的系统框架。

Method: 采用三级级联框架：（1）说话人日志模块基于 DiariZen，通过局部说话人活动估计与全局说话人聚类生成说话人同质片段；（2）长音频多语种 ASR 模块基于 Qwen3-Omni 生成多语种转写，并借助外部 CTC 对齐模型提供精确的词级/字符级时间戳；（3）融合模块将日志输出与带时间戳的转写结果结合，生成带说话人属性的 STM 格式输出。

Result: 在官方评测集上的实验验证了所提框架的有效性，提交系统取得 tcpMER 15.41%，在全部参赛队伍中排名第二。

Conclusion: 级联式"日志-ASR-融合"方案可有效应对多语种对话语音的说话人属性转写任务，并在官方评测中达到具有竞争力的性能水平。
...
### [116] [Towards Secure Cloud-Native Computing: Unveiling Kubernetes Misconfigurations with Large Language Models](https://arxiv.org/abs/2609.20834)
*Mostafa Anouar Ghorab,Mohamed Aymen Saied*

Main category: cs.CL

TL;DR: 本文探讨利用大语言模型（LLM）识别 Kubernetes 配置错误，构建了常见错误配置分类体系，并对现有检测工具进行实证评估，同时分析了易出错对象及问题严重性。


<details>
  <summary>Details</summary>
Motivation: 云原生生态日益普及，Kubernetes 成为容器编排事实标准，但其复杂性导致配置错误频发，威胁安全与性能。现有检测手段存在不足，亟需探索 LLM 等先进方法提升检测能力。

Method: 引入常见错误配置类型分类体系；对当前最先进的检测工具进行实证评估与基准测试；分析最易发生配置错误的 Kubernetes 对象并评估问题严重程度；借助包括 LLM 在内的先进机器学习技术开展研究。

Result: 建立了结构化的配置错误分类框架；对主流检测工具的有效性完成基准对比；识别出高风险的 Kubernetes 对象类型并量化了所发现问题的严重性。

Conclusion: 利用 LLM 等先进机器学习技术可为改进 Kubernetes 配置错误检测方法提供新思路，有助于提升云原生环境的安全性与性能。
...
### [117] [A Generative Grammar Underlying the Voynich Manuscript, the Pastiche Hypothesis: Evidence from Large Language Models](https://arxiv.org/abs/2609.20835)
*Nicolas Turenne*

Main category: cs.CL

TL;DR: 该论文对15世纪伏尼契手稿进行多学科综合分析，结合概率建模、语音分解、稀有事件检测与多模态图像分析，认为其符号为字母级单元、语音特征近闪语系，植物插图与地中海Pseudo-Apuleius草药书高度对应，支持"结构化生成系统"假说。


<details>
  <summary>Details</summary>
Motivation: 伏尼契手稿书写系统未被破译，既有研究仅指出其统计性质类似自然语言、插图类似中世纪草药书，缺乏系统性、跨学科的定量验证。作者希望检验其究竟为自然语言、编码语言还是结构化仿造，并探索概率方法与AI辅助方法在历史手稿分析中的价值。

Method: 基于新转写语料库：(1)用位置依赖的概率文法建模词级与字母级分布；(2)进行语音分解，将语音模式与印欧语、闪语及亚洲语言比较；(3)采用稀有事件检测分析重复首字母序列的概率；(4)应用基于大语言模型的图文对齐方法识别植物学对应关系。整体为概率建模、语音分析与多模态图像分析的结合。

Result: 伏尼契符号表现为字母而非音节单元，但词长分布类似音节结构；语音上与希伯来语、阿拉伯语等辅音密集语言更接近，而非印欧语；概率模型可复现类Zipf分布，并显示重复首字母序列概率极低，说明是对自然语言的结构化模仿；图像分析显示其植物插图与地中海传统的Pseudo-Apuleius草药书插图高度对应，符合对中世纪医书的模仿。

Conclusion: 结果支持伏尼契手稿遵循一种结合语言规律与草药知识的结构化生成系统这一假说，并证明将概率建模与AI辅助方法整合用于历史手稿分析具有重要价值。
...
### [118] [From Generation to Detection: Exploration of Discourse Driven Scenario based LLM Generated Fake News](https://arxiv.org/abs/2609.20838)
*Zeynep Özdemir,Murat Osmanoğlu,Sevgi Yiğit-Sert,Ömer Özgür Tanrıöver,Yılmaz Ar*

Main category: cs.CL

TL;DR: 本研究系统评估了主流大语言模型在四种操纵场景下生成与检测假新闻的能力：构建14000篇合成假新闻语料，分析其语言特征，并测试检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对LLM生成假新闻与检测能力在多种典型生成场景下的系统性对比评估，尤其缺少生成策略对可检测性影响的量化分析。

Method: 采用七个广泛使用的LLM，围绕开放式生成、改写、操纵提示和基于新闻话语框架的属性提示四种场景，生成14000篇假新闻构成语料；分析其结构与语义层面的语言属性；再以基础检测提示和通过真实-虚假配对迭代提炼误导模式得到的改进提示进行检测实验。

Result: 不同模型在生成与检测虚假信息方面差异显著；生成策略强烈影响可检测性；迭代改进提示并未提升检测性能，反而常导致性能下降。

Conclusion: 该研究提供了跨典型生成场景的LLM生成假新闻检测能力系统评估，揭示了模型间差异及生成策略对检测难度的关键作用。
...
### [119] [COAL-SQL: Coverage-Guided Augmentation and Failure-Driven Learning for Text-to-SQL Post-Training](https://arxiv.org/abs/2609.20842)
*Qifeng Cai,Xuanguang Pan,Hao Liang,Chang Xu,Wentao Zhang*

Main category: cs.CL

TL;DR: 本文提出COAL-SQL框架，通过覆盖引导增强（CGA）与失败驱动学习（FDL）相结合，用较少数据提升开源大模型复杂Text-to-SQL生成能力。


<details>
  <summary>Details</summary>
Motivation: 开源大模型在复杂真实场景Text-to-SQL生成中仍需任务特定的后训练。有效后训练需要两要素：覆盖目标任务所需能力的训练数据，以及能使模型习得这些能力的学习策略。现有数据集虽提供监督，但对SQL结构的覆盖不完整；增强方法通常只扩充数据而不识别结构缺口；且单纯SFT或RL无法动态应对训练中暴露的弱点。

Method: 提出统一框架COAL-SQL，包含两部分：(1) Coverage-Guided Augmentation (CGA)，用贪心选择识别原始数据集中缺失的SQL结构并构造互补样本，提升结构覆盖；(2) Failure-Driven Learning (FDL)，以GRPO为主要优化目标，同时为未解决样本提供定向监督——步级层面，对累积失败样本由强LLM生成并验证推理轨迹后施加SFT；轮级层面，基于累积失败检索结构相关样本构造针对性练习，帮助模型习得相应SQL能力。

Result: 仅用12,600条不同的后训练样本，COAL-SQL在BIRD开发集上达到64.9%的执行准确率，并优于同等规模训练的基线方法。代码已开源。

Conclusion: COAL-SQL通过将结构覆盖增强与失败驱动学习统一，在数据规模较小的条件下有效提升开源大模型的复杂Text-to-SQL生成性能，验证了覆盖与动态学习策略结合的有效性。
...
### [120] [Boosting Deepresearch and LongContext Ability with Self-Generated Deepresearch Rollouts Traces](https://arxiv.org/abs/2609.20844)
...
TL;DR: 论文针对DeepResearch智能体因长上下文理解不足导致的预测错误，提出DR-to-Long数据构造方法及DLD-RL三阶段训练框架，以零标注成本提升长上下文与DR能力。


<details>
  <summary>Details</summary>
Motivation: DR智能体通过多轮搜索与网页访问与真实网络环境交互，上下文随时间快速膨胀。作者观察发现，即便经过DR-RL训练，仍有61.6%的剩余预测错误可归因于长上下文理解不足，具体表现为长上下文幻觉与跨文档证据整合失败。因此需要突破DR-RL瓶颈，强化模型的长上下文能力。而有效的长上下文训练并非简单增加上下文长度即可，还面临数据缺口问题。

Method: 提出DR-to-Long数据构造方法：复用DR-RL轨迹（天然包含搜索历史、访问网页、证据片段与最终答案监督），将轨迹中紧凑的证据片段与网页摘要替换为对应URL的完整正文，从而在保留原始证据关系的前提下生成显著更长的多文档上下文，且零标注成本。基于此构建DLD-RL三阶段流程：先进行短程DR-RL收集rollout轨迹并转换为LongQA样本；再用LongQA-RL优化模型以增强长上下文能力；最后进行完整DR-RL继续提升DR能力。

Result: 在三个Deepresearch基准上，DLD-RL相较标准DR-RL提升7.3%；在三个长上下文基准上性能提升13.5%。

Conclusion: 通过将DR-RL轨迹重利用为长上下文QA数据并进行分阶段强化学习，可有效缓解长上下文理解瓶颈，同时提升DR任务与长上下文任务表现，且无需额外标注成本。
...
### [121] [Reading Anxiety or Reading the Label? Comparing Fine-Tuned and Frontier Models for Anxiety Detection on Social Media](https://arxiv.org/abs/2609.20847)
*Cris Huynh,Arlene Pham*

Main category: cs.CL

TL;DR: 该研究在统一的Reddit留出测试集上比较了六种焦虑检测方案（前沿商用模型、自建微调模型与常规分类器），并揭示评估中的词汇混淆：69.3%的焦虑标签帖子含有"anxiety"或其变体，约为对照类别两倍。作者通过删除关键词前后各评测一次来衡量"词汇依赖度"。结果表明前沿模型F1最高（0.846），但110M参数领域适配编码器达0.831且无需外部API，领域预训练仅贡献0.7分；词汇依赖度在8.6–25.4分之间且与模型能力无关，微调3B模型依赖度最高，甚至高于TF-IDF。


<details>
  <summary>Details</summary>
Motivation: 焦虑是最常见心理健康问题之一，用户常在求助临床前就在网上书写相关内容。构建检测工具的从业者面临具体选择：调用前沿商用模型、在本地微调较小模型，还是部署常规分类器，但缺乏在统一协议下对三类方案的系统比较。此外，该任务的评估方式可能存在混淆，需要识别并量化。

Method: 在留出的Reddit测试集上，于同一受控协议下比较覆盖三类方案的六种条件。识别评估混淆：语料中69.3%的焦虑标签帖子含"anxiety"或其变体，约为可比条件的两倍，分类器可凭关键词匹配而非建模病症语言获得高分。因此对每个模型进行两次评测——原始文本与删除这些词后的文本——并以两者差值作为词汇依赖度。

Result: 前沿模型在焦虑F1上领先（0.846），但一个110M参数的领域适配编码器达到0.831且无外部API依赖，其中心理健康领域预训练仅贡献0.7分。词汇依赖度跨度为8.6至25.4分，且不与模型能力相关：经LoRA微调的3B模型是所测条件中关键词依赖最强的，甚至高于TF-IDF分类器，而前沿零样本模型依赖度最低。

Conclusion: 该语料上已发表的指标应被视为上界，且这种虚高对微调模型（此类文献通常报告的对象）最为严重。选择检测方案时须注意，模型能力并不等同于对关键词捷径的鲁棒性。
...
### [122] [Generative Artificial Intelligence Chatbots for Motivational Interviewing: A Scoping Review From System Design to Intervention Outcomes](https://arxiv.org/abs/2609.20902)
*Runze Hu,Jingqi Kong,Yang Yang,Yihang Yang,Jingyao Liu,Haizhou Tang,Shanghang Zhang,Zheng Liu*

Main category: cs.CL

TL;DR: 该范围综述（PRISMA-ScR）纳入47篇报告（48项研究），系统梳理了生成式AI驱动的动机性访谈（MI）聊天机器人在系统设计、安全性、MI质量、用户感知及干预结局方面的证据：多数系统为纯文本、无实体化设计，互动总体符合MI原则且用户评价积极，但干预结局多限于单次会话，对持续性行为与功能改变的证据有限。


<details>
  <summary>Details</summary>
Motivation: MI是一种通过协作方式激发健康行为改变自主动机的方法，生成式AI为以对话系统形式提供MI带来新可能，但相关系统设计、评估与干预转化的证据较为零散，缺乏系统整合，因此有必要开展范围综述以描绘研究全貌并识别证据缺口。

Method: 遵循PRISMA-ScR开展范围综述；检索9个数据库，纳入2015年1月1日至2026年6月2日期间发表或公开的、使用生成式AI生成MI聊天机器人回应或咨询师话语的研究；采用预定义框架提取数据并进行描述性综合。

Result: 共纳入47篇报告（48项研究）。20项（41.7%）聚焦系统设计而未直接让参与者使用，28项（58.3%）涉及直接交互；多数系统为纯文本、无实体化，23项（47.9%）具备动态适应能力。安全措施报告不均衡；直接使用类研究中21/28（75.0%）报告了知情同意或用户教育。30项（62.5%）评估了MI质量，总体上互动与MI原则一致。用户感知积极，尤其在共情、可用性、有帮助性和使用意愿方面，但测量工具异质性大。18项（37.5%）报告了干预结局，多为单次会话后；短期动机方面的阳性结果较一致，而持续性行为或功能改变的证据有限。

Conclusion: 生成式AI-MI聊天机器人能够提供符合MI原则且被用户积极感知的互动，但支持持续性行为或功能改变的证据仍有限。未来研究应加强运行时安全监测、标准化MI质量评估，并采用更长期、含行为与功能结局的对照设计。
...
### [123] [$μ^2$-Bench: A Multilingual Machine Unlearning Benchmark](https://arxiv.org/abs/2609.20945)
*Kyomin Hwang,Hyeonjin Kim,Hyunho Lee,Yearim Kim,Yeji Song,Nojun Kwak*

Main category: cs.CL

TL;DR: 提出 $\\mu^2$-Bench 多语言机器遗忘基准，覆盖多语言记忆—遗忘—评估全流程，揭示现有 MMU 评估不足。


<details>
  <summary>Details</summary>
Motivation: 多语言大模型会通过直接训练与跨语言传播扩散有害内容与隐私数据；现有多语言机器遗忘（MMU）评估不充分，无法确认目标知识是否在所有语言中被真正消除。

Method: 构建 $\\mu^2$-Bench 基准，模拟多语言场景下的完整流程（记忆、遗忘、评估）；其特点为：1) 覆盖广泛语言；2) 同时在训练语言与留出语言上评估；3) 将知识视为分散于多语言中加以评估；并据此开展分析。

Result: 实验表明，成功的 MMU 需要能够反映多语言特性的方法；分析进一步提供了对 MMU 的深入见解。

Conclusion: 多语言机器遗忘评估需纳入多语言特性与跨语言泛化视角，$\\mu^2$-Bench 为系统评估与后续方法设计提供基准与分析依据。
...
### [124] [Scaling Forced Alignment to End-User Devices](https://arxiv.org/abs/2609.21145)
*Lawry Sorenson,Michael Crandall,Eric K. Ringger,Stephen D. Richardson*

Main category: cs.CL

TL;DR: 提出两种优化将Viterbi强制对齐的复杂度从二次降为线性内存，并剪枝加速，兼顾内存、速度与准确率。


<details>
  <summary>Details</summary>
Motivation: 现有Viterbi强制对齐实现具有二次时间与空间复杂度，难以扩展到长输入序列，用于从在线资源挖掘训练数据时效率低下。

Method: 1) 将Hirschberg算法应用于对齐，实现线性内存的原地对齐；2) 将语音与文本的对齐建模为受限随机游走，从而可以任意置信度剪枝搜索空间，并兼顾转写错误。

Result: Hirschberg优化在三小时输入上将内存从140 GB降至5 MB，CPU上与torchaudio产生相同对齐结果且耗时为其三分之一；对超过20分钟的输入，剪枝再带来2倍加速，并在超过98%的测试用例中保持对齐准确率。

Conclusion: 所提优化显著降低内存占用并提升速度，同时保持对齐准确性，使长序列的强制对齐更具可行性。
...
### [125] [CoLearn: An Agentic Tutor that Learns its Learner in a Human--AI Co-Learning Loop](https://arxiv.org/abs/2609.21154)
*Kailai He,Zhihao Wu,Linhai Zhang,Runcong Zhao,Yulan He,Jiazheng Li*

Main category: cs.CL

TL;DR: CoLearn是一个交互式智能体辅导系统，通过持续更新的学习者状态记忆实现个性化出题，盲测中被偏好率68-69%。


<details>
  <summary>Details</summary>
Motivation: 现有辅导工具多采用固定题库，且仅将错误答案视为单一二值信号，无法追踪学习者掌握的知识并诊断错误原因，缺乏真正的个性化。

Method: CoLearn包含三部分：(i) 持久化学习者状态记忆，采用软证据版贝叶斯知识追踪（BKT），由大语言模型充当连续观测函数，按主题更新掌握度；(ii) 自适应题目生成，针对最弱主题与反复出现的错误概念；(iii) 证据视图，通过实时进度可视化与盲A/B对比使个性化过程可见、可验证。

Result: 在盲A/B评估中，基于该记忆条件化生成的题目被偏好比例达68-69%，优于非个性化题目；在隐藏真实掌握度的人设模拟中，智能体的信念收敛于学习者真实掌握水平。

Conclusion: 基于证据的持久学习者记忆与LLM观测函数可有效支撑迭代式个性化辅导，显著提升题目相关性与掌握度估计准确性。
...
### [126] [Not All Irregularity Is Equal: Causally Isolating a Rare Failure Mode in Japanese Morphological Inflection](https://arxiv.org/abs/2609.21179)
*Wen Zhang*

Main category: cs.CL

TL;DR: 该论文对日语过去时动词活用进行正字法感知诊断，发现两个字符级Transformer模型虽总体准确率超97%，但占比不足1%的特定不规则子类（词干以/e/结尾且需促音化）贡献了30-43%的残余错误，其错误贡献约为其流行度的34-48倍。


<details>
  <summary>Details</summary>
Motivation: 神经形态生成系统在基准数据集上常取得高总体准确率，但这种表现会掩盖集中在罕见形态子类中的系统性错误，因此需要更细粒度的子类诊断来揭示被聚合指标遮蔽的失败模式。

Method: 作者将平假名视为编码形态音系结构的表征系统，而非单纯转写媒介，对日语过去时动词活用进行正字法感知诊断；使用两种字符级Transformer架构，在五个随机种子下评估，并进行受控消融实验以隔离该特定不规则子类的因果影响。

Result: 两个系统总体准确率均超过97%，但词干以/e/结尾且过去时后缀前需促音化的不规则子类仅占数据不足1%，却占残余错误的30-43%，对总错误的贡献约为其流行度的34-48倍；仅移除该子类带来的准确率提升大于移除所有不规则动词之和。

Conclusion: 神经形态学习中的错误集中并非由不规则性本身驱动，而是由极低频形态模式与特定正字法过程之间的交互所致；形态学评估应纳入细粒度子类分析，并对数据高效、符合发展规律的语言模型预训练具有启示意义。
...
### [127] [When Better Turns Do Not Make Better Agents: Diagnosing the Gap Between Next-Turn Metrics and Workflow Success](https://arxiv.org/abs/2609.21187)
...
TL;DR: 该论文发现，逐决策的"下一轮成功"评测并不能可靠预测智能体的自主工作流执行能力，因此主张分别报告文本质量、局部动作正确性、工具执行与端到端任务完成度。


<details>
  <summary>Details</summary>
Motivation: 当前智能体模型常采用"一次一决策"方式评测：在黄金交互历史下预测下一动作并与参考答案比对。作者质疑这种逐轮评测的改进是否能预测真实的自主工作流执行能力，即其作为评测代理的可靠性。

Method: 在多轮客户支持工作流上，研究 pre-SFT 与 SFT 的 Qwen3（4B、14B）和 Gemma 3（4B、12B）模型，对比黄金历史评测下的下一轮成功与自主工作流执行的严格轨迹完成评测。

Result: SFT 一致提升文本轮成功，且所有模型在黄金历史评测下下一轮整体成功率都上升；但这些提升并未迁移到自主工作流执行。工具相关的增益因指标和模型而异。四个 SFT 模型在整体工作流评测下均未成功，严格轨迹完成的工作流成功率最高仅 10.4%。

Conclusion: 下一轮评测不是工作流成功的可靠代理，应分别报告文本质量、局部动作正确性、工具执行和端到端任务完成情况。
...
### [128] [Beyond Reference-Based Evaluation: Reward Models for Meta-Evaluation of Grammatical Error Correction](https://arxiv.org/abs/2609.21231)
*Ruotian Wu,Bill E. Johnson,Gene Saunders,Osama Hamzeh,Ankit Vadehra,Pascal Poupart*

Main category: cs.CL

TL;DR: 本文提出RM-EVAL，一种基于人类偏好数据训练的无参考奖励模型元评估器，可进行整句和部分序列质量判断，并可用作RGTG的奖励信号改进GEC生成。


<details>
  <summary>Details</summary>
Motivation: 传统GEC评估指标（如M²、ERRANT）依赖参考集枚举所有有效编辑，因而会惩罚语法正确、语义保持但表达不同的修改。作者希望摆脱对金标准参考的依赖，实现更贴近人类判断的评估，并进一步将评估信号用于提升GEC系统。

Method: 构建RM-EVAL：在SEEDA人类偏好数据上训练奖励模型，作为无参考元评估器，预测整句与部分序列层面的人类化质量判断。同时提出Reward-Guided Text Generation（RGTG）：保持基础GEC模型冻结，进行在线、奖励驱动的解码以改进生成。

Result: 在SEEDA上，RM-EVAL与人类排名高度一致；RGTG在奖励和外部验证上均取得一致提升。

Conclusion: RM-EVAL与RGTG构成统一框架，可在不依赖金标准参考的情况下同时评估与增强GEC系统。
...
### [129] [When Does Reasoning Help in Machine Translation? A Hierarchical Analysis of LRM Reasoning Traces](https://arxiv.org/abs/2609.21247)
*Yuxiang Liu,Jiaming Luo,Eleftheria Briakou,Colin Cherry*

Main category: cs.CL

TL;DR: 该论文系统分析了大推理模型在机器翻译中中间推理轨迹的作用，提出层级元摘要（HMS）框架揭示推理的结构与模式，主张对翻译推理进行模型感知、长度感知与模式感知的调控。


<details>
  <summary>Details</summary>
Motivation: 大推理模型在机器翻译中日益使用中间推理轨迹，但何时这种推理有益、何时有害尚不明确，亟需系统分析推理语言、长度与结构等因素的影响。

Method: 跨模型、语言、领域与数据集分析推理轨迹，关注推理语言、长度与结构；提出可扩展的层级元摘要（HMS）框架，在无预定义分类体系下归纳粗粒度与细粒度的推理结构。

Result: 最优推理语言因模型而异；推理长度与质量呈非单调关系；推理轨迹呈现反复出现的功能模式。HMS揭示了共享组织（理解/规划、翻译/起草、精炼/验证）及领域特定的变异。

Conclusion: 机器翻译中的推理应以模型感知、长度感知和模式感知的方式进行控制，而非一律鼓励使用推理。
...
### [130] [Beyond Atomic Tokens: Factorizing Syllables for Language Model Pretraining](https://arxiv.org/abs/2609.21362)
*Nghia Hieu Nguyen,Thai Bao Huynh,Binh-An Dinh-Le,Phu Gia Hoang,Dat Tien Nguyen,Kiet Van Nguyen,Ngan Luu-Thuy Nguyen*

Main category: cs.CL

TL;DR: 提出音素分词器 Phonemic Tokenizer，将越南语和汉语音节转为 IPA 并分解为声母、韵母、声调三个音系成分，共享一个上下文位置，词表仅 112（中文）和 256（越南语），并实例化为 PhonemicBERT 在多项理解任务上达到或超过字符/子词基线。


<details>
  <summary>Details</summary>
Motivation: 传统分词器以字符或统计子词表示文本，忽略音节内部音系结构，且往往需要大词表。作者希望引入语言学先验，用音系分解实现紧凑、高效且可解释的表示。

Method: 将每个音节转换为 IPA 并因子化为 onset（声母）、rime（韵母）、tone（声调）三部分；三者共同占用一个上下文位置，保持音节级序列长度，同时允许音系相关音节共享表示；非音系及不支持单元采用字符级回退；设计为确定性、无需语料相关词表学习。进一步构建 PhonemicBERT，融合因子化成分嵌入，并用三个预测头重建完整被掩码音节。

Result: 在两种语言上取得显著更高的 Rényi 效率；对标准越南语音节词典的每个条目 Fertility 恰为 1；越南语序列通常比现有预训练分词器更短。受控中文预训练下，PhonemicBERT-Zh 在多种语言理解任务上与字符、子词及 SubChar 方案相当或更优；PhonemicBERT-Vi 与既有越南语及多语言预训练模型相比也具竞争力或更优。

Conclusion: 音系因子化是一种紧凑、高效且可解释的文本表示方案，可替代原子化与统计式分词表示。
...
### [131] [ArenaFlow: From Trajectory Ranking to Hierarchical Credit Propagation for Open-Ended Agent RL](https://arxiv.org/abs/2609.21378)
...
TL;DR: ArenaFlow：面向开放式智能体强化学习的分层信用传播框架，通过锦标赛相对排名获得轨迹级奖励并结合结构化反思评估，将优势传播至关键步骤并构建技能记忆。


<details>
  <summary>Details</summary>
Motivation: 强化学习已在可验证领域的LLM智能体中取得显著进展，但在开放式任务中难以应用：解具有多样性且难以获得可靠的标量奖励。现有成对评估方法虽通过相对偏好缓解奖励判别崩塌，但仍将丰富的比较反馈压缩为单一轨迹级奖励，掩盖了决定性中间步骤，也无法将成功行为固化为可复用技能。

Method: 提出ArenaFlow分层信用传播框架：1）利用基于锦标赛的相对排名导出轨迹级奖励信号；2）每次比较配备结构化反思评估，揭示三类监督信号——关键成功步骤、可复用策略技能、检索技能的使用归因；3）步骤级：依据锦标赛存活深度将轨迹级优势传播至高置信关键步骤，实现局部推理行为的针对性优化；4）技能级：从组级使用归因估计技能效用，通过效用感知的更新、剪枝与检索维护全局技能记忆，高效用技能作为未来探索的策略先验。

Result: 大量实验验证了ArenaFlow在开放式智能体任务上的有效性。

Conclusion: ArenaFlow通过分层信用传播，将轨迹级相对奖励细化为步骤级与技能级监督，有效解决了开放式智能体强化学习中奖励稀疏、信用分配粗粒度及技能难以复用的问题。
...
### [132] [Benchmarking Gender Bias in Machine Translation Evaluation Metrics across Occupations](https://arxiv.org/abs/2609.21490)
*Orfeas Menis Mastromichalakis,Giorgos Filandrianos,Wafaa Mohammed,Giuseppe Attanasio,Chrysoula Zerva*

Main category: cs.CL

TL;DR: 论文研究WMT 2026自动翻译质量评估任务中的性别偏见，使用GAMBIT+职业平衡子集（7个英语源语言对，每语言1308对男/女性别翻译对），发现男性化译文整体得分更高，且差异随职业呈刻板印象分布，但评估器与语言间强度不一致。


<details>
  <summary>Details</summary>
Motivation: 机器翻译在源文本性别未指明时，译文可能采用阳性或阴性形式，而MT系统与自动评估指标可能对二者存在系统性偏好，尽管源文本并未提供区分依据。性别偏见既影响生成译文也影响其自动评估，但现有评估对偏见的方向、幅度、频率缺乏多维度刻画，尤其在共享任务语境下缺少细粒度分析。

Method: 基于GAMBIT+构建职业平衡子集：7个英语源语言对（阿拉伯语、捷克语、希腊语、冰岛语、俄语、乌克兰语，外加德语扩展），每种目标语言1308个男/女译文对，覆盖436个ISCO-08职业组、每组3例。对共享任务提交系统及基线进行评分预测与错误标注评估，考察性别相关差异的方向、幅度与频率。

Result: 译文整体呈现阳性形式得分更高的倾向；不同职业间的差异符合性别刻板印象；该偏好的强度与一致性在评估器之间和目标语言之间存在显著差异。

Conclusion: 性别偏见仍存在于MT评估中；仅靠单一聚合指标无法充分捕捉其程度，需结合评估器行为的多个互补维度进行衡量。
...
### [133] [MIRAGE: Multi-Perspective Creative Language Model Reasoning with Reinforcement Learning Guidance](https://arxiv.org/abs/2609.21554)
*Arash Lagzian,Srinivas Anumasa,Dianbo Liu*

Main category: cs.CL

TL;DR: MIRAGE 是一个推理时创造性思维框架，通过动态切换多种概念视角（如代数、概率）来提升大语言模型在数学、科学和逻辑任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽取得显著进展，但在复杂数学、科学和逻辑任务上仍表现不足。受人类认知灵活性（即动态切换心理视角的能力）启发，作者希望赋予模型多视角推理能力以弥补这一短板。

Method: 提出 MIRAGE（多视角推理时推理，通过智能体引导探索）。框架包含两个核心组件：Selector（选择器）负责优先挑选有效的概念视角（如代数、概率视角）；Reasoner（推理器）顺序求解任务，直至产生高置信度的解，否则将多个视角的结果进行聚合。

Result: 在 GSM8K、MATH500、MMLU-Pro 和 Game-of-24 四个基准上测试，MIRAGE 持续优于 Chain-of-Thought 及多种提示集成方法，在极小的推理开销下显著提升准确率。

Conclusion: MIRAGE 提供了一种可扩展的实用解决方案，通过多视角智能体引导探索有效增强大语言模型的复杂推理能力，兼顾性能与效率。
...
### [134] [Rethinking Human-Aligned Evaluation: An Analysis of Semantic Metrics Beyond WER](https://arxiv.org/abs/2609.21663)
*Hritika Sharma,Thibault Bañeras-Roux,Alessandra Pinto,Petr Motlicek,Hyunggu Jung,Esaú Villatoro-Tello,Somang Nam*

Main category: cs.CL

TL;DR: 本文提出HATS-en英文人类评价数据集，系统比较WER、CER、BERTScore与SemDist与人类判断的一致性，发现WER一致性最低，最佳SemDist配置最高，CER性价比突出。


<details>
  <summary>Details</summary>
Motivation: WER作为ASR最常用指标，将任何词级偏差等同视之，忽略是否改变语义，因此需要检验WER是否真正反映人类对转写质量的判断，并寻找更贴合人类感知的评价指标。

Method: 构建HATS-en英文人类评价数据集；以人类评分为基准，对比词级指标（WER、CER）与多种BERTScore、SemDist配置（改变语言模型、层选择与池化策略）的一致性。

Result: 在所有测试指标中WER与人类判断一致性最低；最佳SemDist配置整体一致性最高，优于CER与BERTScore；不存在跨设置普遍最优的单一模型；CER虽简单低成本，但表现接近最佳配置。

Conclusion: 支持将ASR评价转向CER（对英文及音节文字系统均适用），因其更可解释、低成本且更贴近评价应捕捉的目标，并将SemDist作为补充评价手段。
...
### [135] [PRISM-BN: A Controlled Corpus and Benchmark for Text-to-Parameterized Bayesian Network Extraction](https://arxiv.org/abs/2609.21673)
*Amartya Bhattacharya,Nikhil Singh,Neeti Pokhriyal,Soroush Vosoughi*

Main category: cs.CL

TL;DR: PRISM-BN：5054条文本-贝叶斯网络配对语料，用PRISM边际优先流程构建，支持结构恢复与概率参数估计的分离评测。


<details>
  <summary>Details</summary>
Motivation: 概率图模型（尤其贝叶斯网络）具有有向结构与概率参数，是神经符号AI的天然符号化目标；但训练"文本到参数化贝叶斯网络"系统需要大规模配对的文本-BN资源，此类数据在规模上不可得。

Method: 构建受控语料PRISM-BN：5054条BN grounding描述配对离散参考BN，含变量、状态、有向边、根先验及完整多父节点CPD，覆盖五个领域；实例源自50个Wikipedia种子骨架，概率为内部构造的基准目标而非外部验证的因果估计。采用PRISM边际优先流水线：先诱导边际与局部联合分布，解析恢复归一化CPD，并构建局部重参数化子图。基准定义含语义节点/状态对齐、条件结构评分与严格全CPD评测。

Result: 在六个LLM抽取器上，Node F1为0.56–0.83，条件Edge F1为0.90–0.97，CPD-KL为1.11–3.14。条件状态与边恢复持续较强，严格全CPD一致性仍具挑战；该趋势在独立生成的GPT-5.5参考上保持一致，人类试点亦支持结构可恢复性与相似的概率解释。

Conclusion: PRISM-BN支持对结构恢复与概率参数估计进行分离评测，为神经符号文本到贝叶斯网络的建模与评估提供受控基准。
...
### [136] [Reusing Latent Speech Representations for Query-Conditioned Topic Localization in Transcripts](https://arxiv.org/abs/2609.21844)
*Steffen Freisinger,Philipp Seeberger,Thomas Ranzenberger,Tobias Bocklet,Korbinian Riedhammer*

Main category: cs.CL

TL;DR: 本文研究"查询条件化主题定位"任务：在长转录文本中预测最能回应主题标题查询的句子片段。方法复用ASR编码器状态作为句级表示，并与文本嵌入融合，使轻量片段定位器无需单独音频编码器即可利用语音信息。两个公开数据集上均优于纯文本基线，尤其在严格边界匹配标准下；跨数据集实验显示结构化/半结构化语音收益最大，自发语音增益有限且不稳定。


<details>
  <summary>Details</summary>
Motivation: 长转录文本作为下游NLP系统输入成本高昂，且包含大量无关上下文。需要定位转录中与给定主题查询最相关的句子片段，以降低成本并提升下游效率；同时希望在不额外训练音频编码器的前提下利用语音信息改进定位。

Method: 提出查询条件化物主题定位任务。复用ASR编码器状态作为句级表示，将其与文本嵌入融合，为轻量级片段定位器提供语音线索，避免引入独立音频编码器。

Result: 在两个公开数据集上相较纯文本基线取得一致提升，在严格边界匹配准则下提升尤为明显。跨数据集实验表明，收益在结构化或半结构化语音上最强，而在自发语音上增益有限且表现参差。

Conclusion: 融合ASR编码器状态是一种无需额外音频编码器的有效增强方式，可提升转录中查询条件化的片段定位性能；但其有效性依赖语音类型，对自发语音的迁移增益有限。
...
### [137] [TrialAtlas: Multi-Agent Research Organization for Clinical Trial Design and Optimization](https://arxiv.org/abs/2609.21859)
*Jiacheng Lin,Zifeng Wang,Zheng Chen,Erick Scott,Ziwei Yang,Fanyang Yu,Sheng Zhong,Jimeng Sun*

Main category: cs.CL

TL;DR: TrialAtlas：记忆增强的多智能体研究组织，用于临床开发规划（CDP）；配套TrialAtlasBench基准，在缺陷检测、设计改进建议与技术/监管成功预测三项任务上均超越基线。


<details>
  <summary>Details</summary>
Motivation: 约90%进入临床开发的药物最终失败，药企依赖CDP与技术/监管成功概率评估来预判风险，但现有决策高度依赖人力、主观性强，需跨临床科学、统计、监管事务与竞争情报专家共同获取、整合并推理异构证据，缺乏自动化、可积累经验的支持系统。

Method: 提出TrialAtlas，一种记忆增强的多智能体研究组织，通过协调文献综合、竞争性试验情报、监管先例分析及试验设计与开发风险集成推理等专用智能体，模拟专家协作流程；并从历史临床试验与监管结果（含既往NDA）中学习，将决策建立在积累的开发经验之上。同时构建TrialAtlasBench，基于291份FDA完整回复函（CRL），涵盖试验设计缺陷检测、可执行设计改进建议、技术与监管成功预测三项任务。

Result: 缺陷检测F1达50.0%，超最强基线6.1个百分点；技术/监管成功预测达85.3%平衡准确率与84.7% F1，较最佳基线在平衡准确率上提升6.7个百分点、Cohen's kappa提升12.0个百分点；专家评估中TrialAtlas生成关注的86.4%被判为有效，高于OpenAI DeepResearch的83.1%与Gemini DeepResearch的59.3%。

Conclusion: TrialAtlas以多智能体协作与记忆机制将CDP与监管成功预测从主观、劳动密集的过程转化为可积累经验、可量化评估的自动化流程，并在真实监管场景基准上验证了其有效性与专家认可度。
...
### [138] [RecreationWorld: Scalable and Verifiable Environments for Hybrid Computer-Use Agents](https://arxiv.org/abs/2609.22000)
...
TL;DR: 该论文提出混合计算机使用智能体（hybrid CUA）设定，并构建 RecreationWorld 五平台框架与 RecreationBench 基准，用以评估智能体在无既定流程下"复刻"运行中参考软件的能力。


<details>
  <summary>Details</summary>
Motivation: 现有计算机使用智能体分两条独立路线发展：图形界面交互与通过代码/命令行进行软件开发。而真实数字工作需要在两者之间交替进行，而非端到端串行堆叠，因此需要研究能自主决定何时探索界面、何时实现软件并运行和视觉验证产物的混合型智能体，同时缺乏相应的可复现环境与评测基准。

Method: 提出 RecreationWorld：围绕"复刻"（recreation）设计的五平台框架（Ubuntu、macOS、Windows、Android、Web），提供可复现环境与统一测试台，集成原生 GUI 控制与编码工具。任务设定为：给定一个运行中的参考程序，智能体须自行发现其行为并构建忠实实现，不预设工作流。以运行中的参考作为隐藏行为测试的 oracle，提供基于执行的奖励。利用高质量开源应用扩展轨迹生成规模，并据此训练模型。

Result: 基于所生成轨迹训练的模型在五个分布外编码与混合计算机使用基准上均取得提升，并更频繁地对渲染输出进行验证，显示跨复刻场景的迁移证据。在冻结的 RecreationBench（250 个跨领域、跨平台任务，含基于参考的程序化与视觉断言，经参考程序与人工审阅双重验证）上，GPT-6 Astra 以 58.1% 总体得分领先，但仅 2.8% 的任务通过全部程序化测试。智能体复现静态界面结构比复现交互与计算输出更可靠；生成的应用程序相比参考仍更小、更单体化。

Conclusion: 混合计算机使用（交替进行界面探索、软件开发与视觉验证）是可训练且可评测的方向；当前最强模型在完整通过全部程序化测试的任务上比例极低（2.8%），交互与计算输出仍是主要瓶颈，生成应用在规模与模块化程度上与真实参考存在差距。作者开源基准、环境与测试套件。
...
TL;DR: 本文提出 QuranicMMLU 基准，从语言学五大层级对生成式 AI 的古兰经阿拉伯语能力进行评估，构建 980 道经人工审核的题目，并以开放式与多项选择两种形式测试 12 个系统。


<details>
  <summary>Details</summary>
Motivation: 现有古兰经基准主要关注通用问答与语义检索，未针对具体语言学能力进行探测，也未按认知需求与经文难度进行分层，因而难以细致刻画模型在古兰经阿拉伯语上的真实能力。

Method: 构建涵盖音系学、形态学、句法学、语义学、语用学五大支柱、31 个叶节点的古兰经分类体系；为每个叶节点按 Bloom 认知层级与经文困惑度分层生成题目；采用 LLM-as-a-judge 独立作答与打分并转交人工复核；最终形成 980 道经人工审核的题目，每题同时以开放式与多项选择两种形式呈现；在该数据集上对 12 个系统进行评测。

Result: 伊斯兰专门化模型表现最佳；但所有系统在多项选择准确率上（平均 84%）均高于开放式作答质量（平均 60%）；两种形式的系统排名高度一致（Kendall's τ=0.73）；多项选择评分掩盖了去除选项后才暴露的失败。

Conclusion: QuranicMMLU 为古兰经领域的阿拉伯语 NLP 评估提供了严谨且语言学基础扎实的框架，并揭示多项选择评测的低区分度问题。
...
TL;DR: 提出零参数记忆决策控制器MDL，通过三信号互补编码对检索记忆的可信度进行显式判定，在冲突记忆下将幻觉率降低约56.04%，高危场景接近零幻觉，单次决策仅约0.14 ms。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型记忆系统主要聚焦高效检索，却忽视"检索到的记忆是否应当被信任"这一决策问题。当记忆库中存在冲突立场时，标准RAG会盲目注入记忆并放大幻觉：在易受记忆注入影响的模型上，冲突记忆下的RAG幻觉率显著高于无记忆基线。作者受前额叶皮层记忆信号机制启发，希望补上"检索—生成"之间缺失的可信度判定环节。

Method: 提出Memory Decision Layer（MDL），一个位于检索与生成阶段之间的零参数记忆决策控制器。核心是三信号互补编码器：融合相关性、可靠性与任务风险，通过基于QR的正交子空间投影，并结合元工作记忆信号，形成可解释的决策表示，量化检索记忆的可信程度。在此编码器基础上，MDL显式解耦置信度与一致性，并引入风险反转与显式弃答（abstention）机制。控制器完全白盒，仅依赖几何运算，无需训练参数。

Result: 在主流大语言模型与多个开源数据集上评测：一般场景下冲突记忆导致的幻觉率降低约56.04%，高危场景下接近零幻觉；单次决策仅增加约0.14 ms，比前置的嵌入检索步骤快约50倍，比LLM自评估调用快四到五个数量级。

Conclusion: MDL将记忆系统的研究重点从"检索效率"扩展到"检索可信度决策"，以零参数、白盒、低开销的方式有效抑制冲突记忆引发的幻觉，并提供风险反转与弃答能力，为RAG中的记忆信任问题提供了轻量且可解释的解决方案。
...
TL;DR: 本文评估法语职业事故叙述中“事故过程角色分类”的跨行业泛化能力，发现任务特定微调显著优于冻结预训练表示，前三种方法在三个目标语料上平均平衡准确率约85.6%–85.8%。


<details>
  <summary>Details</summary>
Motivation: 职业事故叙述蕴含工作情境、不利条件、事故事件及后果等信息，自动结构化可支持大规模事故分析与风险预防；但事故描述术语与写作风格因行业和组织而异，自动编码系统能否泛化到训练域之外尚不明确，因此需检验跨行业泛化性。

Method: 构建专家标注语料，将事实单元分为四类角色：工作情境（A0）、明确报告的不利条件（A1）、事故事件或偏差（B）、报告后果（C）。角色分类器仅在来自6,040条建筑行业叙述的42,244个事实单元上开发与选择，随后在未见的冶金、化学—塑料行业语料及独立收集的企业语料上评估，不进行重训练或目标域调优。比较冻结预训练表示、任务特定微调与监督表示学习策略。

Result: 任务特定适配在跨域迁移上始终优于冻结表示；重复训练中，三种领先的任务适配策略在三个目标语料上的平均平衡准确率为85.6%–85.8%。

Conclusion: 研究支持开发可迁移的辅助编码系统，以一致地结构化异构职业事故叙述，服务于专家复核与跨行业预防分析。
...
### [142] [How Many Posterior Samples? Calibrated Stopping for Adaptive Sensing](https://arxiv.org/abs/2609.21813)
*Vincent Corlay,Andriy Enttsel*

Main category: stat.ML

TL;DR: 本文研究分类导向自适应感知中的"停止层"：在不修改后验采样器与感知方向的前提下，将后验样本的类别投票转化为类别声明。作者指出直观的插件式阈值规则（投票份额超过阈值即声明）并不构成置信保证——当真实投票质量恰好等于阈值时，插件规则约有一半概率误声明。为此，作者将固定样本规则与有限时域序贯规则校准到预设的误声明概率，并研究"精确截断"（一旦最终判决被确定即停止）。随后推导一轮声明概率如何决定沿感知路径的后验样本开销与分类精度。在 MNIST 上使用 DDRM 和固定 PCA 引导探测序列，截断最多节省 62% 的后验样本；在匹配工作点上序贯停止开销最低；在高精度下序贯规则可用更多后验样本换取更少测量。


<details>
  <summary>Details</summary>
Motivation: 分类导向自适应感知中，后验样本既用于指导下一步感知方向，又通过类别投票决定是否继续感知。然而，将投票份额与阈值直接比较的插件式停止规则看似自然，却缺乏置信度语义：当真实投票质量等于阈值时，该规则约有 50% 的概率做出声明。因此需要一个显式的"停止层"，把投票转化为具有可校准误声明概率的判决，同时不改变后验采样器和感知方向。

Method: 1) 形式化停止层问题：仅将后验样本的类别投票映射为声明，保持采样器与感知方向不变。2) 分析插件式阈值规则的缺陷，证明其在投票质量等于阈值时声明概率约为 1/2。3) 提出并校准两类具有误声明概率保证的规则：固定样本规则与有限时域序贯规则。4) 研究精确截断（exact curtailment）：当固定池规则的最终判决已被确定时提前停止。5) 推导一轮声明概率与后验样本开销、分类精度沿感知路径的关系。6) 在 MNIST 上用 DDRM 与固定 PCA 引导探测序列进行实验验证。

Result: 理论上：插件式阈值并非置信保证，在投票质量等于阈值时误声明概率约 0.5；固定样本与有限时域序贯规则可校准至预设误声明概率；给出声明概率与样本开销、精度的路径级关系。实验上（MNIST + DDRM + 固定 PCA 引导探测序列）：精确截断最多节省 62% 的后验样本；在匹配工作点下，序贯停止的成本最低；在高精度区，序贯规则可用更多后验样本换取更少的测量次数。

Conclusion: 停止层应被显式校准，而非依赖直观的插件式阈值，因为后者不提供置信保证。可校准的固定样本规则、有限时域序贯规则及精确截断能够以可控的误声明概率显著降低后验采样成本；其中序贯停止在匹配工作点下最省成本，并能在高精度情形下以后验样本换取测量次数，为分类导向自适应感知提供了实用的停止准则。
...
TL;DR: 研究二分类最小问题：能否构造规则使对任意P≠Q的分类准确率严格优于随机猜测；给出核方法、序方法与可测编码的构造，并刻画先验不平衡与自适应选择下的不可能性。


<details>
  <summary>Details</summary>
Motivation: 探讨最弱假设下的二分类可学习性：仅知两分布不同(P≠Q)、无样本量、无先验信息，且目标Y以等概率来自P或Q，是否存在普遍优于随机的规则，并厘清其边界条件（如不平衡先验、自适应类别选择）。

Method: 1) 反例：光滑密度下一近邻可劣于随机，而共同正定协方差的高斯下最近邻有效；2) 构造固定随机化核规则，期望准确率为1/2+MMD_k²(P,Q)/4，并由可数族可测二值问题得到可数生成空间上的特征核；3) R上确定性序规则对任意不同Borel概率测度优于随机；4) 通过可测编码推广到可数生成可测空间（含可分度量空间）；5) 分析未知不平衡类先验与自适应目标类选择下的不可能性。

Result: 存在随机化核规则统一优于随机且准确率由MMD²显式刻画；R上确定性序规则普遍有效并可编码到可数生成空间；但若类先验未知且不平衡，或允许目标类自适应选择，则任何规则（或除公平硬币外）对某有限支撑分布对严格劣于随机。

Conclusion: 在等概率目标先验下，普遍优于随机是可行的，且可通过核MMD与序/可测编码实现分布无关的确定性规则；其根本限制来自不平衡先验与自适应类别选择，此时不存在普遍有效规则。
...
TL;DR: 该综述将大语言模型的复杂问题求解(CPS)重新建模为对潜在解状态的序贯估计-决策问题，以"过程控制"区分于"推理"为组织主线，提出五组件分类框架与"控制匹配"诊断假说，指出可靠验证、针对性恢复、不确定性校准与匹配预算评估是核心开放问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究常把LLM复杂问题求解归因于"推理更强"或"生成更长"，但早期步骤误差放大、提示脆弱性、无法修正错误承诺等现象，仅靠知识缺失或表达容量不足难以解释。因此需要一个能解释这些失败机制并统一组织现有方法的理论视角。

Method: 将CPS形式化为潜在解状态上的序贯估计与决策问题：控制器维护关于未观测解轨迹的信念，随噪声中间证据到达而更新，并决定提交、验证、分支、回滚或弃答以最小化期望损失。区分"推理"(提供候选转移与解释)与"过程控制"(塑造与评估提案、调节后续转移与观测)。据此把现有方法归纳为五个组件——显式状态表示、转移结构化、验证与约束执行、搜索与回滚、不确定性管理；并按所估计的统计量重新解读评估指标。进一步区分系统性/随机性/不可约误差与认知/偶然不确定性，提出"问题-控制匹配"及失配的诊断假说。

Result: 构建了一个统一的估计-决策框架与五组件方法分类体系；建立了评估指标与所估计统计量之间的对应；给出可检验的诊断假说：干预应在针对观测失败所牵涉的误差或不确定性成分时最有效，例如额外采样可降低采样方差却无法改变共同的系统性误差。

Conclusion: 该视角厘清了当前方法实际估计与控制了什么、哪些部分仍不受控，并说明为何可靠验证、针对性恢复、校准不确定性及匹配预算的评估是关键开放问题。
...
TL;DR: 该论文研究非参数回归中谱正则化方法的数据驱动早停策略，提出多尺度停止规则，并结合随机特征近似，实现全平滑度自适应与随机特征数、神经网络宽度的数据驱动选择。


<details>
  <summary>Details</summary>
Motivation: 现有谱正则化早停方法难以在所有平滑度水平上实现完全自适应，且在大数据集上计算成本高；同时随机特征与神经网络宽度等容量参数通常需要先验知识。

Method: 基于差异原则（discrepancy principle）提出适用于一般核估计器的多尺度停止规则，并扩展到随机特征近似；通过随机特征与神经正切核（NTK）区域神经网络的联系，将该规则用于选择网络宽度与停止时间。

Result: 在正确设定情形下实现对所有平滑度水平的完全自适应；随机特征扩展在降低大数据计算成本的同时保持极小极大最优统计保证；可同时数据驱动地选择最优停止时间与所需随机特征数。

Conclusion: 该方法为随机特征数与神经网络宽度提供有原则的数据驱动建议，使神经网络在无需平滑度或容量参数先验知识的情况下达到极小极大最优学习率。
...
TL;DR: 该论文研究等变高斯过程的可扩展性问题，通过均值函数与协方差核实现等变稀疏GP，并给出免积分等变核，构建近乎三重可扩展的等变GP，用于SO(2)等变矢量场与分子性质预测。


<details>
  <summary>Details</summary>
Motivation: 高斯过程具备概率预测与先验（含等变性）编码能力，但在大规模科学问题中受计算成本限制；等变神经网络常用却缺乏GP式的不确定性量化，而分子研究等应用高度需要该量化。高维输入与大对称群进一步要求可扩展性。

Method: 建立GP等变性与条件化相互作用的结果，借此通过合适的均值函数与协方差核获得等变稀疏GP；实例化为免积分等变核的灵活类，实现可扩展且数据高效的推断；采用SO(2)等变稀疏变分GP（SVGP）；并开发免矩阵等变全GP实现，结合精确Kronecker约简与预处理共轭梯度求解，以评估完整联合预测密度；同时比较等变稀疏GP中诱导点选择的不同方法。

Result: 在合成SO(2)等变场以及基于量子化学模拟的N-甲基甲酰胺电偶极矩预测任务上，实现准确且具不确定性感知的预测，计算成本仅为经典GP推断的一小部分。

Conclusion: 所提出的三重可扩展等变高斯过程框架兼具等变性、稀疏可扩展性与不确定性量化，可高效处理高维与大对称群的科学建模任务，在分子性质预测等场景中表现有效。
...
TL;DR: 该论文提出面向矩阵值协变量的个体化稀疏回归框架，并引入对角化注意力机制，用query-key分数定位每个样本专属的信号行、用value矩阵做回归，理论上证明了单头/多头模型可高概率恢复潜在行并给出预测风险界。


<details>
  <summary>Details</summary>
Motivation: 矩阵值表示（行对应token、patch等局部特征）中，预测信息往往稀疏但样本特异，传统共享支撑集的稀疏回归难以适应这种异质性。

Method: 提出个体化稀疏回归框架：每个观测有自身关注的行，但回归效应在全人群共享；引入对角化注意力机制，用query-key分数定位样本特异信号行，用value矩阵进行下游回归。参数维度与样本量无关，且可在无响应的情况下为新样本识别关注行。

Result: 在适当的分数分离与集中条件下，建立存在性定理，证明单头与多头对角化注意力模型能以高概率恢复潜在行，并给出预测风险界；模拟实验在回归和误设定分类中随样本量、维度和信号基数变化均表现出强预测与定位能力；真实情感分析显示分类精度提升且token选择可解释。

Conclusion: 该理论为注意力打分如何在异质矩阵值数据中定位样本特异信号提供了统计学解释。
...
TL;DR: LoSc：一种超快速、准确且具理论支撑的大规模筛查框架，通过仅含逻辑运算的解码算法实现 O(klogn) 混合检测识别所有阳性样本。


<details>
  <summary>Details</summary>
Motivation: 疫情早期需通过大规模筛查识别少数感染者，但受限于试剂与检测能力。现有分组检测方法或计算复杂度高、或识别准确率低，且缺乏对样本使用约束与稀释效应等硬约束下稀疏识别的严格理论分析。

Method: 提出逻辑筛查方法 LoSc，引入采用极简选择策略的新型解码算法，解码仅依赖逻辑运算，支持直接硬件实现；在池化设计中显式纳入稀释效应与样本使用约束，并建立理论保证以指导最优池化配置。

Result: LoSc 仅需 O(klogn) 次混合检测即可识别所有阳性样本，实现超快速计算；大量仿真验证了其在有效性、效率和可扩展性方面的优越性。

Conclusion: LoSc 为自动大规模筛查提供了一种快速可靠的解决方案，兼具超快速度、高准确性与理论严谨性。
...
TL;DR: 本文研究深度ReLU表示后接有界范数预测器（Brownian heads）的条件经验Rademacher复杂度，针对同一样本上进行特征选择与预测器拟合导致的"选择成本遗漏"问题，给出了精确对偶恒等式、锐利界以及coordinated/符号投影阈值迹的形式。


<details>
  <summary>Details</summary>
Motivation: 深度表示学习常在同一批样本上同时进行隐藏特征选择与最终预测器拟合。此后再做固定特征的分析会忽略"选择成本"，从而低估复杂度。作者希望刻画"同一样本选择"下的条件经验Rademacher复杂度，把已实现的尺度与选择复杂度分离开来。

Method: 研究对象为Deep ReLU表示后接加性RKHS或Lévy-Brownian RKHS中的有界范数预测器（Brownian head）。对固定表示，推导精确对偶恒等式，并用激活质量（激活值平均范数）给出锐利界。在同一样本选择下，表示上确界诱导出二次Rademacher过程；利用Brownian layer-cake与Gaussian projection恒等式将其约化为逐坐标或带符号的投影阈值迹，从而分离已实现尺度与选择复杂度。对输入两两不同的样本，构造显式标量ReLU族，使其在已实现迹与包络层面以通用常数匹配有限迹率和VC率。通过诱导范数收缩，将结果推广到矩形、秩亏ReLU网络的架构级界。

Result: 得到了固定表示下的精确对偶恒等式与基于激活质量的锐利界；将二次Rademacher过程约化为阈值迹形式；显式标量ReLU族在实现迹-包络层面匹配有限迹与VC率（至通用常数）；给出了矩形、秩亏ReLU网络的架构级复杂度界。实验验证了锐利界与率，展示了固定激活质量下的"选择间隙"，并评估了Brownian heads的预测可行性。

Conclusion: 同一样本特征选择带来的选择成本不可忽略，需在复杂度分析中显式建模。通过二次Rademacher过程与阈值迹分解，可将已实现尺度与选择复杂度分离，并为深层ReLU网络（含秩亏情形）提供架构级泛化界；实验支持理论界与选择间隙的存在。
...
TL;DR: 该论文基于Breiman关于"在不削弱单棵树的前提下降低树间相关性可提升随机森林"的观察，提出Dirichlet-Multinomial Bagging Random Forest (DM) 与 Dirichlet-Weighted Random Forest (DW) 两种变体，通过浓度参数α调节样本重加权，并给出判定其与标准随机森林行为一致的简化理论准则，据此设计轻量调参策略。


<details>
  <summary>Details</summary>
Motivation: Breiman曾观察到，若能降低树间相关性而不削弱单棵树的强度，则可提升随机森林性能。论文以此原理为出发点，试图通过样本重加权机制进一步控制树间相关性，从而在保持单树能力的同时提升集成效果。

Method: 提出两种随机森林变体：(1) Dirichlet-Multinomial Bagging Random Forest (DM)；(2) Dirichlet-Weighted Random Forest (DW)。二者均通过浓度参数 α>0 对样本重加权进行调制。此外，论文给出一个简洁的理论准则，用于刻画在何种条件下这两种变体与标准随机森林的行为无法区分（即退化为标准RF），并以此准则指导一种轻量级的超参数调优策略。

Result: 在公开分类基准上的受控实验中，DM与DW表现稳定且具有竞争力，往往优于其他随机森林基线方法，且额外运行时间开销可忽略不计。

Conclusion: 通过Dirichlet基的样本重加权机制调节树间相关性，可在几乎不增加计算成本的前提下提升随机森林性能；所提供的理论准则明确了变体与标准RF等价的条件，并为轻量调参提供了依据。
...
### [151] [Riemannian Simultaneous Inference for Tangent Vector Field Regression](https://arxiv.org/abs/2609.21910)
*Xiaotian Chang,Yangdi Jiang,Qirui Hu*

Main category: stat.ML

TL;DR: 提出黎曼流形上切向量场回归的非参数核估计方法，通过平行移动与体积校正实现均匀偏差、协方差与随机率分析，并构造可行的同时置信管。


<details>
  <summary>Details</summary>
Motivation: 不同点处的响应位于不同切空间，无法直接做局部平均，因此需要一种能跨切空间比较和聚合响应的非参数回归框架，并进一步解决同时推断（置信管）问题。

Method: 先通过平行移动将邻近响应传输到目标切空间，再做体积校正的局部平均形成核估计；推导均匀二阶偏差、有限带宽协方差与随机率；将切向范数写成单位切丛上的上确界，经精确协方差白化得到单位方差高斯场，分析其局部协方差几何并导出显式内在常数的Gumbel极限，结合高斯逼近与交叉拟合协方差估计构造同时置信管，并讨论带宽选择与高阶偏差修正。

Result: 得到核估计的均匀二阶偏差、有限带宽协方差和随机率；白化后相关长度在基流形方向为O(h)、纤维方向为O(1)；获得带显式内在常数的Gumbel极限，并据此给出可行的同时置信管；模拟验证推断流程，全球风数据重构展示置信管截面刻画空间变化的不确定性。

Conclusion: 该框架为黎曼流形上的切向量场回归提供了完整的非参数估计与同时推断理论，兼顾有限样本改进与实际数据应用。
...
### [152] [Emergent Intelligence: Resonant Oscillators Produce Proactive Adaptive Behavior](https://arxiv.org/abs/2609.21161)
*Alex Fedosov,Maxim Yakimenko,Sander Stepanov*

Main category: cs.NE

TL;DR: 该论文提出一种无需训练、监督或参数调整的极简脉冲神经回路，通过三个以上反相且时间窗错开的振荡单元，自发在探索性螺旋搜索与利用性追踪之间切换，实现一级对称与二级群组发现。


<details>
  <summary>Details</summary>
Motivation: 现有人工神经网络多被设计为将给定输入映射到输出，而适应性智能体面临更前置的问题：必须在证据不足时行动、主动寻求与世界的接触，并在证据出现时修正行为。作者希望探索智能神经网络的另一出发点——无信号下的主动搜索，即最基础的"好奇心"，并追问这种能力能否来自一个最小的、未经训练的回路。

Method: 研究一种对输入响应反转的脉冲单元：窗口内无信号时放电更快，信号到来后切换为更慢的反转模式。搜索需要三个或更多此类振荡器处于反相，并各自在不同时间窗读取同一输入。系统不进行训练、监督、参数调优，也不设控制器。通过63种配置、63,000次试验的消融实验，检验时间错开与反相这两种要素的作用，并考察旋转扩散与扰动下的稳健性，以及振荡器数量对搜索行为的影响。

Result: 该复合回路无需训练即可自发在探索性螺旋搜索与利用性追踪之间切换，并能发现一级对称与二级群组。切换机制源于其对同一信号的快、慢读法之间的时间不一致。消融实验显示该切换同时需要时间错开与反相，二者单独均不足，表明行为是涌现的而非被编程的。螺旋在零旋转扩散下仍持续，说明其是结构性的，并在扰动下平缓退化。振荡器增多可提升螺旋规则性，但会降低资源获取，因此最小充分回路最优。

Conclusion: 作者将该回路视为"进化训练"的结果：其能力源于结构而非经验。他们提出这一原理可能支撑简单生物的搜索，以及复杂生物的导航与决策，并因过于简单普遍而常被忽视。最终，这类主动原语构成的网络或可为AI架构提供另一种基础，使其探索世界而非仅仅预测序列中的下一个符号。
...
### [153] [A Confidence-Driven Evolutionary Algorithm for Noisy Optimization with Joint Chance Constraints](https://arxiv.org/abs/2609.21318)
*Enrico Halim,Hemant Singh,Tapabrata Ray*

Main category: cs.NE

TL;DR: 提出置信度驱动的进化算法CR-EA-C，用于求解带联合机会约束的噪声黑盒优化问题，通过解析可行性估计、成对统计排序与改进的不可行度驱动生存策略，在保证约束满足的同时提升评估效率与目标性能。


<details>
  <summary>Details</summary>
Motivation: 现实优化问题常含噪声目标评估与概率约束（尤其是联合机会约束），此类约束评估代价高昂，传统方法难以在统计可靠性与计算效率间取得平衡。

Method: CR-EA-C包含三个核心组件：(1) 针对联合机会约束的解析可行性估计；(2) 用于噪声下稳健比较的成对统计排序机制；(3) 改进的不可行度驱动生存策略以加速收敛。这些组件共同实现统计可靠的决策并减少函数评估开销。

Result: 在多种不确定性分布下与四种近期元启发式算法对比；并在两个额外真实优化问题上与传统静态采样方法比较。结果显示CR-EA-C能持续满足规定的联合机会约束，同时整体取得有竞争力的目标值。

Conclusion: CR-EA-C是被证明有效的通用噪声优化方法，兼具约束满足性与目标性能优势。
...
### [154] [Predictive Suppression Layers for Communication-Efficient Spiking Neural Networks](https://arxiv.org/abs/2609.21583)
*Aidin Attar,Michele Rossi*

Main category: cs.NE

TL;DR: 本文提出一种用于脉冲神经网络（SNN）的最小预测编码框架，通过引入误差单元与预测抑制两种层变体，减少层间通信冗余，在N-MNIST和SHD数据集上实现约三倍通信量降低并提升任务精度。


<details>
  <summary>Details</summary>
Motivation: 前馈SNN不加区分地传播每一个生成的脉冲，未从信息论角度判断信息是否冗余，导致层间通信存在高冗余，在多核神经形态硬件或通信主导的IoT无线传输场景中带来昂贵开销。

Method: 以牺牲局部处理换取更精简的网络通道，提出SNN的最小预测编码框架。两种层变体共享一个预测器模块：误差单元（error units）传输带符号的脉冲残差；预测抑制（predictive suppression）利用残差幅度动态门控，仅转发不可预测的"意外"活动。

Result: 在N-MNIST和SHD数据集上，采用将局部处理与跨层通信解耦的诊断指标评估，通信活动减少约三倍，同时两个数据集的任务精度均得到提升。

Conclusion: 预测编码层不仅能最小化通信开销，还能产生具有更高表示能力的输出特征向量，说明精度提升源于表征能力的增强。
...
### [155] [The Internet Archive Music Dataset](https://arxiv.org/abs/2609.20870)
*Paraskevas Stamatiadis,Bernardo V Miranda,Clémentine Berger,Gaël Richard,Mathieu Fontaine,Slim Essid*

Main category: cs.SD

TL;DR: 本文提出互联网档案馆音乐数据集（IAMD），这是目前公开可用的最大音乐-文本描述数据集，包含超过34,000小时音频及标题描述，并提供自动化描述生成流程。


<details>
  <summary>Details</summary>
Motivation: 现有公开音乐-文本描述数据集规模有限，难以支撑音乐理解与生成模型的训练和评估，因此需要一个大规模、可靠标注的音乐描述基准数据集。

Method: 从互联网档案馆中选取声明为Creative Commons许可的内容构建数据集，并与MusicBrainz交叉验证以提升许可信息可靠性。提出自动描述生成流程：以音频-语言模型（ALM）生成的基础描述为核心，融合互联网档案馆的文本元数据以及通过音频分类模型推测的填补元数据。

Result: 构建了包含超过34,000小时音频的IAMD数据集，规模为当前公开音乐描述数据集之最。通过客观与主观评估验证了描述质量，结果表明标注流程可靠，且随规模扩展描述质量不下降。

Conclusion: IAMD为音乐理解与生成模型提供了大规模、可信的公开基准；其自动描述流程具备可扩展性与可靠性，可支持后续研究。
...
### [156] [Composer2Vec: A Continuous Embedding Space of Composer Style Learned from Symbolic Melody Generation](https://arxiv.org/abs/2609.20893)
*Sakutaro Nishio,Osamu Ichikawa*

Main category: cs.SD

TL;DR: 作曲家条件Transformer学习的作曲家嵌入可视为可解释的作曲风格连续隐空间，其主成分与作曲家出生年份高度相关，并显著编码风格时期结构。


<details>
  <summary>Details</summary>
Motivation: 以往作曲家嵌入多被视为生成任务的内部表示，而未被当作可分析的风格隐空间；作者希望检验这种无额外监督的嵌入是否能捕捉音乐史结构。

Method: 在MIDI旋律序列上训练递归预测旋律续写的作曲家条件Transformer（124位作曲家）；对124x128的作曲家嵌入矩阵做PCA；将PC1与出生年份相关分析，并与CLAP、MuQ-MuLan等通用音频-文本嵌入对比；用2,000次置换检验评估风格时期标签的Silhouette分数显著性；并做嵌入空间向量算术。

Result: 第一主成分与作曲家出生年份强相关（r=-0.884, p<0.001, n=123），强于对CLAP、MuQ-MuLan等嵌入做同样分析的结果；风格时期标签的Silhouette分数为0.0110且置换检验显著（p<0.001）；向量算术能捕捉作曲家间有意义的风格关系。

Conclusion: 仅以作曲家身份为监督学习到的作曲家嵌入形成可解释的隐空间，能够反映音乐历史结构，说明生成模型内部表示可作为音乐学分析对象。
...
### [157] [Voice-Light: A Full-Duplex Cascaded Voice Agent with Causal Turn-Taking and Speculative Generation](https://arxiv.org/abs/2609.20995)
*Bertil Braun*

Main category: cs.SD

TL;DR: 提出全双工级联语音代理 Voice-Light，融合声学起音、因果适配器、可逆播放控制与私密推测生成；沉默候选评测中外推学习式完成模型 EOT 召回率仅 12.53%，故保留混合控制器，实测 36 轮响应中位延迟 758 ms。


<details>
  <summary>Details</summary>
Motivation: 自然口语交互不能仅靠流式 ASR、语言生成与语音合成的简单串联：系统需在重叠说话时做出反应而不因每个确认词被取消，需在轮次尚未确定时就预备回复，并保证已取消音频不进入对话历史。

Method: 构建 Voice-Light 全双工级联语音代理，包含即时声学起音、与流式 ASR 编码器共享的因果适配器、可逆播放控制、私密推测式回复生成；结构化工具调用与可听桥接语音并发执行，浏览器确认使已渲染音频成为权威历史；在 1,673 个真实对话沉默候选上锁定评测学习式完成检查点与 Silero 计时策略。

Result: 学习式完成检查点保持 2.70% 误截断率，但端轮召回仅 12.53%，Silero 计时策略达 95.60%；三次无脚本操作员麦克风会话中 36 个响应轮从最终 VAD 端点到首段服务器音频中位 758 ms，21 轮低于 800 ms。

Conclusion: 系统因此保留混合控制器，而非宣称以学习策略替代计时策略；会话属仪表化案例研究而非受控用户评测，并公开合成数据、模型产物、评测代码与摘要、源码及部署配置。
...
### [158] [Online Algorithms for Independent Low-Rank Matrix Analysis and Rank-Constrained Spatial Covariance Matrix Estimation Based on Maximum Weighted Likelihood Estimation](https://arxiv.org/abs/2609.21180)
*Yuto Ishikawa,Norihiro Takamune,Tomohiko Nakamura,Daichi Kitamura,Hiroshi Saruwatari,Yu Takahashi,Kazunobu Kondo*

Main category: cs.SD

TL;DR: 本文提出ILRMA与RCSCME的在线算法，用于扩散噪声下的实时多通道语音提取，并通过近似中间参数、稳定化与加速技术实现实时运行，在仿真与实录信号上均优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 扩散噪声下的实时多通道语音提取在语音识别、助听器等场景应用广泛。作者此前提出的基于ILRMA与RCSCME的分块批处理实时方法假设单批内空间特性平稳，当目标说话人移动时性能下降。

Method: 分三步推导在线算法：首先基于最大加权似然估计构建ILRMA与RCSCME的逐帧代价函数；其次基于辅助函数技术推导逐帧更新规则；鉴于原始更新规则计算代价高，最后通过用估计值近似部分中间参数得到在线算法，并提出稳定化与进一步加速技术。

Result: 在目标说话人静止与移动的仿真场景中，所提方法语音提取性能优于传统方法；使用真实录制信号进一步验证了其在实际场景中的有效性。

Conclusion: 所提在线ILRMA与RCSCME算法在动态条件下兼具实时性与提取性能，适用于实际语音提取应用。
...
### [159] [I'll Keep an Ear Out: Teaching AudioLLMs Proactive Audio Assistance](https://arxiv.org/abs/2609.21183)
*Amit Kumar Singh Yadav,Ritvik Shrivastava,Xuan Zhang,Seungwhan Moon,Shashank Jain,Pinar Donmez,Babak Damavandi*

Main category: cs.SD

TL;DR: 本文提出"主动音频助手"任务，并设计了模型无关的ISM范式，通过在LLM解码中引入<interrupt>和<silent>两个特殊token嵌入主动决策，在Qwen2-Audio-7B上实现高中断F1与去重召回，并验证了实时可行性。


<details>
  <summary>Details</summary>
Motivation: 现有AudioLLM是被动响应式的，仅在用户查询时才作出反应。针对听障（Deaf and Hard of Hearing）用户的佩戴式应用场景，需要模型能够主动监听音频流，并依据单条自然语言意图自主判断何时提醒用户。

Method: 提出Interrupt and Silent Modeling（ISM）范式，模型无关，通过<interrupt>和<silent>两个特殊token将主动决策嵌入LLM解码过程，覆盖四种状态：起始（onset）检测、持续相关性触发、无关性抑制、去重。

Result: 在Qwen2-Audio-7B上，ESC-50上达到99.6%中断F1与完美的去重召回；在含噪的Epic-Sounds厨房音频上，无需领域特定训练即取得最高中断F1，是唯一在保持强起始检测能力的同时不过度触发、不过度抑制的方法；流式评估显示平均延迟3.5秒，具备实时可行性。

Conclusion: ISM为AudioLLM提供了通用、可嵌入解码的主动辅助能力，兼顾检测灵敏度与抑制/去重表现，并支持实时流式场景，适用于听障辅助等佩戴式应用。
...
### [160] [GenTraceBench: A Benchmark for Tracing Audio Deepfakes Across Pre- and Post-training Stages](https://arxiv.org/abs/2609.21738)
*Li Wang,Kunyu Feng,Wan Lin,Dekun Chen,Qinke Ni,Xueyao Zhang,Lei Wang,Jie Shi,Haizhou Li,Zhizheng Wu*

Main category: cs.SD

TL;DR: 本文提出 GenTraceBench 基准，系统评估 TTS 基础生成器指纹在监督微调（SFT）与偏好优化（DPO/GRPO）等适配后的有效性，发现 DPO/GRPO 基本保留指纹，而部分 SFT 与预训练数据变化导致显著漂移。


<details>
  <summary>Details</summary>
Motivation: 现代 TTS 系统通常不会以原始预训练模型直接部署，而是经过 SFT 或 DPO、GRPO 等偏好优化适配。这引出一个音频深度伪造取证的实际问题：从基础生成器学到的指纹在适配后是否仍然有效。现有研究缺乏在受控条件下对该问题的系统评测。

Method: 构建 GenTraceBench 受控基准，覆盖 5 种 TTS 架构、16 个训练前/后变体、49,728 条固定文本与说话人提示生成的语音。采用"基础模型训练、适配模型测试"协议，评估二分类检测、闭集归因和开集验证三类任务，并在三种取证骨干网络上比较效应量，同时进行重复训练与数据混合对照实验。

Result: DPO 与 GRPO 总体上能保留生成器指纹；部分 SFT 与预训练数据变化则导致明显指纹漂移，且效应量在不同取证骨干间存在差异。重复训练验证了 W2V-BERT 归因性能的最大下降；语音质量相当的数据混合对照表明，数据组成变化本身未必引起漂移。在 W2V-BERT 验证中，多片段注册将 SFT 条件的 EER 从 44.4% 降至 11.0%，而仅用 SingNet 的条件 EER 仍保持在 45% 及以上。

Conclusion: TTS 生成器指纹对适配策略的鲁棒性具有条件依赖性：偏好优化较安全，SFT 与数据变化风险较高，且取证结论受骨干模型与注册方式影响。GenTraceBench 为音频深度伪造取证的指纹稳定性研究提供了可复现的受控评测平台。
...
### [161] [Training Music Sample Identification Models on Real Sample Pairs](https://arxiv.org/abs/2609.21911)
*R. Oguz Araz,Joan Serrà,Xavier Lizarraga-Seijas,Emilio Molina,Xavier Serra,Yuki Mitsufuji,Dmitry Bogdanov*

Main category: cs.SD

TL;DR: 本文提出 SI Embeddings (SIE) 模型，通过首个全监督训练方案，在三个基准（含大规模测试集）上取得样本识别任务的最优性能，并验证真实样本对并非性能的全部来源。


<details>
  <summary>Details</summary>
Motivation: 样本识别（SI）需要匹配经音乐变换得到的音轨对，但缺乏大规模的真实样本标注，主流做法依赖人工合成的样本对训练。虽有新数据集提供大规模真实样本对标注，却缺少有效的训练方法。

Method: 提出 SI Embeddings (SIE) 模型，设计首个针对真实世界样本识别的全监督训练配方，并分析模型架构与训练策略对性能的贡献，同时与在人工样本对上训练的先前最优方法进行对比。

Result: SIE 在三个基准（包括一个大规模测试集）上取得最优结果；先前基于人工样本对训练的最优方法在真实样本对上仅部分泛化，且其训练数据限制了性能；真实样本对并不能完全解释 SIE 的表现。

Conclusion: SIE 的架构与训练配方对性能具有实质性贡献。本文提供了首个面向真实场景样本识别的全监督训练方案，为该领域后续研究奠定了坚实基础。
...
### [162] [Investigating the Performance and Energy Costs of Replicating Band-Split RNN for Music Source Separation](https://arxiv.org/abs/2609.21918)
*Paul Magron,Romain Serizel,Constance Douwes*

Main category: cs.SD

TL;DR: 本文复现了音乐源分离模型BSRNN，实现完整流程，并通过实验研究数据预处理、优化协议和架构参数等设计选择，同时报告能耗成本并公开代码与预训练模型。


<details>
  <summary>Details</summary>
Motivation: BSRNN在合理计算资源和公开数据集下接近SOTA，具有可复现研究价值，但完整代码不可用导致复现困难；作者希望通过复现推动更可复现的研究实践。

Method: 通过实现完整流程复现BSRNN，并扩展原论文分析，实验研究数据预处理、优化协议及架构参数等设计选择。

Result: 成功复现BSRNN，并得出关于数据预处理、优化协议和架构参数等设计选择的实验结论；报告了项目能耗成本。

Conclusion: 若完整流程可得，能耗足迹本可大幅降低，倡导更可复现的研究实践；公开代码和预训练模型。
...
### [163] [Spherical Harmonic Sliced Wasserstein Displacement Interpolation for Acoustic Source and Reflection Density Modeling](https://arxiv.org/abs/2609.22028)
*Yuancheng Luo*

Main category: cs.SD

TL;DR: 本文提出在球谐域中利用Wasserstein度量与位移来评估插值的SRIR回声密度，并通过新的幅度平方和展开实现高效概率密度拟合、最大似然、逆采样及球面切片Wasserstein插值，实验验证了模型降阶与插值效果。


<details>
  <summary>Details</summary>
Motivation: 移动声源的空间房间脉冲响应（SRIR）采集困难，需要在多次测量间进行复杂插值以刻画多路径时空动态，因此需要更合适的度量与插值方法来评估和处理SRIR回声密度。

Method: 提出新颖的幅度平方和（sum-of-magnitude-square）展开，用于高效拟合概率密度函数、最大化似然、逆采样以及计算球面切片Wasserstein插值；并将Wasserstein度量与位移用于球谐域中插值SRIR回声密度的评估。

Result: 实验在直线路径上对SRIR图像源密度的线性插值与几何插值进行比较，计算Wasserstein位移与度量，并展示了模型降阶效果。

Conclusion: Wasserstein度量与位移为球谐域中SRIR回声密度的插值评估提供了有效工具，所提幅度平方和展开支持高效计算，并实现模型降阶。
...
### [164] [Listen Before You Speak: Response Planning from Listener Facial Reactions for Conversational Speech Generation](https://arxiv.org/abs/2609.21683)
*Yunji Chu*

Main category: cs.AI

TL;DR: 提出 ReACT-TTS 两阶段框架，利用听者一秒反应前面部序列预测下一话语的情感和韵律，再驱动 Grad-TTS 合成语音；在 MELD 双方对话协议与主观评测中优于纯文本条件。


<details>
  <summary>Details</summary>
Motivation: 对话语音依赖对话上下文以及听者紧邻前一刻的行为，而现有多数 TTS 仅以文本（或文本加说话人自身历史）为条件，忽略了听者在回应前的即时面部动态这一互补线索。

Method: 两阶段框架：第一阶段以一秒反应前听者面部序列作为时间条件，预测下一话语的情感与韵律（VAD）风格；在严格的双方 MELD 协议下与纯文本条件对比，并进行视觉变体消融（含显式早—晚差分）与循环错配对照；第二阶段将预测的回应风格接入 Grad-TTS 主干实现端到端语音合成。

Result: 十个随机种子下，Temporal 条件的平均 macro-F1 与 VAD 一致性高于 Text-only，而准确率基本持平；消融显示时间建模在视觉变体中表现最佳、显式早—晚差分并无必要，正确听者反应平均优于循环错配；20 位语音研究者的语境恰当性评测中，76% 偏好 Temporal，9% 偏好 Text-only，15% 无偏好。

Conclusion: 结果表明反应前听者动态可作为对话回应规划的互补线索；框架可接入 Grad-TTS 完成端到端语音实现，代码已开源。
...
### [165] [MIST: Multimodal Survival Prediction with Genomic-Guided Histology Attention](https://arxiv.org/abs/2609.21811)
*Muhammet Sami Yavuz,Sabri Mustafa Kahya,Richard R. Chen,Jana Lipkova,Benedikt Wiestler*

Main category: cs.AI

TL;DR: 提出MIST，用基因组特征作为查询token去检索紧凑的病理基础模型上下文token，实现基因组引导的组织学注意力融合，在四个外部队列上提升生存预测C-index。


<details>
  <summary>Details</summary>
Motivation: 多模态生存模型可整合全切片图像（WSI）与基因组信息的互补预后价值，但面临外部队列分布偏移与计算复杂度高的挑战，且晚期简单融合效果有限。

Method: 将基因组特征表示为token，作为查询去关注由基础模型导出的紧凑组织学上下文token，从而在预测前用组织学上下文丰富分子信息，而非仅在最终阶段合并各自独立编码的模态。训练结合离散时间生存预测、基因组特征掩码、WSI dropout，以及配对的WSI-基因组对比对齐。

Result: 在结肠、肾、肺与胶质母细胞瘤四个外部评估队列中，MIST在主要比较上相较标准融合基线的外部C-index均有提升。
...
### [166] [Designer-RSI: Evolving Procedural Memory from User Traffic for Agentic Graphic Design](https://arxiv.org/abs/2609.22086)
*Hongyang Du,Lan Yan,Christian Flores,Asim Kadav*

Main category: cs.AI

TL;DR: 提出"程序性记忆"外部框架：冻结前沿模型操作专业设计软件230+工具，通过技能"拓宽"（新增子任务程序）与"深化"（据成功/失败修订）持续积累自然语言设计技能，配合匹配回放门控防止退化；5轮迭代将技能库从76增至139，GenEval2成功率72.7%→99.3%，并在多基准上优于无技能智能体。


<details>
  <summary>Details</summary>
Motivation: 专业平面设计是长时程智能体任务，需经多步相互依赖动作生成结构化可编辑产物，但结果缺乏可靠的程序化评判标准（无oracle）；同时无法依赖权重更新或人工标注实现持续适应，因此需要一种在噪声、不可验证反馈下仍能持续改进的外部机制。

Method: 冻结前沿模型通过230+工具操作专业设计软件；构建外部自然语言技能的程序性记忆，包含两种机制：拓宽（为反复出现的未覆盖子任务获取新程序）与深化（依据自身成功/失败执行修订既有程序）；引入匹配回放门控，仅接受能修复失败且不使已观测成功退化的变更。全程无权重更新、无人工标签，在1,406份真实用户需求与1,869条自动评分轨迹上迭代5轮。

Result: 技能库由76个文档衍生技能扩展至139个；GenEval2执行成功率在Claude-Sonnet-4上从72.7%提升至99.3%，生成质量提升11.99分；在四个专业设计基准上对无技能智能体的胜率分别为61.8%（Claude-Sonnet-4）和67.6%（Claude-Opus-4.6）。在用户流量基准的200份留出需求上，单独拓宽或深化胜率为49.4%/48.6%，二者结合达58.5%（p=0.025），验证机制组合有效性。

Conclusion: 程序性记忆为在噪声且不可验证反馈条件下实现智能体的持续适应提供了一条可行路径；拓宽与深化两种机制结合使用效果显著优于单独使用。
...
### [167] [AutoRecLab: Describe the Experiment, Get the Code!](https://arxiv.org/abs/2609.21863)
*Moritz Baumgart,Philipp Meister,Justus Krell,Michael Schmidt,Bela Gipp,Joeran Beel*

Main category: cs.AI

TL;DR: 提出 AutoRecLab：一个基于 Python 的自主推荐系统实验框架，可将自然语言研究想法自动转化为可执行实验代码并完成端到端评估。


<details>
  <summary>Details</summary>
Motivation: 推荐系统（RecSys）研究中经验评估至关重要，但从实验设计到可执行代码的转化过程高度依赖人工、容易出错，缺乏自动化手段。

Method: AutoRecLab 接收自然语言提示，先推导出显式实验需求，构建并验证原型，再迭代扩展为完整实验。工作流融合了面向文档检索的检索增强生成（RAG）、静态类型验证以及执行引导的树搜索（execution-steered tree search）。

Result: 在演示中，AutoRecLab 自主实现了一项显式反馈到隐式反馈转换的研究。在覆盖 6 种算法、3 个数据集的基线对比中，9 次运行有 8 次成功，使用 GPT-5.4-mini 时平均每次运行成本约 1 美元。

Conclusion: AutoRecLab 展示了以低成本自动化推荐系统实验流程的可行性，能够以自然语言驱动实现从需求推导到完整实验执行的全流程。
...
### [168] [RBS-Attention: Radius-Bounded Sparse Prefill for Long-Context Large Language Models](https://arxiv.org/abs/2609.20971)
*Chuxu Song,Jiuqi Wei,Zhencan Peng*

Main category: cs.AI

TL;DR: 提出 RBS-Attention，一种免训练的稀疏预填充方法，通过质心基分支与救援分支双路选择，缓解"均值稀释"问题，在保持精度的同时大幅加速长上下文推理。


<details>
  <summary>Details</summary>
Motivation: 长上下文 LLM 推理瓶颈在预填充阶段，密集自注意力需处理整个提示。稀疏块选择可降低成本，但块质心可能掩盖块内少数高度相关的 token（即"均值稀释"失效模式），导致重要块被低估。

Method: RBS-Attention 为免训练稀疏预填充方法，含两条互补选择分支：质心基分支捕捉平均相关性；救援分支利用最大 key 块半径及其依赖 prompt、层、头的分布，识别有被低估风险的块。两分支独立阈值化并合并掩码，以控制救援块贡献，同时保留常规块稀疏 FlashAttention 执行。

Result: H100 上，Qwen3-30B-A3B-Instruct-2507-FP8、128K 时：独立预填充注意力加速 20.65×，vLLM 预填充注意力加速 11.92×，端到端首 token 时间加速 5.97×。密集 Qwen3-32B 上 RULER 总分 88.65，对比密集注意力 89.52；另在 LongBench-v2、InfiniteBench、Video-MME 上评测质量。辅助实验测量实际保留率、同密度下比较选择器，并刻画块大小、阈值与内存行为。

Conclusion: 半径自适应双分支选择是长上下文预填充的有效方案，兼顾显著加速与近乎无损的精度。
...
### [169] [Attention-Aware Routing: Coupling Routing and Attention in MoEs](https://arxiv.org/abs/2609.20974)
*Despoina Kosmopoulou,Anastasios Tsetsilas,Efthymios Georgiou,Giannis Karamanolakis,Swastik Roy,Alexandros Potamianos*

Main category: cs.AI

TL;DR: 提出注意力感知路由(AAR)，用注意力权重滑窗特征增强路由器，冻结主干仅训练路由参数，提升推理能力并揭示路由-注意力耦合机制。


<details>
  <summary>Details</summary>
Motivation: MoE路由器仅依赖当前token隐状态，上下文信息有限；作者希望在不改动主干模型的前提下，为路由引入更丰富的上下文表征，并借此隔离路由这一单一变量来研究其作用。

Method: 提出AAR：从注意力权重滑动窗口中提取时序与频域特征，作为路由器输入，这些特征被视为模型上下文状态的摘要，且与隐状态解耦。保持基础Transformer完全冻结，只训练路由参数。

Result: 在OLMoE上，AAR相对仅路由SFT基线在GSM8K上提升+3.37个百分点；路由与注意力构成耦合回路：第l层路由变化经残差流传播，放大第l+1层注意力汇，重塑注意力；减少长发散生成，错误答案变短而正确答案长度不变；AAR呈强深度敏感性。

Conclusion: 路由与注意力存在耦合回路，且不同深度承载的信息不同；跨层盲目应用AAR会损害事实检索，而数学推理收益在更深层引入时仍保持，揭示深度上的检索-推理张力，使层选择性AAR成为探测各层注意力所携带路由相关信息的受控手段。
...
### [170] [CaLR: Causal Latent Revision for Robust Diffusion Reasoning](https://arxiv.org/abs/2609.20981)
*Wei Cai,Jian Zhao,Yuchen Yuan,Xuelong Li*

Main category: cs.AI

TL;DR: 提出Causal Latent Revision (CaLR)框架，将推理重新表述为受约束的隐空间优化，融合自回归模型与扩散语言模型优势，通过因果拓扑矩阵与隐式微分实现梯度引导的"思维修正"。


<details>
  <summary>Details</summary>
Motivation: 自回归(AR)模型存在局部贪心问题，而扩散语言模型(DLM)往往缺乏推理所需的严格因果结构。为兼具两者优势并克服各自缺陷，需一种统一方法。

Method: CaLR采用来自专家模型的因果拓扑矩阵(CTM)与隐式微分，执行梯度引导的"思维修正"以强制逻辑一致性，从而在并行生成过程中对中间步骤进行动态自我纠正。

Result: 在复杂基准上，CaLR取得DLM的SOTA性能，超越强AR基线，并在数独等受约束任务上展现出更优的鲁棒性。

Conclusion: CaLR通过受约束隐空间优化有效结合AR与DLM优势，实现并行生成中的动态自纠错，验证了因果结构约束对推理任务的增益。
...
### [171] [LoRA Enhanced Contrastive Learning with SAS Vision Transformers](https://arxiv.org/abs/2609.21061)
*Dan Zimmerman,Frank E. Bobe,Amelia L. McCormack,Matthew Cook,Gregory D. Vetaw*

Main category: cs.AI

TL;DR: 将DINOv3 ViT经LoRA参数高效适配到水下合成孔径声呐ATR，LoRA带来主要增益（AUPRC 0.300→0.679），而后续的难负样本挖掘与SupCon精炼阶段均未超过对照，表明单一适配阶段已足够。


<details>
  <summary>Details</summary>
Motivation: 合成孔径声呐自动目标识别对海军能力有重要价值，但深度学习受限于目标图像稀缺、背景杂波强以及依赖人工在环评估，因此需要参数高效的适配方案，弥合自然图像预训练与水下声传播之间的域差异。

Method: 采用三阶段参数高效框架适配DINOv3 ViT：阶段1冻结ViT主干，仅用LoRA（秩4，仅训练0.26%权重）；阶段2用难负样本挖掘强化对声学仿似物（岩石、沉积物等）的决策边界；阶段3用监督对比学习分离目标与杂波表示。基于海上SAS实拍数据、按任务级地理划分评估，统一在85%测试召回率下比较，并对每个对比重复3个随机种子。

Result: LoRA为主要的效应来源，在同一冻结主干下将AUPRC从0.300提升至0.679±0.027，秩4即达到该结果。两个精炼阶段均未超过匹配对照：难负样本挖掘相对等规模随机课程使AUPRC变化-0.0045±0.0119；SupCon相对前一阶段变化+0.0002±0.0096，二者均为零结果。

Conclusion: 零结果表明难负样本挖掘作用在编码器已经拟合的数据上，且监督阶段已施加了大部分目标—杂波几何结构。因此一个高效的适配阶段即已足够，堆叠式多阶段精炼并非必要。
...
### [172] [Detecting Hallucination in LLMs: Tracing the Topological Signatures of Impaired Context Sharing](https://arxiv.org/abs/2609.21096)
*Amir Jalilifard,Anderson Rocha,Eric Wong,Marcos Medeiros Raimundo*

Main category: cs.AI

TL;DR: 本文通过分析注意力图的拓扑结构与Forman-Ricci曲率，捕捉注意力头的信息流特征，提出一种单次前向传播的幻觉检测方法，在多个LLM与基准上优于现有注意力类和多响应基线。


<details>
  <summary>Details</summary>
Motivation: 幻觉检测对LLM可靠性至关重要。现有方法多依赖多响应采样或仅利用注意力权重，缺乏对注意力图内部信息流动结构（如信息瓶颈）的刻画，难以有效区分幻觉与非幻觉响应。

Method: 将注意力图视为信息流网络，利用Forman-Ricci曲率识别表征信息瓶颈的结构模式；提出同时捕捉与幻觉响应相关注意力头的半局部与全局信息流特征的方法，实现单次前向传播的检测。

Result: 在多个LLM和两个幻觉检测基准上，该单次传播方法相较现有基于注意力和多响应的基线取得一致的性能提升，并在多种LLM架构上保持有竞争力的表现。

Conclusion: 幻觉响应与因果生成过程中token间上下文共享受损密切相关，典型表现为过度依赖自注意力、对早期token的上下文检索分散，以及信息过度压缩（over-squashing），尤其在最后一个transformer层中尤为明显。
...
### [173] [Decoupling Internal Representational Changes and Causal Importance in Fine-Tuned Large Language Models](https://arxiv.org/abs/2609.21113)
*Lingfang Li,Procheta Sen,Shubham Das,Danushka Bollegala*

Main category: cs.AI

TL;DR: 本文研究微调如何重塑LLM内部表征，发现EAP识别的任务关键组件集中于特定层，但其分布与微调中表征变化最大的层基本不相关；跨任务的EAP组件重叠在任务性质不同时不带来性能迁移，反而可能导致性能下降。


<details>
  <summary>Details</summary>
Motivation: 微调已成为适配LLM下游任务的常用方法，但其如何改变模型内部机制仍缺乏理解。作者希望探究微调对内部表征（注意力模式、逐层激活）的影响，并检验这些变化是否与EAP识别的任务关键组件（注意力头、logit级激活）相关联。

Method: 分析微调前后LLM的内部表征，包括注意力模式和逐层激活；使用EAP（基于激活的路径/组件识别方法）定位驱动任务表现的组件（注意力头与logit级激活）；比较EAP组件分布与表征变化最大的层，并考察跨任务EAP组件重叠与性能迁移的关系。

Result: EAP识别的组件集中在特定层，表明模型内部化任务特定行为存在一定程度的功能局部化；这些组件的层分布与微调中发生显著表征变化的层基本不相关；跨任务的EAP组件重叠在任务性质不同（如分类与生成）时并不转化为跨任务性能迁移；具体而言，当一个任务的微调与另一任务在EAP组件上高度重叠时，前者可能导致后者性能下降。

Conclusion: 微调引起的表征变化与EAP识别的任务关键组件在层级上并不对应，说明功能局部化与表征漂移是相对独立的现象；EAP组件的跨任务重叠不能简单预测正向迁移，任务性质差异下甚至预示负迁移，这对理解微调机制与任务适配策略具有启示。
...
### [174] [TinyCeNN-LM: Quality-Gated Conversion of Pretrained Attention with CeNN-Inspired Cellular-Recurrent Layers](https://arxiv.org/abs/2609.21139)
*Kabeh Mohsenzadegan,Vahid Tavakkoli,Kyandoghere Kyamakya*

Main category: cs.AI

TL;DR: TinyCeNN-LM 提出"质量门控后训练转换"框架，用类 CeNN 的细胞循环层替换预训练语言模型中的注意力层，仅在表示保真度与 NLL 双阈值同时通过时接受替换，否则回滚。在 SmolLM2-135M 与 Qwen3.5-0.8B 上验证了保守转换的可行性。


<details>
  <summary>Details</summary>
Motivation: 直接替换预训练语言模型中的注意力机制存在兼容性问题：看似合理的替代层可能改变后续层所依赖的表示。因此需要一个带质量门控、可回滚的保守转换流程，而非追求普适的注意力替换或加速。

Method: 提出质量门控后训练转换框架（quality-gated post-training conversion），采用受 CeNN 启发的细胞循环层，具备有界局部处理、紧凑循环记忆、路由、融合以及"接受或回滚"验证机制。研究三种实现：Integrated Memory、MemoryFusion、PDelta3-GDN2-CLVR+Local32。严格 PDelta3 转换仅当表示与 NLL 指标均通过固定阈值时才接受该层。

Result: SmolLM2-135M 上，第 0-2 层被接受，累计 ΔNLL=+0.01209；第 3 层虽 NLL 可接受但因表示保真度不达标被拒绝。Qwen3.5-0.8B 上，全注意力层 3、7、11 被接受，最终 ΔNLL=+0.02073。Integrated Memory 将困惑度变化控制在 -0.07%~+0.93%，总缓存最多减少 6.01%。对转换后 Qwen 版本抽样 200 项下游检查，总体准确率为 28.5%~32.0%。

Conclusion: 结果支持采用保守的、质量门控的结构转换策略，而非通用的注意力替换或加速方案；转换的可行性与收益受表示保真度约束。
...
### [175] [Clinician-Grounded Quality Assurance for AI-Assisted Psychiatric Intake](https://arxiv.org/abs/2609.21149)
...
TL;DR: 提出临床医生导向的评估平台InterviewPlayground，用记忆增强患者模拟器支持开放式AI问诊的对比评估，并在6名临床医生试点中验证其可行性与临床相关性。


<details>
  <summary>Details</summary>
Motivation: AI辅助精神科问诊系统在部署前需要常规化、可操作的质量评估手段。由于临床医生的问诊风格各异，评估须满足三点：支持不同问诊方式的横向比较、降低临床医生负担、测量对部署机构有临床意义的性能。

Method: 构建以记忆增强患者模拟器为核心的临床医生导向评估平台InterviewPlayground；将专家撰写的病例脚本转化为可交互患者；搭建模拟问诊平台；设计面向问诊任务的评估模态；开展6名临床医生的25分钟试点评估，并与基于GPT的LLM问诊者进行对比。

Result: LLM问诊者从病例脚本中提取临床相关条目更多（88.0% vs. 38.9%），但产生更多无问诊依据的临床推断（56.8% vs. 27.8%），且对已识别安全问题的特征描述更少（33.3% vs. 66.7%）。

Conclusion: 该平台为AI辅助精神科问诊任务的部署质量保证提供了可行基础，并揭示了LLM在信息采集覆盖度与临床推断可靠性、安全风险评估方面的权衡。
...
### [176] [Can Agents Design Better Chips with a Higher Level Abstraction?](https://arxiv.org/abs/2609.21157)
*Zijian Ding,Yang Zou,Yizhou Sun,Jason Cong*

Main category: cs.AI

TL;DR: 该论文比较了四种LLM智能体芯片设计流程，提出将智能体HLS设计与HLS后RTL精炼结合的AHRR方法，在11项FPGA基准测试中实现相对直接RTL设计2.6倍的几何平均加速。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体芯片设计方法大多直接在RTL层面操作，未能充分利用更高层次抽象来提升设计质量。作者试图探究：智能体能否借助更高层次抽象设计出更好的芯片。

Method: 对比四种设计流程：直接RTL设计、智能体HLS设计、编译器后HLS精炼、HLS后RTL精炼，并将智能体HLS设计与HLS后RTL精炼组合为AHRR。以FPGA作为便于部署的端到端评估平台，在11项多样化任务基准上评测。

Result: AHRR在11项任务基准上相比直接RTL设计取得2.6倍几何平均加速。案例研究表明，HLS将设计知识蒸馏为智能体可利用的抽象，而RTL精炼则恢复底层优化机会。

Conclusion: 结合高层次HLS抽象与低层次RTL精炼的AHRR是面向智能体芯片设计的有前景工作流，且所研究的设计流权衡在很大程度上与目标工艺无关。
...
### [177] [SpecOpt: Contact-Diff Reasoning for Agentic Molecule Optimization Toward Binding Specificity](https://arxiv.org/abs/2609.21165)
*Thao Nguyen,Heng Ji*

Main category: cs.AI

TL;DR: 本文提出"特异性优化"(SpecOpt)任务及智能体框架,通过对已有药物进行受约束的结构修饰,提升其对目标靶点相对已知脱靶靶点的结合偏好。


<details>
  <summary>Details</summary>
Motivation: 脱靶蛋白结合是小分子药物产生不良反应的主要来源,但现有基于结构的分子设计方法多关注从头生成选择性化合物,而非改善已有、表征充分的药物的选择性。

Method: 1) 定义SpecOpt任务:在保持结构同一性和成药性的约束下,对现有化合物做结构修饰以增强其对目标靶点相对脱靶靶点的结合偏好。2) 基于ChEMBL化合物-靶点相互作用数据构建基准数据集,通过药物机制注释确定目标靶点,通过实测活性确定脱靶靶点。3) 开发智能体框架:将化合物分别对接至目标靶点和脱靶靶点,通过残基感知的原子-蛋白接触比较对接构象,将差异性相互作用输入大语言模型以生成针对性的结构修饰;候选分子需通过分子相似性、ADMET及靶点-脱靶靶点对接选择性筛选。

Result: 在915个化合物上,该智能体使84.8%的化合物目标-脱靶结合差距得到改善,平均差距从-0.72 kcal/mol提升至+0.47 kcal/mol,同时与起始化合物的平均Tanimoto相似度为0.72。消融实验表明残基特异性的接触信息是关键优化信号:将残基身份替换为二值接触指示符后,29个消融化合物上均无改善。

Conclusion: 研究确立了SpecOpt作为一个独立的分子设计问题,并证明残基感知的差异性相互作用是提升现有化合物特异性的有效信号。
...
### [178] [Implicit Rule Induction with Test-Time Task Embeddings in ARC-like Tasks](https://arxiv.org/abs/2609.21181)
*Adrien Deliège,Claas Beger,Marc Van Droogenbroeck,Melanie Mitchell*

Main category: cs.AI

TL;DR: 本文提出两步式测试时训练协议（先微调任务嵌入 Embed-TTT，再冻结嵌入微调主干），证明该方法能获得与底层规则更一致的任务嵌入，并更清晰地区分 ARC 类任务中的规则归纳与规则执行。


<details>
  <summary>Details</summary>
Motivation: 现有 ARC 及类似基准虽用于评估模型求解新颖推理任务的能力，但往往无法判断其成功究竟源于推断出预期规则，还是依赖捷径。作者希望厘清这一点，为此研究 VARC 中的测试时任务嵌入。

Method: 在 VARC（预训练主干 + 可训练任务嵌入）基础上提出新颖的两步测试时训练协议：第一步仅微调任务嵌入（Embed-TTT），第二步冻结该嵌入并微调主干。评估覆盖 ARC-AGI-1、ConceptARC 及两个具有已知规则的受控数据集，并借助嵌入检索、线性探针、定性分析与几何结构分析进行检验。

Result: Embed-TTT 持续产生更优的任务嵌入，与底层任务规则对齐更好，提升基于嵌入的检索效果，并支持对已知规则做准确的线性探针；在 ARC-AGI-1 上能识别出更具语义意义的测试—训练任务关系。仅优化任务嵌入（不到模型参数的 0.01%）即可解决相当一部分 ARC-AGI-1、ConceptARC 和 Mini-ARC 任务，完整两步流程进一步提升了最终性能。Embed-TTT 还能恢复参数化规则的底层几何结构，学到支持规则级插值但不支持外推的组合能力。

Conclusion: 研究结果支持在 ARC 类评估中更清晰地区分规则归纳与规则执行，并推动设计能更好区分分布内与分布外规则的基准。
...
### [179] [AI-GRACE: A Use-Case Operationalization Framework for Agentic AI: From Organizational Objectives and Obligations to Deployment Capabilities and Architecture](https://arxiv.org/abs/2609.21192)
*John Cuneo,David Chun,Gaurav Khanna*

Main category: cs.AI

TL;DR: 本文提出AI-GRACE框架（智能体AI的治理、风险、保证、控制与证据），通过用例操作化将组织治理与技术实现相连接，为智能体AI的部署提供可追溯的决策依据。


<details>
  <summary>Details</summary>
Motivation: 部署智能体AI的组织不能仅判断模型是否可信，还需明确用例需验证什么、控制什么、观测什么，以达成预期结果并满足合规义务。现有治理与技术实现之间缺乏衔接。

Method: 基于专业观察与标准及文献的目的性综合，采用设计科学框定方法贡献，并用情境化方法工程指导情境裁剪与复用。框架先确定目标与义务，再在七个风险域（含任务与价值实现）评估风险，导出部署前保证、运行时控制与证据需求，进而指导能力资格认定、差距评估与逻辑架构设计；以“智能体运行包络”（Agent Operating Envelope）规定允许动作与升级条件，以“风险对齐独立性等级”（RAIL）概括授权独立性。并以虚构的零售银行场景进行示例说明。

Result: 框架输出了一个可追溯的基础，用以判断组织必须实现什么、已支持什么以及尚存哪些未解决问题；虚构零售银行用例对该方法进行了演示。

Conclusion: 贡献在于为智能体AI用例的治理与落地提供可追溯的决策基础；其是否改善部署决策、效率与复用仍需实证评估。
...
### [180] [Information-Gain Rewards over Diversity-Pruned Tests: GT-Anchored Verifier Co-Training for Reliable Code Generation](https://arxiv.org/abs/2609.21208)
*Ana Nunez,Peyman Najafirad*

Main category: cs.AI

TL;DR: 本文提出 CoVer，一个单策略 GRPO 框架，通过信息增益奖励与多样性感知筛选，同时缓解自博弈代码生成中的"宽容崩塌"与"集中偏差"两大问题。


<details>
  <summary>Details</summary>
Motivation: 自博弈方法让单一语言模型同时充当编码器与测试作者，有望摆脱固定测试集的限制，但存在两个耦合缺陷：1）宽容崩塌——通过率奖励可被平凡、无区分度的测试最大化；2）集中偏差——独立同分布采样的测试聚集在众数输入上，抬高估计器方差。

Method: 提出 CoVer（协同训练的编码器与验证器），单策略 GRPO 框架。其一，采用信息增益（IG）奖励，以自生成测试的通过/失败向量与锚定真值的分级正确性信号 y∈[0,1]^m 之间的互信息打分，并由协方差符号门控，仅让正区分性测试获得奖励；其二，设计三阶段多样性感知选择步骤（无效性过滤、输入字符串过滤、执行轮廓过滤），将候选池剪枝为行为上非冗余的测试套件，在固定执行预算下提高 IG 估计器的有效样本量。

Result: 在 LiveBench、MBPP、LiveCodeBench、CodeContests、CodeForces 五个基准上，CoVer 相较 Qwen2.5-Instruct 主干将 one-shot pass@1 在 7B 上提升 +5.8 分、14B 上提升 +7.1 分，并在两种规模下取得所有对比方法中最高的宏平均。作为 CodeT 排序流水线的即插即用主干，CoVer-7B 额外提升 +3.5 分。

Conclusion: 实验表明协同训练对生成与选择均具双重收益，验证了 IG 奖励与多样性感知筛选在缓解宽容崩塌和集中偏差方面的有效性。
...
### [181] [Ability-Residual Decoupled Modeling for Affective Cognitive Diagnosis](https://arxiv.org/abs/2609.21214)
*Boyuan Zhao,Meng Ye*

Main category: cs.AI

TL;DR: 提出能力-残差解耦框架用于情感认知诊断，显式建模认知残差以避免情感污染。


<details>
  <summary>Details</summary>
Motivation: 现有情感认知诊断方法假设骨干模型已充分解释能力、题目和概念效应，剩余误差主要归因于情感。但真实教育数据中，题目校准偏差、系统性概念偏差、个性化学生-概念偏差及潜在学生-题目匹配会形成稳定的认知残差。若缺乏显式建模路径，这些残差会泄漏进情感表征，造成情感污染。

Method: 提出能力-残差解耦框架。首先通过学生、题目、概念、学生-概念及低秩学生-题目组件捕获未建模的认知残差；随后用情感模块调制猜测/失误效应；并引入Q矩阵约束的概念残差注意力机制，仅自适应聚合与题目相关的概念残差。

Result: 在ASSIST2017、ASSIST2012、ASSIST2009及Junyi四个数据集上，结合六种认知诊断骨干模型，响应预测性能在报道的对比中均有提升；在情感标签可用时情感对齐总体改善。消融、泄漏探测、主成分分析可视化、长尾分析及案例研究进一步验证其有效性。

Conclusion: 能力残差可吸收稳定认知偏差，减少情感分支中的认知污染，提升认知诊断模型的鲁棒性与预测精度。
...
### [182] [A Fully Differentiable Neuro-Soft-Symbolic Framework for Perceptual Task Planning](https://arxiv.org/abs/2609.21221)
*Hongyan Wei,Wael AbdAlmageed*

Main category: cs.AI

TL;DR: 提出一种全可微的神经-软符号框架，将视觉感知与任务规划统一在单一计算图中，通过软符号状态与可微 soft-T_P 转移算子实现端到端规划，并在 Blocksworld 任务上超越 LatPlan 与推理模型基线。


<details>
  <summary>Details</summary>
Motivation: 感知规划任务需同时具备对不确定场景的准确感知与遵循逻辑规则的有效动作序列规划能力。传统方法将感知转为离散符号事实后再规划，丢弃了感知不确定性，并切断任务级反馈对感知的反向作用。

Method: 构建通用的全可微神经-软符号框架，在单一计算图中连接视觉感知与任务规划：维护连续软符号状态，将领域规则提升为可微 soft-T_P 转移算子，在短规划视野上优化动作 logits；规划目标的梯度可回传更新感知参数，从而在规划过程中精炼任务相关的感知表示。

Result: 在 Blocksworld 上，方法在 LatPlan-40 上求解 40/40，在 PlanBench-600 上求解 596/600，而 LatPlan 为 33/40、推理模型基线为 587/600，且计算量与时间显著更少；感知不确定性消融中，成功率由冻结感知的 59% 提升至 83%。进一步在 Blocksworld 场景开展任务与运动仿真，验证了解码任务计划与下游机器人运动执行的兼容性。

Conclusion: 该框架通过可微软符号表示与任务级梯度回传，弥合了感知与规划之间的割裂，在保持低计算开销的同时提升规划成功率，并具备向机器人运动执行层延伸的可行性。
...
### [183] [CogGym: Towards Large-Scale Comparative Evaluation of Human and Machine Cognition](https://arxiv.org/abs/2609.21259)
...
TL;DR: 论文提出CogGym——一个基于认知科学的可扩展统一框架，用于在匹配实验试次上系统比较模型与人类行为，并初步标准化258个常识推理实验，评测50个大模型，发现模型规模越大越接近人类判断，但拟合度仍远低于人类信度。


<details>
  <summary>Details</summary>
Motivation: 理解与建模人类智能是AI与认知科学的共同目标。随着AI能力增强，需要明确模型回应在哪些方面类似人类、又在哪些方面系统性偏离。但人类任务种类繁多，缺乏可扩展且严谨的人机比较方法。

Method: 提出CogGym框架，采用半自动化、人在回路的流水线，将多样实验范式标准化为任务无关的Experiment Markup Language (EML)，实现大规模可复现的忠实比较。初始版本标准化了100篇论文中的258个认知实验，聚焦人类常识推理，并评测50个大语言模型与人类反应。

Result: 存在清晰的扩展趋势：更大、更新的模型更好地复现人类判断。但AI在常识推理任务上的提升明显慢于数学、编程等形式推理基准；模型—人类拟合度远低于人类分半信度（文本R²=0.93、图像0.95、视频0.92），最佳模型仅为文本R²=0.59、图像0.58、视频0.43。

Conclusion: CogGym旨在成为一个持续演进的评估框架，不断纳入新的认知科学实验，用于刻画模型行为在何处类似人类、在何处系统性偏离，以及这些模式如何随模型和实验演进而变化。
...
### [184] [PlaceReasoner-Beta: Reasoning-Driven Macro Placement and Benchmarking](https://arxiv.org/abs/2609.21263)
*Qiufeng Li,Chengxuan Wang,Rongqian Chen,Quan Cheng,Yihui Ren,Chia-Tung Ho,David Z. Pan,Tian Lan,Weidong Cao*

Main category: cs.AI

TL;DR: 提出验证器引导的多智能体框架 PlaceReasoner-Beta，将宏单元布局重构为闭环推理问题，并发布开源端到端基准 PlaceReasoner-Bench；在DRC-clean条件下取得最佳时序，后布线TNS相对经典基线降低61.2%（1:1）与53.0%（2:1）。


<details>
  <summary>Details</summary>
Motivation: 自动宏单元布局是VLSI物理设计的核心难题。现有方法多优化手工代理目标（如估计线长），且采用一次性数值优化，难以统一融入视觉布局上下文、可编码的设计专家经验以及下游物理设计反馈。

Method: 提出 PlaceReasoner-Beta，一个验证器引导的多智能体闭环推理框架：VLM规划器基于版图图像、宏单元规格与连接结构生成候选布局；几何验证器保证物理合法性与专家布局准则；物理验证器利用早期实现反馈精修候选；后布线优化器基于最终PPA进一步改进有潜力的布局。同时构建 PlaceReasoner-Bench：基于开源RTL、EDA工具与工艺库的全开源端到端基准，含8个设计×2种宽高比共16个任务，固定floorplan与I/O分配，方法仅在宏单元位置和朝向上不同。

Result: 评测采用后布线PPA与DRC而非布线前代理指标。在所有方形任务上，PlaceReasoner-Beta 在DRC-clean方法中取得最佳时序，后布线TNS相对经典基线分别降低61.2%（1:1）和53.0%（2:1）；在多数设计上还缩短了布线线长，尽管从未显式优化线长。

Conclusion: 在物理设计反馈下对空间结构进行推理，可超越代理目标优化，提升端到端布局质量；闭环、验证器引导的推理范式为宏单元布局提供了新路径。
...
TL;DR: 针对生产环境分析型LLM智能体的重复评估，基于574次历史基准运行比较四种采样策略：多维2PL自适应测试保真度最佳（200题、38.5%题量，MAE 1.03pp），但实际部署选择难度分层固定子集，因其运维简单且可跨智能体族与短标定窗口迁移。


<details>
  <summary>Details</summary>
Motivation: 生产环境中的LLM智能体在迭代演进时需反复评估，但完整基准测试重跑成本高昂，因此需要研究高效的低成本重复评估方法，并获取一线部署经验。

Method: 使用生产基准的574次历史运行数据，按时间顺序划分为标定期与留出期；对比随机采样、历史缓存、固定代表性子集与基于IRT的自适应测试四种策略；并检验固定子集在五个其他智能体族上的迁移性及不同标定窗口（最短一天）下的稳定性。

Result: 多维2PL自适应测试在总体分数保真度上最优：执行200题（占完整运行的38.5%）时MAE为1.03个百分点。但最终部署的是难度分层固定子集，因其运维简单，且无需重新标定即可迁移到五个其他智能体族，并在短至一天的标定窗口下保持稳定。

Conclusion: 在准确性与运维成本之间存在权衡，自适应测试虽精度最佳但固定分层子集更适于生产落地；据此总结出面向生产智能体重复评估的实践建议。
...
### [186] [GameASG-Bench: Benchmarking Autonomous Software Generation for Game Development](https://arxiv.org/abs/2609.21293)
*Xiuhui Zhang,Yi Chen,Shusheng Xu,Fan Li,Huan Wang,Tongkai Yang,Binhang Yuan*

Main category: cs.AI

TL;DR: 本文提出 GameASG-Bench，将行为可测试性纳入游戏开发生成任务：先声明评估接口规范，再做静态 L1 与浏览器执行 L2 双层检查，覆盖 47 个浏览器原生游戏任务。九种智能体栈中最高平均 L2 通过率为 93.2%，但最严格任务成功率仅 55.3%（26/47），揭示高平均通过率掩盖的任务级合规缺口。


<details>
  <summary>Details</summary>
Motivation: 自主软件生成（ASG）虽能把人类需求转化为可执行应用，但交付应用并不等于其交互组件满足既定的行为需求。现有基准缺乏对生成结果行为可测试性的系统性评估，因此需要在生成任务中内嵌行为验证。

Method: 提出 GameASG-Bench 基准。生成前先声明评估接口规范，固定合法起始场景、玩家级动作、稳定快照、拒绝行为与不变量，同时保留私有实现开放。检查分两层：(i) 静态 L1 检查，评估源码级合规；(ii) 浏览器执行的 L2 检查，结合语义观察、真实输入与运行时证据。协议实现为 47 个浏览器原生游戏生成任务，覆盖 12 个主要类型及 2D/3D 交互，每个任务均配可执行检查与独立验证的参考实现。

Result: 实验回答四个关键问题：端到端智能体性能、工具访问与名义回合预算、推理努力程度、测试框架选择。九种智能体栈中，最高平均 L2 检查通过率为 93.2%，但要求全部 L1 及适用 L2 前置与核心需求检查的严格任务成功率最高仅 55.3%（26/47）。对 DeepSeek-V4-Flash，完整工具访问与更大名义回合预算带来更多严格任务成功，而严格任务成功率对推理努力程度并非单调。两种测试框架均取得 18 个严格任务成功，但仅 10 个任务在两者下都成功。

Conclusion: 高平均检查通过率掩盖了任务级合规缺口。GameASG-Bench 表明，将行为可测试性前置到生成任务中，能够暴露平均指标无法反映的严格合规问题，为自主软件生成的行为验证提供更严格的评估范式。
...
### [187] [LEGIT: Credentialing Protocol for Trustworthy AI Agent Marketplaces](https://arxiv.org/abs/2609.21325)
*Steve Drew,Jiayu Zhou*

Main category: cs.AI

TL;DR: 本文提出LEGIT——一个连接认证、声誉与市场分配的凭证协议，用于解决智能体市场中买家难以判断哪个AI智能体在自身任务上表现最佳的问题。认证将测得的质量与成本绑定到具体智能体配置、任务领域、评估预算及证据上，形成签名记录；声誉则将历史任务结果记录与同一身份关联，并考虑反馈可靠性。实验表明，配置相近的智能体在任务成功率相似时成本差异显著，且比较结论依赖于评估预算，验证了将性能测量绑定到被测配置和资源限制的必要性。此外，分析量化了在给定女巫攻击模型下操纵声誉所需的押金与费用。


<details>
  <summary>Details</summary>
Motivation: 智能体市场正在兴起，具备不同能力的AI智能体自主为买家完成专业任务。然而买家难以判断哪个智能体在自身任务上表现最佳，所报告的基准分数在不同任务、软件和预算之间难以验证或比较，缺乏可信的凭证机制来支撑市场分配决策。

Method: 提出LEGIT凭证协议，整合认证、声誉与市场分配三个环节。认证通过签名记录，将测得的每任务解决质量与成本绑定到智能体配置、任务领域、评估预算和证据上；声誉将过往任务结果记录与同一身份关联，并受所报告反馈可靠性的约束。买家与智能体可验证凭证记录并查看可选的可视化画像。此外，在给定的女巫攻击模型下，量化了操纵声誉所需押金与费用。

Result: 评估揭示：在观测任务成功率相近的智能体配置之间存在显著成本差异；同时，智能体之间的比较结论取决于评估预算的大小。这些结果表明有必要将性能测量绑定到实际被测配置与资源限制上。补充分析量化了在特定女巫攻击模型下操纵声誉所需的押金与费用门槛。

Conclusion: LEGIT通过将认证、声誉与市场分配相连接的凭证协议，提升了智能体市场中性能与成本信息的可验证性与可比性。结果支持将性能测量绑定到被测配置与资源限制，并为抵御声誉操纵提供了基于押金与费用的经济分析支撑。
...
### [188] [Offline Multimodal Large Language Models for Decision Support in Air Operations](https://arxiv.org/abs/2609.21390)
*Joao P. A. Dantas,Jelton A. Cunha,Gabriel Dietzsch*

Main category: cs.AI

TL;DR: 该论文研究在无网络、受限安全环境下的离线大语言模型作为决策支持工具，提出模块化检索增强架构，并通过巴西空军四名图像分析员的试点研究，验证系统在教义知识评估中达到与人类相同分数(8/10)且速度大幅提升。


<details>
  <summary>Details</summary>
Motivation: 空中作战依赖复杂规则、既定程序，并需要在有限连接和严格安全约束下进行时间关键的分析；分析员须结合书面条令与图像，且常无法访问外部算力资源。为此需要能在隔离受限环境中部署、可追溯至原始来源的离线知识支持工具。

Method: 提出一种适用于无互联网连接的模块化检索增强(RAG)架构，支持来自技术手册的文本和图像输入；并开展含四名巴西空军图像分析员的试点研究，包括：(i)基于其电子目标识别条令的知识评估，比较人类与系统在同一测试上的表现；(ii)测量无AI辅助下人工编制侦察目标报告(REMIR)的认知负荷。

Result: 人工任务要求苛刻，心理需求(6.0/7)和努力程度(5.0/7)较高；所提系统得分与人类持平(8/10)，评估耗时7.1分钟，而人类平均为26.5分钟，从而建立了未来AI辅助评估的基线。

Conclusion: 研究确立了离线检索增强系统在受限环境下作为教义知识决策支持的可行性基线，并提出了未来系统比较人工流程与AI辅助工作流的评估协议。
...
### [189] [DENSE: Distilling Agent Trajectories into Evidence-Grounded Shortcut Trees for Self-Refinement](https://arxiv.org/abs/2609.21423)
*Siyuan Liu,Fan Yu,Dongyu Ru,Yizhu Liu,Yifan Yang,Xuezhi Cao,Xunliang Cai,Yixin Cao*

Main category: cs.AI

TL;DR: 提出 DENSE，一种从嵌套子任务执行轨迹中蒸馏证据、构建可复用反馈的方法，无需事后结果标签；配套 REFIT 评测协议，在 Terminal-Bench 2.1 上严格通过率提升 7.12–15.64 个百分点，重跑 token 减少 19.0–43.6%。


<details>
  <summary>Details</summary>
Motivation: 在线智能体部署产生大量执行轨迹，但任务特定的验证与专家标注难以规模化。作者希望利用轨迹中蕴含的局部进展、恢复过程与未完成需求等证据，在无事后结果标签的条件下，将轨迹蒸馏为可复用的反馈。

Method: 提出 DENSE（Distilling Evidence from Nested Subtask Executions）：将证据组织为“证据锚定的嵌套捷径树”（nested shortcut trees），压缩冗余尝试，利用恢复证据跨层级协调问题，总结已完成分支并展开未解决分支，把可复用进展与剩余义务关联。提出 REFIT 协议：源配对的评测方案，在事后结果盲（outcome blindness）条件下比较来自相同初始轨迹的反馈，并重置环境与模型上下文以进行同一任务的全新尝试。

Result: 在 Terminal-Bench 2.1 上，DENSE 在四个接收模型中取得所测试的非特权反馈方法中最高严格通过率；相对初始执行，严格通过率提升 7.12–15.64 个百分点，重跑中观测到的接收方 token 减少 19.0–43.6%。GPT-5.5 消融实验支持将嵌套子任务分析与捷径构建、问题协调相结合。

Conclusion: 研究结果表明，智能体可借助证据锚定的轨迹复用实现自我改进，减少对外部监督的依赖。
...
### [190] [GVPO++: Group Variance Policy Optimization for LLM Post-Training and On-Policy Distillation](https://arxiv.org/abs/2609.21432)
*Kaichen Zhang,Yuzhong Hong,Junwei Bao,Hongfei Jiang,Yang Song,Dingqian Hong,Hui Xiong*

Main category: cs.AI

TL;DR: 本文提出GVPO，一种将KL约束奖励最大化的解析解融入梯度加权的后训练方法，避免依赖重要性采样，从而缓解训练不稳定，并自然扩展到在线策略蒸馏（OPD）及其扩展目标族。


<details>
  <summary>Details</summary>
Motivation: 现有后训练方法（如GRPO）依赖重要性采样，导致实际部署中训练不稳定，限制了其应用。

Method: GVPO将KL约束奖励最大化的解析解整合进梯度加权方案，其梯度对应隐式奖励中心距离与实际奖励中心距离之间的均方误差；方法不依赖重要性采样，允许灵活的采样分布，并可扩展到在线策略蒸馏及其扩展目标。

Result: GVPO保证唯一最优解，且恰好对应KL约束奖励最大化目标；同时支持灵活采样分布而无需重要性采样，并可优化更广泛的扩展OPD目标。

Conclusion: GVPO统一了理论保证与实际适应性，为可靠且通用的LLM后训练与在线策略蒸馏建立了新范式。
...
### [191] [Risk-Aware Occupancy for Safety-Oriented End-to-End Autonomous Driving](https://arxiv.org/abs/2609.21470)
*Jiaxing Chen,Hengduo Zou,Yiren Zhao,Bolin Gao*

Main category: cs.AI

TL;DR: 该论文提出"风险感知占据"（risk-aware occupancy）这一稠密表示，将全局场景占据、地图交通约束和未来动态智能体占据统一编码进BEV地图，并设计端到端网络ROIDrive，在nuScenes上实现显著碰撞率下降。


<details>
  <summary>Details</summary>
Motivation: 稀疏表示将环境感知建模为对象、车道线等离散元素，在拥挤、遮挡、非结构化障碍物、不确定区域和复杂交互场景中存在安全风险，因此需要更明确、统一地表征与规划相关的风险。

Method: 提出风险感知占据表示，将全局场景占据、地图衍生的交通约束、未来动态智能体占据联合编码为统一BEV地图，在空间和时间维度上捕捉轨迹规划的风险证据；设计端到端网络ROIDrive，通过独立分支预测风险感知占据，并将其注入规划查询以生成面向安全的轨迹；同时构建RiskOcc4D-nuScenes数据集（基于nuScenes和Occ3D-nuScenes）以量化安全问题。

Result: 风险感知占据在nuScenes上实现相对开环碰撞降低：UniAD指标下降低52.9%，ST-P3指标下降低35.0%。

Conclusion: 用稠密、显式、统一的风险感知占据替代稀疏表示，可有效提升端到端驾驶系统的规划安全性，并通过新数据集RiskOcc4D-nuScenes量化验证。
...
### [192] [Driving on Registers, Reasoning on Risk: Risk-Aware Occupancy for Register-Based End-to-End Autonomous Driving](https://arxiv.org/abs/2609.21486)
*Jiaxing Chen,Hengduo Zou,YuKai Qin,Yiren Zhao,Lidong Yu,Bolin Gao*

Main category: cs.AI

TL;DR: 本文提出RRDrive，引入风险感知占据（risk-aware occupancy）作为稠密、时间对齐且可被轨迹查询的表示，用于多模态轨迹预测，并结合RiskOcc4D-NAVSIM数据集与外部RiskOcc预测器，显著提升候选生成质量与排序可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态轨迹预测受限于稀疏场景表示，证据不完整导致候选轨迹生成质量低，且几何相似轨迹间排序不可靠。在register-based基线上，差与较差候选占19.74%，oracle最优候选平均仅排第33.9位。

Method: 提出RRDrive：利用risk-aware occupancy作为稠密、时间对齐、可被轨迹查询的表示；其全局结构引导高质量多模态生成，候选条件风险查询支持细粒度选择。构建带自动风险标注的RiskOcc4D-NAVSIM。进一步开发外部RiskOcc预测器，直接从传感器输入估计风险感知占据。

Result: RRDrive取得选中轨迹PDMS 0.951，较基线0.937相对提升1.5%；平均候选PDMS提升7.7%；在困难场景中候选PDMS提升30.2%，好候选间Spearman相关从0.26提升至0.67（+0.41）。外部RiskOcc预测器性能具竞争力。

Conclusion: 风险感知占据表示能有效提升多模态轨迹生成的覆盖度与排序质量，并可通过感知模块从传感器输入直接估计，验证了该表示的可行性与实用价值。
...
### [193] [LogicTrack: Auditing Reasoning Trajectories of Large Language Models with Formal Logic Solvers](https://arxiv.org/abs/2609.21492)
*Jingyu Hu,Shu Yang,Weiru Liu,Di Wang*

Main category: cs.AI

TL;DR: LogicTrack是一个神经符号框架，通过自动形式化推理步骤并用定理证明器验证，配合基于求解器的回溯奖励（SBR）进行逐步评分与推理时回溯搜索，并生成带回溯轨迹的SFT数据，从而提升大模型思维链的逻辑可靠性与答案正确率。


<details>
  <summary>Details</summary>
Motivation: 现有CoT优化方法多依赖结果导向的反馈，缺乏对中间推理步逻辑有效性的验证，导致模型可能通过逻辑有缺陷的推理链得出正确最终答案。

Method: 提出LogicTrack神经符号框架：将每个推理步骤自动形式化为符号表示，并用自动定理证明器验证；引入Solver-Based Backtracking Reward（SBR），一种逐步评分机制，量化逻辑合理性并指导推理时的回溯树搜索；进一步利用其轨迹中的回溯痕迹构建监督微调（SFT）数据，使微调模型内化逐步审计能力。

Result: 在8个推理基准和7个大语言模型上的大量实验表明，LogicTrack有效提升了推理链的可验证性与最终答案通过率。

Conclusion: LogicTrack通过神经符号逐步验证与回溯奖励机制提升了思维链质量与可信度，适用于高风险领域的推理应用。
...
### [194] [PolyBridgeBench: Benchmarking Multimodal LLMs for Physics-Grounded Bridge Design](https://arxiv.org/abs/2609.21493)
*Zicheng Zhao,Dongyin Chen,Rui Xu,Yinghui Xu*

Main category: cs.AI

TL;DR: 该论文提出 PolyBridgeBench，一个可执行的桥梁设计多模态基准，用于检验 MLLM 能否生成完整承重结构并在仿真失败后进行修复。


<details>
  <summary>Details</summary>
Motivation: 现有基准多评估空间推理、结构合法性或物理构建，但无法判断 MLLM 是否能综合出完整承重结构，并在仿真执行暴露失败后进行修复。视觉理解与结构化生成能力并不等于工程设计可执行。

Method: 模型接收视觉场景与结构化工程约束，生成完整的节点—构件—材料拓扑；确定性合法性检查作为原生动态物理仿真的执行门控。执行失败后，基准返回失败推演的时间序列视觉证据，并在固定交互预算下评估修复。分别测量确定性有效性、动态功能成功与失败后恢复。

Result: 六个代表性 MLLM 在 189 个关卡上的实验显示：确定性有效性与动态成功之间存在显著差距，对材料预算高度敏感，且在主要严格预算设置下失败后恢复能力有限。

Conclusion: PolyBridgeBench 通过分阶段测量定位设计失败的环节，揭示了 MLLM 在可执行工程设计中的能力缺口，尤其是动态承重成功与失败后修复方面。
...
### [195] [The Communication Bottleneck: A Round-Trip Study of Tree-Structured Expression Serialization in Language Models](https://arxiv.org/abs/2609.21509)
*Xavier Suau,Alex Ferrando de las Morenas,Luca Zappella,Samy Bengio*

Main category: cs.AI

TL;DR: 提出"往返协议"实证测量语言模型在自然语言序列化中保留树结构组合信息的能力，发现该信道有损且不对称、失败多源于生成端且受树结构驱动、并可通过少量微调提升。


<details>
  <summary>Details</summary>
Motivation: 语言模型在进行思维链推理或交换自由文本中间结果时，会将结构化信息序列化为自然语言。作者希望量化这一"瓶颈"中究竟有多少树结构组合内容能够存活。

Method: 设计往返协议：生成器将程序化生成的算术表达式转化为文字应用题，独立的提取器仅从文字应用题还原表达式，并以符号等价作为精确评测基准。对16个模型的所有两两组合进行评测，得到通信矩阵，其边际分布可分离生成质量与提取质量。另进行共享语义的约3600条微调实验，以及引入新算子与新词表的非同分布域实验。

Result: 信道有损且不对称：交换生成与提取角色可使准确率变化达60.4个百分点，最佳组合在两端使用不同模型而非同一模型，达92.9%。至少73.6%的往返失败源自生成端，难度由树结构（算子数量、深度、右分支）而非模型家族驱动。约3600条与评测共享算子与树形的微调样本可将所有开放权重模型提升至未训练Gemini-3.1-Pro之上；非同分布域实验同样提升所有开放权重模型，证明增益并非语义匹配的假象，但与前沿模型仍有差距。

Conclusion: 树结构表达式的序列化是模型通过自然语言传递层次结构时的主要限制因素，信道具有可训练性，生成端是主要瓶颈。
...
### [196] [Learning-to-Optimize as the Missing Architectural Layer of AI-Native Networks](https://arxiv.org/abs/2609.21519)
*Giambattista Amati,Federica Mangiatordi,Pierpaolo Salvo,Emiliano Pallotti,Simone Angelini*

Main category: cs.AI

TL;DR: 本文提出将Learning-to-Optimize (L2O)视为连接优化算法与AI原生智能的缺失架构层，通过四阶段工作流把优化算法转化为离线知识生成器，训练神经代理模型实现低时延在线推理，并以NR-V2X中继选择问题（MILP求解器生成解训练GNN）进行验证。


<details>
  <summary>Details</summary>
Motivation: 未来AI原生通信网络正把AI作为基本设计原则，以支持自主资源管理、自适应控制与零接触运维。然而现有AI原生架构虽在各网元中嵌入智能，却缺乏对"优化知识应如何被系统化生成、迁移与利用"的指导，即缺少一个在优化与AI原生智能之间的架构抽象层。

Method: 提出以L2O作为该缺失架构层的新范式，将优化算法重新定义为离线知识生成器，而非仅作为在线决策引擎，从而为神经代理模型产生高质量监督信息。给出通用四阶段L2O工作流：优化求解、知识生成、代理学习与运行时推理；其代理模型继承优化专长，同时支持动态网络环境所需的低时延推理。与聚焦算法加速的现有L2O方法不同，该框架将L2O确立为可跨异构通信与计算系统应用的架构抽象。

Result: 以NR-V2X中继选择问题作为示例验证，使用混合整数线性规划（MILP）求解器生成的优化解训练图神经网络（GNN），该GNN能够在实时条件下复现近最优决策。

Conclusion: 该观点将Learning-to-Optimize定位为未来AI原生网络的关键架构使能者，为优化知识与AI原生智能之间的衔接提供了系统化抽象与可操作工作流。
...
### [197] [Dual-Interest Sequential Product Recommendation With Multi-Granular SSM](https://arxiv.org/abs/2609.21548)
*Shuiying Liao,P. Y. Mok*

Main category: cs.AI

TL;DR: 提出DSRec，一种双兴趣跨SSM模型，通过显式解耦长短期语义上下文中的物品角色，并结合Mamba与时序调制SSM及残差交叉融合，提升序列推荐效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的序列推荐虽性能优异但成本高；SSM方法虽实现高效长程建模，却多以单一静态上下文角色编码物品，忽视物品多义性，也难以捕捉不同时间粒度下的动态行为。

Method: 将序列物品编码为长期兴趣嵌入（历史聚合捕获稳定偏好）与短期兴趣分支（以点击时间间隔调制局部会话意图）；分别用全序列Mamba与时间调制SSM编码；采用残差交叉融合机制在两个分支间交换上下文信息，同时保持语义独立性。

Result: 在公开基准数据集上的实验表明，DSRec优于其他最先进方法。

Conclusion: DSRec通过双兴趣解耦与跨粒度对齐有效建模物品多义性与动态行为，验证了SSM双分支架构在序列推荐中的有效性。
...
### [198] [Beyond Accuracy: Centroid-Guided Contrastive Loss for Structured Fraudulent Job Posting Detection](https://arxiv.org/abs/2609.21599)
*Syed Ali Ahmed,Malaika Raza,Muhammad Shoaib Siddiqui,Muhammad Rafi*

Main category: cs.AI

TL;DR: 本文提出Centroid-Guided Contrastive Loss（CGCL），将分类与密集聚类统一，通过质心驱动的top-k推拉机制重塑潜在空间，用于欺诈招聘帖检测。


<details>
  <summary>Details</summary>
Motivation: 现有欺诈招聘帖检测方法难以同时实现高准确率和具有意义的潜在空间结构表示，无法捕捉虚假帖子间的细微差异。

Method: 提出CGCL损失函数，统一分类与密集聚类，通过质心驱动的top-k推拉机制一致地重塑潜在空间，兼顾类间可分性与类内紧凑性。

Result: 在公开基准数据集EMSCAD上，实验表明该方法达到SOTA性能。

Conclusion: CGCL能够同时实现准确决策边界与高聚类紧凑度，有效捕捉类可分性和潜在结构；代码已开源。
...
### [199] [Reducing Barriers to Academic Support: Evaluating a Course-Specific RAG System for Addressing Help-Seeking Disparities in Higher Education](https://arxiv.org/abs/2609.21600)
*Andy Gray,Jake Hobbs*

Main category: cs.AI

TL;DR: 该论文提出并评估了Beacon——一个面向特定课程的RAG学术支持系统，通过将回答锚定于经批准的课程材料，降低学生求助门槛，同时促进自主学习。


<details>
  <summary>Details</summary>
Motivation: 学术支持的可及性是学生成功的关键决定因素，但学生获得支持并不平等：部分学生因焦虑、害怕被评判、对期望不确定或缺乏自信而犹豫求助。这一问题在编程等累积性强、认知负荷高的计算教育中尤为突出。通用生成式AI工具虽被广泛使用，却可能产生不准确、缺乏情境化或与课程期望不符的回答。

Method: 采用设计型研究方法，迭代开发课程专属的检索增强生成系统Beacon，并以混合方法进行评估，结合问卷与半结构化访谈，研究对象为某高等教育机构的学生与教职工。

Result: 学生认为Beacon的回答与课程内容高度契合，比不受限的生成式AI工具更可信；他们尤其看重其使用伪代码与支架式解释而非直接给出答案。参与者对未经核实即信任AI生成内容仍持谨慎态度，但将系统视为在咨询讲师或查阅官方资源之前的有价值的第一支持点。

Conclusion: 精心设计的课程专属AI系统可通过占据自主学习与正式支持之间的中介空间来降低学术支持障碍。教育AI并非取代教师，其最大价值在于拓宽获取指导的渠道，同时保留讲师的教学角色。
...
### [200] [Calibrating Teacher--Student Discrepancy for On-Policy Distillation](https://arxiv.org/abs/2609.21619)
*Qiangqiang He,Jin Li,MingCai Chen*

Main category: cs.AI

TL;DR: Cal-OPD通过正负特权干预估计教师自身的自偏差区域，并据此校准师生差异信号，在仅保留约52–65%原始优化信号的情况下，在数学推理基准上稳定优于标准OPD及其变体。


<details>
  <summary>Details</summary>
Motivation: 在线策略蒸馏（OPD）通过让学生模型学习更强的教师模型与其自身在token级别的差异来提升推理能力，但该差异并不纯粹反映师生能力差距，还混入了教师自身产生的偏差，被标准OPD不加区分地学习。特权OPD会进一步放大这一问题：特权信息诱发更大的教师侧似然偏移，使学生学到更多教师自身的偏差。

Method: 提出Calibrated On-Policy Distillation（Cal-OPD）。该方法通过正向与负向特权干预来估计教师的自偏差区域，并利用该区域对原始师生差异进行校准，仅保留位于该区域之外的分量作为优化信号。

Result: 在数学推理基准上，Cal-OPD在仅保留约52–65%原始师生差异作为优化信号的前提下，在不同模型规模上均持续优于标准OPD及其变体。

Conclusion: 师生差异中混入的教师自身偏差会对在线策略蒸馏产生负面影响，通过特权干预识别并剔除该自偏差区域、仅保留真正的能力差距信号，可在更少优化信号的条件下获得更优的推理性能。
...
### [201] [One Prompt Does Not Fit All: Self-Meta-Evolve for Personalized Information Extraction](https://arxiv.org/abs/2609.21626)
*Hongliang Li,Lu Wang,Yong Xu,Hanyang Chen,Zhitao Hou,Xiaoting Qin,Song Ge,Qingwei Lin,Dongmei Zhang*

Main category: cs.AI

TL;DR: 论文将企业信息抽取建模为面向单用户的提示自适应问题，提出双层进化框架Self-Meta-Evolve，并发布292个模拟企业用户的人格驱动基准，成功率74.58%，较最强基线提升13.56个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法依赖单一提示并针对全局目标优化，与企业真实工作场景中同一文档需按不同用户需求重新组织的用户异质性本质相冲突，因此需要面向单用户、基于交互反馈的提示自适应方法。

Method: 提出Self-Meta-Evolve分层框架：为每个用户维护专属提示，通过双循环持续精炼——内循环依据人格条件化反馈编辑结构化提示，外循环通过蒸馏成功编辑模式来进化元提示本身；同时基于O*NET职业分类体系构建可复现的人格生成流水线，发布含292个模拟企业用户的人格驱动IE基准。

Result: 在该基准上成功率达74.58%，比最强提示优化基线高出13.56个绝对百分点；仅需两次迭代即达到52.54%。与20位真实从业者的双盲人工研究进一步表明，本框架适配的提示在71%的成对比较中优于静态基线。

Conclusion: 面向单用户的提示自适应是企业IE的有效建模范式，双层元提示进化能在有限迭代内显著超越全局优化基线，并在真实专业人员评审中获得多数偏好，验证了用户异质性建模的必要性与实用价值。
...
### [202] [Accelerating Dense LLMs via L0-regularized Mixture-of-Experts](https://arxiv.org/abs/2609.21672)
*Zhenyu Zhang,Jiudong Yang,Zhaowen Tao,Meng Chen*

Main category: cs.AI

TL;DR: L0-MoE：用L0正则化的轻量级MoE加速稠密LLM，最高2.5倍加速且性能几乎无损。


<details>
  <summary>Details</summary>
Motivation: LLM推理慢、成本高；现有加速方法常带来明显性能损失，而常规MoE需大量计算资源。

Method: 提出轻量级MoE方法L0-MoE，采用L0正则化加速稠密LLM；引入聚类混淆矩阵进行领域感知的数据集构建，并用动态批处理实现高效训练。

Result: 相较稠密模型最高2.5倍加速，同时保持有竞争力的性能，优于现有LLM加速基线。

Conclusion: L0-MoE可在近乎不损失性能的前提下有效加速稠密LLM，是一种有前景的加速方案。
...
### [203] [GUARD: Natural Forgetting in Large Reasoning Models via Guided Answer-Reasoning Distillation](https://arxiv.org/abs/2609.21677)
*Zeyu Yan,Guanghao Zhou,Minghui Qiu,Ming Gao,Cen Chen*

Main category: cs.AI

TL;DR: 本文提出 GUARD（Guided Answer-Reasoning Distillation），通过将模型生成的不安全披露转化为"安全退出轨迹"，用引导 token 对齐冻结的大推理模型并将该行为蒸馏进参数，实现"自然遗忘"；同时提出 NFRS 指标评估替换质量，在 R-TOFU 与 STAR-1 衍生场景上显著降低泄露且保持推理能力。


<details>
  <summary>Details</summary>
Motivation: 大推理模型（LRM）的中间思维链（CoT）会在最终答案前暴露受保护事实或不安全推理，而现有 unlearning 方法只抑制目标内容或重定向内部表示，未规定遗忘后的轨迹应如何延续，导致幻觉替代、边界错乱或重复输出。

Method: 提出 Guided Answer-Reasoning Distillation (GUARD)：将模型自身生成的不安全披露改写成"连贯的非披露 CoT + 稳定拒绝式回答"的安全退出轨迹；通过 guidance tokens 对齐一个冻结的 LRM；再将这种引导行为蒸馏到模型参数中。同时提出 NFRS 指标，从结构稳定性、流畅性、无支撑替代内容三方面衡量替换质量。

Result: 在 R-TOFU 和一个由 STAR-1 派生的有害意图设定上，针对两种广泛使用的蒸馏式 LRM 进行大量实验，GUARD 在显著减少不安全与隐私披露的同时保持了推理效用。

Conclusion: LRM 遗忘应学习"自然遗忘轨迹"而非单纯抑制；GUARD 结合轨迹蒸馏与 NFRS 评估，为兼顾安全性与推理效用的遗忘提供了有效框架，代码已开源。
...
TL;DR: 通过机制分析与因果干预，作者证明TaxiGPT的行为失败并非缺乏世界模型，而是叠加路口特征之间的干扰破坏了内部定位。


<details>
  <summary>Details</summary>
Motivation: 先前研究将transformer在导航任务中的行为失败解读为其内部世界地图不一致或缺失世界模型的证据，作者认为这种推断可能错误：模型或许具有忠实的环境表征，但行为失败来源于表征使用层面的问题，因此需要从机制上重新审视世界模型的有无。

Method: 以在曼哈顿随机游走数据上训练的transformer（TaxiGPT）为对象，采用机制分析与因果干预方法，识别模型中表征路口、街道、自身位置及目标方位（goal compass）的具体组件，定位失败来源；并提出“affordance packing”策略，将具有相同合法动作的路口表征归组；同时构造机制性指标用于跨模型比较，并追踪训练不同阶段世界建模能力的出现过程。

Result: 模型确实表征了路口与街道、能够追踪自身位置并利用目标方位进行导航；其失败可归因于叠加（superposed）路口特征之间的干扰，导致内部地图中的定位被破坏。Affordance packing能够限制此类错误的后果。机制性指标显示世界建模能力在训练的不同阶段逐步涌现，且各模型之间存在差异。

Conclusion: 应把研究问题从“模型是否具有世界模型”转向对世界建模的机制性研究，即考察模型表征环境并据此引导行为的多个相互作用能力。行为失败不足以判定世界模型缺失。
...
TL;DR: 论文提出并定义"ECG Mirage"现象，即视觉语言模型(VLM)在急诊多模态预测中表面具备多模态能力，实则未有效利用患者特异性ECG信息，并通过对比实验验证该问题并提出视觉提示微调缓解策略。


<details>
  <summary>Details</summary>
Motivation: 急诊决策依赖病史、生命体征、化验和心电图等异构信息，VLM虽可联合处理这些模态，但强预测性能未必意味着真正利用了正确的患者ECG。作者关注这种"假性多模态能力"的失效模式，旨在揭示并量化模型对患者特异性ECG信息缺乏有效依赖的问题。

Method: 定义ECG Mirage的两种形式：ECG忽视(ECG几乎不带来预测增益)与ECG混淆(匹配ECG优于无图像输入但不优于错配ECG)。实验在MDS-ED数据上，固定临床文本与预测目标，对比匹配ECG、结局不一致的错配ECG和无图像输入三种条件下的预测。随后在冻结VLM主干的前提下，用监督学习加条件直接偏好优化(conditional DPO)训练四个受限视觉提示。

Result: 在四种VLM上，匹配ECG对ICU入住或临床恶化预测均无一致优势，证实ECG Mirage存在。经视觉提示训练后的模型在ICU入住预测上达70.6%平衡准确率，恶化预测达67.5%，并将匹配-错配性能差距分别提升至约16.5和5.5个百分点。

Conclusion: 研究识别了多模态临床预测中的ECG Mirage现象，表明VLM的强性能可能掩盖其对患者特异性ECG的无用依赖；视觉提示微调是一种高效缓解策略，能显著增强模型对匹配ECG的依赖并提升预测表现。
...
TL;DR: 在严格选择协议下，一个提取滑窗统计量、用MAD转导鲁棒模型打分并按域选择特征子集的简单流程，在TSB-AD-U上达到0.529 VUS-PR，超过最强神经(0.45)与统计(0.44)方法，逼近最强预训练基础模型。消融表明候选池决定性能上限，选择与聚合搜索仅为二阶因素。故用多模态LLM以内嵌示例窗口按域生成候选池，其与手工池在匹配选择下相当且覆盖互补域，二者并集选择在全部12个生成器-种子组合上提升，将流程提升至0.588，与排行榜最佳条目持平。


<details>
  <summary>Details</summary>
Motivation: 探究在严格选择协议下，简单统计流水线在单变量时间序列异常检测中能达到何种水平，并厘清决定性能的关键因素（候选池、选择策略、聚合方式）的相对重要性。

Method: 方法为：在滑动窗口上提取小型统计量池，用转导鲁棒（MAD）模型对窗口打分，并在留出调优集上按域选择特征子集；进一步通过提示多模态LLM、提供该域的内嵌示例窗口来生成候选池，并在手工池与生成池的并集上进行选择。

Result: 在TSB-AD-U上达到0.529 per-series VUS-PR，高于公开排行榜上最强神经（0.45）与统计（0.44）条目，距最强预训练基础模型差0.06。消融显示：三种选择策略及事后oracle使分数变动0.031，聚合网格变动0.096，而更换候选池变动0.226。生成池在匹配选择下与手工池相当，域覆盖互补；并集选择在全部12个生成器-种子对中均提升，分数升至0.588，与排行榜最佳条目持平。

Conclusion: 候选池设定性能上限，而对其搜索仅为二阶因素。利用多模态LLM按域生成候选池并与手工池联合选择，可显著提升简单统计流水线，达到与排行榜最优条目相当的水平，表明该任务的瓶颈在于候选统计量的覆盖而非模型或搜索复杂度。
...
### [207] [EnterpriseVal: Quantifying the Efficacy, Reliability and Value of Generative AI in the Enterprise](https://arxiv.org/abs/2609.21841)
*Abbas Raza Ali,Muhammad Ajmal Siddiqui,Moona Zahid*

Main category: cs.AI

TL;DR: 论文指出企业GenAI落地失败的核心是"测量问题"，提出EnterpriseVal——一套用例级评估系统，将模型能力基准转化为部署决策依据，并在某全球银行三个工作流中试点验证。


<details>
  <summary>Details</summary>
Motivation: 前沿语言模型已能在大量经济价值任务上产出与人类专家相当的专业交付物，但多数企业GenAI项目未能体现可测量的业务收益，且相当比例的智能体项目预计将被取消。作者认为这主要是测量问题：公开基准回答的是"模型能做什么"，而部署决策需要回答"该工作流在此处、基于我们的数据与管控条件下，是否适配、可靠、安全且值得规模化"。

Method: EnterpriseVal由五部分构成：(i) 用例及冻结的社会技术配置的形式化规范，涵盖模型、提示、检索、工具、护栏与人工监督，并以自主性等级与后果层级共同确定评估强度；(ii) 覆盖保真度、效用、效率、可靠性、保障与监督的指标目录；(iii) 通过预测驱动推断（prediction-powered inference）以校准的LLM-as-judge评分扩展盲审专家判断的评分协议；(iv) 以可执行算法表述的两级阈值门，将带置信界的指标向量映射为拒绝/有条件/规模化决策；(v) 将审查者捕获率作为实测参数的价值-风险模型。

Result: 在某全球银行三个工作流的试点中：信贷备忘录撰写任务里，最优模型的人工评分引用精确率达88%、幻觉率1.6%，对应门槛为70%与5%；流程改造任务中，分析师精修工作量从估计的每份文档27.4小时降至2.9小时。作者区分了已确立结果、有记录的试点证据、所提出系统与开放假设，并列出完整验证所需的实验。

Conclusion: 该工作将企业GenAI的部署决策重新定义为可度量的用例级评估问题，提供了从形式化规范、指标体系、评分协议、阈值门到价值-风险模型的完整框架，并以银行试点给出初步证据；但作者明确其系统部分与开放假设尚需专门设计的实验才能完成全面验证。
...
TL;DR: 本文研究部分证据下的交互式检索问题，提出 RAVEL——一个面向交互式行人重识别的检索感知在线强化学习框架，直接优化问题策略以最大化检索收益。


<details>
  <summary>Details</summary>
Motivation: 现有系统通过模仿候选问答对的离线排序来训练提问决策，但问题价值实际上取决于其引出的回答以及对检索的下游影响；候选区分度与主观有用性只能提供弱监督，无法对齐真实检索目标。

Method: RAVEL 从监督式问题生成模型初始化，直接观测当前 Top-4 候选，并利用完整"提问—回答—检索"闭环中的排序反馈，通过在线强化学习优化提问策略。

Result: 在 Interactive-PEDES 数据集上，RAVEL 在五轮交互中实现逐步增强的检索性能；分析显示其将提问预算重新分配给局部化开放式属性，这类问题提供更有用的检索证据，并在初始困难查询上取得最大提升。

Conclusion: 以检索结果为终局反馈的在线强化学习优于离线模仿排序范式，能够有效分配提问预算并显著提升交互式检索，尤其是困难查询场景。
...
TL;DR: AutoViewMem 通过“数据结构化优先”的思路，将长时记忆在写入阶段组织为自配置、低重叠的语义视图，从而在标准 top-K 检索下提升长程问答与个性化能力。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 智能体记忆系统多采用固定粒度或静态 schema，难以处理单条混合表示中同时包含偏好、事件、约束和时间更新等异构信息的问题；由此产生的语义干扰使 top-K 检索对噪声敏感，相关证据排序不佳。

Method: 提出 AutoViewMem 数据驱动框架：在索引前将长期对话记忆组织为自配置、低重叠的语义视图。具体包括从交互轨迹中发现候选视图、选择紧凑的互补视图集合，并以此指导写入阶段的结构化抽取，生成带溯源依据的记忆；同时引入离线整合以提升记忆紧凑性与一致性。

Result: 在 LoCoMo 与 PersonaMem 基准上，基于 Qwen3-8B 和 Qwen3-14B 骨干模型的实验表明，AutoViewMem 在长程问答与个性化任务上优于强记忆基线，同时保持简单推理流程。

Conclusion: 将语义解耦从检索阶段前移至写入阶段，可有效缓解混合表示带来的语义干扰，使标准 top-K 相似度检索无需显式路由或迭代检索即可获取聚焦证据，为长时记忆提供了一种可扩展且推理简便的解决方案。
...
TL;DR: 该研究在运动诱发应激与跨会话条件下评估心电生物识别，采用Siamese ResNet与晚期多导联融合，在运动-时间感知协议下取得1.7%的会话内静息至峰值EER，并在CYBHi数据集上达到最优的3.9%。


<details>
  <summary>Details</summary>
Motivation: 现有心电生物识别研究多局限于单会话静息数据，对时间与生理变化（如运动应激）的鲁棒性缺乏验证，而这对其在敏感心脏数据保护及自监督预训练任务中的应用至关重要。

Method: 构建Siamese ResNet模型并采用晚期多导联融合策略，基于心肺运动试验提取的大规模心电数据集训练，使用运动-时间感知协议评估，并在公开基准（如CYBHi）上进行测试。

Result: 在生理与时间双重变化下，会话内静息至峰值EER为1.7%，在CYBHi数据集上取得3.9%的最新最优性能。

Conclusion: 结果表明存在一种对生理与时间漂移具有鲁棒性的内在心脏特征签名，支持心电生物识别在现实变异性条件下的可行性。
...
TL;DR: 本文提出PIR（Probe of Internal Recognition），借鉴测谎中的隐藏信息测试范式，通过读取模型内部状态判断它是否"识别"出正确答案，从而区分模型"不愿回答"与"不能回答"。


<details>
  <summary>Details</summary>
Motivation: 大语言模型可能掌握知识却不报告，例如在能力评估中故意放水（sandbagging）或违背内部已知答案作答。仅凭输出无法判断模型是隐藏了答案还是根本没有该知识，这对模型审计与评估构成障碍。

Method: PIR将一个问题连同候选答案一同呈现，从模型内部状态中读取其"识别"为正确的候选答案，类似法医隐藏信息测试中通过真实细节与干扰项的比较识别"有罪知识"。该方法是无需参考的（reference-free），不需要诚实参考模型，也不需要标注真值语料。

Result: 在来自五个家族（Gemma、Qwen、Llama、Mistral、Phi）的八个模型上，PIR以0.70–0.87的平衡准确率恢复出被识别的答案，显著高于未知项的0.28–0.40基线和0.25的随机水平。在所有测试的隐藏形式（提示欺骗、训练sandbagging、外部密码锁定、circuit-broken检查点）下保持可读性，识别率0.85–0.93。模型隐藏已知答案时识别率仍高；当遗忘（unlearning）真正移除知识时，识别率降至模型从未知晓该问题的水平。

Conclusion: PIR能区分"不愿回答"与"不能回答"的模型，支持sandbagging审计和遗忘验证。该信号具有因果性，提供了超越黑箱行为线索的额外信息，并可从多项选择题扩展到自由生成任务。
...
TL;DR: CodeMidas将开源代码库中已实现的功能转化为可执行RL环境，仅以源码为任务特定输入，构建了覆盖23种语言、15个领域的5,545个训练任务，训练MiMo-V2.5后在五类基准上全面提升。


<details>
  <summary>Details</summary>
Motivation: 训练具备编码能力的智能体需要通过强化学习获得多样化且具备可靠验证器的任务。开源代码库蕴含丰富任务来源，但现有方法多依赖issue、commit等开发过程产物，限制了可提取任务的范围，难以规模化构建RL环境。

Method: CodeMidas是一条智能体化流水线，将代码库中已实现的功能转换为可执行RL环境。它把智能体算力分配到环境构建的每个阶段：智能体探索已实现功能以形成行为规范；基于原始代码的实际执行构建测试；通过执行检查与多次解轨迹采样对候选任务进行验证和过滤。

Result: 构建的数据集包含来自3,185个开源代码库的5,545个训练任务，覆盖23种编程语言与15个技术领域。用GRPO训练MiMo-V2.5后在五个基准上均获提升：issue修复（DeepSWE +11.7%）、整程序构建（ProgramBench +17%）、终端任务（Terminal-Bench v2.1 +8.5%）。消融实验表明增加高质量训练任务数量可提升性能；轨迹分析显示RL训练后的智能体表现出更多代码库探索与更多样化的自验证行为。

Conclusion: 以源代码作为构造RL环境的可扩展基础是可行的，能够提升编码智能体在多样软件任务上的表现。仅依赖源码作为任务特定输入即可大规模生成高质量可验证任务，任务规模与质量对性能具有正向作用。
...
### [213] [Verify, Don't Trust: Agentic Model Development for Video Discovery Retrieval at Scale](https://arxiv.org/abs/2609.21257)
*Hao Fu,Baiting Zhu,Minglei Chen,Yinjie Huang,Shuai Ding*

Main category: cs.IR

TL;DR: 本文提出EvoPilot——一种人类把关的长周期在线自动研究方法，通过角色化智能体、版本化领域技能、类型化适配器、持久记录与确定性检查保障实验有效性，并在37天VDD检索系统研究中修正评测缺陷、取得离线+3.20pp与在线GSRR相对提升0.66%的结果。


<details>
  <summary>Details</summary>
Motivation: 现有autoresearch多局限于分钟级、自包含程序的短周期迭代；而真实在线自动研究涉及异步系统、数小时至数周的长周期实验，且成品运行仍可能因空操作代码变更、数据窗口泄漏、评测语义漂移或两组实验经由不同服务漏斗而产生无效结论。因此需要一个面向长周期在线场景、以人类把关与有效性校验为核心的自动研究方法。

Method: EvoPilot采用人类把关（human-gated）的长周期在线autoresearch流程：每轮由角色专用智能体通过版本化的领域技能（domain skill）和类型化适配器（typed adapter）执行；持久化记录保存实验与失败；确定性检查（deterministic checks）强制落实已记录的教训。方法上还包括人类把关的验证、匹配对比（matched comparison）、研究后重放与变异测试，以及持久状态恢复与工件复用。

Result: 在37天、覆盖七个方向、基于数亿视频小时级刷新索引的VDD检索系统研究中：早期人工实验未证明交互头有收益，一次原始autoresearch尝试将离线命中率下降22个百分点错误归因于该交互头；引入EvoPilot后，人类把关验证定位到该下降源于既有评测缺陷（输出深度为3,000与600），修复后匹配对比测得离线提升3.20个百分点。重放与变异测试拒绝了无效对比、接纳了有效对比；持久状态恢复了一个被中断的轮次；工件复用节省约5个GPU小时；另经7天随机在线评估，VDD切片上GSRR相对提升0.66%。

Conclusion: EvoPilot证明在长周期在线自动研究中，人类把关、持久状态、确定性检查与工件复用可有效防止无效归因与评测缺陷导致的错误结论，同时支持有效优化：在VDD检索系统中实现离线+3.20pp与在线GSRR相对+0.66%的可信收益，并降低算力开销。
...
### [214] [Hybrid GPU-CPU Retrieval for Personalized Search at Ultra-Large Scale](https://arxiv.org/abs/2609.21281)
...
TL;DR: 本文提出一种GPU-CPU混合协同服务架构，通过编排而非新模型来解决超大规模个性化检索中"深度与广度"冲突的悖论，并在生产环境验证其有效性与经济性。


<details>
  <summary>Details</summary>
Motivation: 在万亿级用户生成内容上的嵌入检索面临"个性化-规模悖论"：深度表达性个性化与大规模库存覆盖在固定延迟与资源预算下相互冲突。将全部服务库存放入GPU显存资源开销过大，而CPU无法在延迟关键的路径上运行同样交互密集的模型。

Method: 提出混合GPU-CPU协同服务系统，通过编排解决悖论：GPU高深度路径在约十亿文档的精选在线池上融合检索与交互式预排序；CPU高广度路径以轻量个性化打分搜索约二十倍规模的独立在线库存。每条请求可运行一条或两条路径，候选在共享下游排序前去重。

Result: 系统已部署生产。与仅用CPU的旧配置进行全系统A/B测试，模型打分相关性与实质性互动均有提升；分路径实验在各自部署范围内也显示正向价值。检索日志表明两条路径贡献结构上不同的候选；生产服务测量刻画了其延迟；匹配容量规划量化了"GPU负责建模深度、CPU负责库存广度"的经济性依据。

Conclusion: 研究结果验证了一种实用、可独立演进的"深度-广度"架构，适用于超大规模个性化搜索。
...
### [215] [Auto-Bidding with Disentangled Advertiser Profiles and Train-Free Adaptation](https://arxiv.org/abs/2609.21308)
*Songyue Cai,Shan Gu,Wei Chen,Ziru Xu,Lianyu Wang,Jian Xu,Xiaofeng Zhu*

Main category: cs.IR

TL;DR: 提出 ADAPT 自动出价框架，通过解耦广告主画像与免训练自适应，提升个性化出价效果。


<details>
  <summary>Details</summary>
Motivation: 基于画像的个性化方法在推荐等领域有效，但在自动出价中应用有限。主要挑战在于：难以提取纯净画像、难以同时建模共性与私有信息、画像更新与冷启动自适应困难。

Method: ADAPT 采用两阶段训练范式并支持免训练自适应：(i) 阶段一通过广告主记忆库上的对比学习提取纯净的静态与动态画像；(ii) 阶段二将动态画像解耦为共性画像与私有画像，并与静态画像结合以联合条件化出价策略；(iii) 训练完成后，无需重新训练即可为新广告主构建画像、为既有广告主更新画像。

Result: 在大规模自动出价基准上，ADAPT 持续取得更优性能，消融实验进一步验证了各模块的有效性。

Conclusion: ADAPT 通过画像解耦与免训练自适应，有效应对自动出价中的画像构建、建模与更新难题，代码将开源。
...
### [216] [Adaptive Preference Modeling via Explicit Indirect Relational Learning for Personalized Fashion Matching](https://arxiv.org/abs/2609.21475)
*Shuiying Liao,Li Li,P. Y. Mok*

Main category: cs.IR

TL;DR: 本文提出APCL框架，通过相关性引导的自适应聚合显式建模直接与间接的用户-物品和物品-物品关系，并引入功能视图对比学习对齐直接与间接偏好/兼容性表示，结合多模态信息提升稀疏交互下的时尚互补推荐性能。


<details>
  <summary>Details</summary>
Motivation: 个性化时尚互补推荐需要在稀疏且多模态的数据条件下同时建模用户偏好与物品兼容性。现有方法多通过图传播隐式捕获高阶关系信号，或依赖直接交互数据，难以显式建模间接的偏好与兼容性关系，从而限制了表达能力。

Method: 提出APCL（Adaptive Preference with Contrastive Learning）框架：1）通过相关性引导的自适应聚合机制构建间接的用户-物品与物品-物品关系，并将其表示为个性化视图和兼容性视图；2）引入功能视图对比学习策略，对齐直接与间接的偏好表示、直接与间接的兼容性表示，以增强跨关系上下文的一致性；3）融合多模态视觉与文本信息与显式间接关系建模。

Result: 在两个基准时尚推荐数据集上的实验表明，APCL持续优于代表性基线方法。

Conclusion: 显式建模间接关系并结合对比学习与多模态信息，可捕获更丰富的语义特征，并在稀疏交互场景下提升鲁棒性与推荐性能。
...
### [217] [Predictable Failure in Multi-Hop Retrieval: Score-Distributional Confidence Scoring and Abstention](https://arxiv.org/abs/2609.22056)
*Andre Bacellar*

Main category: cs.IR

TL;DR: 论文提出多跳检索失败具有结构性分布规律，并通过CWAR可归约性与特征机制互补性两个定理加以形式化，实例化为RegimeAbstain方法，显著降低置信错误率并具跨数据集迁移性。


<details>
  <summary>Details</summary>
Motivation: 多跳检索的失败并非在查询间均匀分布，而是聚集于结构上可预测的子群体；现有置信度方法与仅依赖稠密检索的设置难以有效抑制"置信但错误"的失败。

Method: 证明两个定理：CWAR可归约性（当且仅当检索特征携带关于成功的互信息时，置信失败可归约）与特征机制互补性（无单一ANN分数特征在所有失败机制下最优）。据此提出RegimeAbstain，计算由最多九个查询-ANN结构特征构成的检索置信分数RCS（逻辑函数形式，无需额外LLM调用），并实现校准弃答策略，定义CWAR指标。

Result: 在三个多跳基准（MuSiQue、2WikiMultiHopQA、HoVer）与两种检索架构（LLM-judge、dense-only）的五个失败机制（CWAR 14.5%–62.1%）上，RCS在AUC-AC上对标八个置信基线取得最优或并列最优。MuSiQue（LLM-judge）上50%覆盖率时将CWAR从39.5%降至20.6%（相对下降47.8%），ECE=0.035；MuSiQue训练的模型迁移至2WikiMultiHopQA仅损失0.5个百分点AUC。

Conclusion: 多跳检索失败具备机制相关的可预测结构，RCS与校准弃答可有效降低置信错误且具域无关性，验证了机制特征的跨数据集可迁移性。
...
### [218] [WM-VS: Progress-Aligned World Models for Closed-Loop Visual Servoing](https://arxiv.org/abs/2609.20892)
*Guanzhong Sun,Junyi Ma,Yixuan Zhou,Yuxuan Wu,Yanzi Miao,Hesheng Wang*

Main category: cs.RO

TL;DR: 提出 WM-VS，面向闭环视觉伺服的目标中心、进度对齐世界模型框架，解决“预测-控制失配”问题。


<details>
  <summary>Details</summary>
Motivation: 闭环视觉伺服中，仅判断动作是否“合理”不足以保证任务误差收缩，需要预测动作是否真正减小任务误差；作者将此缺口称为预测-控制失配。

Method: 利用离线目标区域 DINOv2 对应关系定义平移、尺度和平面内旋转的四维带符号伺服坐标。第一阶段让动作条件潜变量转移与该坐标对齐；第二阶段冻结世界模型，训练反应式联合速度策略，结合动作模仿、后果监督和偏向误差收缩的短想象 rollout。部署仅用 RGB、反应式执行，无需在线轨迹优化。

Result: 真实 7-DoF 眼在手系统上，30/30 次试验角点 RMSE 达到不超过初值 10%，最终有效帧保留该标准 25/30（83.33%）；移除未来误差对齐后保留率降至 26.67%。学习到的进度信号与未用于训练/控制的 AprilTag 角点误差一致（Spearman rho 均值 0.8778）。不重训练时，两个未见 3D 目标平移误差分别降低 86.48% 和 90.27%，旋转误差降低 70.01% 和 65.70%。

Conclusion: 结果表明显式对齐动作后果中的进度信号可支撑闭环反复修正与迁移；代码与数据将开源。
...
### [219] [Do Spinning Radar Doppler Velocity Measurements Improve Vehicle Detection and Tracking?](https://arxiv.org/abs/2609.21000)
*Eric Xie,Daniil Lisus,Timothy D. Barfoot*

Main category: cs.RO

TL;DR: 本文研究旋转式FMCW雷达的多普勒速度测量能否提升车载检测与跟踪性能，通过自动标注管线构建数据，验证了多普勒去畸变可提升检测精度，多普勒速度先验可显著提升跟踪精度。


<details>
  <summary>Details</summary>
Motivation: 旋转式FMCW雷达因对恶劣天气鲁棒且具备360°视场，在自动驾驶感知中日益流行，且扫描雷达可生成逐方位多普勒速度。但现有带真值动态目标标签的数据集缺乏多普勒旋转雷达数据，因此作者希望探究这些多普勒速度测量是否能改善旋转雷达的车辆检测与跟踪性能。

Method: 检测方面：估计自车运动，用于消除雷达图像的多普勒距离畸变后再输入网络。跟踪方面：提出估计每辆车速度的新方法，并将其作为跟踪器运动模型的先验。为获得训练数据，作者提出自动标注管线，用集成微调的现成激光雷达检测器标注Boreas Road Trip数据集的全部643 km数据，再将检测结果迁移到雷达，使用超过250 km车辆密集序列作为真值训练数据，并训练评估两个SOTA检测器。

Result: 多普勒去畸变可将检测精度提升最多2.37个点的平均精度均值（mAP）；多普勒速度先验相比零速度初始化基线可将多目标跟踪精度（MOTA）提升13.68个点，同时达到使用真值速度作为先验所获MOTA的99.7%。

Conclusion: 研究表明，旋转雷达的多普勒速度测量能有效提升车辆检测与跟踪性能：去畸变改善检测，速度先验显著改善跟踪，且接近使用真值速度的上限效果。
...
### [220] [Robust Structureless Monocular Visual Inertial Initialization Exploiting Line Features and Vanishing Points](https://arxiv.org/abs/2609.21186)
*Junwan Choi,Woongrae Jo,Dong-Uk Seo,Jinwoo Jeon,Hyun Myung*

Main category: cs.RO

TL;DR: 提出SLIM-init，一种无结构单目VIO初始化器，直接利用2D线特征的几何约束（消失点、线对极残差、线法向投影残差），在退化运动下实现鲁棒的旋转与平移初始估计。


<details>
  <summary>Details</summary>
Motivation: VIO初始化对精度至关重要，但在退化运动下常呈病态。现有方法或要求严格的激励运动以保证可观性，或依赖计算昂贵的3D结构重建，难以高效实用部署。

Method: SLIM-init为无结构（structureless）单目VIO初始化器，无需显式3D路标重建：1）利用线导出的消失点（VP）作为平移不变的方向线索，在低视差或平移主导等退化场景下提供仅旋转约束；2）引入线对极残差约束平移；3）引入线法向投影残差改善线性对齐的条件数。

Result: 在公开基准及自建的挑战性退化运动序列上进行大量实验，相比当前最先进的初始化方法，精度与鲁棒性均有提升。代码开源：https://github.com/cjunwan/SLIM-init。

Conclusion: 通过直接利用2D线特征的几何约束而非3D结构重建，SLIM-init在退化运动下实现了高效、鲁棒且精确的单目VIO初始化。
...
### [221] [FOCAL-VLA: Subtask-Guided Geometry Distillation and Implicit World Modeling for Vision-Language-Action Models](https://arxiv.org/abs/2609.21228)
*Zhiyuan Gao,Di Wen,Yanxiang Zhan,Mohammad Khoshnazar,Jeroen Schäfer,Kunyu Peng,Michael Beetz*

Main category: cs.RO

TL;DR: FOCAL-VLA：通过子任务引导的几何蒸馏与隐式世界建模，为 VLA 模型注入任务相关的空间结构与未来交互动力学表征，在仿真与真实操作任务上超越基线。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型直接将当前 2D 观测映射为动作，缺乏充分的空间与时间理解，难以应对精细和长时序操作。已有的几何监督与未来状态预测方法覆盖整个场景，引入冗余信息，反而干扰模型学习与当前交互相关的几何和动力学。

Method: 提出 FOCAL-VLA 框架，融合两项互补表征：(1) 子任务引导的几何蒸馏——将 VGGT 的几何知识迁移到 VLA 模型，通过将几何隐变量与子任务相关图像区域的特征对齐，使几何学习聚焦当前子任务；(2) 隐式世界建模——利用当前与未来演示帧的 Track4World 特征，捕捉当前交互的未来 3D 演化。两种表征联合引导动作生成，且推理阶段无需运行 VGGT 或 Track4World。

Result: 在仿真基准和真实世界操作任务上，FOCAL-VLA 均优于基线方法。项目网站：https://zhiyuan-gao.github.io/FOCAL-VLA/。

Conclusion: 通过聚焦子任务相关几何并隐式建模未来 3D 动力学，可在不增加推理开销的前提下有效提升 VLA 模型在精细与长时序操作中的表现。
...
### [222] [VLA-Scope: Shift-Aware Failure Prediction for Vision-Language-Action Models](https://arxiv.org/abs/2609.21246)
*Kaiwen Zhu,Dongfang Liu,Liangkai Liu*

Main category: cs.RO

TL;DR: 提出VLA-Scope两阶段框架，结合输入偏移特征与执行历史，在OOD场景下预测VLA模型执行失败。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作（VLA）模型将视觉观测与自然语言指令映射为机器人动作，但分布偏移会损害其可靠性。由于模型在OOD条件下仍可能成功执行，仅检测OOD输入不足以预测执行失败，需进一步预测失败风险。

Method: VLA-Scope为两阶段框架：第一阶段利用池化的图像与语言表征检测OOD输入并分类其偏移类别；第二阶段针对被标记为OOD的输入，融合预测类别、动作前缀特征与执行进度特征，采用跨偏移类别共享的逻辑回归模型，随执行推进更新失败风险。

Result: 在OpenVLA与十个LIBERO-Spatial任务上采用留一组交叉验证：OOD检测ROC-AUC为0.9454，偏移分类准确率91%；在全部1400条OOD轨迹上（独立于OOD门控评估），执行60步后失败预测ROC-AUC为0.8497，无执行进度特征时为0.7906；且优于ActProbe与SAFE-MLP基线。
...
### [223] [ProTracer: Proprioception-Guided Failure Diagnosis in Robot Manipulation](https://arxiv.org/abs/2609.21369)
*Chang Dong,Mehdi Hosseinzadeh,King Hang Wong,Lingqiao Liu,Francois Fraysse,Feras Dayoub,Minh Hoai Nguyen*

Main category: cs.RO

TL;DR: 本文提出 ProTracer——一个免训练框架，结合本体感知信号与视觉语言模型（VLM），实现机器人操作失败的二分类检测、失败分类、解释生成及失败起点定位，并构建 FailTime 基准进行评测。


<details>
  <summary>Details</summary>
Motivation: 机器人操作失败分析通常只做失败检测或分类，缺乏对失败发生时刻的细粒度定位，且现有方法多依赖额外模型训练，难以兼顾时间精度与多模态推理能力。

Method: ProTracer 无需训练：利用本体感知动力学识别具有时间信息的动作边界，并将机器人状态信号转化为结构化自然语言描述，与视觉观测一并交由现有 VLM 联合分析，从而融合本体感知的时间精度与 VLM 的多模态推理能力。同时构建 FailTime 基准，提供视觉与本体感知同步观测，用于评测传统失败诊断任务与失败起点定位。

Result: 实验表明，ProTracer 在传统失败诊断任务和新增的失败起点定位任务上均取得较强性能，凸显本体感知推理对细粒度时间失败分析的重要性。

Conclusion: 该工作表明，将本体感知信号与 VLM 结合可在免训练条件下完成多层级失败分析，尤其是失败起点定位，本体感知推理是实现细粒度时序失败分析的关键。
...
### [224] [2nd Place Solution to the HANDS 2026 Workshop Challenge-Dexterous Grasp Motion Track: Single-Shot Trajectory Warping for Grasp Motion Generation](https://arxiv.org/abs/2609.21511)
*Muneeb A. Khan,Woojin Kim,Shinwoo Kim,Muhammad Munsif,Binod Bhattarai,Seungryul Baek*

Main category: cs.RO

TL;DR: 面向12自由度LinkerHand O6的抓取动作生成，提出"编辑单条成功演示"而非逐步生成的方法：策略观测一次物体后输出12维形变参数，开环回放演示轨迹，并用单步PPO在全部4824个训练物体上并行训练。私有测试集上简单赛道成功率94.61%（所有提交中最高），困难赛道57.18%，获HANDS 2026@ECCV挑战赛灵巧抓取动作赛道第二名。


<details>
  <summary>Details</summary>
Motivation: 任务难点在于：（1）每次抓取需逐步策略做出约70步、每步12维的决策（约70×12维），误差随时间累积；（2）测试物体与物理动力学可能与训练分布不同。逐步生成的开环/闭环策略易受累积误差与域偏移影响，因此需要一种对分布偏移更鲁棒的范式。

Method: 不在仿真中逐步生成动作，而是对单条成功的GraspM3演示进行编辑：策略仅观测一次物体，输出一个12维的warp（形变/变形）参数，将演示轨迹整体变形后以开环方式回放。训练上采用单步PPO，在全部4824个训练物体上并行优化该warp策略。

Result: 在私有测试集上：简单赛道成功率94.61%，为所有提交中最高；困难赛道成功率57.18%。总成绩为HANDS 2026（与ECCV 2026同期举办）灵巧抓取动作赛道第二名。

Conclusion: 以"一次性物体观测+单条演示形变+开环回放"替代逐步动作生成，可有效规避多步决策的误差累积并提升对未见物体和动力学差异的鲁棒性；单步PPO配合大规模并行训练（4824物体）是该方案高效可行的关键。该方法在挑战赛中取得第二名的成绩，验证了演示编辑范式在灵巧抓取动作生成上的有效性。
...
### [225] [GALA: Geometry-Aware Latent Action Modeling for Vision-Language-Action Model Pretraining across Embodiments](https://arxiv.org/abs/2609.21948)
*Yichen Liu,Puzhen Yuan,Xiang Zhu,Yanjiang Guo,Jianyu Chen*

Main category: cs.RO

TL;DR: GALA（Geometry-Aware Latent-Action）框架通过将3D末端执行器几何运动融入图像潜动作建模，并引入统一末端执行器运动表示（UEMR），实现跨本体细粒度动作的通用表征学习，用于多本体VLA预训练，在RoboCasa-GR1达68.3%成功率、真实世界达75.5%。


<details>
  <summary>Details</summary>
Motivation: 从多本体数据集学习大规模VLA模型面临挑战：不同末端执行器的动作空间异质。现有图像型潜动作模型（LAM）虽能从多样视频中学习本体无关的动作表示，但难以捕捉细粒度末端执行器关节运动，尤其是人手与灵巧机械手的指级几何变化。因此需要引入几何信息以增强细粒度动作建模能力。

Method: 提出GALA几何感知潜动作建模框架：在图像型潜动作基础上增强3D末端执行器几何运动。针对直接使用点云会得到细粒度但共享语义有限的动作表示、阻碍跨本体预训练的问题，提出统一末端执行器运动表示（UEMR），在保留细粒度运动信息的同时提升潜动作的跨本体泛化性。基于UEMR，GALA融合捕捉场景级动态的视觉潜动作与捕捉共享细粒度末端执行器关节运动的几何潜动作，为多本体数据（含无动作的自中心人类视频）的VLA预训练提供有效监督。

Result: 在细粒度运动探测、跨本体检索与下游VLA评估实验中，GALA展现出跨本体建模通用细粒度运动的有效性，在RoboCasa-GR1上取得68.3%成功率，真实世界任务取得75.5%成功率。

Conclusion: 通过在图像型潜动作中引入几何感知的UEMR表示，GALA有效解决了现有LAM细粒度末端执行器建模不足的问题，提升了跨本体动作表示的通用性与VLA下游性能，验证了几何信息对多本体VLA预训练的价值，代码、附录与演示已开源。
...
TL;DR: 本文构建了涵盖大豆、番茄、甜菜、草莓四种作物、多生长阶段与循环光照的植物生长与阴影数据集，并提出融入时间条件的扩散生成模型以实现逼真阴影模拟，支持机器人感知、光照控制与视角规划等下游任务。


<details>
  <summary>Details</summary>
Motivation: 植物生长与农业生产是国家可持续发展与民生的重要基础，前沿AI推动了科学农业的发展。阴影是影响植物生长的关键因素，但植物阴影模拟本身具有重要性与内在复杂性，现有工作缺乏系统性研究，因此本文旨在推进该领域以产生更广泛的社会效益。

Method: 工作包含两部分：（1）构建综合的植物生长与阴影数据集，覆盖大豆、番茄、甜菜、草莓四种植物，采用俯视视角并辅以沿圆周轨迹运动的补充光源，在多个生长阶段产生动态阴影，涵盖多样的观测复杂度；（2）提出基于扩散模型的生成式阴影模拟方法，通过引入时间条件（temporal conditioning）实现不同时间阶段的灵活阴影生成，可为未见植物生成逼真阴影。

Result: 通过定量与定性评估验证了模型性能，模型能够为未见植物生成逼真阴影，并支持机器人感知、光照控制与视角规划等下游任务。

Conclusion: 本研究为植物感知的阴影建模提供了基础性工作，对更广泛的农业与机器人应用具有意义。
...
### [227] [Visual Navigation Transformer with Pose Attention](https://arxiv.org/abs/2609.21212)
*Beiming Li,Jaime Romero,Jonathan Diller,Vijay Kumar,Alejandro Ribeiro*

Main category: cs.RO

TL;DR: 提出VNT-PA，一种以位姿索引的深度关键帧集合为上下文的Transformer导航规划器，注意力依赖位姿差而非时间顺序，在HM3D点目标导航中达到93.3%成功率和90.4% SPL。


<details>
  <summary>Details</summary>
Motivation: 现有学习式导航策略将观测作为时序历史消费，位置编码把每个观测绑定到其被看到的时间，导致难以复用早前遍历同一环境的经验。复用经验的系统通常需构建显式表征（地图或拓扑图）并在其上规划，存在局限。

Method: VNT-PA以相机位姿索引的深度关键帧集合作为上下文，用位姿作为位置编码，使注意力取决于关键帧间的位姿差而非时间顺序。模型通过模仿在真值场景网格上运行的最短路径规划器进行训练，仅凭当前位姿和目标位置查询空间上下文来预测动作。

Result: 在HM3D验证场景的点目标导航中，VNT-PA达到93.3%成功率与90.4% SPL，在导航性能和训练效率上均优于将相同上下文编码为时序序列或把位姿作为输入特征的基线；由于空间上下文是位姿索引集合，测试时可融合不同轨迹的帧；在定位噪声下比基于显式地图规划的传统基线退化更平缓。

Conclusion: 带位姿标记的经验可直接作为学习式规划器的环境表征；让注意力依赖位姿差而非时间顺序，能加速训练并改善长时程导航。
...
### [228] [Fewer Steps, Better Actions: Rethinking Flow-Matching Inference for VLA Policies](https://arxiv.org/abs/2609.21216)
*Zhipeng Tang,Xinda Chen,Weining Rao,Xiao Li,Wenting Tan,Yuning Wang,Xiao Shi,Xiaofang Zhao*

Main category: cs.RO

TL;DR: 提出 Coda，将流匹配 VLA 策略的部分积分预算重新分配给单一学习式端点校正，在不增加推理成本的前提下提升闭环成功率。


<details>
  <summary>Details</summary>
Motivation: 基于流匹配的 VLA 策略通过反复评估动作专家生成动作块。增加积分步数会提高推理成本，却不一定提升闭环成功率，即积分步数与任务成功率之间存在明显的质量-延迟失衡问题。

Method: 冻结原策略，先以少步数完成从噪声到动作的轨迹；再引入轻量级 Transformer 校正器，利用候选动作、源噪声与共享观测前缀缓存预测演示监督的残差；训练时仅更新校正器。该设计将部分积分预算转为一次端点校正。

Result: 在 50 个 RoboTwin Easy 任务上，五步 Coda 将成功率从 71.64% 提升至 74.68%，相比默认十步策略前向延迟降低 30.2%；两步配置达 71.88% 成功率，加速 2.12 倍。独立 13 任务对照在近乎相同延迟下取得 5.69 个百分点提升。将该设计用于冻结的官方 SmolVLA，两步成功率由 60.8% 提升至 69.4%。

Conclusion: 端点校正可有效替代额外的积分步数，改善冻结流匹配策略的质量-延迟权衡，是一种通用且低成本的性能增强方案。
...
### [229] [KnowDemo: Knowledge-Guided Robot Demonstration Generation from Human Videos](https://arxiv.org/abs/2609.21229)
*Zhiyuan Gao,Yanxiang Zhan,Mohammad Khoshnazar,Jeroen Schäfer,Michael Beetz*

Main category: cs.RO

TL;DR: 提出KnowDemo框架，利用人视频中的结构化操作知识生成多样化的机器人演示数据，以缓解真实机器人演示数据采集成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 机器人操作策略学习通常依赖大量演示数据，而在真实机器人上采集成本高昂。现有方法通过适配人视频中恢复的运动并在仿真中验证来生成演示，但以运动参考适配为中心会因保留原演示的接触策略与子任务顺序而限制行为多样性；同时对任务需求与场景关系理解不足会导致生成无效候选，降低演示生成效率。

Method: 构建基于VLM的知识抽取与推理模块，将物体与动作描述同推断出的任务条件、演示参考及可允许的执行变化关联起来，从而区分任务需求与演示特有条件；再将知识描述与目标场景中的实体和几何进行解析对齐，用于引导候选生成与筛选，最后经运动规划与仿真验证生成带结构化执行标签的演示。

Result: 生成的演示通过替代接触策略和有效子任务顺序展现出多模态行为，并带有结构化执行标签；相比仅基于参考的配置，可获得更多经过验证的执行模式；借助任务引导的抓取采样，提升了候选规划成功率；在仿真数据上微调预训练的π_{0.5}模型，实现了三个任务的sim-to-real迁移。

Conclusion: 以结构化操作知识为核心可有效突破运动参考适配带来的多样性与效率瓶颈，生成多样化且带标签的机器人演示数据，并支撑策略的仿真到真实迁移。
...
### [230] [AtomEgo: Exploring Ego-Robot Integration for Embodied Foundation Model Pretraining](https://arxiv.org/abs/2609.21461)
...
TL;DR: 本文提出 AtomEgo，基于约 2659 小时第一人称人类交互数据与可扩展处理管线，系统研究 ego-robot 协同预训练。在 VLA 与世界-动作两类架构上比较三种范式：带域特定动作头的联合协同训练、通过具身对齐的渐进式 ego-to-robot 迁移、以及联合视频-动作建模。真机多任务实验与语言条件跨具身表征分析表明：数据规模 × 对齐质量 → 能力增益。


<details>
  <summary>Details</summary>
Motivation: 具身基础模型受限于机器人演示数据的规模与多样性，因此需要引入大规模第一人称人类交互数据；但人与机器人之间存在显著的具身差异与动作空间鸿沟，如何将此类数据有效纳入具身模型预训练尚不清晰。

Method: 构建约 2659 小时的第一人称人类交互语料及可扩展数据处理管线；覆盖视觉-语言-动作（VLA）与世界-动作两类模型架构；系统研究三种代表性预训练范式：1）带域特定动作头的联合协同训练；2）通过具身对齐实现渐进式 ego-to-robot 迁移；3）联合视频-动作建模。通过多任务真机实验与语言条件跨具身表征分析进行评估。

Result: 第一人称数据能够提升泛化能力，但其价值取决于对齐与利用的有效程度；不同范式在真机多任务与跨具身表征上表现出差异，最终归纳出“数据规模 × 对齐质量 → 能力增益”的简单原则。

Conclusion: 第一人称人类数据对具身预训练具有可观价值，但并非规模越大越好；关键在于具身与动作空间的对齐质量及使用方式。该原则为可扩展的 ego-robot 预训练提供了实践指导。
...
### [231] [Potential-Field Action Representation for Reinforcement Learning in Contact-Rich Manipulation](https://arxiv.org/abs/2609.21609)
*Xinyu Liu,Gökhan Solak,Arash Ajoudani*

Main category: cs.RO

TL;DR: 提出PA-RL框架，用人工势场作为强化学习的动作表示，策略调节势场参数而非直接输出运动，在轴孔装配任务中取得100%仿真成功率并实现零微调真机迁移。


<details>
  <summary>Details</summary>
Motivation: 无模型强化学习可习得接触丰富的操作技能，但策略需同时学习任务策略与底层运动生成，学习负担重。直接笛卡尔指令接口要求策略在每个决策步生成运动，将任务级适应与连续底层控制耦合。因此动作表示的设计成为关键问题，需要一种能解耦任务决策与运动生成的表示方式。

Method: 提出PA-RL：以人工势场作为动作表示。策略不直接指令运动，而是调节类能量势场的参数，由该势场产生状态相关的引导方向，并通过笛卡尔阻抗控制器执行。在轴孔插入这一代表性接触丰富任务上，与笛卡尔速度、笛卡尔位姿、可变阻抗三种动作空间在相同RL算法下对比。

Result: PA-RL是唯一在给定训练时间内达到100%评估成功率的方方法，最优基线为92.6%。相较最优基线，关节力矩变化降低55.4%，笛卡尔加速度变化降低70.8%，且奖励中未显式加入运动质量惩罚项。仿真训练的策略无需微调即完成9/9真机插入。

Conclusion: 人工势场动作表示能有效解耦任务级适应与底层运动生成，提升学习效率与运动质量，并具备良好的仿真到现实部署可行性。
...
### [232] [SynthDemo-RL: Breaking the Zero-Reward Barrier in VLA Adaptation with LLM-Guided Synthetic Demonstrations](https://arxiv.org/abs/2609.21650)
*Hiroaki Kingetsu,Hiroaki Kurihara,Kaoru Yokoo,Kenji Fukumizu,Manohar Kaul*

Main category: cs.RO

TL;DR: 提出 SynthDemo-RL 教师-学生框架：自动教师在仿真中用特权状态合成成功轨迹，经 SFT 蒸馏到 VLA 学生模型，再用稀疏二值奖励的 PPO 精调，从而在无人演示的情况下解决稀疏奖励探索难题。


<details>
  <summary>Details</summary>
Motivation: VLA 模型微调依赖人类遥操作演示，成本高；而稀疏二值奖励的强化学习在成功轨迹罕有采样时面临严重的探索难题。

Method: 教师-学生框架：1) 自动教师将仿真器特权状态（privileged state）转换为成功操作轨迹；2) 学生 VLA 通过监督微调（SFT）从合成轨迹中蒸馏；3) 使用二值任务成功奖励的 PPO 进一步精炼学生策略。每任务合成 50 条轨迹，不使用新的人类演示。提出"奖励覆盖度"（至少观测到一次成功的任务比例）作为平均成功率的补充指标。

Result: 在 LIBERO-PRO（57 个计分任务）上，基于原始 LIBERO 微调的 pi_0.5 策略有 27 个任务成功率为 0%；同等 PPO 配方与算力从该策略直接做 PPO 仅挽救 10 个，仍留 17 个为 0%；SynthDemo-RL 挽救全部 27 个，Position 与 Task 轴平均成功率分别达 97.8% 与 97.1%。标准 LIBERO 上达到 96.0%，与使用每任务 50 条人类演示训练的 pi_0.5 相差 1.7 个百分点。并在 RoboTwin 2.0 上验证，且在 MuJoCo 孪生中训练的策略轨迹可在物理机器人上开环执行。

Conclusion: 自动合成的教师轨迹加稀疏奖励 RL 精炼，可在零人类演示条件下显著提升 VLA 模型的成功率与奖励覆盖度，优于直接用 PPO 的方案，并具备跨仿真平台与真机开环执行的可行性。
...
### [233] [Outcome-Conditioned End-Effector Geometry Across Vision-Language-Action Policies](https://arxiv.org/abs/2609.21659)
*Xingyu Lin,Zhuang Li,Zhongrun Wu,Shouquan Zhou,Dehui Du*

Main category: cs.RO

TL;DR: 论文研究跨策略末端执行器几何一致性，发现双双成功配对的DTW距离显著小于单侧成功配对，但低距离不意味着策略可互换。


<details>
  <summary>Details</summary>
Motivation: 仅凭任务成功率无法判断不同VLA策略在同一操作任务中的物理执行是否一致，需要研究跨策略末端执行器轨迹几何。

Method: 基于四个策略在LIBERO上的15,000次闭环回放，构造3,600个配置匹配（因而相互依赖）的策略配对，采用归一化DTW距离度量，并在九种采样与带限表示下检验；另用同任务示教作基线，并施加复合视觉扰动。

Result: 双双成功配对的中位归一化DTW距离为0.0120 m，单侧成功为0.0380 m，该排序在所有任务、策略配对及九种表示中稳定，但比值随表示变化数倍；双失败配对分离更大但支撑样本单薄，仅作探索性报告。成功执行中，更换合作策略造成的分离大于更换初始状态；匹配基线仍显示可测且异质的残余策略差异。成功执行与同任务示教的距离约等于示教之间的距离。72动作窗口保留排序但幅度下降；端点与时长调整后混合结果系数仍为正，幅度依赖设定。复合视觉压力下策略排名与配对构成同时变化。

Conclusion: 跨策略成功执行存在与任务相关的几何一致性，显著高于混合/失败配对，但低跨策略距离不等于策略可互换；结论应报告方向而非固定倍数，并注意表示与视觉条件的敏感性。
...
### [234] [ForceTwin: Physics-informed Digital Twins for Robotic Manipulation from Instrumented Human Interaction](https://arxiv.org/abs/2609.21751)
*Tim Engelbracht,René Zurbrügg,Mayank Mittal,Marco Hutter,Marc Pollefeys,Hermann Blum,Zuria Bauer*

Main category: cs.RO

TL;DR: ForceTwin：通过带力传感手柄的人机交互，辨识铰接物体的物理信息数字孪生，涵盖惯性、库仑摩擦、粘性阻尼及状态依赖机构力的神经残差建模，实现高精度动力学预测与机器人操作策略。


<details>
  <summary>Details</summary>
Motivation: 铰接物体的操作不仅依赖运动学，还取决于惯性、摩擦以及弹簧、闭门器等机构特性，这些物理属性随构型与速度变化且无法从外观直接观测（外表相同的门操作难度可能差异巨大）。现有数字孪生流程多仅恢复运动学，或依据视觉与语言先验赋予静态物理参数，导致估计物理上不合理，状态依赖的机构动力学无法被辨识，也无法在标准资产格式中表达。

Method: 提出ForceTwin系统：由人手持带力感知的夹爪对物体进行探测交互，获取同步的位姿与交互力数据；在此基础上估计铰接运动学，并辨识参数化动力学模型，包含惯性、库仑摩擦、粘性阻尼，以及一个结构化神经残差项用于捕获状态依赖的机构力。

Result: 惯性参数误差相比VLM先验降低近一半；作为阻抗控制的前馈动力学模型，在Spot与Franka FR3、九组物体-本体配对上达到87%目标完成率，而VLM先验为60%、仅运动学孪生为57%；在强机构导致两种基线停滞的物体上提升最大。所辨识孪生还被用于训练全身开门通行策略并在真实世界部署。

Conclusion: 通过仪器化人类交互可辨识包含状态依赖机构力的物理信息数字孪生，显著优于视觉语言先验与纯运动学方法，并可直接支撑真实机器人的阻抗控制与全身操作策略训练。
...
### [235] [When Should a Failing Robot Ask? Initiating Corrective Human-Robot Dialogue from Audited Sensor Evidence](https://arxiv.org/abs/2609.21942)
*Eshika Pathak,Leela Krishna*

Main category: cs.RO

TL;DR: 该论文构建了一个注入式故障诊断基准，测度不同传感器对故障原因的信息量，并测试六个开源视觉-语言模型在"自主行动/查阅自身传感器/询问人类"三动作决策中的表现，发现模型决策跟随提示表面形式而非证据，询问率不随成本变化。


<details>
  <summary>Details</summary>
Motivation: 机器人在任务失败后面临纠正对话中的首要决策：依据自身诊断行动、查阅其他机载传感器，还是打断人类求助。要做出合理选择，需要知道传感器对故障原因揭示了多少信息，以及机器人自身诊断的可靠性如何。现有模型是否具备这种基于证据的元认知决策能力尚不清楚，且缺乏真值已知、可严格检验数据泄漏的评测环境。

Method: 构建仿真基准，通过人为注入故障使每个失败的真实原因已知，并显式检查数据泄漏，量化各传感器（相机图像、力数据）对故障原因的可诊断性。随后测试六个开源视觉-语言模型在不同提示变体（选项顺序、示例有无）下的诊断准确率、拒绝率与置信度校准。将决策形式化为三动作问题（自主行动、查阅自身传感器、询问人类），并考察模型策略是否遵循由实测准确率推导出的最优策略，包括改变提问成本后的询问率响应。

Result: 部分故障只能由力数据诊断（力数据达0.99，无图像方法超过0.55）。模型行为跟随提示表面而非证据：将拒绝选项从末位移到首位，使六个模型-家族对中三对的拒绝率从78–100%骤降至0–6%。在所有提示变体（含/不含示例）下，基于帧的准确率均不高于多数类基线，且声明置信度与正确性无关。将力数据以十行文本形式提供后，六个模型中有四个首次出现高于基线的诊断，说明失败多源于传感器数据缺失而非能力缺失。模型不遵循最优策略，其询问率对四倍的提问成本变化不敏感。向人类提一个问题可将其从基线提升至回答者自身可靠性水平（提问时0.70–0.81）。

Conclusion: 是否提问的决策应基于实测准确率与明示成本，而非模型自身置信度。模型当前缺乏基于证据的元认知决策能力，其求助行为易受提示形式操纵且对成本不敏感；补齐传感器数据可显著改善诊断表现。
...
### [236] [Diverse and Adaptable Arm Coordination for Octopus-Crawling via Diffusion-Based Uncertainty-Aware Optimization](https://arxiv.org/abs/2609.21138)
*Seung Hyun Kim,Heng-Sheng Chang,Kimia Kazemi,Prashant Mehta,Mattia Gazzola*

Main category: cs.RO

TL;DR: 本文提出基于扩散的不确定性感知优化算法DUO，为仿真肌肉驱动仿生章鱼学习无演示爬行控制器，通过多样性协调策略实现鲁棒适应。


<details>
  <summary>Details</summary>
Motivation: 章鱼爬行启发的软体多臂机器人存在冗余性，如何发现并组织多样的协调模式以适应动态约束仍是难点。现有方法依赖演示，泛化与适应能力不足。

Method: 提出DUO（Diffusion-based Uncertainty-aware Optimization）算法，学习无演示爬行控制器；采用对称结构化策略表示，将径向等效控制器折叠至规范方向扇区；结合在线黑盒优化策略发现并保留多样协调模式；提出控制编辑技术，无需再训练即可适应新执行器约束。

Result: 首次将扩散控制应用于接触丰富仿真中的软体多臂机器人；仿真章鱼能够在共享控制分布中嵌入多种运动行为，应对动态物理约束，验证学习到的协调多样性天然促进鲁棒适应。

Conclusion: 学习到的协调多样性使运动冗余成为软体多臂机器人实际可用的适应资源，为无演示软体机器人控制提供了新范式。
...
### [237] [FootQuery: Future-Touchdown-Guided Retrieval from Depth History for Perceptive Humanoid Locomotion](https://arxiv.org/abs/2609.21447)
*Tao Dong,Jia Yu,Yuxuan Fan,Linna Zhao,Jiaqi Gong,Andong Yang,Chao Gao,Guyue Zhou*

Main category: cs.RO

TL;DR: 提出FootQuery感知运动框架：利用每只脚预测的下一落足点查询稀疏历史深度帧，结合全局视觉记忆生成控制动作，实现人形机器人在复杂地形上的连续行走。


<details>
  <summary>Details</summary>
Motivation: 人形机器人在复杂地形行走需预判落足点，但触地时该区域可能已被遮挡或不在相机视野内。受限于相机覆盖范围与自遮挡，必须从早期观测中检索相关地形信息。

Method: 以本体感觉预测各脚落足位置及其不确定性，结合逐脚特征查询稀疏采样的历史深度帧；训练时将实际接触投影到历史图像中，监督接触可见区域的检索；检索到的逐脚特征与全局视觉记忆融合生成控制动作；采用渐进式力辅助课程支持早期探索，并用事件一致的踏面中线整形鼓励协调的楼梯接触。部署仅需本体感觉与机载深度图像。

Result: 仿真中最具挑战性的楼梯、间隙与平台场景下，完整框架优于各组件消融版本。在Unitree G1真实实验中，单一策略可连续通过室外楼梯，以及包含上下楼梯、平台与间隙的室内路线。

Conclusion: 结果表明，围绕预期接触点组织视觉历史，是构建感知式人形机器人运动的有效途径。
...
### [238] [Adaptive Rollout Truncation Based on Epistemic Uncertainty for Efficient Offline World Model Training](https://arxiv.org/abs/2609.21482)
*Nikodem Sebastian Zymla,Laurin Thiele,Johannes Pitz*

Main category: cs.RO

TL;DR: 提出基于认知不确定性的自适应 rollout 策略训练离线世界模型，按自课程在不确定性超阈值时截断自回归展开，在保证/提升预测精度的同时大幅减少计算量。


<details>
  <summary>Details</summary>
Motivation: 多步自回归训练可提升长时程预测能力，但固定 rollout 长度会增加计算开销，且在模型尚不准确时放大早期训练误差；现有方案在整个优化过程中使用固定展开长度，未考虑模型当前预测可靠性。

Method: 面向离线世界模型训练，采用自课程（auto-curriculum）自适应展开：当认知不确定性超过由 warm-up 阶段校准的阈值时终止自回归 rollout。比较两类不确定性估计器：共享循环骨干的五头集成、Monte Carlo Dropout；并用两阶段 warm-up 稳定不确定性估计后再启用自适应截断。

Result: 在 ANYmal-D 与 ANT 上，基于集成的自适应截断在预测精度上达到或优于固定视界训练与 RWM-U 基线，同时累计 rollout 步数显著减少；ANYmal-D 上以约 72% 更少的 rollout 计算量达到与基线相当的最终性能。

Conclusion: 认知不确定性不仅可用于下游策略正则化，也能提升世界模型训练本身的计算效率，使离线世界模型训练更省算力且不牺牲精度。
...
### [239] [From Pretraining to Proficiency: Real-World Subtask RL for Long-Horizon Manipulation with Minimal Human Intervention](https://arxiv.org/abs/2609.21788)
*Sichang Su,Benjamin Yang,Zhiyun Deng,Boyuan Liang,Yip Fun Yeung,Zelin Wang,Lingfeng Sun*

Main category: cs.RO

TL;DR: PARTS是一个真实世界子任务级强化学习微调框架：冻结预训练策略输出名义动作，由智能体生成的选择器/成功验证器激活残差修正并提供局部结果奖励，聚焦少数瓶颈子任务，从而用稀疏奖励解决长程任务。


<details>
  <summary>Details</summary>
Motivation: 预训练机器人基础策略可完成长程任务的大部分，却常在少数关键子任务上反复失败。补充全任务演示做SFT需重复已掌握行为，成本高；而现有RL微调方法在仅用稀疏奖励时难以解决长程任务。因此需要一种能集中练习瓶颈子任务、且训练rollout对人类干预需求最小的真实世界RL微调方法。

Method: PARTS在真实世界中进行子任务级RL：冻结的预训练策略全程提供名义动作；智能体生成的选择器与成功验证器激活残差修正，并提供局部结果奖励，使得即使在整任务成功稀缺时也能从成功子任务中学习。训练结合在线RL与成功重加权再训练（success-reweighted retraining），每次再训练的残差策略重新部署以收集更多经验。人类仅在设置阶段识别瓶颈，并在需要时进行物理重置。

Result: 在双臂YAM和单臂Franka任务上，PARTS将完整任务成功率分别从32%提升至61%、从50%提升至95%，平均每任务仅需数十分钟的真实世界RL rollout。与既有真实世界RL微调方法相比，在相同机器人rollout预算下，整任务成功率提升超过25%，且人类参与更少。

Conclusion: 通过将RL练习集中于瓶颈子任务、借助选择器—验证器提供稠密的局部结果奖励，PARTS在稀疏奖励与有限人工干预条件下有效微调预训练机器人策略，显著提升长程任务成功率与rollout效率。
...
### [240] [AeRove: A Compact Bimodal Aerial-Terrestrial Drone with Rapid Bistable Reconfiguration for Close-Range Pipeline Inspection](https://arxiv.org/abs/2609.20965)
*Caleb Polillio,Petras Swissler*

Main category: cs.RO

TL;DR: 提出双模态空中-地面机器人 AeRove，用螺旋桨护罩作为滚轮沿管道滚动，配合弹簧双稳态机构实现 200 ms 内地面/飞行模式切换且维持状态不需持续供电；滚动巡航使电流降低 14 倍、估算行程从 144 m 提升至 2057 m，并在 51 cm 钢管上完成导航、越障与泄漏检测验证。


<details>
  <summary>Details</summary>
Motivation: 近距管道巡检对标准无人机困难：悬停功耗高、对气流敏感，且螺旋桨下洗气流会干扰气体传感。

Method: AeRove 将螺旋桨护罩用作滚轮在地面滚动，采用弹簧加载双稳态机构在 200 ms 内完成地面/飞行模式重构；飞行仅用于越障，地面滚动用于连续行进。收敛式护罩几何提升推力效率。另设 CO2 传感实验，对比栖息（滚动/停驻）与悬停条件下的气体检测性能，并比较机身底部螺旋桨吸气式与伸出探针式进气方案。

Result: 相比持续飞行，电流消耗降低 14 倍（0.7 A vs 10 A），估算行程由 144 m 增至 2057 m。51 cm 直径钢管自主试验完成直道与弯道导航、越障及模拟标记泄漏检测。停驻检测的浓度响应显著大于悬停；飞行中底部螺旋桨吸气式方案比伸出探针响应更快、更强。

Conclusion: 双模态空中-地面设计可显著降低巡检能耗并延长航程，同时利用螺旋桨诱导气流增强气体检测，适用于近距管道巡检；代码与设计以 CC-BY 许可开源。
...
### [241] [Shake to Learn: Dynamic Interrogation of Hidden Object Physics for Robotic Manipulation with Physical Reservoir Computing](https://arxiv.org/abs/2609.20970)
*Wen Sin Lor,Jun Wang,Suyi Li*

Main category: cs.RO

TL;DR: 该研究提出"shake-to-learn"机械询问范式：利用折纸启发软体机械臂作为物理储备池计算机，通过固定晃动激励并读取耦合系统振铃响应的动力学，用轻量线性读出解码隐藏物体属性（如质心），完成质心朝向推断、质心距离估计及指导再抓取三项任务。


<details>
  <summary>Details</summary>
Motivation: 许多与机器人操作相关的物理属性无法通过视觉获取，例如密封物体的质心位置或内部内容物，只有在提举、摇晃或动态扰动后才可能显现。因此需要一种新的感知模态，将交互诱发的动态响应用作推断隐藏物体物理属性的信息来源。

Method: 采用折纸启发的软体机械臂构建物理储备池计算机；抓取物体后在基座施加固定晃动激励，通过摄像机跟踪或嵌入式传感器记录振铃响应；由于输入在多次试验中保持一致，隐藏物体属性通过影响机器人-物体耦合系统动力学而被编码；再用轻量线性读出解码这些动力学；并提出振铃响应的动态摘要表示以提升预测精度。

Result: 该软体机械臂储备池完成了三项难度递增的任务：推断物体隐藏质心的朝向、推断质心与抓取点之间的距离、以及利用推断出的质心信息指导后续再抓取；动态摘要表示进一步提高了预测准确率。

Conclusion: 这些结果确立了"shake-to-learn"机械询问作为一种有前景的策略，使机器人系统能够将短暂的物理交互转化为关于隐藏物体属性的可操作线索，用于下游操作任务。
...
### [242] [ForeTac-VLA: A Forecasting-Based Tactile-Vision-Language-Action Model for Contact-Rich Robotic Manipulation](https://arxiv.org/abs/2609.20980)
*Zhengyu Tao,Xin Li,Xin Wang*

Main category: cs.RO

TL;DR: 提出ForeTac-VLA，一种基于触觉预测的视觉-语言-动作融合模型，通过预测未来触觉状态指导动作生成。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型依赖视觉感知，在接触密集环境中鲁棒性不足，关键物理交互状态难以从视觉观测；已有触觉增强VLA多为被动响应式，未显式建模接触的演化过程。

Method: 将近期触觉观测编码为时序表示，通过双向交叉注意力与视觉-语言特征融合；用Transformer预测模块预测多步未来触觉状态；融合多模态表示与预测触觉状态输入VLA主干以条件化动作生成；训练初期采用真值到预测的课程学习稳定训练。

Result: 在四项真实接触密集操作任务上平均成功率95%，较微调VLA模型提升36.25个百分点，较最先进触觉增强VLA基线提升超22个百分点；在低光照与视觉杂乱条件下仍保持较强性能。

Conclusion: 显式预测未来触觉状态可有效增强接触密集操作中VLA模型的物理接地与鲁棒性。
...
### [243] [PIVOT: Physically Informed Vision-Language Off-Road Traversability for Field Robot Navigation](https://arxiv.org/abs/2609.20983)
*Aoran Jiao,Wenda Zhao,Hshmat Sahak,Timothy D. Barfoot*

Main category: cs.RO

TL;DR: PIVOT 是一种物理信息引导的视觉-语言越野可通行性导航系统，在几何规划基础上引入 VLM 语义推理并进行物理标定，通过两级导航架构实现高效重规划，闭环实验中自主率从 59.6% 提升至 97.0%。


<details>
  <summary>Details</summary>
Motivation: 越野移动机器人需要在非结构化、几何复杂环境中安全可靠导航。传统基于几何的地形评估计算快，但在非结构化环境中往往过于保守，限制了自主导航能力，需要引入语义层面的可通行性推理来弥补几何方法的不足。

Method: 提出 PIVOT 系统：用视觉-语言模型（VLM）进行语义可通行性推理，对 VLM 预测的通行能耗代价、机器人振动与车轮打滑三类输出与真实测量值的相关性进行量化，以实现物理落地；并据此构建统一可通行性评分，按各模态的预测-测量相关性加权。导航上采用两级架构，以几何规划为常规模式，仅当几何模式无法找到路径时才触发语义重规划。

Result: 在约 6.4 km 混合地形路线上进行五组重复闭环试验：相比纯几何导航，整体自主率由 59.6% 提升至 97.0%，人工干预次数由 11 次降至 3 次，平均干预间隔距离（MDBI）由 69.2 m 提升至 412.9 m。

Conclusion: 物理落地的 VLM 地形评估可显著拓展自主导航能力，突破纯几何方法的局限，同时保留高效几何规划作为常规模式。
...
### [244] [Project SCOUT: Interceptor Drone for Perimeter Defense](https://arxiv.org/abs/2609.21005)
*Azmain Yousuf,Siwei Cai,Knut Peterson,Lifeng Zhou,David Han*

Main category: cs.RO

TL;DR: 本文提出 SCOUT，一个集成于 ROS 的机载感知与控制框架，用于无人机边界防御中的实时目标跟踪与闭环拦截，实测有效检测帧率达 92.2%。


<details>
  <summary>Details</summary>
Motivation: 未经授权的无人机快速增多，边界防御亟需具备抗干扰能力的反无人机系统；现有方案多依赖外部传感设施或地面站计算，难以满足机载实时自主防御需求。

Method: SCOUT 在防御无人机上直接完成视觉检测、目标关联、航迹滤波与控制指令生成。感知流水线采用 TensorRT 加速的无人机检测、基于 ByteTrack 的关联以及轻量级航迹保持状态机，以抑制目标突变跳变、在检测短暂退化时维持目标连续性，减少误检与目标切换导致的不稳定控制响应。硬件部署下执行平面"守门"式拦截策略。

Result: 真实飞行实验中，SCOUT 在实时机载检测速率下保持 92.2% 帧的有效目标检测，防御无人机能够跟踪来袭目标并调整运动以维持保护区边界附近的阻挡构型，验证了视觉跟踪与闭环控制用于无人机边界防御的可行性。

Conclusion: 该工作表明基于机载视觉感知与闭环控制的无人机边界防御方案具有工程可行性，为不依赖外部基础设施的自主反无人机系统提供了实践参考。
...
### [245] [SPARROW: Survival-POMCP for Adaptive Robot Routing, Observation, and Waiting](https://arxiv.org/abs/2609.21008)
*Hshmat Sahak,Aoran Jiao,Nicholas Rhinehart,Timothy D. Barfoot*

Main category: cs.RO

TL;DR: 将临时障碍下的图导航建模为部分可观测半马尔可夫决策过程，提出基于POMCP的置信空间规划器SPARROW，联合搜索通行、观测与有限等待动作，并在线学习障碍清除生存模型，仿真与真机实验均显著优于OSCAR。


<details>
  <summary>Details</summary>
Motivation: 机器人在规划路线上可能遇到临时性障碍，此时需要在"等待障碍清除、绕行、先获取更多障碍信息"之间做出序贯决策，传统方法难以权衡观测成本与未来导航收益。

Method: 将问题形式化为POSMDP；提出SPARROW，基于POMCP在置信空间上规划，搜索通行、观测和有限时长等待动作；用粒子滤波维护对潜在障碍类别与清除时间的置信；通过清除观测与右删失遭遇在线学习类别条件生存模型；用生成模型模拟障碍到达与清除以考虑替代路径上的阻塞；引入"学习价值"准则，权衡采集带标签生存数据的即时成本与未来导航后悔的期望降低。

Result: 在两个仿真图和多种障碍类别设置下，SPARROW将平均到达目标时间较OSCAR降低12-26%；在真实移动机器人上较OSCAR降低20.5%，并能随环境条件变化自适应地观测、等待与绕行。

Conclusion: SPARROW表明将临时障碍导航纳入置信空间规划并主动学习生存模型，能在仿真和真实场景中一致地提升导航效率与自适应性。
...
### [246] [Towards Effective Visual-Inertial SLAM with Passive-Only Sensors for Low-Cost Autonomous Underwater Vehicles](https://arxiv.org/abs/2609.21015)
*Grant Schwidder,David Widhalm,Junaed Sattar*

Main category: cs.RO

TL;DR: 该研究证明，使用总价低于1万美元、仅配备消费级立体相机、MEMS IMU和深度传感器的开源AUV，即可在无缆、无约束6-DOF水下环境中实现可用的VI-SLAM导航，为低成本海洋机器人提供基准。


<details>
  <summary>Details</summary>
Motivation: 高端AUV依赖DVL、USBL等昂贵传感器套件，成本高昂，限制了先进海洋机器人从专业实验室走向更广泛的研究与爱好者应用。因此，亟需验证低成本平台上能否实现鲁棒、高质量的视觉-惯性SLAM导航。

Method: 采用价格相近的开源AUV平台，仅搭载消费级立体相机、MEMS IMU和深度传感器；在完全无约束的6-DOF水下环境中评估其性能；分析现成SLAM软件包的有效性，并针对水下无缆作业的视觉与物理挑战提出传感器融合优化方案。

Result: 实验验证了在低成本消费级硬件上可获得可用的SLAM解；现成SLAM包经传感器融合优化后，能缓解非结构化水下环境带来的视觉与动力学难题，实现实时导航。

Conclusion: 低成本、消费级硬件可支撑鲁棒的水下VI-SLAM，使SLAM技术可被大众获取，为苛刻实时海洋任务提供无需工业级硬件预算的性能预期基准。
...
### [247] [Catch Me If You Can: Real-Time Feedback Denoising for Responsive VLAs](https://arxiv.org/abs/2609.21022)
*Yiheng Ji,Xingru Zhou,Luis Sentis,Mingyo Seo*

Main category: cs.RO

TL;DR: 提出VLA-Feedback，一种双时间尺度架构，将低频扩散规划与高频视觉反馈结合，把扩散去噪的最后一步作为轻量反馈接口，实现执行前的实时动作校正。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型虽泛化性强，但基于扩散的动作生成器通常对动作块进行开环执行，当物体移动、接触变化或场景演化时缺乏响应能力。

Method: 采用双时间尺度架构：低频扩散规划器负责生成动作块，保留其最终去噪步骤作为轻量反馈接口，在每次动作执行前利用最新观测对动作进行校正，无需重跑完整的视觉-语言-扩散模型。

Result: 在静态LIBERO任务上与GR00T持平；动态仿真任务平均成功率从27.5%提升至85.0%；真实机器人任务平均成功率从51%提升至73%。

Conclusion: 该设计在保留扩散规划器表达能力的同时实现了实时动作校正，显著提升了动态场景与真实环境下的操作成功率。
...
### [248] [Adapting Rigid-Body Dynamics Derivatives for Constraint Embedding Closed-Chain Models](https://arxiv.org/abs/2609.21024)
*Daniel J. Volpi,Patrick M. Wensing*

Main category: cs.RO

TL;DR: 本文将刚体动力学一阶导数的现有算法扩展到基于约束嵌入建模的闭链运动系统，通过移除"关节速度效应局部构型不变"假设，导出适用于更一般关节类型的动力学导数算法。


<details>
  <summary>Details</summary>
Motivation: 现有高效动力学导数方法假设关节速度效应在局部构型下不变，该假设不适用于约束嵌入闭链模型中的一般关节类型，限制了对更真实驱动子机构的精确动力学计算。

Method: 移除关节速度效应局部构型不变的假设，推导适配的动力学导数算法，将导数计算扩展至约束嵌入闭链模型中的一般关节类型；对比传统销轴关节机器人模型与捕捉局部闭链的更完整驱动模型。

Result: 泛化引入的附加项在仅对驱动运动学建模时计算开销较低；但当驱动链包含额外刚体（如电机转子）或考虑非局部闭环时，计算成本显著升高。
...
### [249] [MAPLE-RF: Efficient Probabilistic RF Source Localization in Partially Explored Environments](https://arxiv.org/abs/2609.21026)
*Haozhe Lei,Sundeep Rangan*

Main category: cs.RO

TL;DR: 研究部分探索地图下的单快照射频发射源定位，比较数字孪生方法与MAPLE-RF两种输出位置后验的方法，结果表明混合地图覆盖训练对两者都至关重要。


<details>
  <summary>Details</summary>
Motivation: 许多机器人应用中只有部分地图可用，尤其是机器人边探索边定位射频发射源（结合SLAM）时，传统定位方法依赖完整环境模型来预测障碍物对信号的阻挡与反射，因此需要适配部分已知地图。

Method: 对比两种方法：第一种将数字孪生方法扩展至部分地图，把未探索空间视为自由空间并对混合地图覆盖进行训练，对每个候选位置进行射线追踪；第二种为MAPLE-RF，将估计的到达路径角和信噪比编码为与地图已知度、占据状态、视距可见性对齐的网格通道，用U-Net一次前向为所有候选位置打分，推理时不模拟传播。

Result: 室内环境的射线追踪仿真表明：混合地图覆盖训练对两种方法都必不可少；数字孪生方法在多数单快照指标上更准确，MAPLE-RF接近其精度，但查询成本不依赖传播模型，比使用通用射线追踪的全网格查询低两个数量级以上；两者均优于高斯和高斯混合基线；在由自身估计引导的探索路径上，融合的MAPLE-RF后验在真实源附近赋予的概率高于对比方法。

Conclusion: 混合地图覆盖训练是部分地图下发射源定位的关键；数字孪生精度更高，MAPLE-RF以极低且与传播模型无关的查询成本接近其性能，并在探索式定位中表现更优，代码与数据将公开。
...
### [250] [DEXTERA: From a Single Image to Deployable Dexterous Manipulation via Real-to-Sim-to-Real](https://arxiv.org/abs/2609.21045)
*Jin Wu,Lianjie Yuan,Zeyan Sun,Yuanyuan Lei,Disi A,Bicheng Han,Fangzhou Xia*

Main category: cs.RO

TL;DR: DEXTERA是一个自动化的real-to-sim-to-real框架，可从单张RGB图像生成可部署的灵巧操作策略，通过仿真数据或仿真-真实联合训练实现零样本或高效的真实机器人迁移。


<details>
  <summary>Details</summary>
Motivation: 真实世界灵巧操作数据采集成本高、耗时长；虽可用高保真物理仿真器进行可扩展的数据合成与策略学习，但手动构建可部署的数字孪生工作量大，且视觉、几何与动力学残差阻碍了可靠的sim-to-real迁移。

Method: DEXTERA包含四个统一阶段：(1) 单图像场景分解为静态高斯背景与可交互的刚体/铰接资产，并用VLM推断物理参数；(2) 度量场景全局对齐、物体规范化与形态均衡的机器人标定；(3) 可扩展的仿真器任务原语构建、VR遥操作与以物体为中心的运动轨迹合成；(4) 支持模仿学习与强化学习的共享多模态策略接口。

Result: 在13个任务-具身配对、2个灵巧机器人平台、6种策略架构上评估：DEXTERA在视觉保真度与3D几何重建上优于生成式基线；跨域轨迹回放验证了较强的物理交互一致性；仅仿真训练的策略可实现可行的零样本真实机器人部署，仿真-真实联合训练将平均物理策略成功率从29.2%提升至61.9%。

Conclusion: DEXTERA证明了自动化real-to-sim-to-real流程可为灵巧操作提供高保真、可部署的策略学习方案，仿真-真实联合训练能显著提升真实机器人操作成功率。
...
### [251] [Constraint-Unified MPC for Over-Actuated Surface Vehicles with Post-Detection Fault Reconfiguration](https://arxiv.org/abs/2609.21046)
*Sebastian Burmester,Ruiheng Jiang,Noa Sendlhofer,Raffaello D'Andrea,Aswin Ramachandran*

Main category: cs.RO

TL;DR: 提出将轨迹跟踪、推力分配与执行器力/速率限制统一为单个二次规划（QP）的自主水面艇控制架构，并通过故障标志实现容错重构，已在真实表演中部署验证。


<details>
  <summary>Details</summary>
Motivation: 编排水上表演要求小型自主水面艇在推力器力与速率限制下精确跟踪路径，且需在推力器故障后仍保持控制性能；传统级联架构在故障后性能退化明显。

Method: 将轨迹跟踪、推力分配以及每个推力器的力与速率限制统一在一个关于每推力器指令的二次规划中求解；故障重构由外部检测器提供的每个推力器一个二值标志引入。

Result: 在苏黎世湖现场表演中，10分钟定点保持的RMS位置误差为1.6 cm，10 m方形轨迹（0.6 m/s）为4.3 cm；0.5 m/s时失去前推力器误差增至11.6 cm，失去右舷推力器增至11.5 cm；同时失去两个推力器时以0.4 m/s跟踪方形轨迹RMS误差为1.25 m。与自研级联基线相比，统一形式在标称方形轨迹上精度相当，但定点保持更紧且推力约为一半；故障后统一控制器保持在参考路径24 cm内，而级联架构则偏离路径。

Conclusion: 单一QP统一形式在正常与故障工况下均优于级联基线，具备实际部署的容错跟踪能力，但双推力器同时失效时性能显著下降。
...
### [252] [Design of Adaptive PID Controller Based On Asynchronous Advantage Actor Critic Learning Method for QuadCopter Control](https://arxiv.org/abs/2609.21082)
*Ali Jokar,Aria Alasty*

Main category: cs.RO

TL;DR: 本文将A3C强化学习算法与PID控制器结合，用于四旋翼姿态与轨迹跟踪，A3C通过并行agent与神经网络动态整定PID参数，并结合系统辨识模块预测状态；与A2C对比，仿真表明两者均能精确跟踪，但A3C损失收敛更好、参数优化更优。


<details>
  <summary>Details</summary>
Motivation: 四旋翼具有非线性特性和对扰动敏感的问题，传统基础PID控制器应对复杂工况能力不足，因此需要更具自适应性的控制方案。

Method: 提出集成A3C算法与PID控制器的控制系统，利用并行agent通过神经网络动态优化PID参数；并设置系统辨识模块为互补系统预测系统状态，以获得最优控制策略；与标准A2C模型进行对比。

Result: 仿真结果显示两种控制器都能精确跟踪；但基于A3C的控制器损失函数收敛更好，奖励曲线与损失曲线均验证其参数优化效果更佳。

Conclusion: A3C与PID结合的方法提升了四旋翼控制性能，有效融合强化学习与传统控制，实现更高的适应性。
...
### [253] [Dynamic Modeling and LQR Control of a Single Coaxial Drone with 2DOF Thrust Vectoring Mechanism](https://arxiv.org/abs/2609.21099)
*Ali Jokar,Amin Talaeizadeh,Aria Alasty*

Main category: cs.RO

TL;DR: 本文提出一种采用二自由度摆式机构的主动推力矢量同轴旋翼无人机，以较低机械复杂度实现滚转与俯仰控制，并通过拉格朗日建模、LQR控制、EKF状态估计及高保真仿真验证其可行性与节能潜力。


<details>
  <summary>Details</summary>
Motivation: 同轴旋翼无人机具有能效高、体积小的优势，但滚转与俯仰控制存在固有欠驱动问题；现有斜盘机构虽能克服该限制，却显著增加机械复杂度。因此需要一种机械结构更简单、同时能实现主动推力矢量控制的替代方案。

Method: 提出由二自由度摆式机构实现主动推力矢量的同轴无人机架构；建立完整拉格朗日动力学模型，涵盖机体、舵机臂和电机组件的惯性贡献；在悬停平衡点线性化后设计LQR控制器；并结合执行器动态与传感器噪声开展高保真仿真；采用EKF融合GPS、气压计与IMU进行状态估计。

Result: 高保真仿真（考虑执行器动态与传感器噪声）验证了所提架构的有效性；EKF融合GPS、气压计与IMU实现高精度状态估计；结果表明该方案在节能型快速同轴无人机方面具有潜力与可靠性。

Conclusion: 二自由度摆式推力矢量机构可作为一种机械复杂度较低的同轴无人机欠驱动解决方案，配合完整动力学建模、LQR控制与EKF状态估计，能够可靠实现滚转与俯仰主动控制，具备节能与快速飞行的应用前景。
...
### [254] [Dynamics-Induced Commitment in Learning-Based Robotic Penalty Kicks](https://arxiv.org/abs/2609.21100)
*Ruize Geng,Hao E. Zhang,Yisen Li,Yikai Wang,H. Eric Tseng,Ding Zhao*

Main category: cs.RO

TL;DR: 该研究提出动力学诱导承诺映射（DIC-Map），在分层人形-四足点球系统中刻画"身体可执行性"对博弈学习的约束，并给出简化零和博弈的闭式界与估计器中介的学习通道分析。


<details>
  <summary>Details</summary>
Motivation: 机器人博弈学习不仅受策略信息约束，还受身体能否继续执行动作（延续能力）的约束。作者希望定量刻画这种"策略-身体"耦合，尤其是何时承诺（commitment）不可逆地发生、以及承诺后剩余交互能否简化为零和博弈。

Method: 构建分层人形-四足点球系统：博弈层自对弈策略指挥固定的足球全身控制器（S-WBC）；人形射门技能基于自采集动作捕捉数据初始化，四足扑救技能用强化学习训练。提出DIC-Map：估计延续能力、定位终端备选首次持续丧失的时刻，并检验剩余交互是否可约减为零和博弈。对对称终端备选推导最优策略集中度的闭式界（由响应者的延迟价值决定）；并分析当响应者经由估计器行动时，相等响应值会消除直接的终端分配梯度，仅保留估计器中介的一阶学习通道。

Result: 实验定位承诺发生在触球前约0.29秒；仅改变球速即可改变延迟覆盖范围。在四种响应者策略下，替换估计器使扑救率从0.240提升至0.472；而通过等待获得的相近读球准确率提升仅将扑救率提高到0.246。由于可用的覆盖项是观测代理，故采用后验分析进行均衡比较。

Conclusion: 身体动力学造成的承诺会重塑博弈结构：承诺时刻可被定量定位，且其后的交互常可约减为零和博弈；学习效果的关键瓶颈在于响应者的估计器而非单纯读球准确率，改进估计器远比延后决策更有效。
...
### [255] [Learning Scene-Aware Humanoid Locomotion through 3D Clutter from Immersive Human Demonstrations](https://arxiv.org/abs/2609.21107)
*Beichen Wang,Tong Xu,Daniel Kosukhin,Yuen-Hei Yeung,Yuanjie Lu,Xuesu Xiao*

Main category: cs.RO

TL;DR: MTC是一个从示范中学习场景感知人形运动的框架，利用VR生成环境采集人类动作并经场景感知重定向训练运动策略，在Unitree G1上实现密集障碍环境中的全身避障穿越。


<details>
  <summary>Details</summary>
Motivation: 现有从人类运动学习的方法虽能让仿人机器人在无障碍空间中完成高动态技能（如跳舞、武术），但对密集杂乱三维受限环境中的穿越问题研究不足。此类环境几何约束强，要求全身运动与场景几何紧密耦合以实现避障，而真实场景搭建成本高昂。

Method: 提出MTC（Moving Through Clutter）从示范学习框架：使用程序化生成的虚拟现实环境进行沉浸式数据采集，规避昂贵的实体场景搭建；提出场景感知运动重定向算法，将人类示范转换为仿人轨迹，并严格约束机器人—场景间隙以保证无碰撞穿越；以所得参考轨迹训练场景感知运动策略，部署于Unitree G1仿人机器人。

Result: 在自提出的MTC-Challenge多障碍穿越基准上评估，策略在多样化场景中实现70.2%的无碰撞率，并通过爬行通过低净空通道、挤过狭窄缝隙等多种全身技能成功穿越复杂环境。

Conclusion: MTC验证了从人类示范学习并结合场景感知重定向的可行性，为仿人机器人在密集杂乱三维环境中的全身避障运动提供了有效途径。
...
### [256] [Demonstration Synthesis from a Single Scan via Gaussian Splatting for Visuomotor Policy Learning](https://arxiv.org/abs/2609.21112)
*Beichen Wang,Yuen-Hei Yeung,V. R. Sridhar Devarakonda,Xuesu Xiao*

Main category: cs.RO

TL;DR: GaussianFactory是一个仅需一次视频扫描作为人类输入、无需物理引擎参与生成循环的高保真数据引擎，通过3D高斯泼溅重建场景并运动学规划抓取与轨迹，合成逼真演示数据，在仿真与真实UR10e机器人上训练的扩散策略分别达到95.1%和84.2%的成功率。


<details>
  <summary>Details</summary>
Motivation: 训练视觉运动策略需要大量与目标环境高度匹配的演示数据，而重新采集成本高昂。现有演示合成方法虽能降低成本，但仍受限于高人工投入、有限视觉保真度或对物理仿真器的重度依赖。

Method: 以单次视频扫描为唯一人类输入，将场景重建为可编辑的3D高斯泼溅（3DGS）副本，并从场景支持的对象组合任务中采样；对每个任务在几何重建上纯运动学地规划抓取与轨迹，渲染照片级逼真演示；仅在接触力决定结果的抓取形成阶段引入物理动力学，通过预先在交互数据集上训练一次的习得接触模型实现。

Result: 实现了端到端的扫描到部署工作流，在两种设置下评估：替代真实世界以保证可复现性的仿真场景，以及配备物理UR10e机器人的真实工作空间。仅使用合成演示训练的标准扩散策略在两个设置下分别取得95.1%和84.2%的成功率。

Conclusion: GaussianFactory证明仅凭单次视频扫描、无需物理引擎即可大规模生成高保真演示，所合成数据能有效支撑下游视觉运动策略训练，并在仿真与真实机器人部署中均表现出高成功率。
...
### [257] [Noctif3R: Feed-Forward Monocular Real-Time SLAM for Photon-Limited Scenes on Embedded Hardware](https://arxiv.org/abs/2609.21114)
*Mihir Chauhan,Aditya Uday Abhang,Kevin Biju Mathew,Aniket Bera*

Main category: cs.RO

TL;DR: 该论文提出SYS——一个面向暗环境机器人的单目视觉里程计/建图流水线，基于低光前馈pointmap前端与显式匹配门控，在极低信噪比下给出更可靠轨迹与更高精度，并在Jetson AGX Orin上实现能效与内存更优的嵌入式实时执行路径。


<details>
  <summary>Details</summary>
Motivation: 机器人在极暗环境中需依靠单目RGB相机、低功耗板载计算机实时定位，而现有实时单目系统（DROID-SLAM、DPV-SLAM等）在低信噪比下退化或失效；离线低光重建精度虽高但无法实时。作者实测了这些系统在暗光下的失效模式，发现它们会输出看似完整却完全不含相机运动信息的"无信息轨迹"，因此需要一种在暗光下既实时又可靠的方法。

Method: SYS构建于低光前馈pointmap前端之上，并引入显式匹配门控（match gate）以判断跟踪是否有效，从而避免输出无信息轨迹。同时提出面向Jetson AGX Orin的嵌入式执行路径：地图、关键帧与后端以384像素运行，跟踪以256像素运行，并对逐帧位姿求解进行两处修正。评估使用标定且比特精确可复现的噪声阶梯、重标注的真实暗光曝光数据，以及从Boston Dynamics Spot机器人采集的新暗室视频阶梯。

Result: 在场景中最暗的九个暗度等级上，DROID-SLAM在全部九个上返回无信息全长轨迹，VGGT-SLAM与CUT3R在八个、pi^3在七个、DPV-SLAM在四个上如此。SYS返回三条被跟踪轨迹且无无信息轨迹，在其可跟踪范围内误差最低（为无信息上限的24–47%，最强基线为56–73%）。在86.5%帧全黑的实际机器人视频中，所有配置均在明亮开头后停止，而DROID-SLAM与DPV-SLAM对全部1178帧都输出了位姿。嵌入式优化实现可复现的帕累托改进：一个场景吞吐提升1.28倍、误差为0.964倍，另一场景1.42倍、误差0.68倍，峰值GPU内存减少47%，每个位姿能耗降低29%。

Conclusion: 在暗光实时单目定位这一交叉约束下，SYS通过匹配门控与前馈pointmap前端避免了现有方法常见的无信息轨迹输出，在可跟踪范围内取得更低误差，并通过嵌入式执行优化在Jetson AGX Orin上实现吞吐、精度、内存与能耗的多目标改进，适用于低功耗机器人暗环境部署。
...
### [258] [MetaPusher: Meta Learning and Planning for Nonprehensile Manipulation of Unseen Objects with Rapid Online Adaption](https://arxiv.org/abs/2609.21122)
*Donghyung Lee,Seyedali Golestaneh,Jaskrit Singh,Zhuoyun Zhong,Athanasios Kapoutsis,Constantinos Chamzas*

Main category: cs.RO

TL;DR: MetaPusher：结合元学习动力学模型与自适应kinodynamic规划，实现未知物体非抓取式操作的快速适应与长时程规划，仿真与sim-to-real任务成功率最高提升20%。


<details>
  <summary>Details</summary>
Motivation: 未知物体操作困难在于其动力学依赖摩擦、质量分布等无法仅从感知推断的隐式物理属性。跨物体经验可提供初始估计，但该估计不确定且在sim-to-real迁移中会退化；通过交互自适应可逐步精化，但更新模型会使已规划轨迹失效。因此需要兼具快速动力学适应与可融合模型演化的规划策略。

Method: 提出MetaPusher框架：1）元学习动力学模型，在执行任务过程中从交互快速适应；2）自适应kinodynamic规划器，通过复用并精化已有搜索树来更新长时程规划。二者耦合，使操作与适应无需独立的数据采集阶段。

Result: 在仿真及sim-to-real场景中对未知物体进行评估，与微调、主动学习方法、MPPI控制及强化学习策略对比：MetaPusher获得更低的预测误差，任务成功率最高提升20%。

Conclusion: 将元学习动力学适应与树复用式自适应规划耦合，可在无物体特定先验交互条件下高效完成未知物体非抓取式操作，并在仿真到现实迁移中保持性能优势。
...
### [259] [SAGE: Safety-Aligned Gradient Enforcement for Human--Robot Collaboration](https://arxiv.org/abs/2609.21130)
*Yisen Li,Hao Zhang,Ruize Geng,Yves Tseng,Ding Zhao,H. Eric Tseng*

Main category: cs.RO

TL;DR: 本文提出SAGE方法，通过SAIL层与TALO优化解决可解释决策树策略与CBF滤波结合时在多智能体强化学习中产生的两类学习不匹配问题，实现多人-机器人协作的安全与性能兼顾。


<details>
  <summary>Details</summary>
Motivation: 多人-机器人协作要求机器人决策既可解释、可审计，又须在物理交互中满足安全约束。将可解释决策树策略与控制屏障函数（CBF）滤波结合是有前景的架构，但在多智能体强化学习中引入两个学习不匹配：(1) 安全投影改变了实际作用于环境的动作；(2) 耦合的提议图使独立优化的actor更新与团队级更新产生错位。

Method: 提出安全对齐梯度强化（SAGE）。其一是shield-annealed internalization layer（SAIL），使用可微的有限惩罚提议映射，同时保留精确的CBF二次规划用于执行，从而保持约束-范数敏感性以内化反复激活的安全约束。其二是team-averaged Lyapunov policy optimization（TALO），构建团队感知的更新参考，并施加Lyapunov半空间修正以规范独立的actor更新。

Result: 在两个人形机器人与一名人类伙伴的物理实验中验证了部署可行性。在九个仿真场景中，SAGE达到71.0%成功率，每千环境步0.5次碰撞步。消融显示：直接CBF滤波将碰撞频率降低98.5%，但成功率从67.3%降至59.3%；SAIL将提议违反降低48.8%、提议-执行修正降低85.2%；TALO将更新一致性差距降低50.8%。

Conclusion: 直接CBF滤波虽显著提升安全性但损害任务成功率，而SAGE通过SAIL将安全约束内化、通过TALO对齐团队更新，在保证安全的同时维持并提升任务性能，兼顾可解释性与安全性，具备实际部署可行性。
...
### [260] [Same World, Different Knowledge: When Isolated Audits Misjudge World-Model Repairs](https://arxiv.org/abs/2609.21155)
*Rui Min,Xianyao Li,Fang Xu,Sofiane Lachab,Jing Du*

Main category: cs.RO

TL;DR: 论文提出面向世界模型的"信息接口审计"，区分保真度缺口（精确输入变为估计）与可用性缺口（输入缺失），用固定权重干预测量预测误差、输入依赖与闭环收益，指出在共享故障信息时优越的修复可能反而更差。


<details>
  <summary>Details</summary>
Motivation: 在孤立输入故障下表现良好的修复方法，当部署模块共享同一故障信息时可能反而更差；因此需要从部署所使用的信息路径出发评估修复方案，而非仅看单一输入扰动下的表现。

Method: 提出信息接口审计，区分保真度缺口（精确输入被估计替代）与可用性缺口（输入缺失）；采用固定权重干预，测量预测误差、输入依赖以及成对的闭环收益，并纳入由重构引入的依赖关系。在仿真四旋翼模型预测控制中验证。

Result: 耦合、反号的10%质量/推力标定误差使基于物理锚定模型的成功率从69%降至8%，不确定性训练恢复到65%；风场重构恢复了控制收益但继承了标定依赖。正标定偏移下，仅重构受污染时有利于不确定性训练的重构，而共享污染时有利于基线。加速度诊断揭示重构偏差与标称模型误差之间存在补偿，扰动观测器中亦观察到该现象。

Conclusion: 修复方案的选择取决于部署中使用的信息路径；应基于信息接口审计而非孤立输入故障下的表现来评判世界模型修复的有效性。
...
### [261] [MA-LIPP: Cooperative Multi-Agent Load-Aware Informative Path Planning for Heterogeneous Robot Teams](https://arxiv.org/abs/2609.21167)
*Hojune Kim,Guangyao Shi,Gaurav S. Sukhatme*

Main category: cs.RO

TL;DR: 提出Multi-Agent LIPP（MA-LIPP）问题，通过异步"dead drops"实现异构多机器人采样与运输分工，以MIQP精确建模并用Pairwise LNS启发式求解。


<details>
  <summary>Details</summary>
Motivation: 单机器人LIPP中采样与搬运强耦合，机器人必须运送所有样本，导致频繁返回基地，严重限制空间覆盖范围。

Method: 将MA-LIPP精确建模为混合整数二次规划（MIQP），并针对大规模实际应用提出可扩展的Pairwise大邻域搜索（LNS）启发式。

Result: 启发式在95.5%的已验证案例中达到精确最优，在最多12个机器人的较大规模实例上将加权后验方差较顺序基线降低16.1%–19.8%。

Conclusion: MA-LIPP通过异构团队与异步dead drops分工，为协作式物理采样任务提供了稳健框架。
...
### [262] [OpenRoIS: A Community-Driven Open-Source Middleware Implementing the Robotic Interaction Service (RoIS) Framework for Physical Robots and Virtual Agents](https://arxiv.org/abs/2609.21178)
...
TL;DR: 本文介绍 OpenRoIS——一个社区驱动的开源中间件，作为 OMG 标准化 RoIS Framework 2.0 的具体实现，将平台无关的机器人交互服务标准从规范推向工程实践。


<details>
  <summary>Details</summary>
Motivation: 人机交互（HRI）服务应用通常针对某一平台的硬件特定接口编写，更换硬件即需重写应用，造成严重的碎片化问题。OMG 的 RoIS Framework 2.0 虽定义了平台无关模型，但仅有规范无法提供实际采用所需的可维护实现、SDK 与适配器。

Method: OpenRoIS 提供 RoIS Framework 2.0 的开源实现，其核心包括：递归引擎架构（单一引擎类同时实现主 HRI 引擎与子 HRI 引擎角色）；内部的五方法组件契约（区别于五个外部 RoIS 接口）；将接口映射到基于 WebSocket 的 JSON-RPC 2.0；单一数据源类型流水线生成三套一致的语言栈；TypeScript 与 C# 客户端 SDK（支持 Web 与 Unity）；以及支持 ROS 2 的 Python 适配器 SDK。

Result: 通过统一的 RoIS 接口，服务应用可经由互联网同时访问物理机器人与虚拟代理；生成的三套语言栈类型保持一致，客户端与适配器 SDK 覆盖 Web、Unity 与 ROS 2 场景。

Conclusion: 一个开放开发、范式中立的实现是将标准从规范转化为实践的关键。全部源代码、接口类型与文档以 Apache-2.0 许可发布，并在 openrois.org 上开放开发。
...
### [263] [When to Waddle: A Comparative Study of Bipedal Torso-Stabilization on Low-Friction Surfaces](https://arxiv.org/abs/2609.21185)
*Naomi Oke,Ben Gu,George Ortiz,Stacy Ashlyn,Cordelia Pride,Sarah Bergbreiter,Aaron M. Johnson*

Main category: cs.RO

TL;DR: 受企鹅摇摆步态启发，研究在低摩擦表面上通过躯干侧向运动与质心（COM）位置调整来改善双足运动，并在MuJoCo仿真与五自由度硬件上验证其摩擦依赖性效果。


<details>
  <summary>Details</summary>
Motivation: 低摩擦表面限制了迈步时可用接触力，给双足运动带来挑战，需要探索新的步态策略与质心配置以维持运动性能。

Method: 使用五执行器双足机器人，对比直立步态与"躯干位于支撑腿上方"的企鹅式步态；在MuJoCo中扫描正弦腿/髋驱动参数，覆盖μ=0.1、0.3、0.5、0.7四种摩擦系数，并测试多种COM位置，随后在硬件上验证。

Result: 仿真中，低摩擦下躯干位于支撑腿上方产生更多成功控制器和更高前进速度，高COM配置速度最高。硬件实验在μ=0.12时呈相同趋势，前进速度提高、运输成本降低，高COM同时改善两项指标并保持低侧向足部运动。在μ=0.45时趋势反转：低COM更快更节能，步态策略对速度影响小但仍改变侧向足部运动。

Conclusion: 躯干侧向运动与COM位置的效果取决于可用摩擦：前进速度、能耗与打滑相关的足部运动可通过企鹅式躯干运动在硬件上调制。
...
### [264] [Stochastic Neural Signed Swept Volume for Real-time Chance-Constrained Trajectory Optimization](https://arxiv.org/abs/2609.21211)
*Qingyi Chen,Kevin Zhang,Lucas Chen,Zachary Kingston*

Main category: cs.RO

TL;DR: 提出学习扫掠体积的符号距离函数作为概率场，量化认知不确定性并融入机会约束轨迹优化，实现高维操作的无碰撞运动规划。


<details>
  <summary>Details</summary>
Motivation: 无碰撞运动规划需可靠碰撞模型与连续轨迹状态验证，但主流规划器仅在离散状态、对单一确定化环境模型做碰撞检测，带来安全性与计算效率的权衡。现有连续碰撞检测方法计算昂贵或过于保守，数据驱动方法虽能学习扫掠体积，但其神经模型存在近似误差，通常只能作下游碰撞检测器的粗过滤器。

Method: 将扫掠体积的符号距离函数建模为概率场进行学习，从而量化认知不确定性、纳入感知噪声，并最终集成到机会约束轨迹优化框架中。

Result: 在具有显著传感器噪声的高维操作问题上验证了该方法，涵盖仿真与真实硬件实验。

Conclusion: 通过学习扫掠体积的概率符号距离场，可将不确定性显式纳入规划，在噪声环境下兼顾安全性与计算可行性，并在高维真实操作任务中有效。
...
### [265] [Safe Real-Time Policy Steering via Noise-Space Trajectory Optimization for One-Step Generative Policies](https://arxiv.org/abs/2609.21220)
*Qingyi Chen,Joseph Ruan,Zachary Kingston*

Main category: cs.RO

TL;DR: INSPO将一步生成策略的推理时引导建模为输入噪声空间中的轨迹优化，用种群粒子优化在线求解，实现高效的碰撞避免与姿态维持。


<details>
  <summary>Details</summary>
Motivation: 生成式机器人策略能表达多样化、多模态行为，但将预训练策略适配到部署时约束（如避碰、姿态维持）仍具挑战。现有推理时引导方法需通过迭代扩散或流过程进行梯度引导，实时控制时计算开销大。

Method: INSPO将一步生成策略的推理时引导形式化为策略输入噪声空间中的轨迹优化。它在评估诱导状态轨迹上的约束的同时优化输入噪声，从而在不直接修改生成动作的情况下搜索策略诱导的行为空间。优化包含正则化项以保持解与策略输入分布一致，并通过基于种群的粒子优化在线求解。

Result: INSPO在Push-T、Can pick-and-place和LIBERO-Spatial任务上，对基于状态与图像的任务专用策略以及通用视觉-语言-动作策略进行了评估。相比best-of-N采样和动作投影，INSPO提升了任务成功率与约束满足度，并且以更低运行时间优于梯度引导生成方法。

Conclusion: INSPO通过在输入噪声空间中做轨迹优化，为一步生成策略提供了一种高效、有效的推理时引导方案，在满足部署约束的同时保持低运行时开销。
...
### [266] [SafeStage: Evaluating Safety Before, During, and After Vision-Language-Conditioned Robot Manipulation](https://arxiv.org/abs/2609.21223)
*Jinzhu Luo,Qi Zhang,Wei Wang,Wei Jiang*

Main category: cs.RO

TL;DR: 本文提出 SafeStage——一个按任务生命周期（执行前/中/后）划分结构的机器人操作安全基准，包含 97 个风险场景，将任务成功率与分阶段安全结果解耦评估。实验表明任务完成常与安全违规并存，不同 VLA 策略呈现差异化失效模式。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-条件机器人策略评估多聚焦于任务成功率、单一物理约束、语义拒答或已发生的物理损伤，难以揭示闭环操作中安全失效发生的位置（何时、何阶段）。作者希望将任务成功与安全表现分离，并定位违规发生的阶段，从而提供统一诊断工具。

Method: 构建 SafeStage 基准：97 个专门设计的风险场景，划分为三阶段——初始状态危害（操作前需解决的安全相关关系）、执行时安全（执行中的不安全接触、轨迹、区域进入、物体交互）、最终状态危害（名义任务完成后残留的不稳定/不安全状态）。采用事件级与状态级检查评估真实交互，并在统一闭环协议下独立报告原生任务成功与各阶段安全结果；对比评测直接动作型 VLA 策略与世界模型型策略。

Result: 名义任务完成与安全违规经常同时出现；不同策略在三阶段上呈现各异的失效画像（failure profiles）。

Conclusion: 通过将任务成功与安全解耦并定位违规发生阶段，SafeStage 为评估和改进视觉-语言-条件机器人操作策略提供了统一诊断测试平台。
...
### [267] [AirSplan: Risk-Aware Motion Planning for Quadrotors in Cluttered 3D Gaussian Splats](https://arxiv.org/abs/2609.21226)
*Seth Isaacson,William Hong,Katherine A. Skinner,Ram Vasudevan*

Main category: cs.RO

TL;DR: AirSplan：用归一化3D高斯溅射建模场景几何，结合基于可达性的运动规划器，为四旋翼生成连续时间无碰撞轨迹，在挑战性测试中成功率81.2%，显著优于基线51.2%。


<details>
  <summary>Details</summary>
Motivation: 四旋翼在农业、设施巡检与维护等场景中需在复杂几何环境中严格无碰撞飞行，且空中碰撞往往导致机器人损毁。这带来两个挑战：一是无真值障碍数据时仍需高保真表示复杂场景结构；二是规划器需基于该表示求解无碰撞路径。

Method: 提出AirSplan系统：采用归一化变体的3D高斯溅射（3D Gaussian Splatting）编码高保真场景几何；并设计新颖的基于可达性的运动规划器，利用四旋翼的微分平坦特性（differential flatness）计算连续时间碰撞约束，紧密过近似机器人占用空间。

Result: 在挑战性测试用例中成功找到路径的比例为81.2%，较最接近的基线方法51.2%有显著提升。

Conclusion: 将高保真辐射场式场景表示与可达性规划相结合，可有效解决无真值障碍数据下的空中无碰撞导航问题，验证了该方法在复杂场景中的有效性。
...
### [268] [LOInK: Learned Optimal Inverse Kinematics via Structured Neural Surrogate Models](https://arxiv.org/abs/2609.21275)
*Michael Somerfield,Damian Abood,Ruigang Wang,Ian R. Manchester*

Main category: cs.RO

TL;DR: LOInK通过双Lipschitz可逆映射将构型空间解耦为任务/潜空间，并将代价最小解置于原点，配合基于算子分裂的网络反演算法，实现近似最优逆运动学解的高效采样。


<details>
  <summary>Details</summary>
Motivation: 逆运动学问题通常需在满足任务约束的同时最小化代价，传统方法依赖约束优化，计算开销大、速度慢；现有生成式方法则需大量采样并评估候选解才能获得高质量结果。因此需要一种能高效、直接生成近似最优解的方法。

Method: 在由采样构型、对应任务变量及给定代价函数组成的数据上训练，学习一个从构型空间到解耦的任务/潜空间的双Lipschitz可逆映射；通过构造使代价最小解位于潜空间原点，并采用基于算子分裂的网络反演算法，高效采样代价最小化解。

Result: 在三个问题上验证：三自由度机械臂的说明性算例；四足攀爬机器人上，LOInK生成近优解平均比约束优化快31倍，最高快100倍；在仿真软体驱动器这一纯数据驱动任务中，LOInK能直接生成高质量解，而现有生成式方法需依赖多样采样与候选解评估。

Conclusion: LOInK通过学习具有良好几何性质的可逆潜空间表示并结合网络反演，为近似最优逆运动学求解提供了高效、直接且适用于纯数据驱动场景的新范式。
...
### [269] [Stability-aware Residual Reinforcement Learning Framework for Robotic Manipulator Disturbance Compensation](https://arxiv.org/abs/2609.21307)
*Jihong Kim,Joonhyuk Kwon,Hwa Soo Kim,TaeWon Seo,Hyung-Tae Seo*

Main category: cs.RO

TL;DR: 提出残差强化学习干扰观测器(RRL-DOB)框架，用解析观测器与RL策略互补，并引入干扰感知估计器与基于ISS的状态相关动作约束，保证跟踪误差收敛到认证包络，在6-DOF机械臂上实现零样本仿真到现实迁移。


<details>
  <summary>Details</summary>
Motivation: 传统控制器与干扰观测器在精密跟踪中面临参数不确定性、非线性摩擦与复合干扰，模型无法完全刻画这些残差，因此需要一种能补偿未建模残差同时保证稳定性的方法。

Method: 将确定性解析观测器作为基线在可靠区间工作，RL策略专门补偿模型无法捕获的残差；设计估计器网络，将观测历史与特权干扰上下文对齐，按干扰工况组织隐空间以支持干扰切换下的快速适应；基于输入到状态稳定性(ISS)分析推导并施加状态相关动作边界，使任意策略输出下闭环误差被限制在认证包络内。

Result: 在6-DOF机械臂实验中，干扰估计与跟踪性能一致提升：真实硬件上零样本仿真到现实迁移使跟踪误差降低27.8%；在训练中未观测到的基座振动干扰下误差降低38.0%。

Conclusion: 将解析干扰观测器与残差强化学习结合，并配合干扰感知表征与ISS稳定性约束，可在保证理论稳定性的同时显著提升机械臂在不确定与未见干扰下的跟踪精度。
...
### [270] [NaViRrator: Robot Navigation from Human-Readable Maps through a Learned Visual Route](https://arxiv.org/abs/2609.21316)
*Ayun Lee,Jiseon Kim,Giseop Kim*

Main category: cs.RO

TL;DR: 本文提出NaViRRator框架，将人类可读地图上的起点与终点转化为导航指令，供预训练的VLN策略使用，实现地图到语言的模块化接口。


<details>
  <summary>Details</summary>
Motivation: 人类可读地图是直观的机器人目的地指定界面，但其示意几何与机器人自中心观测之间的连接仍是挑战；同时希望地图到语言的模块无需随执行器更换而重新训练。

Method: 核心方法RouteScribe将路径推断与语言化分离：先用起点—终点线条件流匹配（SGL-CFM）将直线起点—终点路点序列变形为地图条件下的显式路径骨架（地图图像坐标），再由预训练视觉语言模型（VLM）将骨架转成导航指令；执行时VLN策略仅接收指令与自中心观测，地图与骨架留在上游。

Result: 真实世界实验表明，相比直接地图到指令生成、基于A*的骨架生成和高斯源条件流匹配，该方法的成功率与SPL更高；定性结果显示显著转弯更清晰、预期机动序列保持更好。

Conclusion: 基于路径接地的语言可作为一种模块化接口，连接人类可读地图与预训练导航策略，并支持执行器替换而无需重训地图到语言模块。
...
### [271] [LEMCA: LLM-Guided Synthesis of Efficient Mode-Switching Control Architectures](https://arxiv.org/abs/2609.21319)
*Arjun Krishna,Vincent Pacelli,Dinesh Jayaraman*

Main category: cs.RO

TL;DR: 提出LEMCA框架，利用大语言模型的语义先验、推理与编码能力，在进化循环中自动合成可解释的模式切换控制架构（MSC），从而在简单任务阶段回收浪费的资源，超越单体设计的Pareto前沿。


<details>
  <summary>Details</summary>
Motivation: 自然界的物理控制任务（如驾驶、物体操作）在感官与计算复杂度上常随时间剧烈变化，因此资源高效的自然选择是动态切换具有不同资源分配的控制模式。但模式切换控制器历史上需要大量专家驱动、针对每项任务重复进行费力设计与合成；受限于此，现代机器人控制架构常退化为浪费的“单体”一刀切结构，将资源分配永久锚定在最困难、最耗资源的任务阶段。

Method: 将MSC设计表示为可解释程序，在进化循环中迭代优化。为评估设计适应度，提出MSC兼容的自动化控制器合成扩展（如仿真中的强化学习）。随后利用LLM的语义先验、推理与编码能力，迭代编辑控制器模式、对应的感官-计算资源分配以及模式切换。

Result: 在多种控制基准任务上，LEMCA一致地发现超越单体设计Pareto前沿的策略，其方式是在“容易”的任务阶段回收被浪费的资源。

Conclusion: LEMCA为合成资源高效的模式切换控制设计提供了一条自动化、低人工投入的路径。
...
### [272] [The EventCV Library for Event-Based Robotic Vision](https://arxiv.org/abs/2609.21330)
*Adam D. Hines,Michael Milford,Tobias Fischer*

Main category: cs.RO

TL;DR: EventCV：面向事件相机的开源可扩展Rust库，配OpenCV风格Python绑定，集成去噪、几何变换、增广、角点检测、无监督特征学习、对比度最大化运动估计、视频-事件仿真及ONNX推理，并接入Neuromorphic Drivers实现实时处理。


<details>
  <summary>Details</summary>
Motivation: 事件相机具备微秒级异步响应、高动态范围、低功耗，适合高速或困难光照下的机器人，但缺乏即插即用驱动、事件流文件格式互不兼容、多依赖定制研究级代码，导致接入真实机器人流水线门槛高。

Method: 以Rust实现开源可扩展库并提供OpenCV风格Python绑定；功能涵盖去噪滤波、几何变换、数据增广、角点检测、无监督特征学习、对比度最大化运动估计、视频到事件仿真、ONNX模型推理；集成Neuromorphic Drivers支持事件流实时处理；在Jetson Orin AGX上部署。

Result: 现有工具包均无法在单一包内覆盖如此广泛操作；EventCV构建表示与解码文件的速度比现有库快1.1×至3.7×；在Jetson Orin AGX上完成三个机器人案例研究（目标检测、端侧模型推理、定位）。

Conclusion: EventCV通过统一、开源、可扩展的工具链降低事件相机使用门槛，并具备实时处理、部署与效率优势，适用于机器人视觉任务。
...
### [273] [FAN: Foresight Action Normalization for Continual Adaptation of Vision-Language-Action Models](https://arxiv.org/abs/2609.21358)
*Yijun Hong,Jiarun Zhu,Xiaoquan Sun,Le Xu,Qijun He,Xin Jin,Mingqi Yuan,Wenjun Zeng,Jiayu Chen*

Main category: cs.RO

TL;DR: 本文聚焦VLA模型持续适配中被忽视的动作归一化机制，系统评估五种归一化策略在四个真实任务流上的表现，提出3C设计原则与前瞻动作归一化（FAN），该方法在持续学习中保持最优且稳健的性能。


<details>
  <summary>Details</summary>
Motivation: 大规模闭源数据预训练的VLA模型在多种机器人操作任务中表现优异，但长期真实部署要求持续习得新技能并保留旧能力。现有持续VLA适配研究多依赖经验回放、强化微调等技术，却忽视了决定策略感知与执行物理动作坐标系的动作归一化这一基础机制，导致任务间坐标漂移、运动覆盖不足、训练-测试坐标失配等严重失效模式。

Method: 作者在涵盖单臂与双臂操作的四个真实世界任务流上，系统评估五种归一化策略，分析其失效机理，进而提出一致性、覆盖性、因果性（3C）三条设计原则，并据此设计前瞻动作归一化（FAN）：在持续学习开始前，仅用一个小规模、与任务无关的校准集估计一次归一化统计量，并在整个适配过程中将其冻结。

Result: 分析表明现有归一化协议会因任务间坐标漂移、运动覆盖有限或训练-测试坐标不匹配而诱发严重失败模式。在所有评估任务流中，FAN均取得最高性能，并展现出稳定的鲁棒性。

Conclusion: 动作归一化是持续VLA适配的关键基础机制。基于3C原则的FAN通过在持续学习前一次性估计并冻结归一化统计量，为构建稳定的动作表示、实现有效的终身VLA适配提供了具有指导意义的方案。
...
### [274] [MicroHookACT: Monocular Microscopic Vision Guided Visuomotor Policy for Flexible Microelectrode Hooking](https://arxiv.org/abs/2609.21365)
*Yitong Chen,Fangbo Qin,Yang Wang,Ruihua Hu,Kui Zhang,Shan Yu*

Main category: cs.RO

TL;DR: 本文提出MicroHookACT——一种基于模仿学习的视觉运动策略，在单目显微视觉下实现微电极植入中针-环自动3D钩挂，总体成功率96.7%，平均耗时11.5 s。


<details>
  <summary>Details</summary>
Motivation: 柔性微电极（FME）植入中，自动化针-环钩挂是关键步骤，但显微尺度下存在景深模糊、目标细微、对准精度要求高等难题，传统方法难以胜任。

Method: 采用三项关键技术：1）单向钩挂策略，利用离焦线索与光轴引导实现无触诊的精确对准与接触丰富的穿线；2）基于冻结ViT骨干的动作监督目标注意力模块，直接从人类示教中学习聚焦微针尖与微环，无需人工视觉标注；3）以注意力为中心的全局粗特征与局部细特征，依据预测动作进度动态加权，使单一ACT策略适应操作中变化的离焦模糊与视觉需求。

Result: 基于60组人类示教训练视觉运动策略，在5种不同难度设置下评估，MicroHookACT取得最高96.7%的总体成功率，平均执行时间11.5 s。
...
### [275] [AVT-Fabric: Active Visuo-Tactile Perception via Adaptive Evidence Selection for Efficient Robotic Fabric Comparison](https://arxiv.org/abs/2609.21377)
*Chang Gao,Zhuo Chen,Suhang Xia,Jihong Zhu,Jiankang Deng,Shan Luo*

Main category: cs.RO

TL;DR: 本文提出AVT-Fabric，一种RGB优先的机器人织物比较框架，通过自适应门控按需调用触觉证据，以7B MLLM达到98.0%准确率并大幅降低延迟与计算量。


<details>
  <summary>Details</summary>
Motivation: 机器人织物比较需同时结合视觉外观与触觉线索，但被动地固定使用全部触觉阶段会造成计算与延迟浪费；因此需要一种根据每个比较任务难度自适应分配触觉证据的方法。

Method: AVT-Fabric为RGB优先框架：双尺度门控（dual-scale gate）依据答案token置信度与原始logit间隔，判断是否需要追加带力标记的GelSight触觉观测；紧凑文本记忆保存已执行历史；多数投票整合所选预测。

Result: 在400个留出比较任务上，7B MLLM达98.0%准确率，超90B MLLM-Fabric基线的94.0%（+4.0个百分点），平均仅用5个阶段中的1.60个；相比匹配的被动推理提升9.25个百分点，模型侧延迟降低61.8%，并优于纯RGB方案；四种额外MLLM骨干验证了通用性、准确性与效率。真实机器人系统上达78.1%成对排序准确率，8个应用场景中7个正确选布。

Conclusion: 自适应证据选择能够同时提升机器人视触觉推理的准确性与效率。
...
### [276] [MarineCraft: Enabling Rapid Prototyping of Underwater Robots via Modular Construction](https://arxiv.org/abs/2609.21396)
*Yuta Sugiura*

Main category: cs.RO

TL;DR: MarineCraft 是一种模块化水下机器人工具包，通过自包含防水推进模块消除集中布线与反复密封，实现分钟级重组与快速迭代原型开发，可在 2.5 米水深下无泄漏可靠运行。


<details>
  <summary>Details</summary>
Motivation: 水下机器人开发受限于防水与布线复杂度，导致快速原型迭代周期被显著拖长；作者希望降低水下机器人研发门槛，将模块化作为快速物理原型设计的手段。

Method: 提出 MarineCraft 模块化工具包，采用结构可重构设计；核心是自包含防水推进模块，将电源、无线通信与驱动集成于一体，从而取消集中式布线和重复密封环节，支持多种机器人几何构型在数分钟内完成组装与测试。

Result: 实验表明该可重构架构支持快速迭代设计循环，并能在最深 2.5 米水深下保持可靠运行与无泄漏性能。

Conclusion: 该工具包通过模块化显著降低水下机器人研发门槛，使组装与测试从数天缩短至数分钟，为水下机器人快速物理原型开发提供有效途径。
...
### [277] [Stabilizing Trajectory Outputs in End-to-End Autonomous Driving via SC-IMM Based Teacher Signals](https://arxiv.org/abs/2609.21404)
*Siewoo Kim,Seung-Hyun Kong*

Main category: cs.RO

TL;DR: 本文提出基于场景条件交互多模型（SC-IMM）的离线教师信号生成与学习方法，通过将专家轨迹转化为路径-速度状态并生成辅助监督信号，提升端到端自动驾驶模型轨迹输出的稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统基于路点的模仿学习主要最小化坐标级误差，难以捕捉随场景变化的路径-速度变化以及路点输出间的时间不稳定性，导致闭环行为不稳定。

Method: 将专家轨迹转换为路径-速度状态，基于场景线索进行IMM更新，生成路径-速度教师标签和模式后验概率；这些信号作为辅助监督加入原始轨迹损失进行训练，而推理结构与路点控制器保持不变。

Result: 在CARLA Town12的100条短路线闭环评估中，相比基线，驾驶得分提升28.0%，每公里碰撞数降低62.3%，同时jerk和轨迹变化指标也有所改善。

Conclusion: 嵌入场景条件运动模型线索的离线教师信号能够引导轨迹输出式驾驶模型实现更稳定的闭环行为。
...
### [278] [A Unified Dynamic Force Guidance Framework for Performance-Optimized Kinesthetic Teaching](https://arxiv.org/abs/2609.21416)
*Chunxin Li,Jianhua Wu,Zhenhua Xiong,Xiangyang Zhu*

Main category: cs.RO

TL;DR: 该论文提出一种融合性能约束与优化机制的在线动态力引导方法，用于提升协作机器人拖动示教过程中的操作性能与工作效率。


<details>
  <summary>Details</summary>
Motivation: 协作机器人在频繁换产的工业场景中需求增加，拖动示教可实现快速部署，但用户示教时易忽略机器人构型，导致操作性能下降。操作性能反映机器人生成运动的能力，可由雅可比矩阵最小奇异值量化。

Method: 提出在线动态力引导方法，集成性能约束与优化机制：采用可变导纳控制将机器人操作性能维持在预设阈值以上；同时施加虚拟力，主动引导用户将机器人拖向性能更优的构型。

Result: 在六自由度协作机器人上开展实验，对比任务空间中的三条典型路径；通过轨迹回放实验分析机器人操作性能与工作效率的关系。

Conclusion: 所提方法有效提升了机器人操作性能，进而改善工作效率，对缩短工业生产节拍时间具有重要价值。
...
### [279] [Robotic Multiphase Interaction: Manipulating Coupled Liquid and Solid Dynamics with a World Model](https://arxiv.org/abs/2609.21448)
*Yixuan Feng,Peng Wang*

Main category: cs.RO

TL;DR: 提出"机器人多相交互(RMI)"新设定:液体进入多孔材料并与其变形固体骨架发生机械耦合,以含水海绵为首个案例,构建基于SPH的不可压缩多孔流仿真环境,并结合动作条件世界模型与扩散/整流流策略,显著降低保水预测误差。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作研究多针对纯固体对象,或虽同时涉及液体与固体但保持两相空间分离;而液体渗入可变形多孔骨架的耦合场景(如含水海绵)缺乏建模与操作方法,且液体溢出在家庭与制造环境中存在安全隐患,因此需要研究这种操作会同时改变孔隙体积、挤出或重分布液体并影响抓取稳定性的新问题。

Method: 1) 定义RMI设定:液体进入多孔材料并与其变形固体骨架机械交互。2) 以充水海绵为实例,采用隐式不可压缩多孔流与光滑粒子流体动力学(SPH)作为动力学引擎。3) 为支持机器人操作,加入库仑接触记忆、混合速度与力调节,以及用于举升的稳定性门控。4) 构建将机器人指令与耦合液固状态关联的环境;使用以动作为条件的世界模型预测候选指令下状态演化;用时间UNet结合扩散策略或整流流匹配生成动作。

Result: 世界模型将保水量预测误差较基线降低超过60%;在世界模型从策略提案中选出的最优动作序列进一步将预测的终端水量误差降低约一半。

Conclusion: 对耦合液固状态进行建模有助于机器人预测其动作如何同时影响多孔物体及内部液体,验证了RMI设定与所提方法的有效性。
...
### [280] [Learning Distance-Conditioned Object Transport for Humanoid Loco-Manipulation from a Single Motion Clip](https://arxiv.org/abs/2609.21467)
*Yuhyeon Hwang,Daniel Sungho Jung,YongHyeok Seo,Mingi Jung,Chang Nho Cho,Jung-Hoon Hwang,Dongin Shin*

Main category: cs.RO

TL;DR: 提出距离条件参考重组(DCRR)方法,将演示的终止片段重定位至中间搬运状态,通过冻结跟踪教师闭环回放、按到达物体位置重标注并蒸馏出无参考策略,实现距离可控的人形搬运操作。


<details>
  <summary>Details</summary>
Motivation: 基于单一重定向动作片段的运动跟踪策略只能复现演示中的运输结果。源轨迹虽经过多个中间物体位移,但"终止"仅演示于终点位置,形成"终止-经过"差距(termination-versus-passage gap):中间位移只被观测为经过状态而非终止完成的结果,因而策略缺乏距离条件化的监督信号。

Method: DCRR:将演示的终止片段重定位到各中间运输状态,重组出距离条件化的参考动作;用冻结的运动跟踪教师模型在闭环动力学下回放这些重组参考,保留可行轨迹并按其实际达成的物体摆放位置重新标注;最后将标注数据蒸馏为无需参考的策略(BC),并辅以RL微调。

Result: 在Carry、Kick-Push、Crouch-Push、Drag四种交互模式下,DCRR-BC实现命令相关的运输,整体归一化距离MAE为0.15,优于仅用源行为的BC(0.28);RL微调进一步提升了命令响应与执行鲁棒性,在训练仿真器及sim-to-sim迁移下均有效;硬件实验验证了四种模式下的运输距离调制能力。

Conclusion: 通过将演示终止片段重定位到中间状态并闭环回放重标注,DCRR可从源动作蕴含的交互行为中构造距离条件监督,使无参考策略具备可控运输距离,并具备仿真与真机迁移的可行性。
...
### [281] [FORTE: Task-Adaptive Force Capability Optimization for Mobile Manipulators](https://arxiv.org/abs/2609.21497)
*Xiao Wang,Heng Zhang,Gokhan Solak,Fei Zhao,Arash Ajoudani*

Main category: cs.RO

TL;DR: 提出任务导向的力能力优化框架，用VLM推断物体属性生成任务力序列，并以任务力不确定球与动态残余力多面体(RFP)的有符号距离作为力能力度量，融入全身多目标轨迹优化，使移动机械臂在重载时保证力能力、轻载时保持高可操作度。


<details>
  <summary>Details</summary>
Motivation: 机器人操作中的有效物理交互控制不仅需要运动学可行，还需足够的力交互能力。现有冗余度求解方法要么忽略任务特定的力需求，要么不加区分地最大化力能力，在不需要大力裕度时牺牲了可操作度，因而缺乏任务自适应性。

Method: 1) 用视觉语言模型(VLM)从RGB图像和任务描述中推断物体物理属性，生成刻画重力与惯性需求的任务力序列；2) 定义任务导向力能力度量为任务力不确定球与动态残余力多面体(RFP)之间的有符号距离，量化任务需求与机器人剩余驱动能力间的兼容性；3) 将该度量与可操作度、关节限位规避、轨迹平滑性和基座振荡抑制一起纳入全身多目标轨迹优化问题。

Result: 在移动机械臂执行举升与单点保持任务、并在不同负载条件下进行实验，所提方法对重载提供足够的力能力，对轻载保持高可操作度，实现了任务自适应的权衡；而固定能力最大化基线（RFP内切半径、RFP锥）与仅可操作度优化方法无法实现这一效果。

Conclusion: 通过将任务力需求与动态残余力能力显式耦合于全身轨迹优化，可实现力能力与可操作度的任务自适应平衡，验证了任务导向力能力优化在冗余移动机械臂物理交互中的有效性。核心实现已开源于 https://github.com/yeying256/FORTE。
...
### [282] [DPed-VLN: A Benchmark for Socially Compliant Vision-and-Language Navigation in Dynamic Pedestrian Environments](https://arxiv.org/abs/2609.21504)
*Haojie Dai,Xiangyi Wang,Liuyi Wang,Kai Sheng,Zongtao He,Chengju Liu,Wei Ye,Qijun Chen*

Main category: cs.RO

TL;DR: 本文提出DPed-VLN——基于Habitat 3.0的动态行人视觉语言导航基准，包含33,093个导航回合、配对指令、ORCA行人与社会约束专家路径，并配套DPet行人感知策略网络及VLM适配实验。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航（VLN）研究多集中于静态室内环境，而真实场景中的机器人需在有人群活动的空间中理解语言，同时应对移动行人并满足社会安全约束，缺乏相应的基准与评估手段。

Method: 构建DPed-VLN基准：基于Habitat 3.0，含33,093个导航回合，配备全局指令与增强先验指令的配对设计、ORCA控制的人形行人、社会约束专家路径，以及联合评估导航效率与社会安全的指标；同时提出DPet动态行人感知策略网络，采用强化学习与模仿学习训练；并通过LoRA微调将NaVILA、StreamVLN等代表性VLM导航模型适配至该基准。

Result: LoRA适配在多个成功率和安全性指标上提升了零样本VLM基线，尤其降低了StreamVLN的碰撞率；在所有评估方法中，DPet-RL取得最高的SR、SPL与STL。

Conclusion: DPed-VLN为动态行人场景下的视觉语言导航提供了可控、可复现的评测平台，验证了行人感知策略学习与VLM适配在该任务中的有效性，DPet-RL在导航效率与社会安全指标上表现最优。
...
### [283] [Skel-WAM: A Hand-Skeleton-Conditioned World Action Model for Human-to-Robot Manipulation Transfer](https://arxiv.org/abs/2609.21514)
*Zetao Cai,Yaping Li,Yiqun Wang,Xinyu Zhan,Yuyin Yang,Haoxiang Ma,Kailin Li,Tao Lu,Jiangmiao Pang,Linning Xu,Dahua Lin*

Main category: cs.RO

TL;DR: Skel-WAM 是一种世界动作模型，通过统一的“手部骨架运动接口”对齐人类视频与机器人演示，将视频/关键点专家与机器人动作专家解耦，实现人类数据无需机器人动作标签即可共同监督共享动力学，显著提升双臂操作成功率与任务泛化。


<details>
  <summary>Details</summary>
Motivation: 机器人演示采集成本高、任务变体覆盖有限；人类视频虽廉价且具互补操作经验，但存在视觉外观与动作空间的具身鸿沟，直接利用困难。

Method: 构建统一手部拓扑作为跨具身接口：用骨架叠加将运动锚定到场景，用结构化2.5-D关键点编码显式手部运动学；采用Mixture-of-Transformers让Video Expert与Keypoint Expert联合学习视觉与骨架动力学；另设由机器人数据训练的动作专家，将预测映射为可执行控制，从而人类视频无需动作标签即可监督共享动力学。

Result: 在4个真实双臂任务与7个仿真任务上平均成功率分别达79.86%与63.29%，较最强基线高出22.22与8.28个百分点；人机协同训练将机器人训练数据未覆盖的真实任务变体成功率从38.89%提升至86.11%，翻倍以上。

Conclusion: 共享骨架接口可有效实现人类与机器人数据的联合学习，并借由互补的人类演示扩展机器人任务覆盖范围。
...
### [284] [SABER: Learning Attention-based Semantic Affordance for Legged Locomotion](https://arxiv.org/abs/2609.21572)
*Hari Prasanth Palanivelu,Samuel Sze,Kennard Garrison Johannes,Albertus Hendrawan Adiwahono,Meng Yee,Chuah*

Main category: cs.RO

TL;DR: 提出 SABER：一种无规划器、端到端的强化学习运动策略，在统一地形可供性图上联合推理几何与语义接触许可，通过带符号语义偏置的交叉注意力实现语义接触选择，并在 Unitree B2 上完成 sim-to-real 验证。


<details>
  <summary>Details</summary>
Motivation: 现有感知式足式运动虽已将地形几何融入学习策略，但缺乏对“地形语义/含义”的整合：管道、草地、脆弱箱子等在几何上可通行，却不适合踩踏。在工业场景中，一次误踩可能损坏设备、使机器人失稳或危及场地安全。因此需要让策略在决定落足点时同时考虑几何可通行性与语义接触许可。

Method: SABER 为无规划器（planner-free）的强化学习策略，输入为统一的地形可供性图（terrain-affordance map），每格编码局部 3D 几何与语义接触代价（semantic contact cost）。核心为在交叉注意力（cross-attention）中引入可学习的带符号语义偏置：作为注意力 logits 上的加性项，并由接触代价门控，按“距最近足端的距离”对被标记单元格重新加权——使危险区域仅在仍能影响下一落足点的位置重塑注意力，距离过远时其影响衰减。最终策略在允许支撑面上选择落足点，并在整个摆动相使腿部避开禁区。作者进行了系统性消融，隔离各架构组件的贡献。

Result: 消融实验表明：仅移除语义偏置一项，违规（禁区）接触增加 55%，而速度跟踪性能不变，说明该组件专门负责语义接触选择而非影响运动跟踪。在 Unitree B2 上完成验证，实现了室内外环境、四类语义障碍上的 sim-to-real 语义接触选择。

Conclusion: 该工作表明，将语义接触许可以带符号、距离门控的注意力偏置形式注入学习策略，可在不牺牲速度跟踪的前提下显著提升足式机器人的语义安全落足能力，并具备从仿真到真实（sim-to-real）的迁移可行性，适用于工业等对误踩敏感的接触选择场景。
...
### [285] [Tilt as a Certified Resource: Preserving Motor Wrench-Rate Authority on Articulated Multirotors](https://arxiv.org/abs/2609.21580)
*Giuseppe Silano,Martin Saska*

Main category: cs.RO

TL;DR: 本文利用主动形态（伺服倾转）打破定几何对称平台中"就绪性-力旋量跟踪"的零和退化，构造了一个依赖构型、仅由电机决定的就绪性证书（可达力旋量速率集的体积对数），排除了伺服容量以避免"幽灵容量谬误"，并将其作为控制屏障函数嵌入统一物理指令二次规划中。仿真表明，经典分配器会发散，未认证的关节式分配器违反安全下限，而所提出的CBF滤波器能约束系统状态并保持车辆权限。


<details>
  <summary>Details</summary>
Motivation: 全驱动多旋翼不仅要跟踪标称力旋量，还须在扰动下保持快速调制力旋量的"就绪性"。经典的省力分配器忽略这一动态限制，而最大化就绪性又会产生拓扑上不连通的、需要物理上不可能执行器速率的优化面。在固定几何对称平台上施加就绪性安全下限会遭遇零和退化：电机转速的重分配无法在不牺牲力旋量跟踪的前提下提升权限。

Method: 使用主动形态学打破退化，将伺服倾转视为提供静态转子所无法提供的权限恢复方向的几何资源。构造依赖构型、仅含电机的就绪性证书——可达力旋量速率集的体积对数，该证书明确排除伺服容量，从而避免"幽灵容量谬误"（否则证书会错误地将缓慢的机械运动学极限计入，而非在电机饱和时准确崩溃）。该证书作为控制屏障函数（CBF）在作用于电机力矩和伺服设定点的统一物理指令二次规划中强制执行。

Result: 在严重阵风扰动下对关节式八旋翼进行闭环仿真显示：经典分配器发散，未认证的关节式分配器违反安全下限，而所提出的CBF滤波器能够约束系统状态并保持车辆权限。

Conclusion: 主动形态学（伺服倾转）为全驱动多旋翼的力旋量速率就绪性提供了静态转子无法获得的权限恢复方向，从而打破定几何平台的零和退化；基于电机专属就绪性证书的CBF滤波能在严重扰动下保证安全下限并维持车辆权限。
...
### [286] [A High-Payload Wall-Climbing Robot Using Passive Bistable Suction Cups](https://arxiv.org/abs/2609.21584)
*Andrew Nguyen,Mingyuan Li,Daniel Bruder*

Main category: cs.RO

TL;DR: 本文提出一种利用被动双稳态吸盘实现高负载壁面爬行的机器人，通过轨道系统自动吸合/脱开吸盘，在光滑表面实现爬行，最大拖曳负载7.940 kg，负载自重比2.25。


<details>
  <summary>Details</summary>
Motivation: 壁面爬行机器人可自动化窗户清洗、检测、维护与施工等危险或高强度任务。主动粘附方式负载能力强但维持吸附需持续供电；被动粘附（如吸盘）无需供电维持吸附，但负载能力受限。因此需要一种兼具高负载能力且无需持续供电的粘附方案。

Method: 采用被动双稳态吸盘产生粘附力，且无需将吸盘压向墙面即可吸附；机器人搭载轨道式系统，自动完成双稳态吸盘的吸合与脱开，从而实现光滑表面上的运动。

Result: 机器人可在玻璃、木材、金属及涂漆表面实现垂直壁面爬行，并支持侧向与倒挂爬行；可拖曳7.940 kg负载，负载自重比达2.25。

Conclusion: 该工作验证了被动双稳态吸盘在高负载壁面爬行机器人中的可行性，在无需持续供电维持吸附的前提下实现了较高负载能力与多表面、多姿态爬行能力。
...
### [287] [CounterPlay: Counterfactual Post-Training for Self-Play Driving Policies](https://arxiv.org/abs/2609.21617)
*Jiarong Wei,Yin Wu,Runkai He,Abhinav Valada*

Main category: cs.RO

TL;DR: 该论文提出CounterPlay，一种反事实自博弈后训练方法，通过回溯失败任务并在不同驾驶风格下重试，用极少训练量显著提升驾驶策略闭环性能。


<details>
  <summary>Details</summary>
Motivation: 高吞吐仿真器中的自博弈能产生闭环性能稳健的驾驶策略，但随着训练规模扩大，单位仿真带来的改进逐渐递减。策略早期就学会处理常见情形，而后续大量 rollout 反复遭遇未解决的失败。现有后训练方法主要在已访问状态上评估替代动作或续接，但成功恢复驾驶往往需要更早地改变驾驶风格。

Method: CounterPlay包含三个关键组件：1）失败驱动回溯，利用策略的价值估计选择更早的存储状态来重试任务；2）奖励条件化，使单一策略能以从谨慎到激进的不同候选风格从该状态重试；3）验证机制，仅当重试中无其他车辆相对事实分支产生新的或更早的碰撞或越界事件时，才保留完成任务的重试。通过验证的新随机性重试随后在部署条件下蒸馏进策略。

Result: 在BehaviorBench上，CounterPlay仅用1B后训练迁移量，就在全部八种交通规则的Interactive和Random划分上取得最优分数，仅为基线100B自博弈训练预算的1%。相对基线的改进在三种评估驾驶风格下均成立。CounterPlay解决了基线在BehaviorBench上一大部分超时情况，并实现了任务完成与安全之间的平衡，这是持续自博弈或采用更激进驾驶风格都无法达到的。

Conclusion: CounterPlay通过反事实回溯与风格条件化重试，以极低训练成本有效解决驾驶策略的顽固失败案例，在性能与安全性间取得优于持续自博弈的平衡。
...
### [288] [Towards Fine-Grained Object Manipulation: SAM3-Guided Visuomotor Policy with Persistent Memory Learning and Focused Visual Conditioning](https://arxiv.org/abs/2609.21621)
*Haolong Meng,Fangbo Qin,Mengchen Bai,Houwu Wang,Cirong Liu,Shan Yu*

Main category: cs.RO

TL;DR: 提出 SAM3 引导的视觉运动框架，通过持久对象记忆与聚焦视觉条件实现细粒度物体操作。


<details>
  <summary>Details</summary>
Motivation: 细粒度物体操作要求机器人区分视觉相似的指定目标并可靠执行动作，但场景级视觉条件缺乏显式目标选择，类别级引导又难以区分同类细粒度物体。

Method: 提出 FOM-SAM3，在 SAM3 冻结条件下从有限多视角配准图像学习可复用记忆 token，通过 one-vs-rest 学习编码持久记忆以定位目标并排除相似干扰，并存入记忆库；另提出 FSAE，将目标内部局部外观特征与显式边界框坐标结合，用于条件化动作策略（DP、ACT）。

Result: 在包含四大类 30 个物理物体的 FO-30 数据集上验证 FOM-SAM3；在三个真实机器人细粒度操作任务中，策略对干扰物具有鲁棒性，可区分相似细粒度物体并扩展至新物体。

Conclusion: SAM3 引导的持久对象记忆与聚焦视觉条件能有效支持细粒度物体操作，兼顾抗干扰、相似物区分与泛化能力。
...
### [289] [RAYA: Learning Where and When to Intervene for Robot Recovery](https://arxiv.org/abs/2609.21690)
*Ishaan Mahajan,Charles Chen,Frederike Dümbgen,Brian Plancher*

Main category: cs.RO

TL;DR: 提出混合学习-解析控制框架RAYA，将学习到的有限时域可恢复性裕度嵌入带硬约束的最优控制器，并用有界学习调度器动态调整任务权重；在仿真与35g Crazyflie实机上均显著提升生存率并实现零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 机器人即使能预测失败也可能无法避免失败：安全机制反应时，标称规划已消耗掉恢复所需的控制权限，而固定的任务优先级又会阻碍剩余可用响应。作者认为这两个问题都应在控制器内部解决——可恢复性必须在动作选择时参与决策而非事后否决，任务目标应随可恢复性收缩而自适应调整。

Method: RAYA是一个混合学习-解析框架：在带硬约束的最优控制器中嵌入学习的有限时域可恢复性裕度（recoverability margin），使可恢复性在动作选择阶段即影响决策；并配以有界的学习调度器（bounded learned scheduler），动态移动任务权重以促进恢复。学习组件支持零样本迁移。

Result: 在四旋翼与自动驾驶车辆基准上，每个控制器进行7,200次仿真回合；RAYA不仅提升生存率，还将学习组件零样本迁移到未见过的轨迹、扰动、被控对象偏移与摩擦布局。嵌入式实现部署于35g Crazyflie四旋翼；在风扰加气动失配或未建模40%电机指令损失的40次硬件飞行中，三个基线全部失败，而RAYA在10/10次六周期任务中完成。

Conclusion: 可恢复性应作为动作选择的内在约束而非事后否决，任务优先级应随可恢复性自适应；将学习到的可恢复性裕度与有界任务权重调度嵌入硬约束最优控制，可在仿真和真实微型四旋翼上显著提升任务生存能力并具备强泛化性。项目网站：https://raya-control.github.io/。
...
### [290] [NeuRIO: A Streaming Neural Estimator for Zero-Shot Sim-to-Real Multi-Robot Relative Inertial Odometry](https://arxiv.org/abs/2609.21707)
*Zhehan Li,Jiadong Lu,Shengwei Ren,Chao Xu,Yanjun Cao*

Main category: cs.RO

TL;DR: NeuRIO：一种基于图神经网络的无锚点6自由度相对惯性里程计流式神经估计器，仅利用机器人间识别到的方位、距离和IMU测量，实现零样本仿真到现实迁移，并具有极强的计算可扩展性。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统需要无锚点、仅依赖相对观测（方位、距离、IMU）的6-DoF相对惯性里程计，且要求能适应不同团队规模和时变观测图，现有优化方法在大团队下计算开销过大。

Method: 将测量规范化到重力对齐坐标系；以机器人作为节点、相互观测作为因子，构建图网络；使用注意力进行空间推理、GRU进行时间建模；网络中共享节点级和因子级算子以支持不同团队规模与时变观测图；在耦合多种运动模式、设备级传感器特性及建模的时序持续传感器退化的模拟器上训练，从而实现零样本仿真到现实迁移。

Result: 在24条真实世界序列上达到14.1 cm位置RMSE与3.9°旋转RMSE；计算可扩展性强，仿真中400个机器人时更新成本仍低于20 ms，而优化方法在仅24个机器人时已超过20 ms；即使仅在有限团队规模上训练，也能无需改变架构或参数直接迁移到未见过的更大团队。

Conclusion: NeuRIO以图神经网络与流式估计实现了高精度、强可扩展、可零样本迁移的无锚点6-DoF相对惯性里程计，在精度和计算效率上均优于优化方法，并具备对大团队的泛化能力。
...
### [291] [AgenticSwarm: Semantic Perception and Adaptive Task Allocation for Heterogeneous Multi-UAV Missions](https://arxiv.org/abs/2609.21716)
*Muhammad Ahsan Mustafa,Yasheerah Yaqoot,Faryal Batool,Roohan Ahmed Khan,Valerii Serpiva,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: 提出AgenticSwarm智能体框架，实现异构多无人机任务的语义感知与自适应任务分配，在仿真与实测中验证其语义推理与约束分配的有效性。


<details>
  <summary>Details</summary>
Motivation: 复杂环境下的多无人机任务要求系统同时理解周围场景与人类操作员意图，并在任务条件变化时保持可行的任务分配，现有方法难以将语义理解与约束分配统一起来。

Method: 智能体解析航拍图像与自然语言指令，构建将感知对象/区域与任务需求、能力约束、任务依赖关联的接地任务表示；该表示增强带约束的任务分配过程，在分配前纳入障碍感知路径可行性、能耗与受保护返航要求。执行中，当出现无人机故障、电池衰减或任务变更时，基于当前系统状态进行残余任务重构，同时保留已完成工作与侦察进度。基于SAM3的感知流水线用于语义分割与标注。

Result: 在五个Gazebo环境和室内真实测试环境评估。与Grounding DINO+SAM 2.1基线相比，SAM3流水线将类别感知召回率提升25.2个百分点，语义标签准确率提升29.5个百分点。消融残余任务重规划后，平均重复工作从0%升至61.7%，事件后恢复时间增加58.6%。

Conclusion: AgenticSwarm能够将语义推理与约束分配相连接，实现自适应的多无人机任务执行；自适应重规划对减少重复工作、缩短恢复时间具有关键贡献。
...
### [292] [A Novel Path-Tracking Algorithm for Automated Tractor-Trailer Forward and Backward Maneuvers](https://arxiv.org/abs/2609.21718)
*Alexandre Lombard,Florent Perronnet,Nicolas Gaud,Abdeljalil Abbas-Turki*

Main category: cs.RO

TL;DR: 本文提出一种面向单挂车铰接式车辆的新型路径跟踪算法，通过在挂车层面施加横向控制律并结合牵引车转向角的短时域预测调整，实现前进与倒车工况下稳定收敛的路径跟踪；方法基于几何、无需逐车标定或训练，并在高保真物理仿真中匹配或优于经典与前沿方法。


<details>
  <summary>Details</summary>
Motivation: 全自主牵引车-挂车系统在物流、农业和工业场景中日益普及，对精确鲁棒的路径跟踪需求强烈。但牵引车与挂车之间的铰接引入额外非线性，显著增加横向与纵向控制难度，尤其是倒车机动。

Method: 提出面向单挂车铰接式车辆的路径跟踪算法：在挂车层面施加横向控制律，并对牵引车转向角进行短时域预测性调整；方法为纯几何式，无需逐车标定或训练。

Result: 在高保真物理仿真器中的仿真研究表明，该控制器在精度、稳定性及抗扰鲁棒性方面可匹配或优于经典方法和当前最优方法。

Conclusion: 该几何式控制方案可在前进与倒车中实现稳定收敛，且无需标定/训练，具备较好的通用性与鲁棒性。
...
### [293] [ZeroTouch: Tactile-Supervised Visual Contact Estimation for Contact-Rich Manipulation](https://arxiv.org/abs/2609.21726)
*Dmitriy Kosenkov,Daniia Zinniatullina,Miguel Altamirano Cabrera,Iana Zhura,Mikhail Derevianchenko,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: ZeroTouch 通过腕部 RGB、夹爪状态与局部重力方向，在训练期用触觉测量作特权监督，预测密集接触形变、瞬时六轴力/力矩及抓取相关的目标压缩量，部署时无需触觉硬件。完整模型将法向力 MAE 从 2.017 N 降至 0.531 N；物理实验中在未见物体、已见物体/未见抓取、内容/负载偏移下成功率分别为 95%、80%、90%，显著优于 OpenVLA 与 SmolVLA。


<details>
  <summary>Details</summary>
Motivation: 可靠抓取需要估计动态变化的物理交互并选择与抓取相关的压缩目标。触觉传感器能提供直接交互测量，但部署时需额外专用硬件，成本与集成难度高，因此需要一种无需部署触觉硬件、仍能获得触觉级交互估计的方法。

Method: 提出 ZeroTouch 触觉监督框架：输入为腕部 RGB 观测、夹爪状态和局部重力方向，输出为密集接触形变、瞬时六轴力/力矩以及抓取相关的期望压缩目标。触觉测量仅在训练阶段作为特权监督信号，部署阶段不使用。

Result: 在完整验证集上，完整架构将法向力 MAE 从仅状态基线的 2.017 N 降至 0.531 N。物理评估中每种条件 20 次试验：ZeroTouch 在未见物体上成功率 95%，已见物体/未见抓取为 80%，内容/负载偏移下为 90%；相同协议下 OpenVLA 分别为 25%、40%、55%，SmolVLA 分别为 10%、25%、35%。

Conclusion: ZeroTouch 表明，以触觉作为训练期特权监督，可在部署时仅依赖视觉与本体感知实现接近触觉水平的交互估计与压缩目标预测，并在多种泛化条件下显著超越现有 VLA 基线，验证了触觉监督视觉抓取框架的有效性与泛化能力。
...
### [294] [Visual Proactivity: Enhancing Human-Robot Collaboration Through Intent Communication](https://arxiv.org/abs/2609.21729)
*Valerio Bo,Edison Bejarano,Anaís Garrell,Alberto Sanfeliu*

Main category: cs.RO

TL;DR: 本文提出"视觉主动性"（visual proactivity）方法，让机器人通过视觉反馈传达自身意图，从而影响人类行为并提升交互的透明性与流畅度。


<details>
  <summary>Details</summary>
Motivation: 机器人正从执行重复任务转向与人协作，理解人类意图对有效交互至关重要。已有研究多关注机器人如何推断和响应人类意图，而较少关注机器人如何传达自身意图。

Method: 提出视觉主动性方法，使机器人通过视觉反馈传达意图；在人到机器人（human-to-robot）物品交接场景中开发并评估反应式、预期式和主动式三类机器人行为，并通过用户研究验证人类对这些行为的感知。

Result: 用户研究证实了人类对反应式、预期式和主动式行为的不同感知；有效的视觉主动性促进了更好的人机对齐与协调。

Conclusion: 视觉主动性是一种简单而有效的方式，可提升人机协作的透明性与流畅性，为更直观的人机协作奠定基础。
...
### [295] [When Should Robots Intervene? Balancing Engagement and Intrusiveness in Human-Robot Interaction](https://arxiv.org/abs/2609.21734)
*Lavinia Hriscu,Valerio Bo,Alberto Sanfeliu,Anaís Garrell*

Main category: cs.RO

TL;DR: 本文通过32人模拟医院任务实验，对比"持续寻求互动"与"情境感知选择性介入"两种机器人干预策略，发现更高互动频率并不带来更好参与度，感知支持与侵入性之间存在系统性权衡，有效参与依赖自适应、情境敏感的干预策略。


<details>
  <summary>Details</summary>
Motivation: 面向任务的HRI设计需要在用户参与度与可接受的社会性侵入水平之间取得平衡，但不同机器人介入策略如何影响用户体验、互动动态、感知侵入性与支持感尚缺乏实证依据。

Method: 提出并对比两种策略：（1）持续寻求参与的机器人策略；（2）基于用户状态与任务情境选择性介入的情境感知策略。两者均依赖多模态行为线索（身体朝向、注意信号）驱动机器人动作。在模拟医院环境中开展32名被试的用户研究进行评估。

Result: 更高的互动频率未必带来更好的参与：实验观察到感知支持与侵入性之间存在系统性权衡，并受物理距离、用户投入努力等因素影响。

Conclusion: HRI中的有效参与取决于自适应、情境敏感的干预策略，而非单纯提高介入频率。
...
### [296] [Sandwich-Residuals: Parameter-Efficient Test-time Adaptation of World Models](https://arxiv.org/abs/2609.21740)
*Krishnam Soni,Aditya Sehgal,Vedant Dave,Elmar Rueckert*

Main category: cs.RO

TL;DR: 提出Sandwich-Residuals：冻结预训练世界模型，仅在线学习预测器周围的小型残差修正，以极少参数量实现高效测试时自适应。


<details>
  <summary>Details</summary>
Motivation: 潜在世界模型虽能通过预测动作效果进行规划，但当测试条件偏离训练分布时预测会失效。现有测试时自适应方法需更新预训练模型的部分参数，涉及数百万参数更新，且需要人为选择适配的内部组件。

Method: Sandwich-Residuals 保持预训练世界模型完全冻结，仅在预测器周围学习小型残差修正；残差利用模型的自监督预测误差在线优化，无需奖励、标签或源域数据。

Result: 在 AdaJEPA 基准的 21 种条件下，成功率达到冻结模型的 1.3 倍，保留了最强 AdaJEPA 变体 95% 的性能，同时少适配 97–99% 的参数；在复合偏移下提升至冻结模型的 1.9 倍，与内部块适配相当。并在 DINO-WM 三维操作模型上验证了同样的适配原理。

Conclusion: 世界模型的有效测试时自适并不必然需要修改预训练内部权重，轻量残差修正即可在极少参数开销下取得有竞争力的性能。
...
### [297] [Understanding Engagement and Intrusiveness in Assistive Human-Robot Interaction Using Individual Traits](https://arxiv.org/abs/2609.21744)
*Valerio Bo,Lavinia Hriscu,Alberto Sanfeliu,Anaís Garrell*

Main category: cs.RO

TL;DR: 本文研究了机器人接近性与干预频率如何共同影响用户的投入感与侵扰感，并发现人格等个体差异在其中起关键调节作用。通过32名被试与两种不同干预策略辅助机器人的交互实验，结果表明整体投入感在条件间保持稳定，但情感反应与侵扰感知随人格特质显著变化，且人格对交互动态的影响取决于机器人行为。研究强调人机交互应针对个体差异定制机器人行为。


<details>
  <summary>Details</summary>
Motivation: 在陌生任务中，用户既需理解任务要求又需与机器人协作，因此机器人辅助尤为关键。然而，以往关于机器人空间关系（proxemics）对用户投入感影响的研究结论不一致：部分研究认为更近的接近性可增强交互，另一些则报告其可能令人感到侵扰。作者提出，侵扰感不仅取决于空间接近性，还取决于机器人干预的频率，并受人格与人口统计特征等个体特质的强烈影响，因此需要对此进行系统探究。

Method: 开展了一项实验，共32名参与者，与两种辅助机器人进行交互。两种机器人提供相似的任务支持，但在干预策略上存在差异。研究考察了空间接近性、干预频率与个体特质（如人格、人口统计特征）对用户投入感、情感反应与侵扰感知的影响。

Result: 总体投入感在各实验条件间保持稳定；但情感反应与感知到的侵扰程度随人格特质呈现显著差异。此外，人格对交互动态的影响因机器人行为的不同而有所差异。

Conclusion: 有效的人机交互应考虑个体差异，通过定制机器人行为来维持用户投入感，同时尊重每位用户独特的情感与行为特征。
...
### [298] [PSR: Predictive Sensorimotor Representation Learning for Contact-Rich Manipulation](https://arxiv.org/abs/2609.21753)
...
TL;DR: 提出预测性感觉运动表征（PSR）学习框架，通过多模态Transformer预测未来交互动力学，并将预测表征融入VLA策略的动作流，在六个真实接触密集型操作任务上实现91.7%成功率。


<details>
  <summary>Details</summary>
Motivation: 接触密集型操作需在视觉之外推理接触力、机器人构型与交互历史以生成精确动作。现有方法仅被动地以力反馈为条件，缺乏对未来接触动力学的主动预测，限制了高精度动作的生成能力。

Method: 提出PSR学习框架：预训练阶段用多模态Transformer联合预测未来交互动力学，学习分层的预测性表征；随后将学到的层级表征增强至动作流，使策略能在多个深度上利用接触相关线索。并将其实例化于视觉-语言-动作（VLA）模型，得到PSR-VLA。

Result: 在六个真实接触密集型操作任务上评估，PSR-VLA总体成功率达91.7%，分别较π0.5、ForceVLA-π0.5、ForceVLA2-π0.5提升30.0、22.5、19.2个百分点。

Conclusion: 结果验证了PSR在力感知、接触密集型操作中的有效性，表明主动预测未来接触动力学并融入动作流可显著提升策略精度。任务视频与稳定性测试见 https://psr-vla.pages.dev/。
...
### [299] [CRISP: Contact-Rich Robotic Simulation Platform with Extensive Geometries and Contact Solvers](https://arxiv.org/abs/2609.21761)
*Somang Lee,Sunkyung Park,Jinhee Yun,Seoki An,Dongjun Lee*

Main category: cs.RO

TL;DR: 本文提出 CRISP，一个面向复杂多接触场景（如紧公差机器人操作）的高保真物理仿真平台，结合多几何表示、基于优化的碰撞检测与增广拉格朗日接触求解器。


<details>
  <summary>Details</summary>
Motivation: 机器人仿真需要高物理保真度，要求既具备表达力强的几何与接触建模，又具备鲁棒的数值求解。现有仿真器在几何表示支持、接触建模简化程度或数值求解精度与鲁棒性上存在不足，尤其难以处理接触密集和尖锐交互。

Method: 开发新仿真器 CRISP：支持多样几何表示，采用基于优化的精确碰撞检测，并将接触建模与鲁棒的增广拉格朗日接触求解器相结合，从而在复杂几何上高效一致地获取接触信息，并在无病态松弛的情况下求解多接触约束。

Result: 将仿真器的物理保真度与当前先进平台进行对比验证，并通过复杂机器人操作场景展示其能力。

Conclusion: CRISP 通过几何表示、碰撞检测与接触求解的一体化设计，实现了复杂多接触仿真的高保真度，可用于接触密集的机器人操作任务，且已开源。
...
### [300] [Scaling Vision-Language Reward Learning for Robot Manipulation in Parallel Simulation](https://arxiv.org/abs/2609.21767)
*Lobna Joualy,Eric Demeester,Nikolaos Tsiogkas*

Main category: cs.RO

TL;DR: RAPID 系统通过 GPU 并行 rollout、数据感知策略更新、单请求偏好标注、自动奖励稳定化和代表性图像采样，将基于视觉语言模型的偏好奖励学习训练时间从 9.18 小时降至 1.15 小时，API 调用从 19,840 次降至 896 次，最终成功率从 86.3% 提升至 98.7%。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型可替代人工标注者进行基于偏好的奖励学习，但顺序 API 请求和单环境数据采集导致训练缓慢且成本高昂。作者旨在解决这一效率与成本瓶颈。

Method: 提出 RAPID（Reward learning with Adaptive Parallel Image Diversity）系统，耦合四项组件：GPU 并行 rollout 与数据感知策略更新、单请求偏好标注、自动奖励稳定化、代表性图像采样。在 IsaacLab 的五个 Franka Panda 操作任务上进行评估，并利用 Gemma 3 12B 与 GPT-4.1 mini 进行离线评估。

Result: 在匹配的两阶段提示下，并行 rollout 与自适应更新将平均运行时间从 9.18 小时降至 3.13 小时；启用全部 RAPID 组件后，训练在 1.15 小时内完成，每次运行的 API 调用从 19,840 次降至 896 次，聚合最终成功率从 86.3% 提升至 98.7%。实现 8.0 倍端到端加速和 95.5% 的 API 使用量削减。离线评估表明单请求提示在两个模型上均降低标注延迟与成本。

Conclusion: RAPID 通过并行化、数据感知更新、单请求标注、奖励稳定化与图像采样等协同设计，显著提升基于 VLM 的偏好奖励学习效率并降低成本，同时提高任务成功率。代码已开源。
...
### [301] [TRACE: Coverage Path Planning for Unknown Environments Using Hierarchical Coverage Tree](https://arxiv.org/abs/2609.21777)
*Zongyuan Shen,Haodong Liu,Gao Wang,Shancheng Zhao,Dehua Zhou,Yaming Ou,Zhongqiang Ren,Yikui Zhai,C. L. Philip Chen*

Main category: cs.RO

TL;DR: 提出在线覆盖路径规划算法TRACE，基于分层覆盖树实现未知环境实时覆盖，兼顾计算效率与覆盖完整性。


<details>
  <summary>Details</summary>
Motivation: 未知环境下的实时覆盖路径规划（CPP）需要在环境逐步揭示过程中高效规划路径。环境探索会因新发现的障碍物和被覆盖单元将剩余未覆盖空间分割成多个不连通区域，全局重规划计算负担大且难以保持覆盖进程的连续性。

Method: TRACE构建分层覆盖树，作为未覆盖空间连通性的全局表示。当新障碍或已覆盖单元导致未覆盖空间碎片化时，递归扩展相应树节点以显式表示并组织这些区域；基于更新后的树维护增量式全局巡回（global tour）指导覆盖过程，仅局部细化受影响部分并保留未变区域的访问顺序，避免全局重规划；局部规划器在全局巡回引导下生成往复（back-and-forth）覆盖路径，并在需要时切换到全局巡回感知规划以高效完成目标区域。

Result: 理论分析给出了TRACE的计算复杂度、完整覆盖性质，并推导出增量全局巡回细化的近似界。通过高保真仿真与移动机器人实机实验，与六种现有CPP方法对比，TRACE在覆盖时间、路径长度、重叠率和转弯次数方面均有显著改善。

Conclusion: TRACE通过分层覆盖树与增量全局巡回机制，实现了未知环境下高效、连续的在线覆盖路径规划，在多项性能指标上优于现有方法。
...
### [302] [Compact but Moving: Intervention-Relevant Geometry in Recurrent World Models](https://arxiv.org/abs/2609.21787)
*Yuming Chen,Yang Liu*

Main category: cs.RO

TL;DR: 在受控循环世界模型中，反事实速度干预的紧凑结构并非固定子空间，而是随自主推演移动到状态依赖的低维局部几何中，同时保留反事实预测功能。


<details>
  <summary>Details</summary>
Motivation: 探究学习到的世界模型在循环状态高维时，紧凑干预结构进入模型后会发生什么变化，特别是先验工作发现的检查点特定秩-4一次性反事实速度干预接口在模型内部如何演化。

Method: 在受控循环世界模型中进行实验：追踪干预方向在自主推演中离开固定入口子空间的过程；通过事实循环雅可比链传输入口方向获得低秩图像；比较独立训练的结构化GRU与参数匹配的LSTM；刻画有限时域未来响应算子在完整循环载体上的性质，并进行方向面板扰动排名与全幅度检验。

Result: 修正迅速离开固定入口子空间，但通过雅可比链传输得到的低秩图像仍能捕获大部分非线性修正；使用切空间预测修正重启可保留显著反事实未来功能；该传输/功能模式在多个模型上重复出现；补丁使未来敏感主方向向匹配的本征反事实组织移动，局部算子能准确排序扰动效应；全幅度检验发现事实端点切残差，并在三个检查点中的两个支持响应重构。

Conclusion: 紧凑干预结构可以作为嵌入高维循环动力学中的移动、状态依赖局部几何持续存在，而不意味着存在固定或动力学封闭的低维状态。
...
### [303] [AcousticDiffusion: Semantically Conditioned Audio-Guided Diffusion Policy for Search-and-Rescue Assistance](https://arxiv.org/abs/2609.21792)
*Iana Zhura,Didar Seyidov,Dmitrii Plotnikov,Hajira Amjad,Miguel Altamirano Cabrera,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: 本文提出 AcousticDiffusion——一种语义条件化、音频引导的扩散策略，用于面向人类呼叫者的救援机器人导航；它融合声源识别与麦克风阵列方位估计构建贝叶斯鸟瞰信念场，并由扩散模型生成航点轨迹，在仿真与四足机器人在线部署中均优于经典规划器。


<details>
  <summary>Details</summary>
Motivation: 救援机器人在视觉受阻或遮挡的环境中作业时，朝向人类呼叫者移动是一项重要能力，但视觉方法失效，需依赖声学信息；而声学定位存在不确定性（尤其是距离不确定性），需要将不确定的声学观测转化为可靠的导航决策。

Method: 采用冻结的预训练音频识别器处理 10.24 秒窗口，通过语音门控与求救优先机制将识别输出转化为声源级导航角色；将麦克风阵列到达方向（DoA）测量递归整合进以机器人为中心的贝叶斯鸟瞰信念场，并进行自我运动补偿以对齐连续观测，在约束声源位置的同时保留由方位引起的距离不确定性；以语义信念、近期声学观测、音频特征与机器人状态为条件，用扩散模型生成航点轨迹。

Result: 在基于录制音频的合成导航验证集上，平均终点方位误差为 11.20°，91.78% 的轨迹在呼叫者 30° 范围内；干扰源抑制率为 89.20%–98.99%，在 91.07% 的窗口中选择 HELP 标记呼叫者而非竞争说话人。无需额外重训练即可在线部署于 ZSL-1 四足机器人，平均方位误差 64.9°，优于 A* 的 98.2° 与 RRT 的 90.4°，平均规划计算时间 6.07 ms；平均最终声源距离由经典规划器（基于 ODAS 引导）的 3.96 m 降至 2.48 m，提升 37.4%。

Conclusion: 尽管声学定位并不完美，该框架能够将不确定的声学观测转化为更接近人类呼叫者的接近行为，验证了音频引导扩散策略在视觉退化场景下实现面向人类导航的可行性。
...
### [304] [Contact-Rich Motion Planning via GPU-Parallel Mode Evaluation](https://arxiv.org/abs/2609.21803)
*Jiayun Li,Georgia Chalvatzaki*

Main category: cs.RO

TL;DR: 提出 CoMET 方法，利用 GPU 并行轨迹评估配合贪心接触模式扩展，验证了在接触丰富运动规划（CRMP）中广泛显式评估接触模式序列的有效性。


<details>
  <summary>Details</summary>
Motivation: 接触丰富运动规划（CRMP）因接触决策的组合爆炸而计算困难。现有方法多通过搜索启发式或优化重构来回避对接触模式序列的广泛评估，本文重新审视这一做法，考虑现代 GPU 硬件带来的可能性。

Method: 提出 CoMET（Contact-Mode Expansion with parallel Trajectory optimization），将 GPU 并行轨迹评估与贪心接触模式扩展相结合，对接触模式序列进行广泛显式评估。

Result: 在平面推动基准上，CoMET 在解质量与规划时间上与基于优化、采样和树搜索的基线方法相当；在几乎所有实例上以更少的评估次数和更短的规划时间达到全枚举参考解的性能。消融实验表明性能提升主要来自高吞吐轨迹评估器。在双臂非抓取操作中，随着模式空间增大，GPU 友好的局部模式扩展成功率高于所测试的自适应树搜索。
...
### [305] [A Sim-to-Real Integration Pipeline for Training and Deployment of Chunk-Based VLA Manipulation Policies](https://arxiv.org/abs/2609.21817)
...
TL;DR: 提出开源sim-to-real实验协议：在仿真中生成专家轨迹，在真实Franka FR3上开环回放并采集真实视觉与本体感受观测，转换为VLA训练格式；同一部署栈闭环评估策略，同步量化sim-to-real差距，并开源数据与代码。


<details>
  <summary>Details</summary>
Motivation: VLA模型通常预测末端执行器位姿空间的动作块序列，其训练与评估依赖大规模真实世界示范数据；这类数据多靠人工遥操作采集，成本高、耗时长、难以扩展，构成数据瓶颈。

Method: 开源sim-to-real实验协议：仿真生成专家轨迹→在真实Franka FR3上开环回放→记录对应的真实视觉与本体感受观测→转换为兼容VLA训练的格式。数据采集与评估复用同一部署栈，评估时以闭环方式在该硬件配置上运行训练好的策略。

Result: 每条真实记录都与生成它的仿真轨迹配对，从而可直接测量sim-to-real差距；在Hugging Face发布采集的数据集，并公开流程源代码。

Conclusion: 该协议通过仿真生成、真机回放与同一部署栈复用，缓解VLA数据采集瓶颈，实现数据采集与评估硬件配置一致，并提供sim-to-real差距的直接度量；数据集与代码开源以促进复现与扩展。
...
### [306] [LunaDrive: A Delay-Compensated High-Voltage GaN FET-Based Motor Driver for Dynamic Robots with Flat BLDC Motors](https://arxiv.org/abs/2609.21818)
*Sota Yuzaki,Temma Suzuki,Hiromi Tada,Masanori Konishi,Kento Kawaharazuka,Kei Okada*

Main category: cs.RO

TL;DR: 本文开发了一款面向高功率扁平BLDC电机的GaN FET驱动器，支持96 V、30 A连续电流与80 A峰值电流，最高电频率3110 Hz，并验证了超额定转速下的延迟补偿效果及动态机器人应用可行性。


<details>
  <summary>Details</summary>
Motivation: 动态机器人需要快速能量输出，但商用伺服电机多限于48 V及以下电压，限制了最高转速；同时Si MOSFET驱动器难以兼顾高压与大电流，而可安装于扁平BLDC电机背侧的紧凑型GaN驱动器仍较缺乏。因此需要超额定电压驱动电机以提升瞬时最高转速。

Method: 采用GaN FET构建适用于高功率扁平BLDC电机的紧凑型电机驱动器，并研究在超出额定工作范围的高速区域中的延迟补偿方法。

Result: 在96 V工作条件下，加装散热器时实现30 A连续电流；确认80 A峰值电流和3110 Hz最高电频率。还进行了由24S LiPo电池（100 V）驱动的高速负载提升实验。

Conclusion: 所开发的GaN FET驱动器可在超额定电压与高速条件下稳定运行，延迟补偿有效提升高速区性能，并展示了对动态机器人运行的适用性。
...
### [307] [PopNavShift: Stress-Testing Social Navigation under Behavioral Population Shift](https://arxiv.org/abs/2609.21838)
*Kaizhen Tan,Diyu Zheng,Tim Guangyu Wu,ChengHe Guan*

Main category: cs.RO

TL;DR: 提出PopNavShift仿真框架，用大模型生成人群条件的行人运动画像，对三种社交导航策略在人群分布偏移下进行压力测试，发现评价指标（尤其行人负担）会导致控制器排名反转。


<details>
  <summary>Details</summary>
Motivation: 现有社交导航算法通常在固定行人行为分布下评估，忽略了个体与社会情境导致的行人反应差异；这种单一分布假设可能使策略排名不可靠，因此需要在人群分布偏移下系统检验策略稳健性。

Method: PopNavShift：用MatrAIx Persona 1M中的600条合成persona记录提示Gemini 3.7 Flash，得到人群条件下的行人运动画像，并将回答确定性映射为有界运动参数；在8种人群条件下对反应式避障、提前让行、互惠避碰三种策略进行7,488次匹配机器人运行对比；另对同一202个persona做仅改变时间压力的匹配干预，并在第二种行人动力学模型下复现。

Result: 同一202个persona的匹配干预中，仅改变时间压力就使基于机器人通行时间的控制器排名反转8.6%，基于平均行人延误反转22.4%，基于最差十分位延误反转23.9%；跨人群条件时，这种敏感性对行人负担大于对机器人通行时间，且在空间受限场景中更强；定性模式在第二种行人动力学模型下依然成立。

Conclusion: 社交导航策略评估应跨越多种行为人群，并同时采用机器人性能与行人负担两类指标，否则单一人群分布下的排名结论可能不稳健。
...
### [308] [VIRGA: Virtual-Agent-Intermediated Riemannian Geometry for Active-Sensing Air-Ground Coordination](https://arxiv.org/abs/2609.21883)
*Fenghe Guo,Runjie Shen,Chenyang Sun,Junrui Zhang*

Main category: cs.RO

TL;DR: 本文提出VIRGA框架，将双LiDAR观测转化为有界黎曼场，通过虚拟智能体与互惠弹性反馈实现空地协同，在仓库与洞穴场景中安全完成观测约束下的协同任务。


<details>
  <summary>Details</summary>
Motivation: 空地自主协同在UAV需持续被UGV搭载的云台LiDAR观测时难度显著上升：平台需在同一闭环内规避动态障碍，同时协调异构运动、有限感知与变化的任务主导权。

Method: VIRGA将双LiDAR观测转化为有界、来源特定的黎曼场，并通过一个虚拟智能体以互惠弹性反馈进行耦合；再经平台感知执行映射，将共享协同参考转为可行的UAV、UGV与云台指令，并施加主动观测安全约束。

Result: 对比三类基线：Ray-RMP响应最快但间隙不足；稠密解析黎曼场避障更好但计算代价高，难以稳定维持视场；ColAG延迟最低但仍出现安全与可观测性违规。VIRGA在全部成对仓库条件下安全完成，并在未重训练的远距离洞穴压力测试中保持协同。消融验证了在线几何评估、虚拟智能体中介与互惠反馈的贡献。

Conclusion: VIRGA通过黎曼场与虚拟智能体中介，在观测约束下实现了安全、可泛化的空地协同，消融证明各关键组件均有实质贡献。
...
### [309] [CommitFlow: Semantic Commitment Verification and Local Correction for Long-Horizon Robot Manipulation VLA Execution](https://arxiv.org/abs/2609.21908)
*Zixiang Zhao,Yansong Feng,Yang Yang,Chaoyu Wang,Haoran Xiao,Hui Zhang,Chuang Cheng,Jianjun Ma*

Main category: cs.RO

TL;DR: CommitFlow：一种结合语义承诺监控与局部修正的闭环执行框架，冻结基座策略即可在RoboTwin 2.0十项任务上平均成功率75.9%，较pi0.5提升22.7%。


<details>
  <summary>Details</summary>
Motivation: VLA策略虽发展迅速，但长时程执行中常出现"语义承诺—物理条件—实际物理状态"三者失配：在所需物理效果尚未建立或维持前，就推进到下一任务阶段。由于单一动作指令无法确认物理条件是否达成，局部偏差会逐步传播并最终导致任务失败。因此需要一种在保持基座策略不变的前提下，对承诺条件进行监控并对局部偏差进行修正的机制。

Method: CommitFlow为闭环执行框架，包含三个组件：(1) Semantic Commitment Monitor（SCM）将阶段需求与当前状态证据进行比对，当所需条件未满足或被破坏时，暂缓依赖该条件的动作；(2) BoundaryFlow以当前状态和基座动作为条件，生成局部修正；(3) Relation and Gain Calibration（RGC）在满足相关约束的前提下，选择最小的修正强度。整个框架不更新基座策略参数。

Result: 在RoboTwin 2.0的十项常见基准任务上，CommitFlow平均成功率达75.9%，相比基座策略pi0.5提升22.7个百分点；跨策略实验也显示出一致的性能增益。

Conclusion: 通过将语义承诺监控与可标定的局部修正结合，CommitFlow在不重新训练基座策略的情况下显著缓解了长时程执行中的语义-物理失配问题，为可靠的长时程机器人执行提供了可行路径。
...
### [310] [MAAP: Multi-Agent Active Perception for Collaborative Manipulation](https://arxiv.org/abs/2609.21929)
*Bruno N. Y. Chen,Li Kang,Heng Zhou,Xiufeng Song,Zhemeng Zhang,Jiahua Ma,Yiran Qin*

Main category: cs.RO

TL;DR: 提出MAAP框架与RAIL控制器，将多臂协作机械臂的腕部相机作为主动感知视角，显著提升操作成功率。


<details>
  <summary>Details</summary>
Motivation: 多智能体操作中每个机械臂都携带腕部相机并在执行任务时移动，天然产生多个任务驱动的视角，但这些观测通常未被充分利用；主动感知在操作领域仍常被视为需要一个专用感知智能体。作者希望证明协作操作本身即可作为一种主动感知机制。

Method: 提出MAAP（Multi-Agent Active Perception），让每个机械臂双重用途：既执行操作动作，又通过其腕部相机作为团队的移动视角；并配以RAIL（Role-Aware Imitation Learning）控制器，预测每个机械臂当前角色及其动作块，并以角色为条件生成动作，从而在单一网络中表示角色相关的动作。

Result: 在四个仿真任务中，感知范围扩大使平均成功率从固定相机的56.5%提升至单腕视角的62.5%，全腕视角达70.0%，MAAP+RAIL达79.2%。RAIL的额外增益集中在三臂Microwave任务上，在相同多腕输入下成功率从47%升至82%。在双臂平台上，MAAP+RAIL在20次放置试验中成功14次，而固定视角ACT为0次。

Conclusion: 协作操作本身即可作为一种主动感知机制，多臂腕部视角的有效利用与角色感知的动作生成能显著提升操作性能。
...
### [311] [CARF: Contrastive Attraction-Repulsion of Failure-Guided Flow Matching](https://arxiv.org/abs/2609.21982)
*Shuqi Zhao,Bang Du,Cheng-En Wu,Yichen Xie,Yixiao Wang,Masayoshi Tomizuka*

Main category: cs.RO

TL;DR: 提出CARF框架，通过对失败轨迹中"进步片段"与"失败关键片段"施加吸引-排斥的非对称监督，更全面地利用不完美机器人数据。


<details>
  <summary>Details</summary>
Motivation: 机器人演示采集常混入失败轨迹。现有方法多只提取失败轨迹中仍有进展的片段进行模仿，忽视了直接导致任务失败的"失败关键行为"。作者认为这两类片段提供本质不对称的监督信号：进步片段应被模仿，失败关键片段应被显式规避。

Method: CARF引入基于进展的重要性打分器，仅用成功专家演示及其扰动结果训练，估计每步对任务完成的贡献并识别失败轨迹中的信息区域；这些分数指导统一的流匹配目标，使策略被吸引向进步行为、被排斥远离失败关键行为，同时排除模糊片段。

Result: 在仿真与真实世界多种失败场景下，CARF持续优于对比基线；消融实验验证了所提打分机制与吸引-排斥机制的有效性。

Conclusion: 对失败轨迹中的进步片段与失败关键片段进行非对称（吸引/排斥）监督，能更全面利用不完美数据，并避免模糊失败片段带来的不可靠监督。
...
### [312] [SkelWAM: A Skeleton-Guided World-Action Model for Zero-Shot Cross-Embodiment Manipulation](https://arxiv.org/abs/2609.21983)
*Pengjun Niu,Yujia Xie,Rui Peng,Hang Zhao,Ke Liu*

Main category: cs.RO

TL;DR: 本文提出 SkelWAM，一种以骨架几何为共享表征的世界-动作模型，实现单源跨形态机器人操作迁移，无需目标域演示或策略微调。


<details>
  <summary>Details</summary>
Motivation: 跨机器人形态复用操作经验可显著降低重复的任务特定数据采集成本，但形态差异会改变视觉外观、动作维度与语义，以及实现同一工具位姿所需的整体构型。

Method: 构建由手臂中心线几何、工具中心点（TCP）位姿与平行夹爪指令组成的共享25维状态，统一用于规范化的第三人称与腕部观测以及未来全身动作目标；采用含视频-动作混合的 Transformer 在预测性视觉监督下预测规范化骨架动作序列，再由形态特定的约束解码器转换为关节或连续体机器人控制指令，无需关节一一对应。

Result: 提出 LIBERO-Cross10 单源跨形态迁移基准（含十项任务、四个形态组下的十个目标形态）；Franka 训练的策略在该基准1000个回合中取得43.3%成功率，较最佳评估基线高出36.2个百分点；并将 JAKA mini2 训练的策略部署至 Feagine A03 连续体机器人完成三项桌面操作任务。

Conclusion: 显式几何骨架表征可作为感知与控制的耦合接口，在无目标任务演示与目标策略更新的条件下实现有效跨形态迁移，并具备真实世界应用潜力。
...
### [313] [Gripper-Aware Automatic Dense Packing of Irregular Objects](https://arxiv.org/abs/2609.22062)
*Tianhao Qin,Connor McCann,Berk Calli,Jing Xiao*

Main category: cs.RO

TL;DR: 本文提出一套面向真实机械臂的闭环密集堆叠流水线，将感知、考虑夹爪的放置优化与力引导执行集成，在Franka Panda上实现不规则物体的自动密集装箱。


<details>
  <summary>Details</summary>
Motivation: 仓库作业对自动密集堆叠需求强烈，但现有不规则物体堆叠研究多局限于理想接触的仿真，将物体视为孤立刚体，夹爪仅作事后可行性检查，且未处理执行过程中的感知与接触漂移，导致难以落地于真实机器人。

Method: 方法为闭环流水线：优化器将物体与夹爪建模为分层球树构成的单一复合体，在GPU上以CMA-ES搜索五自由度位姿，竖直坐标依据当前高度图解析确定；执行时采用力监测垂直下降，首次接触即停止；释放后通过整理推挤消除夹爪感知规划残留的横向间隙；每次放置后重新感知容器以避免漂移累积。

Result: 在Franka Emika Panda上对3D打印的平面、曲面与凹面物体集合及YCB物体子集进行验证；消融实验分离了夹爪感知优化、整理推挤与网格几何对端到端成功率、达到密度和计算代价的贡献；并与高度图最小化方法作为代表性基线进行对比。

Conclusion: 该系统证明闭环集成感知、夹爪感知规划与力控执行可有效提升真实场景下不规则物体密集堆叠的成功率与密度，且各组件贡献可量化。
...
### [314] [Duty Factor Predicts Robust Constrained Quadrupedal Locomotion Across Gait Types](https://arxiv.org/abs/2609.22073)
...
TL;DR: 本文通过三种不同的四足机器人运动控制方法（全身轨迹优化+LQR反馈、学习型控制器、质心模型预测控制），研究步态参数（特别是占空比）与运动鲁棒性的关系，证明占空比比名义步态类型更能预测局部误差收敛，是理解和选择鲁棒四足运动的简单有效基础。


<details>
  <summary>Details</summary>
Motivation: 四足机器人越来越多地应用于需要抵抗扰动和适应受限地形的环境。步态类型（如行走或小跑）通常被用来表征四足运动，但步态类型并不能唯一确定运动特性，因为占空比、速度、站立宽度等参数可在同一步态类型内变化。因此需要探究这些步态参数之间的关系，寻找更有效的鲁棒性表征指标。

Method: 采用三种不同的四足运动控制方法：(1) 全身轨迹优化结合LQR反馈控制；(2) 学习型运动控制器，研究占空比选择；(3) 质心模型预测控制（centroidal MPC）框架。并在物理四足机器人上进行窄地形实验验证。

Result: 在三种控制方法中均发现一致趋势：占空比是比名义步态类型更强的局部误差收敛预测因子；占空比可作为适应窄地形环境中运动鲁棒性的低维参数；质心MPC框架下趋势依然成立，并通过物理四足机器人窄地形实验得到验证。
...
### [315] [LIMBO: Learning and Internalizing Model-Free Barrier Objectives for Agile and Safe Whole-Body Control](https://arxiv.org/abs/2609.22075)
*Jake Gonzales,Arturo Flores Alvarez,Yu-Ming Chen,Aaron D. Ames,Lillian J. Ratliff,Manikantan Nambi*

Main category: cs.RO

TL;DR: LIMBO 从黑盒转移与失败规范中学习状态-动作 Q-CBF，并将其蒸馏进任务策略，在29自由度人形机器人上实现无在线安全滤波器的全身敏捷控制。


<details>
  <summary>Details</summary>
Motivation: 全身控制需在高维非线性动力学下同时协调避障与平衡，安全证书难以设计与跨行为复用；而 Q-CBF 在完整控制维度上合成通常不可行。

Method: 在冻结基控制器周围的残差动作空间上，利用黑盒转移和基于状态的失败规范学习状态-动作控制屏障函数（Q-CBF）；合成时用学到的安全价值驱动风险引导采样，聚焦于可恢复性边界附近；任务学习阶段将其作为教师提供动作级安全反馈。

Result: 在29自由度人形机器人上完成躲避球规避与低矮障碍下行走；在相同安全规范下改变采样集中度，可产生从下蹲到后仰 limbo 式动作的不同策略；策略无需在线安全滤波即可迁移到硬件。

Conclusion: 学习型安全综合可扩展到敏捷全身控制，风险引导边界采样为探索可恢复性边缘提供了有理论依据的方法。
...
### [316] [SeeQ: Training Generalist Value Functions for Long-Horizon Robotic Manipulation](https://arxiv.org/abs/2609.22085)
*Saksham Singh,Zheyuan Hu,Max Sobol Mark,Jeffrey Yu,Zackory Erickson,Aviral Kumar*

Main category: cs.RO

TL;DR: SeeQ提出为当前活跃子任务学习Q值函数，将长时程信用分配缩短为子任务级，并通过自回归预测自然语言子任务实现无需人工标注的测试，在真实双臂机器人任务上显著提升策略引导效果。


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略在复杂长时程任务上表现脆弱：这类任务包含多个阶段，或需在同一子阶段反复尝试与推理。Q值函数可通过排序候选动作或引导策略改进来缓解该问题，但从稀疏任务级奖励学习面临信用分配时程长、Bellman备份困难、数据覆盖要求高等挑战。

Method: 提出SeeQ（Subtask-elicited Q-functions），为当前活跃子任务学习Q值，从而缩短价值预测时程并可用时序差分（TD）目标有效学习。训练时利用离线机器人数据中的子任务级标注提供分解，支持从广泛且可能次优的数据中学习。为免除测试时的人工标注或模块化子任务预测系统，Q函数架构被训练为先以自然语言自回归预测活跃子任务，再估计其价值。基于视觉-语言骨干实例化，在多样开源机器人操作数据上预训练，并在下游任务上微调。

Result: 在两个双臂机器人平台的四个真实世界操作任务上，SeeQ价值函数显著提升了best-of-N策略引导的效果。

Conclusion: 为子任务而非整任务学习Q值可有效缩短信用分配时程，结合自回归子任务语言预测可去除测试时的人工标注依赖，为长时程机器人操作提供有效的价值引导方案。
...
### [317] [The Weight Is Over - Interactive Diffusion on Consumer GPUs](https://arxiv.org/abs/2609.21849)
*Frieder Ganz,Maximilian Müller*

Main category: cs.LG

TL;DR: 本文探讨扩散模型在端侧推理中的性能、质量与模型体积权衡，提出三项贡献：用嵌入翻译器将小文本编码器映射到大编码器空间以降低权重与延迟；给出可复现的扫描配方以导航速度/质量/内存三角；并实现交互式端侧图像生成编辑器，在近期GPU上达到亚秒级TTFI。


<details>
  <summary>Details</summary>
Motivation: 端侧推理发展迅速但主要集中在语言模型；扩散流水线内存需求大、对延迟敏感，且需编排嵌入器、Transformer、解码器及后处理，流程标准化程度不及LLM推理循环。目标是覆盖尽可能多的客户端设备，需在性能、质量与模型体积间权衡。

Method: 提出嵌入翻译器（embedding translator），将小型文本编码器映射至大型编码器空间以减少权重和延迟；设计可复现的扫描配方（sweep recipe），用于在扩散流水线中导航速度/质量/内存三角；构建交互式端侧图像生成编辑器。

Result: 在近期GPU上实现亚秒级TTFI（time-to-first-image），并可在速度、质量与内存之间进行可复现的权衡探索。

Conclusion: 通过嵌入翻译与系统化扫描方法，可在端侧扩散流水线中有效平衡速度、质量与内存，实现面向广泛客户端设备的低延迟交互式图像生成。
...
### [318] [ResNLS: An Improved Model for Stock Price Forecasting](https://arxiv.org/abs/2312.01020)
*Yuanzhe Jia,Ali Anaissi,Basem Suleiman*

Main category: cs.LG

TL;DR: 提出ResNLS混合模型，用ResNet提取相邻股价依赖作为残差、LSTM处理原始时序，取前5个交易日收盘价为输入的ResNLS-5表现最优，较SOTA基线至少提升20%，并经量化回测验证可在跌势中减亏、涨势中获利。


<details>
  <summary>Details</summary>
Motivation: 股价预测长期是难题，已有研究多忽视股价之间不同程度依赖关系的重要性，本文旨在通过强调相邻股价间的依赖来提升预测精度。

Method: ResNLS由ResNet与LSTM两种神经架构组成：ResNet作为特征提取器识别股价间依赖关系，该依赖以残差形式与原始时间序列数据结合，再由LSTM进行建模分析；同时构建量化交易框架用于策略回测。

Result: 以连续5个交易日收盘价为输入时（ResNLS-5）性能最优；ResNLS-5较当前最先进基线至少提升20%；回测显示基于其预测的交易策略能在股价下跌阶段成功减少损失，并在上涨阶段产生收益。

Conclusion: 显式建模相邻股价依赖的ResNLS-5在预测精度与实盘策略收益两方面均优于现有基线，可为投资者规避风险与获取利润提供支持。
...
### [319] [A Hybrid Computational Intelligence Framework for scRNA-seq Imputation: Integrating scRecover and Random Forests](https://arxiv.org/abs/2511.16923)
*Ali Anaissi,Deshao Liu,Yuanzhe Jia,Weidong Huang,Widad Alyassine,Junaid Akram*

Main category: cs.LG

TL;DR: 提出SCR-MF——一个两阶段模块化工作流，先用scRecover进行有原则的dropout检测，再用missForest进行稳健的非参数插补，在多数情况下性能可比或优于现有方法，并在精度与效率间取得平衡。


<details>
  <summary>Details</summary>
Motivation: scRNA-seq可实现细胞分辨率的转录组分析，但普遍存在dropout事件，掩盖了真实生物学信号。现有插补方法在准确性、可解释性、生物保真度与计算效率之间难以兼顾。

Method: SCR-MF采用两阶段模块化流程：第一阶段用scRecover进行dropout检测（识别真正需要插补的零值），第二阶段对检测到的dropout用missForest进行稳健的非参数插补。

Result: 在公开数据集与模拟数据集上，SCR-MF在多数情况下取得稳健且可解释的性能，可比或超过现有插补方法；同时保持生物学保真度与流程透明性。运行时间分析表明其在精度与计算效率间具有竞争力的平衡，适用于中等规模单细胞数据集。

Conclusion: SCR-MF通过“检测+插补”的两阶段设计，提供了稳健、可解释、保真且高效的单细胞dropout插补方案，适合中等规模scRNA-seq数据的下游分析。
...
### [320] [Making Latent Evolution Explicit: Operator-Structured Transitions for World Action Models](https://arxiv.org/abs/2608.27259)
*Xiaoxiao Lu,Yunlong Dong,Jiahao Shi,Ye Yuan*

Main category: cs.LG

TL;DR: 本文将潜空间世界动作模型中的"潜状态转移实现方式"视为独立于预测表征与预测-策略耦合的架构选择，提出基于算子的潜演化网络LEON，用上下文调制的算子传播加可加驱动力建模潜演化，在受控动力学系统与两种WAM框架上验证了该演化归纳偏置的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型（WAMs）日益在潜表征空间预测任务相关场景状态的演化，以避免全外观生成并保留控制相关信息。但潜转移普遍由Transformer预测器实现，其归纳结构以token交互为中心，而非时间演化本身。作者认为"转移实现方式"是与预测表征、预测-策略耦合相区分的独立架构选择，且缺乏专门针对演化结构的归纳偏置。

Method: 提出潜演化算子网络LEON。以受控Koopman生成元视角为理论基础，在学习到的可观空间中对潜演化建模：采用上下文调制的算子式传播刻画围绕共享演化算子结构的上下文相关转移变化，并保留一条用于可加变化的互补路径（additive forcing）。

Result: 在受控动力学系统上验证了LEON所具有的演化专用归纳偏置，以及算子传播与驱动力二者的互补作用；在两种以不同方式将潜预测接入策略的WAM形式下，LEON提升了闭环性能与鲁棒性，且在完全替换转移模块时仍然有效。

Conclusion: 转移实现方式（transition realization）是潜空间世界动作模型中一个具有实质影响的架构选择，值得作为与表征学习、策略耦合并列的独立设计维度加以研究。
...
### [321] [BI-Agent and BI-Bench: Towards Automating End-to-End Business Intelligence](https://arxiv.org/abs/2609.20886)
*Chuxuan Hu,Yeye He,Penny Zhou,Wee Hyong Tok,Daniel Kang,Surajit Chaudhuri*

Main category: cs.LG

TL;DR: 该论文构建了首个端到端商业智能（BI）基准BI-Bench，发现前沿LLM准确率不足50%；提出工具增强的BI-Agent及后训练框架，实现显著准确率提升。


<details>
  <summary>Details</summary>
Motivation: 传统BI工作流需用户手动完成识别相关表、数据变换、构建连接关系等繁琐准备步骤，耗时且复杂。鉴于LLM在数据处理上的强能力，作者希望研究其能否端到端回答BI问题，从而免去手动准备环节。

Method: 1) 从公开来源采集大量真实BI项目，并从真实用户仪表盘中人工抽取（问题，真值答案）对，构建BI-Bench基准；2) 设计工具增强的BI-Agent，将BI工作流分解为结构化数据上的子任务（搜索、连接、变换），并编排各BI阶段的数据管理方法；3) 开发后训练框架，从真实BI项目合成训练轨迹，采用监督微调（SFT）与强化学习（RL）对BI-Agent进行后训练。

Result: 前沿LLM在BI-Bench上表现不佳，准确率低于50%；BI-Agent相较原始LLM最高提升40个百分点；经后训练的BI-Agent进一步提升最高30个百分点。

Conclusion: 在复杂BI工作流中，将工具增强推理与领域特定后训练相结合至关重要，并为未来研究指明了有前景的方向。
...
### [322] [Bio-MF: Low-Latency and High-Fidelity EEG-to-fNIRS Cross-Modal Generation for Hybrid Motor-Imagery Brain--Computer Interfaces](https://arxiv.org/abs/2609.20904)
*Boyuan Zhao,Sifan Zhang,Luping Chen*

Main category: cs.LG

TL;DR: 本文提出 Bio-MF，一种无潜空间、单步 MeanFlow 框架，用于 EEG 条件生成 fNIRS 信号，兼顾低延迟生成与任务相关血流动力学保真度，在混合运动想象 BCI 解码中显著优于纯 EEG 基线。


<details>
  <summary>Details</summary>
Motivation: 混合 MI-BCI 结合 EEG 与 fNIRS 可优于纯 EEG 系统，但配对采集不便。现有 EEG-to-fNIRS 跨模态生成方法生成慢且常需预训练，难以满足实时 MI-BCI 需求；一步生成模型虽低延迟，却缺乏迭代细化，易降低保真度并引入非生理伪影。

Method: 提出 Bio-MF：无潜空间单步 MeanFlow 框架，直接进行信号空间 x-prediction，并将其转化为 MeanFlow 速度监督，推理仅需一次网络评估。为在异构传感器布局下保留任务相关血流动力学结构，集成时空交互 4D 编码、跨模态无分类器引导及噪声级门控 FFT 正则化。

Result: Dataset 1 上 EEG+合成 fNIRS 相比纯 EEG，HbR 与 HbO 的 ACC 分别提升 3.37 与 4.15 个百分点；Dataset 2 在未见过的 64 通道 EEG 蒙太奇下提升 2.98 与 2.50 个百分点。RTX PRO 6000 GPU 上单试次生成 7.0 ms，相比 1000 步 SCDM 延迟实现 857 倍加速。

Conclusion: Bio-MF 实现了快速 EEG-to-fNIRS 合成，同时保持面向下游混合 MI 解码的任务相关生成质量，验证了单步 MeanFlow 在跨模态生理信号生成中的有效性。
...
### [323] [Scaling Discovery through Test-Time Communication](https://arxiv.org/abs/2609.21032)
*Jongho Park,Vasilis Kontonis,Shivam Garg,Akshay Krishnamurthy,Dimitris Papailiopoulos*

Main category: cs.LG

TL;DR: 测试时通信的多智能体团队显著优于独立并行尝试，优势随规模增长并可迁移至研究型任务，但需充足算力与清晰反馈。


<details>
  <summary>Details</summary>
Motivation: 科学进步依赖协作，但现有智能体系统鲜有体现；通信智能体是否有益此前结论不一，尚属开放问题。

Method: 研究无预设角色、通过共享目录通信的k个智能体团队（team@k）在ARC-AGI-3上的测试时通信扩展效应；并迁移至polyomino packing与MNIST分类器压缩等研究型任务。

Result: team@k匹配4k个独立智能体的成功率且优势随k增长；单智能体无法解决的任务团队可稳定解决。polyomino packing超过best@k及此前最优分数；MNIST压缩中四人团队以1,957字节达到99.4%测试精度，优于已知人类最优与最佳单智能体结果。算力受限或无清晰进展度量时独立智能体可能更优。

Conclusion: 测试时通信可带来复合增益且能迁移到研究型任务，但收益非无条件：需充足算力与清晰反馈时，多智能体通信才稳定取得更强结果。
...
### [324] [EnSol: an environment-aware graph neural network for molecular solubility prediction](https://arxiv.org/abs/2609.21151)
*Thao Nguyen,Saman Shafaei,Zhengyi Zhang,Huimin Zhao,Heng Ji*

Main category: cs.LG

TL;DR: EnSol：一种环境感知的概率框架，用分子图+交叉注意力建模溶质-溶剂相互作用，并通过特征调制注入温度、用混合密度网络预测完整溶解度分布；在SolProp和Leeds上Spearman相关分别为0.876和0.601，实验验证为0.715。


<details>
  <summary>Details</summary>
Motivation: 分子溶解度影响反应可行性、制剂性能、分离效率与溶剂选择等关键环节，但实验测量成本高、覆盖稀疏。现有计算模型多依赖固定溶剂假设、确定性建模或简化的溶质-溶剂相互作用表示，难以刻画复杂分子相互作用、连续温度效应及实验不确定性。

Method: 提出EnSol——环境感知的概率框架。将溶质与溶剂分别表示为分子图并学习各自独立表示，再通过交叉注意力融合以捕捉溶质-溶剂相互作用；温度经特征级调制直接注入溶剂环境表示；采用混合密度网络预测完整溶解度分布，以同时建模温度依赖行为与实验不确定性。

Result: 在独立的SolProp与Leeds基准上，EnSol的Spearman相关系数分别为0.876与0.601，在两个基准上均优于当前最先进的溶解度预测模型；在化学多样性溶质-溶剂对的实验验证中保持较强预测性能并支持可靠溶剂排序，Spearman相关系数为0.715。

Conclusion: EnSol可在多种化学体系中实现可靠的溶解度预测与溶剂选择，并能给出预测不确定性，为环境感知的概率化溶解度建模提供了有效方案。
...
### [325] [SWE-Proof: Can Language Models Resolve Real-World Issues with Machine-Checked Proofs?](https://arxiv.org/abs/2609.21190)
...
TL;DR: 提出 Benchproofer 流水线，将带正确补丁的编码任务转为形式化验证任务，构建 SWE-Proof（500 个真实 issue），揭示测试通过的补丁中大量存在反例，并把"忠实规格合成"定为核心开放问题。


<details>
  <summary>Details</summary>
Motivation: 现有 agentic 代码生成基准依赖留出测试集判断正确性，测试集本身不完整且易被记忆；形式化验证虽能规避这两个问题，但已有工作只覆盖规格作为输入给定的独立任务，无法处理涉及大型仓库、意图由模糊自然语言表述的真实 issue。

Method: Benchproofer 流水线：对已知正确补丁的编码任务，(1) 为新代码编写形式化规格；(2) 用公理概括被调用既有函数的行为；(3) 仅在机械门与对抗门一致同意后接纳该实例。据此将 SWE-bench Verified 转为 SWE-Proof，并扩展至 SWE-bench Pro。

Result: 在两种前沿模型上，验证捕获了测试遗漏的错误：25%–50% 通过测试的补丁存在反例；结构化自然语言规格无法解决该问题，而正确形式化规格将 Opus 4.8 的解决率从 85% 提升到 95%。自写规格的模型相对无辅助基线无增益，仅 62% 的规格通过审计；主要失败模式是忠实性不足（规格只约束部分必需行为）。规格质量与结果相关：未解决实例中 89% 规格失败，已解决实例中为 47%。

Conclusion: 测试通过不等于正确，形式化验证可暴露测试遗漏的反例并显著提升解决率；瓶颈在于模型自主编写忠实规格的能力，忠实规格合成仍是具体且迫切的开放问题。
...
### [326] [Deep Reinforcement Learning with Buffered Quantile Objectives](https://arxiv.org/abs/2609.21327)
*Mohammad Alipour-vaezi,Sajad Khodadadian*

Main category: cs.LG

TL;DR: 提出无模型分布式强化学习框架Deep-BQRL，将缓冲分位数学习扩展到神经函数逼近，直接由采样转移学习条件回报分位数并构建缓冲动作分数。


<details>
  <summary>Details</summary>
Motivation: 基于分位数的强化学习可优化累积回报分布的指定分位点，实现可解释的风险敏感决策，但点分位数目标存在两大难题：回报分布的微小扰动会导致分位数骤变；精确的分位数敏感规划需要计算开销大的分布式优化。下缓冲分位数通过对目标水平下方相邻分位数取平均缓解了前者，但现有方法仍为基于模型、依赖显式回报分布规划，难以扩展到小型表格问题之外。

Method: 提出Deep-BQRL，一种无模型分布式强化学习框架，将缓冲分位数学习推广到神经函数逼近。方法直接从采样转移中学习条件回报分位数，基于所学分位数函数的相关区域构造缓冲动作分数，并利用集成不一致性引导探索；同时采用增广输入表示，使所学策略能够响应轨迹信息，而无需显式复现精确规划所需的分位数-状态递归。

Result: 在资产出售最优停止问题和打滑FrozenLake上进行实验，与基于模型的UCB-BQRL以及表格型PPO、TRPO比较。在资产出售任务中，Deep-BQRL在报告的目标水平上取得比PPO和TRPO更小的平均累积点分位数策略差距，而UCB-BQRL的差距最小；所学停止决策随目标分位数变化，体现了方法的可解释风险敏感行为。

Conclusion: Deep-BQRL将缓冲分位数学习成功扩展到无模型神经函数逼近，在保持风险敏感可解释性的同时缓解了点分位数目标的非光滑性与计算难题；实验验证其在最优停止问题上的有效性，但相较基于模型的UCB-BQRL在策略差距上仍有差距。
...
TL;DR: 将冻结的时序基础模型与工程知识耦合，构建 KG-Chronos-2 代理模型，用于 HEC-RAS 水面高程预测，在固定基准上取得最低误差。


<details>
  <summary>Details</summary>
Motivation: 探究将时序基础模型与水利工程知识耦合，能否提升 HEC-RAS 水面高程（WSE）代理预报的精度。

Method: KG-Chronos-2 = 冻结的 Chronos-2 预测器 + 精确状态残差解码 + 图条件历史检索 + 输入对齐校正；对比对象包括持续性基线、残差 LSTM、项目条件循环 GeoFNO、DCRNN 类水力模型与冻结 Chronos-2。用 2008 模拟进行任务拟合；评估使用 2011 与 2002 模拟中 64 个固定 24 小时窗口，覆盖共享几何上 71 个河段的 4,675 个断面。

Result: 事件均衡 RMSE 为 0.246970（原生 WSE 单位）；较冻结 Chronos-2 降低 14.13%，较 DCRNN 类模型降低 29.38%，较循环 GeoFNO 降低 39.54%。与冻结 Chronos-2 的 RMSE 差 95% 层次自举区间为 [-0.075177, -0.016317]。同时取得六个完整系统中最低的活跃窗口与末端提前期 RMSE。

Conclusion: 在固定基准上，将冻结时序预测器与项目知识耦合，可有效支持 HEC-RAS 热启动预报。
...
### [328] [OneBid: A Unified Auto-Bidding Foundation Model for Diverse oCPX Advertising Scenarios](https://arxiv.org/abs/2609.21550)
*Yewen Li,Peng Jiang,Yitian Li,Pengfei Lv,Xialong Liu,Peng Jiang,Qingpeng Cai*

Main category: cs.LG

TL;DR: 本文提出OneBid，一种统一的自动出价基础模型，从异构oCPX日志中学习可复用骨干，并通过离线后训练适配各场景，在快手全量部署后实现oCPX广告整体+2.2% ADVV提升，ROAS场景峰值+13.1%。


<details>
  <summary>Details</summary>
Motivation: 现有自动出价方法（规则控制器、强化学习、Decision Transformer等）与主流oCPX范式日益不匹配：oCPX涵盖注册、购买等异构场景，且每场景单独建模，导致流水线碎片化、跨场景建模探索不足。受LLM等基础模型启发，作者希望将多个oCPX场景统一到一个模型中，但面临三大挑战：多目标控制、严格时延下的可扩展容量、安全离线策略改进。

Method: OneBid基于Decision Transformer，将单一Return-to-Go条件扩展为两个原子信号——衡量转化价值的Return-to-Go与衡量成本比的Cost-to-Go，并加入价值感知的下一动作预测正则。为吸收分布异构性，设计序列级Mixture-of-Experts架构：共享专家编码跨场景知识，稀疏路由专家以低时延捕获场景特有模式，实现模型规模与数据的一致扩展。后训练阶段采用Critic引导的相对离线策略优化（CROP）：学习到的critic对候选动作进行组内相对打分，避免GRPO式微调的在线探索风险，同时约束策略偏移以降低OOD风险。

Result: 通过在线A/B测试验证并在快手全量部署，OneBid在oCPX广告上实现整体+2.2%的ADVV提升，在ROAS场景中峰值达+13.1%。

Conclusion: OneBid验证了以基础模型范式统一异构oCPX自动出价场景的可行性：双信号条件、序列级MoE与安全离线策略优化三者结合，可同时满足多目标控制、低时延可扩展与安全离线改进，并在大规模真实系统中取得显著业务收益。
...
### [329] [On Repulsive and Attractive Teachers: Separating Correctness from Behavior in Self-Distillation](https://arxiv.org/abs/2609.21561)
*Anton Baumann,Akmal Ashirmatov,Leo Schmidt-Traub,Frederike Lübeck,Jonas Hübotter,Thomas Kleine Buening,Andreas Krause*

Main category: cs.LG

TL;DR: 该论文研究on-policy自蒸馏中特权信息引发的行为偏移问题，发现吸引式与排斥式自蒸馏会产生相反的行为变化，提出对比式自蒸馏（同时吸引正确教师、排斥错误教师）可使行为偏移相互抵消，从而保留更纯粹的token级正确性信号，在多种模型上提升推理性能并保持响应长度稳定。


<details>
  <summary>Details</summary>
Motivation: on-policy自蒸馏通过特权信息提供密集的token级监督，但特权信息不仅改变教师"知道什么"，还改变其"行为方式"，将正确性学习信号与意外的行为偏移纠缠在一起，导致学习信号不纯。

Method: 在推理任务中对比两种目标：吸引式自蒸馏（向特权教师靠近）与排斥式自蒸馏（远离特权教师）；进而研究对比式自蒸馏，即结合向正确解条件教师的吸引与向错误解条件教师的排斥，并剥离GRPO目标、单独隔离研究该自蒸馏目标本身的行为。

Result: 两种目标均引发强烈且相反的行为偏移：吸引抑制探索性推理、促使回答更短更自信；排斥增加响应长度、可能触发模型潜在思考模式的意外切换、最终变得不稳定。对比式蒸馏中两位教师共享的行为偏移大部分相互抵消，留下更直接反映正确性的token级信号；在非思考、仅指令、已思考三类模型上均提升推理性能，且响应长度保持稳定。

Conclusion: 对比式自蒸馏通过正确教师吸引与错误教师排斥的组合，使行为偏移相互抵消，从而获得更纯粹的正确性信号，可在不引入GRPO目标的情况下单独提升推理性能并维持稳定的响应长度。
...
### [330] [Matrix AdaGrad: Row-wise and Column-wise Adaptive Subgradient Methods](https://arxiv.org/abs/2609.21815)
*Wenpeng Zhang,Runsheng Yu,Peilin Zhao*

Main category: cs.LG

TL;DR: 本文提出一种面向矩阵参数的在线镜像下降（OMD）框架，通过行/列矩阵近端函数推导出Row-AdaGrad与Column-AdaGrad，理论证明其遗憾界在结构化梯度下优于逐元素AdaGrad，并在矩阵分解和深度网络训练中验证了优势。


<details>
  <summary>Details</summary>
Motivation: 现有AdaGrad、Adam等自适应优化方法主要针对向量参数设计，未显式利用矩阵结构；近期矩阵感知优化器虽展现结构化优化的好处，但缺乏类似AdaGrad那样可用于推导矩阵感知自适应的通用理论框架。

Method: 构建带自适应近端函数的矩阵参数在线镜像下降（OMD）通用框架，通过在线遗憾最小化推导矩阵感知自适应优化。引入行向与列向矩阵近端函数，分析由此产生的遗憾权衡，据此导出Row-wise Matrix AdaGrad和Column-wise Matrix AdaGrad，其自适应缩放由累积的行向或列向梯度范数决定。

Result: 建立了遗憾保证，证明在结构化梯度下矩阵感知遗憾界可严格紧于逐元素AdaGrad；在矩阵分解与深度神经网络训练实验中，将自适应缩放与矩阵结构对齐带来更优的优化稳定性，并能在更大学习率和更深网络下保持可训练性。

Conclusion: 矩阵感知的自适应缩放对应矩阵结构可带来理论更紧的遗憾界与实证更优的稳定性及可训练性，为矩阵参数优化提供了与AdaGrad对等的通用理论框架。
...
### [331] [Federated Deep Clustering Networks for High-Dimensional and Heterogeneous Data](https://arxiv.org/abs/2609.21829)
*Morris Stallmann,Charalampos S. Kouzinopoulos,Marcin Pietrasik,Anna Wilbik*

Main category: cs.LG

TL;DR: 提出FedDCN，将深度聚类网络推广到联邦场景，同时优化重构损失与聚类损失，并通过合成数据增强和几何正则化实现非IID数据下的潜空间对齐。


<details>
  <summary>Details</summary>
Motivation: 高维数据聚类是无监督学习的核心任务，集中式场景下深度聚类方法已较成熟，但联邦学习中数据分布在客户端且涉及隐私，深度聚类研究较少。已有联邦深度聚类方法在客户端数据非独立同分布（non-IID）时性能不够可靠。

Method: 将Deep Clustering Networks推广至联邦场景，提出FedDCN：联合优化重构损失与聚类损失；生成合成数据增强；在学习目标中引入几何正则化以对齐潜空间。

Result: 实验评估表明，在IID与非IID假设下该方法均表现出有效性，并据此指出了未来研究方向。

Conclusion: FedDCN为联邦非IID场景下的深度聚类提供了兼顾重构与聚类的方案，通过数据增强与几何正则化提升鲁棒性和潜空间对齐，但具体性能细节与未来方向需参考原文。
...
### [332] [Neural Cellular Automata Learn General Features in their Hidden Channels](https://arxiv.org/abs/2609.21870)
*Etienne Guichard,Stefano Nichele*

Main category: cs.LG

TL;DR: 本文研究神经细胞自动机（NCA）的隐藏通道内部动力学，提出将教师模型隐藏状态注入学生模型的迁移学习机制，在少样本MNIST上以约9800参数实现优越泛化。


<details>
  <summary>Details</summary>
Motivation: 深度模型依赖过参数化带来泛化，但在少样本场景下易过拟合与记忆。NCA虽参数高效，但研究多聚焦输出而忽视其内部隐藏通道的作用。

Method: 探究NCA隐藏通道的内部动力学，设计迁移学习机制，将预训练教师模型的隐藏状态注入学生模型以引导其早期优化；在少样本与尺度变化的MNIST基准上，与同规模循环及前馈架构进行比较，并进行机制性分析。

Result: NCA在约9800参数的极小预算下优于可比的循环与前馈架构，泛化更强。隐藏通道将特征提取与统一分类共识解耦，通过吸收形态复杂度并收敛到相互正交状态实现；其捕获的是通用的尺度不变拓扑基元而非类别特定模板，使学生模型可借助仅在数字0-5上训练的教师特征，在未见类别上取得强少样本性能。

Conclusion: 隐藏状态动力学可作为稳健、去中心化的计算基底，用于参数高效的迁移学习，凸显NCA内部动力学在少样本泛化中的价值。
...
### [333] [Detecting Pretraining Data in Large Language Models from a Free-Energy Perspective](https://arxiv.org/abs/2609.21888)
*Chenye Ke,Zirui Liu,Qi Liu,Yan Zhuang,Jintao Zhang,Zhenya Huang,Shijin Wang*

Main category: cs.LG

TL;DR: 该论文提出能量转移检测（ETD），通过在预测损失与预测熵的联合空间中引入倾斜边界（熵校正分数），从宏观残余自由能转移视角实现更优的预训练数据检测。


<details>
  <summary>Details</summary>
Motivation: 在大语言模型中检测预训练数据存在困难：高似然既可能源于训练曝光（成员），也可能源于强泛化（非成员）。仅基于似然的检测器在损失-熵联合空间中使用水平边界，容易将可预测的非成员误判为成员。

Method: 引入相对预测熵评估预测损失的倾斜边界，并进行均值-方差分析：证明熵校正可保留期望成员信号同时降低其方差，从而提升标准化的成员-非成员可分性；进一步将该分析推广到具有非零均值熵差的更一般情形。该熵调整分数具有Helmholtz自由能解释，据此提出ETD，从宏观残余自由能转移角度刻画预训练数据检测。

Result: 大量实验表明ETD取得最佳平均检测性能，平均AUROC最高提升3.5%，TPR@5%FPR最高提升5.1%，并在多种设置下保持鲁棒性。

Conclusion: 相对熵的倾斜边界优于水平边界，熵校正能在保持成员信号期望的同时降低方差；ETD以自由能转移视角统一解释该检测问题，并在实验中取得领先且鲁棒的检测效果。
...
### [334] [Layerwise Decoupling for Stable Structured Sparsification of Fully Connected Layers](https://arxiv.org/abs/2609.21126)
*Charles Kulick,Armenak Petrosyan,Sui Tang*

Main category: cs.LG

TL;DR: 本文提出一种解耦、逐层（layerwise）的结构化稀疏化方法，用于对预训练神经网络全连接层进行剪枝，并证明其与特定联合惩罚在最优性上等价，实验表明该方法比联合（coupled）基线更稳健。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常对所有层联合施加惩罚（joint penalty），在剪枝时会同时影响全部层，导致正则化强度可用范围窄，且容易出现灾难性过度剪枝（catastrophic over-pruning）。作者希望找到一种更稳健的结构化稀疏化方式，以拓宽可用的正则化范围并降低过度剪枝风险。

Method: 提出解耦的逐层方法：从预训练网络中抽取浅层两层子网络（two-layer subnetwork），对内部权重进行归一化，并对每个块的外层权重矩阵施加结构化组惩罚（structured group penalty）；按顺序逐层处理以剪除神经元、缩减每层宽度。理论上证明，对于任意正齐次激活函数（positively homogeneous activation），约束解耦目标在最优处等价于对内外权重的特定联合惩罚，因而可得到简洁的投影与近端（projected and proximal）形式。

Result: 解耦重述比耦合方法更稳健：数值实验中提供了更宽的正则化强度可用范围，且灾难性过度剪枝率低于所测试的联合基线，同时保持可比的准确率。在受控的分类与稀疏恢复研究中验证了这些性质，并在高维 PINN 压力测试和 OPT-1.3B 的前馈层中考察了其适用范围。

Conclusion: 解耦、逐层的结构化稀疏化框架在理论上与联合惩罚等价，在实践中比耦合方法更稳健，兼具更宽的可用正则化范围与更低的过度剪枝率，且准确率相当，具有在多种网络与任务上的适用潜力。
...
### [335] [Multi-Domain Clustering via Measure Quantization](https://arxiv.org/abs/2609.21664)
*Rafael Pereira Eufrazio,Eduardo Fernandes Montesuma,Charles Casimiro Cavalcante*

Main category: cs.LG

TL;DR: 提出基于测度量化的多域聚类通用框架：用Sinkhorn散度或MMD匹配各域概率测度与原型测度学习共享聚类原型，支持最近质心或最优传输分配，并用小批量优化实现可扩展性。


<details>
  <summary>Details</summary>
Motivation: 聚类是数据分析的基础任务，通常以K-means等质心方法解决。但多域场景下，来自多个域的数据需要共享的聚类结构，传统方法难以在概率测度层面统一建模并兼顾可扩展性。

Method: 给定多个域的样本，学习一组共享的聚类原型，通过最小化每域概率测度与原型测度之间的概率度量（如Sinkhorn散度或最大均值差异MMD）来拟合。数据点分配采用两种方式：最近质心，或最优传输——后者在同域内耦合所有样本的协作式策略。引入小批量优化策略，使拟合与分配均可扩展，降低内存与计算开销。

Result: 在覆盖图像、音频和传感器数据的5个多域基准上，基于Sinkhorn的方法一致优于经典聚类与多域聚类基线；规模扩展到数十万样本时该优势依然保持。

Conclusion: 测度量化提供了一个统一、可扩展的多域聚类框架，Sinkhorn散度作为目标度量表现最佳，最优传输分配提供协作式聚类能力，小批量优化在保持性能的同时显著提升可扩展性。
...
### [336] [Geometric Mean Pooling for Equal-Weight Multiplicative Coarse-Graining](https://arxiv.org/abs/2609.21876)
*Ang-Kun Wu,Fangdi Wen,Jingtao Zhang*

Main category: cs.LG

TL;DR: 论文提出几何平均池化（GMP），一种带符号池化算子，将特征符号乘积与特征幅值的几何平均结合，在合成任务上优于平均池化和最大池化，但在图像与分子任务上效果依赖具体设置，定位为特定情形下的补充性归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 平均池化具有可加性偏置，最大池化具有极值偏置，二者均无法刻画特征间的乘性合成关系；受量子多体物理中局部到全局组合的启发，作者希望设计一种能同时保留联合符号信息与乘性尺度的池化算子。

Method: 提出几何平均池化（GMP）：将特征符号的乘积与特征幅值的几何平均相结合，构成带符号的池化算子；该算子不引入可学习参数，并采用非重叠的层级式GMP以保持全局乘性统计量。在合成序列任务、迭代粗粒化、图像分类与分子亲脂性回归上进行评估。

Result: 在合成任务上，GMP比平均池化和最大池化更准确地恢复基于乘积的信号，并在所测试的乘性输入噪声水平下保持预测性能；在图像与分子数据上，其有效性取决于表示方式、目标参数化以及局部与全局池化的位置安排。

Conclusion: GMP是一种互补的、依赖具体情形的归纳偏置，适用于等权乘性组合合理的任务，而非对标准池化算子的通用替代方案。
...
### [337] [Sparse Priors for Efficient Distribution Learning](https://arxiv.org/abs/2609.20883)
*Saumya Goyal,Barnabás Póczos*

Main category: cs.LG

TL;DR: 本文提出"稀疏先验"类别与"稀疏维度"概念，证明在k-稀疏先验下分布学习的贝叶斯风险下界为Ω(√(k/n))，并在TV距离上给出匹配上界，从而在依赖n的层面克服维度灾难，同时表明该结果同样适用于"学习采样"。


<details>
  <summary>Details</summary>
Motivation: 现有生成式AI方法虽广泛应用且成功，但从n个样本学习d维支撑分布的理论保证退化为O(n^{-1/Θ(d)})，即使已是极小极大最优，仍被认为过于悲观。作者假设：仅靠光滑性假设不足以刻画真实应用中分布的固有结构，因此需要引入能反映分布结构的先验类以获得更紧的界。

Method: 1) 引入"稀疏先验"类（sparse priors）；2) 定义"稀疏维度"（Sparse Dimension）作为所有分布空间上先验稀疏性的度量；3) 在k-稀疏先验下分析分布学习的贝叶斯风险，推导下界；4) 在温和附加假设下针对TV距离证明匹配上界；5) 在贝叶斯框架下证明"分布学习"与"学习采样"的统计等价性。

Result: 在k-稀疏先验下，常见距离度量上的贝叶斯风险下界为Ω(√(k/n))；在温和附加假设下，TV距离上获得匹配上界（渐近于n,k，至多相差对数项）。由于分布学习与学习采样的统计等价性，这些结果同样适用于学习采样。k仍可依赖维度d或本质维度。

Conclusion: 在合适的先验（即k-稀疏先验）下进行学习，可在关于n的依赖上克服维度灾难，即使k仍可能依赖d或本质维度。这表明现有基于光滑性的悲观界源于未充分利用分布结构，稀疏先验提供了更精确的理论保证，并统一了分布学习与学习采样两类任务。
...
### [338] [Elastic Threshold Attention: Learned Contextual Sparsity for Long-Context Decoding](https://arxiv.org/abs/2609.20888)
*Themistoklis Haris,Henry Li,Maryam Karimzadehgan*

Main category: cs.LG

TL;DR: 提出弹性阈值注意力（ETA），一种端到端可训练稀疏注意力架构：由查询表示动态预测上下文阈值，训练时对低于阈值的logits做乘法抑制而非删除，形成平滑注意力下限；推理时以Triton核在O(1)时间筛选KV块，实现长上下文解码加速且保持稠密模型质量。


<details>
  <summary>Details</summary>
Motivation: 长上下文解码时海量KV缓存造成严重显存带宽瓶颈。现有稀疏注意力依赖刚性启发式选择性加载，会丢弃必要上下文，导致质量下降。作者希望在不牺牲稠密模型质量的前提下获得硬件加速的解码速度。

Method: ETA从查询表示直接预测动态、上下文相关的阈值，对困难检索或推理步骤分配类稠密上下文、对常规token剪枝。训练时对低于阈值的logits做乘法抑制趋向零（而非删除），形成平滑均匀的注意力下限与分布式概率蓄水池；推理时硬剪枝无信息KV块，并吸收GPU粗粒度块选择附带选入的token；同时实现基于缓存几何-概率界的Triton解码核，O(1)时间筛选KV块；另提供离线校准算法，冻结每头常数阈值以消除预测器开销。

Result: 1.45B预训练ETA在语言建模、常识推理与长上下文针检索上媲美稠密注意力，训练稀疏度约85%、活跃解码密度约38%。平滑注意力下限使初始token上的局部注意力汇消失。自定义Triton解码核在最长512K token序列上相较FlashAttention-2实现最高2.5倍墙钟解码加速。离线校准进一步削减注意力计算27%。

Conclusion: ETA通过可学习的动态阈值与乘法式软抑制训练策略，兼顾稀疏效率与稠密质量；结合定制GPU内核与离线校准，可在长上下文场景实现显著解码加速，并适用于领域特定部署。
...
### [339] [Continuous Delayed-Memory Stochastic Gradient Descent and Continuous-Time Reinforcement Learning from History of Astrophysical Time Series Studies](https://arxiv.org/abs/2609.20906)
*Debartha Paul,Juncheng Yi*

Main category: cs.LG

TL;DR: 本文回顾了将随机微分方程（SDE）与神经网络参数化结合用于类星体光变曲线建模的历史，并提出一种依赖离散迭代过程历史状态的连续延迟记忆随机梯度下降（Continuous-Delayed-Memory SGD），在二维景观仿真中相较Vanilla SGD展现出更广的探索与更精确的收敛；同时提出一种无需求解HJB偏微分方程的连续时间策略梯度强化学习结构，并证明其最优性条件可恢复已有工作中的Gibbs策略。


<details>
  <summary>Details</summary>
Motivation: 类星体亮度呈随机变化，承载着其背后超大质量黑洞的信息；从地基巡天观测得到的光变曲线时间序列中建模这些随机变化是一个统计难题，需要借助SDE与神经网络参数化等手段加以解决。

Method: 1）回顾SDE结合神经网络参数化用于类星体光变曲线建模的发展历程；2）提出Continuous-Delayed-Memory SGD，其更新依赖于离散迭代过程的过去状态；3）在若干二维景观上进行仿真，并通过调节超参数与Vanilla SGD对比；4）提出一种采用连续时间策略梯度的强化学习结构，用于探索性策略且无需求解HJB偏微分方程，并分析其最优性条件。

Result: 在二维景观仿真中，通过调整超参数，Continuous-Delayed-Memory SGD相比Vanilla SGD表现出更广泛的探索行为和更精确的收敛性能；所提出的连续时间策略梯度强化学习结构的最优性条件可恢复前人工作得到的Gibbs策略。

Conclusion: 该工作表明，将SDE与神经网络参数化结合是应对类星体光变曲线统计建模挑战的有效途径，且带延迟记忆的SGD与连续时间策略梯度方法在优化与强化学习层面具有理论联系与实用价值。
...
### [340] [Do Quantum Models Scale Like LLMs?](https://arxiv.org/abs/2609.20912)
*David S. Berman,Ying-Jer Kao,Roger G. Melko,Alexander G. Stapleton*

Main category: cs.LG

TL;DR: 本文研究RydbergGPT（基于Rydberg原子阵列量子比特投影测量数据训练的自回归Transformer）的神经标度律，发现标度行为依赖于模型-数据对：临界点附近损失与数据集规模呈带损失下限修正的幂律，远离临界点时幂律描述显著变差。


<details>
  <summary>Details</summary>
Motivation: 神经标度律通常被视为模型的固有属性，但标度行为是否依赖于训练数据本身的统计结构尚不清楚。作者希望借助具有可调控临界性的量子多体系统，检验数据统计特性（尤其是多尺度依赖性）对稳定神经标度的贡献。

Method: 以交互Rydberg原子阵列的量子比特投影测量数据训练自回归Transformer（RydbergGPT），通过改变激光失谐参数使系统处于临界点附近或远离临界点；测量不同数据集规模下的测试损失并拟合幂律（含损失下限修正）；同时使用熵归一化、有限采样校正的互信息"两点"函数，比较Rydberg测量数据与自然语言语料的统计结构。

Result: 临界点附近，损失随训练集规模呈带损失下限修正的幂律；远离临界点时幂律拟合质量大幅下降。两点函数分析显示，临界附近的两点函数衰减特征与自然语言最为接近，而远离临界点的量子比特构型其两点函数衰减更快。

Conclusion: 多尺度依赖性有助于稳定的神经标度行为，神经标度律应被视为模型-数据对的联合属性，而非仅由模型决定的性质。
...
### [341] [When AI Reviews Train AI Reviewers: Scientific-Judgment Collapse and Mitigation](https://arxiv.org/abs/2609.20942)
*Sy-Tuyen Ho,Minghui Liu,Furong Huang*

Main category: cs.LG

TL;DR: 该研究识别出LLM参与同行评审时的"科学判断坍塌"现象，并提出TrustReviewer系统加以缓解。


<details>
  <summary>Details</summary>
Motivation: LLM日益参与科学评审，模型生成的评审意见会进入公开数据与未来训练语料，导致AI评审可能形成递归反馈循环——后续评审者会学习先前模型产生的判断。作者希望研究这一反馈环路中的单步效应及其风险。

Method: 受控实验：以Llama 3.1 8B为起点，先在2018–2023年ICLR官方评审上微调出评审模型，再用ICLR 2024数据训练四个后继模型，并系统变化官方评审与模型生成评审的混合比例。缓解方案TrustReviewer采用两阶段干预：训练期在精选语料上单阶段训练核心评审模型以减少低质量与语义退化监督；测试期使用配对激活引导（paired activation steering）抑制残余的坍塌倾向，无需再训练或额外专家标注。

Result: 引入合成评审会压缩评分分布，并降低同篇论文内与语料库级别的语义多样性，作者称之为"科学判断坍塌"（scientific-judgment collapse）。

Conclusion: 递归评审训练存在具体风险；TrustReviewer的训练期预防与测试期纠正能在无需额外训练或标注的情况下，帮助保持判断多样性并改善推荐对齐。
...
### [342] [Efficient Bayes-Adaptive Reinforcement Learning with Temporal Logic Specifications](https://arxiv.org/abs/2609.20954)
*Jonathan Hau,Alessandro Abate*

Main category: cs.LG

TL;DR: 提出一种基于模型的端到端强化学习算法，将LTL任务转化为LDBA并与BAMDP环境表示同步，配合新型BAMCP算法实现未知环境中高效策略合成。


<details>
  <summary>Details</summary>
Motivation: 在未知环境中，针对给定LTL规范（如安全性或可达性）进行策略合成时，传统非贝叶斯方法探索-利用权衡不佳，且无模型方法样本效率低、易产生任务违规，亟需更高效的端到端方案。

Method: 将LTL任务的LDBA表示与环境的BAMDP表示进行同步，借助贝叶斯RL获得更优的探索-利用权衡；并进一步提出一种新型贝叶斯自适应蒙特卡洛规划（BAMCP）算法，在同步后的BAMDP结构上进行近似贝叶斯最优策略合成。

Result: 在多种有限与无限时域任务实验中，方法在性质满足度和样本效率上均优于传统无模型方法；消融研究验证了新型BAMCP相较经典BAMCP对LTL任务满足的优势；并成功应用于“谨慎”RL以减少训练中的任务违规次数。

Conclusion: 该工作表明，通过LDBA与BAMDP同步并结合定制化的贝叶斯自适应蒙特卡洛规划，可在未知环境中实现高效、样本友好的LTL策略合成，并可用于减少任务违规。
...
### [343] [From Switching to Dynamic Regret: A Simple Reduction via Unbiased Random Sequences](https://arxiv.org/abs/2609.20968)
...
<details>
  <summary>Details</summary>
Motivation: 在非平稳在线学习中，动态遗憾是衡量学习器相对时变比较器序列表现的重要指标。针对强凸与指数凹损失，现有达到最优界的分析往往复杂繁琐，缺乏简洁统一的处理方式。

Method: 作者提出一个归约框架：对任意比较器序列构造一个每轮无偏、方差受控且切换次数可控的辅助随机序列；结合合适的替代损失，将动态遗憾分解为对该随机序列的期望切换遗憾与其受控方差两部分，从而可利用现成的具有切换遗憾保证的算法推导动态遗憾界。

Result: 对强凸和指数凹损失，得到 $\widetilde{O}(T^{1/3}P_T^{2/3})$ 的动态遗憾界；对一般凸损失，同一归约恢复 $O(\sqrt{T(1+P_T)})$ 的动态遗憾界。上述结果均与相应三类损失的极小极大最优结果相匹配。

Conclusion: 该工作表明动态遗憾最小化可简洁地归约到切换遗憾最小化，利用现成算法即可获得最优界，凸显了所提框架的通用性与简洁性。
...
### [344] [Generative inversion for early ranking of competing geologic interpretations](https://arxiv.org/abs/2609.20978)
*Harun Ur Rashid,Daniel O'Malley*

Main category: cs.LG

TL;DR: 提出将竞争性地质解释转化为空间先验，并用水头观测一致性进行排序的工作流，结合文本到图像基础模型、变分自编码器与反演网络，在合成基准与真实场址概念模型上实现定量判别。


<details>
  <summary>Details</summary>
Motivation: 高后果地下决策常在严重数据稀缺条件下做出，专家对同一地下系统可能给出相互竞争的解释，而项目早期缺乏实用手段判断哪种解释最接近真实。此类不确定性往往要等到钻进多口井后才能消解，代价常达数百万美元。现有评估地质解释的方法要么依赖主观判断，要么依赖早期调查中罕见的密集数据。

Method: 1) 将竞争性地质解释翻译为不同的空间先验；2) 对每种解释，用文本到图像基础模型生成1600张地质图像集成，并单独训练一个变分自编码器提供该解释专属的潜表示；3) 用监督反演网络将水头观测映射到该潜空间，由冻结解码器输出图像并映射为对数导水系数场；4) 进行稳态流动模拟得到预测水头，将失配转换为高斯型相容性得分，据此对解释排序。

Result: 在基于Johansen组、包含三种一致性递减解释的合成基准上，925个测试案例的平均水头RMSE从"Precise & Accurate"解释的0.197升至"Accurate"的0.227，再到"Mismatched"的0.280。随后应用于WIPP场址Culebra白云岩段两个已发表概念模型，修订模型相容性权重为0.991，原模型为0.009，与独立证据一致。

Conclusion: 该工作流能在数据稀缺的早期阶段将竞争性地质解释转化为可定量比较的空间先验，并通过水头观测的一致性实现客观排序，为高后果地下决策提供实用判别依据，且在合成与真实场址案例中均得到验证。
...
### [345] [ASGARD: Action-Space Guard for UAV Resilience via Reinforcement Learning](https://arxiv.org/abs/2609.20982)
*Mohsen Salehi,Karthik Pattabiraman*

Main category: cs.LG

TL;DR: 提出 ASGARD 两阶段“教师-学生”框架，使基于强化学习的无人机控制在运行时能抵御动作空间攻击，保证任务完成。


<details>
  <summary>Details</summary>
Motivation: 强化学习控制器已用于无人机导航与控制，但易受动作空间攻击——攻击者在策略生成动作之后、执行器执行之前篡改动作指令。现有防御多针对策略输入攻击，而针对动作空间攻击的方法需在训练时重训策略，无法在运行时应对被篡改的动作。

Method: ASGARD 采用教师-学生两阶段流水线。教师阶段：编码器将无人机物理状态与动作攻击相关特权信息融合，生成“动作攻击感知”的隐表示，用于训练强化学习控制策略以及一个向执行器输出修正动作指令的监控器。学生阶段：编码器与监控器通过监督学习从教师模型蒸馏，仅依赖无人机物理状态历史即可在机载运行。

Result: 在针对无人机不同动作指令的多类攻击场景下评估：ASGARD 对动作空间攻击具有韧性，在攻击存在时仍能完成任务；且能泛化到未见攻击类型，并对隐蔽性攻击保持韧性。

Conclusion: ASGARD 通过特权信息蒸馏与运行时监控修正，弥补了现有动作空间防御需训练时重训、无法运行时防护的不足，实现无人机强化学习控制的运行时动作攻击韧性，并具备泛化性与隐蔽攻击防御能力。
...
### [346] [From Stress to Affect: Multimodal Deep Learning for Physiological Emotion Recognition Across Wearable Sensor Modalities](https://arxiv.org/abs/2609.20991)
*Desta Haileselassie Hagos,Saurav Keshari Aryal,Legand L. Burge*

Main category: cs.LG

TL;DR: 比较研究：在WESAD与EmoWear双模态可穿戴数据集上评估Bi-LSTM、TCN与Transformer，发现架构优劣依赖数据集特性，多模态传感优于单部位，4 Hz为实用采样频率。


<details>
  <summary>Details</summary>
Motivation: 现有生理情感识别研究通常只评估单一模型、传感配置或数据集，难以理解这些因素如何影响识别性能，缺少系统性的横向对比。

Method: 在WESAD与EmoWear两个多模态可穿戴数据集上，对双向LSTM、时序卷积网络（TCN）和Transformer进行对比；在仅手腕、仅胸部与多模态三种传感配置下，采用被试独立留一被试交叉验证（LOSO-CV）评估；并考察软投票集成、传感器消融、采样频率与基于梯度的显著性分析。

Result: Transformer在WESAD多模态上取得最高准确率（99.02% ± 0.51%）；LSTM在EmoWear上取得最佳多模态表现，唤醒度91.80% ± 1.06%、效价89.96% ± 0.36%。架构相对性能取决于数据集特性，不存在普遍最优架构。多模态传感在两个数据集上均稳定优于仅手腕和仅胸部配置。采样频率分析表明4 Hz是实用工作点，性能与更高频率相当但训练成本显著降低。

Conclusion: 可穿戴生理情感识别中架构选择应结合数据集特性，优先采用多模态传感，并在精度与成本间折中时选择约4 Hz采样频率，为模型、模态与采样率的选择提供实践指导。
...
### [347] [MOSAIC-SR: Transformer-Guided Symbolic Regression for Scientific Equation Recovery](https://arxiv.org/abs/2609.20997)
*Peiyi Zheng,Yanming Kang,Hans De Sterck,Giang Tran*

Main category: cs.LG

TL;DR: MOSAIC-SR结合预训练Transformer生成多个初始表达式草图来初始化搜索，并通过尺度感知常数优化与局部符号修复联合恢复结构，在多数据集上取得最高符号解率。


<details>
  <summary>Details</summary>
Motivation: 符号回归需从观测中恢复闭式方程以获得可解释模型。现有方法难以兼顾灵活的结构搜索与高效推理：基于搜索的方法结构可调但依赖昂贵组合优化与随机初始化；预训练神经模型生成快但常含符号错误。

Method: 用预训练Transformer提出多个初始草图，在若干有前景区域初始化搜索以规避广阔表达式空间中的随机起点；每个搜索通过尺度感知常数优化与局部符号修复联合恢复结构与常数。

Result: 在含/不含虚拟变量的SRSD-Feynman数据集及六个额外基准上评估，MOSAIC-SR在每个数据集上符号解率最高，预测精度位列前二；存在无关虚拟输入时优势依然保持。

Conclusion: 学习到的先验可将搜索聚焦于有前景的方程结构，数值优化与符号修复对恢复至关重要。
...
### [348] [On the Limits of Maximal Coding Rate Reduction for Out-of-Distribution Generalisation](https://arxiv.org/abs/2609.21001)
*Menghui Zhou,Gaoshan Bi,Vitaveska Lanfranchi,Po Yang*

Main category: cs.LG

TL;DR: 本文揭示了最大编码率约简（MCR²）目标在分布偏移下可能完全失效，证明其编码最优并不保证稳定的预测关系。


<details>
  <summary>Details</summary>
Motivation: MCR²作为信息论框架，被认为能学习结构化、判别性的表征并支持可解释白盒架构，但作者观察到它在分布偏移下会彻底失败，因此需要研究其分布外泛化的根本局限。

Method: 通过构造极端反例并进行理论分析：一是构造仅依赖不稳定环境特征即可达到全局编码最优、却在相关性反转后预测完全失败的精确最优解；二是分析在训练中所有测试输入均可出现的条件下编码质量任意接近最优而错误率接近100%的情形；三是检验引入IRM与REx所依据的不变性原理能否消除该失败。

Result: MCR²存在两个OOD泛化局限：第一，仅用MCR²目标可容许完全预测失败，失败表征在所有训练环境中共享相同最优编码算子；第二，直接融入IRM和REx的不变性原理无法消除该失败，说明共享编码最优性不蕴含稳定预测。

Conclusion: MCR²要实现可靠的OOD保证，需要引入能跨环境建立稳定预测关系的新假设或学习原则，单纯依赖编码最优性或现成不变性原理并不充分。
...
### [349] [Stiefel-AdamW: Geometry-Aware AdamW for Linear Factorization Blocks](https://arxiv.org/abs/2609.21039)
*Emanuele Zangrando,Marco Sutti,Francesco Tudisco*

Main category: cs.LG

TL;DR: 本文提出 Stiefel-AdamW：在含线性分解块 W=BA 的模块上，将其中一个因子约束到 Stiefel 流形，把 GL(r) 规范对称性约化为紧正交对称性，从而在保留 AdamW 逐坐标对角预条件的同时消除因子膨胀。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习中的线性分解块（如 LoRA、低秩压缩层、自注意力中的 query-key 乘积）普遍存在分解非唯一性，即 GL(r) 规范对称性，可能导致训练不稳定并限制可用学习率。然而此类模块通常仍使用忽略底层几何结构的欧氏优化方法（如 AdamW），存在方法错配。

Method: 提出 Stiefel-AdamW，作为 AdamW 的近似即插即用替代：对一个因子施加 Stiefel 流形约束，另一个因子保持欧氏。矩估计仍在环境欧氏空间中进行，几何仅通过切空间投影与流形回缩进入；由此将 GL(r) 规范对称性松弛为紧正交对称性，同时保留 AdamW 的逐坐标对角预条件。实现开销极小。

Result: 理论上证明该优化器兼具 Riemannian 方法的稳定性优势与标准收敛保证。实验上在 GPT2、ViT、Mistral 7B 的 LoRA 式微调，以及 GPT2 在 OpenWebText 上的完整预训练中，相比强基线取得一致提升，且相比 AdamW 几乎无额外成本。

Conclusion: Stiefel-AdamW 是一种在含线性分解块场景中几乎零额外开销、可直接替换 AdamW 的优化器，通过流形几何约束有效消除因子膨胀、提升训练稳定性，并在多任务上验证了一致收益。
...
### [350] [A Lightweight Plug-in Gate for Transformer-Based Time-Series Forecasters](https://arxiv.org/abs/2609.21044)
*Hongkai Zhuang,Tao Huang,Chen Hou*

Main category: cs.LG

TL;DR: 本文研究"编码器前协变量准入"作为输入端接口，用轻量级表示级门控（sigmoid评分）调节协变量表示后再送入编码器，并作为插件模块在TimeXer、iTransformer、PatchTST上以零额外调参协议评估。


<details>
  <summary>Details</summary>
Motivation: 协变量丰富的时间序列预测中，外部变量如何进入目标预测路径是关键问题；现有Transformer类预测器通常直接构建协变量表示并送入编码器，缺少显式的"准入"阶段。

Method: 在编码器前实现表示级轻量门控接口，对表示单元分配sigmoid分数，并研究对平均准入施加惩罚的使用正则化变体；以插件形式接入TimeXer、iTransformer、PatchTST，采用零额外调参协议继承各自基线配置。

Result: 在ETTm1、ETTm2、Traffic、Energy和ILI数据集上开展配对预测对比、门控位置消融、初始化消融、受控协变量准入分析及VIF-informed PFI诊断案例研究；在测试设置下门控与对应基线具有竞争力，使用惩罚降低了平均准入分数且预测误差接近未惩罚的TimeXer设置。

Conclusion: 编码器前协变量准入可作为有效且易插拔的输入端接口，在几乎不损失预测精度的情况下实现对协变量表示的显式调控，并可通过使用正则化压缩不必要的准入。
...
### [351] [FedeRage: Provably Convergent Agnostic Federated Learning under General Client Drift](https://arxiv.org/abs/2609.21057)
*Herlock Rahimi,Dionysis Kalogerias*

Main category: cs.LG

TL;DR: 本文刻画了在客户端参与概率完全未知时FedAvg实际求解的优化问题，并提出将CVaR嵌入本地目标的联邦风险规避平均算法FedeRage，仅需每客户端一个标量即可提升精度、公平性与收敛速度。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在非独立同分布数据和随机客户端参与下性能下降。现有基于FedAvg的补救方法通常假设服务器已知客户端参与概率，而这在实际部署中很少成立。

Method: 首先刻画"分布不可知"FedAvg在参与未知、可能高度偏斜且各轮规模可变时实际求解的优化问题，证明均匀聚合最小化一个由参与诱导边际加权的随机目标，凸且可能非光滑时达O(1/√T)速率。基于该刻画提出FedeRage，在自然DRO框架下将CVaR嵌入本地目标，隐式提升高损失与低参与频率客户端的权重，仅增加每客户端单个标量。

Result: FedeRage具有O(κ/√T)速率，κ为风险规避"代价"的上界；与需要可用性分布作为输入的最优传输聚合对齐方案不同，FedeRage对分布保持不可知。在三个异构基准上，精度、公平性和收敛速度一致优于最先进方法。

Conclusion: FedeRage在无需知晓参与分布的前提下实现了风险规避的联邦聚合，理论速率与实验表现均具优势。
...
### [352] [Toward individual-level calibration in affect recognition with perceptual adjustment queries](https://arxiv.org/abs/2609.21073)
*Xuanzhou Chen,Sankaraleengam Alagapan,Ashwin Pananjady*

Main category: cs.LG

TL;DR: 该研究提出用轻量级知觉调整查询（PAQ）估计个体JND，将刺激距离重表达到知觉空间以构建难度等化的面部情感任务，并在2AFC中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 行为任务通常假设相同刺激对所有被试具有相同知觉难度，但个体知觉敏感性差异会系统性破坏该假设，导致任务难度不等化。

Method: 以情感知觉任务为测试平台，通过认知轻量的知觉调整查询（PAQ）直接估计每位被试在面部情感谱上的最小可觉差（JND），并用推断的JND重表达刺激距离，在知觉空间中构建难度等化任务；在二选一强迫选择（2AFC）任务中，以二元元认知难度判断和反应时方差分解两种互补行为指标进行验证。

Result: 与非校准基线和群体级Weibull校准相比，PAQ校准在个体水平显著等化知觉任务难度，同时降低平均反应时和反应时的被试间方差。

Conclusion: PAQ可作为面部情感识别中个体化知觉校准的原则性且实用的工具。
...
### [353] [REFINEPPO: Learning Continuous Control Policies by Iterative Action Refinement](https://arxiv.org/abs/2609.21108)
*Sachini Weerasekara,Sagar Kamarthi,Jacqueline Isaacs*

Main category: cs.LG

TL;DR: 本文提出迭代动作精化（IAR）方法，通过共享精化网络对初始动作提案进行多步残差修正，并与PPO结合形成REFINEPPO，在14个连续控制基准上匹配或超越标准PPO，且在多个任务上收敛更快。


<details>
  <summary>Details</summary>
Motivation: 主流深度强化学习（DRL）连续控制策略通常将策略建模为从状态到动作（或动作分布）的直接映射，需由单次前馈网络一次性构造最优控制决策。这种一次性预测的范式缺乏对动作进行重新考虑或逐步改进的机会，可能限制策略的表达与优化空间。作者因此希望探索：策略能否学习迭代地改进动作，以及这种迭代过程能否在策略学习阶段带来优势。

Method: 提出迭代动作精化（IAR）：从初始动作提案出发，使用一个共享的精化网络反复以观测状态与当前动作提案为条件进行残差修正，每一步都可在前序步骤构造的动作基础上进行修订，最终精化提案决定智能体实际执行的动作。将该迭代动作构造机制与近端策略优化（PPO）集成，得到REFINEPPO。实验在14个基准控制任务上评估，并进行了精化深度与更新调度（update schedules）的受控消融，以及旨在解释迭代精化有效性的分析。

Result: 在14个基准控制任务上，REFINEPPO的性能与标准PPO相当或更优，并在若干任务上展现出更快的收敛速度。消融实验考察了精化深度和更新调度的影响，并提供了对迭代精化为何有效的分析。

Conclusion: 迭代动作构造（而非仅直接预测动作）是一种可行且有效的连续控制策略范式：通过共享精化网络的逐步残差修正，REFINEPPO在不牺牲（甚至提升）性能的同时加快了部分任务的收敛，表明迭代精化能为策略学习带来实质优势。
...
### [354] [Talk to Me, Jarvis: An Open-Source Edge-Deployable Voice Assistant Framework for Autonomous Racecars](https://arxiv.org/abs/2609.21109)
*Daniel Henel,Frederik Werner,Alexander Langmann,Johannes Betz*

Main category: cs.LG

TL;DR: 本文提出Jarvis——一个面向自动驾驶车辆高级行为指令的离线语音助手，其核心是基于Mistral 7B领域微调的文转指令分类器，在保证低延迟的同时实现了97.63%的意图识别准确率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为语音助手后端组件效果显著，但在线托管模型存在网络依赖和推理延迟波动问题，难以满足自动驾驶等时间关键场景的需求，因此需要一种离线、低延迟的语音助手方案。

Method: 构建Jarvis离线语音助手架构，将语音识别、语音合成与自然语言指令分类集成到轻量级本地框架中；核心组件为文转指令分类器，通过对Mistral 7B模型进行领域特定的微调实现低延迟推理。

Result: 实验评估表明，该方案优于更大的在线托管模型，意图识别准确率达97.63%，平均处理延迟为1.39秒，适合对响应速度要求较高的操作场景。

Conclusion: Jarvis验证了基于小规模本地微调模型构建离线语音助手的可行性，兼顾了准确率与低延迟，并开源实现以支持后续研究与微调。
...
### [355] [Signal-Centric Remote Sensing via Alternative Preprocessing and Acoustic Processing for ML-Driven Applications](https://arxiv.org/abs/2609.21123)
*Logan Luna,Sirio Jansen-Sánchez,Ilteris Demirkiran,Leo Ghelarducci*

Main category: cs.LG

TL;DR: 论文提出用CSV格式数据替代图像化表示来处理声呐数据，实验显示处理时间减少91.18%，机器学习目标检测精度提升，且SNR、PSNR等指标均改善。


<details>
  <summary>Details</summary>
Motivation: 主流声呐数据处理依赖图像化表示，需要在自主系统上对图像数据进行预处理，计算开销大、效率低，因此作者寻求一种替代性的遥感数据处理方法。

Method: 提出一种基于逗号分隔值（CSV）格式数据的数据处理方法，替代传统的图像化表示方式，用于遥感（声呐）应用。摘要未给出更具体的算法、网络结构或实验设置的细节。

Result: 实验表明：处理时间减少91.18%；机器学习的目标检测准确率有所提升；信噪比（SNR）、峰值信噪比（PSNR）以及其他评价指标均有提高。

Conclusion: 以CSV格式数据直接处理声呐数据是图像化表示方法的有效替代方案，在效率与多项信号质量及检测指标上均表现更优。摘要信息较为简略，方法细节与实验条件有待正文补充。
...
### [356] [HMB-GAN: Hybrid Multi-Bézier GAN for Vector Shape Synthesis](https://arxiv.org/abs/2609.21158)
...
TL;DR: 本文提出HMB-GAN，一个端到端可微的混合量子-经典GAN框架，通过拼接多段Bézier表示生成闭合的CAD矢量几何图形，并对比量子增强生成器与经典生成器的性能。


<details>
  <summary>Details</summary>
Motivation: 现有生成工作多局限于栅格化或单一Bézier域，无法直接生成CAD可用的矢量几何；同时需要探索混合量子-经典架构在结构化几何建模中的可行性。

Method: 提出HMB-GAN（Hybrid Multi-Bézier GAN），通过拼接多段Bézier表示构建闭合形状，并由构造过程强制保证几何连续性，形成端到端可微的生成框架；在同一架构下分别实现量子增强生成器与经典生成器进行对比。

Result: 量子生成器收敛更快、模型参数量更少，在点云分布度量上略有提升；但模拟器开销过大，导致经典模拟评估受限于硬件约束。

Conclusion: 结果证明了通过混合量子架构建模结构化几何的可行性，同时凸显了当前硬件条件的局限。
...
### [357] [M2G-LLM: Enhancing Clinical Prediction via Multimodal Graph Reasoning and LLM Context Injection](https://arxiv.org/abs/2609.21164)
*Inyoung Choi,Sukwon Yun,Jiayi Xin,Jie Peng,Tianlong Chen,Qi Long*

Main category: cs.LG

TL;DR: 提出M2G-LLM框架，用图神经网络增强大语言模型，实现多模态医疗数据融合与对齐，并在MIMIC-IV与MIMIC-CXR上取得临床预测性能提升。


<details>
  <summary>Details</summary>
Motivation: 临床决策需要整合临床笔记、检验结果、医学影像等多种数据模态，而现有大语言模型虽擅长处理非结构化临床文本，却难以纳入非文本模态，限制了其在医疗场景中的应用。

Method: M2G-LLM通过图神经网络（GNN）建模患者多次就诊之间的时序关系，在临床相似患者间传播信息，并对异质数据源进行对齐，构建富含信息的多模态上下文向量；这些向量被注入LLM的中间层，从而支持文本与非文本模态的联合推理。

Result: 在MIMIC-IV和MIMIC-CXR数据集上的评估表明，M2G-LLM在临床预测任务上优于强基线模型。

Conclusion: 将LLM的语言理解能力与GNN的关系推理能力相结合，为全面、多模态的医疗分析提供了有前景的路径。
...
### [358] [TierKV: Long-Context On-Device LLMs via Predictive Multi-Tier KV Caching](https://arxiv.org/abs/2609.21172)
*Zhihao Shu,Md Musfiqur Rahman Sanim,Jie Hu,Kun Yuan,Minghai Qin,Gagan Agrawal,Wei Niu*

Main category: cs.LG

TL;DR: 本文提出 TierKV 移动端 LLM 推理框架，基于 Predictive Multi-Tier Cache Optimization（PMCO），在解码前预测未来缓存需求并将 token 联合分配到精确、低秩和闪存卸载三层，缓解 KV 缓存内存瓶颈。


<details>
  <summary>Details</summary>
Motivation: LLM 部署到移动设备后需处理文本、图像、视频、音频等多样负载，且常需长上下文。KV 缓存随序列长度线性增长并在每个解码步被访问，成为主要内存瓶颈。已有方法（低秩压缩、token 驱逐、闪存卸载）分别带来重建开销、不可逆 token 损失或 I/O 停顿，抵消了省内存收益。

Method: 提出 Predictive Multi-Tier Cache Optimization（PMCO）：在解码开始前，从 prefill 隐状态预测未来缓存需求，在设备内存和精度预算下将 token 联合分配到精确层、低秩层和闪存卸载层。该建模保留完整上下文访问，消除反应式驱逐的循环依赖，并可用闭式求解器在运行时确定层级边界与每层秩。

Result: 在三种移动 SoC 上的八种文本、视觉和音频模型上，TierKV 相比现有移动 LLM 框架将 prefill 吞吐提升最高 17.6 倍，将驻留内存的 KV 缓存减少 12.5–34%，从而在相同内存预算下支持显著更长的上下文，且仅带来轻微精度下降。

Conclusion: TierKV 通过预测式多层缓存优化，在可接受的精度损失下显著降低移动端 KV 缓存内存占用并提升 prefill 吞吐，使移动设备在相同内存约束下能处理更长上下文。
...
### [359] [Reliability-Centered Evaluation of Sparse Longitudinal CT Lesion-Size Forecasting with Conformal Interval Calibration and Gompertz-Inspired Regularization](https://arxiv.org/abs/2609.21197)
*Lingfei Kong*

Main category: cs.LG

TL;DR: 该研究基于DeepLesion与DLT构建五访视同病灶轨迹基准（129例患者、205条轨迹），在固定预测时域（T3→T4）下逐步加入早期观测，评估点预测精度、不确定性可靠性、保形校准、亚组表现与Gompertz轨迹正则化。结果显示RMSE在m=2最低，增加历史观测未提升精度；不同方法不确定性行为差异明显，轨迹一致性与精度、可靠性并不协同提升。


<details>
  <summary>Details</summary>
Motivation: 稀疏纵向CT随访中仅有少量先验观测，使病灶尺寸预测困难；同时现有研究往往孤立评估预测精度与不确定性，缺乏在固定预测时域下对精度、不确定性可靠性、轨迹一致性的联合评估，且缺少标准化的同病灶多访视轨迹基准。

Method: 构建五访视DLT衍生同病灶轨迹基准（DeepLesion + Deep Lesion Tracker，205条轨迹/129例患者）。采用固定访视索引时域设计作为主分析，预测T3→T4的常见对数变化，并逐步增加更早观测（m=1,2,3,4）；另做稀疏至终点的探索性分析。评估点预测精度（10个训练种子的留出RMSE）、不确定性可靠性（Cohort-Level Feature GP、MC Dropout、Deep Ensemble、残差尺度区间）、患者级事后保形区间校准、解剖亚组表现，以及Gompertz启发的轨迹正则化（患者分组交叉验证选λ*）。

Result: 10个种子平均留出RMSE：m=1为0.4726，m=2为0.4305，m=3为0.4499，m=4为0.4513，m=2最低，增加历史观测未改善RMSE。m=4时原始Cohort-Level Feature GP覆盖接近95%名义水平，而MC Dropout、Deep Ensemble与残差尺度区间偏保守；患者级保形校准总体达到近名义或保守覆盖，但区间更宽。患者分组交叉验证选择Gompertz项λ*=0。全局人群参考常与病灶级变化方向相反，且预测难度随解剖亚组变化。

Conclusion: 在控制预测时域后，额外历史观测带来的预测收益有限；预测精度、不确定性可靠性与轨迹一致性未必协同改善，因此在稀疏纵向影像分析中应对三者进行联合评估，而非单独依赖某一指标或单一不确定性方法。
...
### [360] [MIRCID: Inferred Hub-miRNAs Drive Cross-Task Improvements in Drug Mechanistic Modeling](https://arxiv.org/abs/2609.21280)
...
TL;DR: 提出MIRCID框架，利用推断的枢纽miRNA（HubmiR）与TF活性辅助基因表达进行药物机制（MoA）建模。


<details>
  <summary>Details</summary>
Motivation: 药物MoA建模依赖扰动转录组，但配对的miRNA测量常缺失，需要可扩展的推断性调控特征来复用现有数据。

Method: 构建MIRCID框架，在通路分类与基于相似度的MoA检索任务中，对比基因表达、推断的TF活性与推断miRNA表达；HubmiRNet从977个L1000 landmark基因推断414个泛癌枢纽miRNA，并有1,298输出的变体；引入通用嵌入对照与互补性分析。

Result: HubmiRNet达到87.72%的Pearson相关系数；1,298输出变体在全miRNA任务上以71.21%优于SiCmiR的67.30%；miRNA增强带来的增益比TF活性更一致；通用嵌入对照的效用依赖模型；识别出保留基因来源结构、可部分线性恢复的独立表征；示例性rescue案例显示弱转录信号样本的分类改善与合理的miRNA模式相关。
...
### [361] [Multi-Subject Pretraining Enables Short-Calibration Personalization for Closed-Corpus Surface EMG Speech Decoding](https://arxiv.org/abs/2609.21288)
*Chenqian Le,Beatrice Fumagalli,Yasamin Esmaeili,Xupeng Chen,Tianyu He,Nikasadat Emami,Adeen Flinker,Yao Wang*

Main category: cs.LG

TL;DR: 在27名受试者、人均约21分钟sEMG数据的少数据设定下，采用单受试者检查点初始化＋多受试者预训练＋目标受试者微调，闭集50句任务的CER降至21.7%、WER降至31.9%；3分钟校准即可媲美约13分钟数据。


<details>
  <summary>Details</summary>
Motivation: sEMG默读语音接口受跨用户差异与校准负担制约，需探究在极少量数据下能否实现低成本个性化。

Method: 27名语音正常受试者在Aloud与Mimed两种模式下人均贡献<0.5 h（均值21.3 min）数据；闭集50句语料，采用留一受试者评估；流程为从公开单受试者检查点初始化→在非留出受试者上预训练→目标受试者微调；并做消融：无目标校准、直接检查点微调、随机初始化多受试者预训练、预训练受试者数量、3分钟vs约13分钟校准池、受试者专属适配器。

Result: 主流程CER 21.7%、WER 31.9%；无目标校准CER 49.3%；直接检查点微调CER 68.0%；随机初始化预训练后微调CER 44.9%且在27折中5折固定调度下不收敛；宏观平均CER随预训练受试者数由1人的74.4%降至26人的21.7%；3分钟校准CER 20.5%、WER 31.7%，与约13分钟池无显著差异；受试者适配器无显著收益；若训练数据排除5句评估句，CER/WER升至78.6%/99.9%。

Conclusion: 在标准化电极布置、闭集语料条件下，短时校准的个性化可行；检查点初始化对优化稳定性和精度均关键。
...
### [362] [FairLMs: A Turnkey Library for Fairness in Language Models](https://arxiv.org/abs/2609.21296)
*Jiale Zhang,Michael Larionov,Zichong Wang,Zhipeng Yin,Wenbin Zhang*

Main category: cs.LG

TL;DR: FairLMs 是一个 Python 库，通过显式声明模型能力与输入需求，整合语言模型公平性研究中的偏见测量、缓解与证据检验，实现组件兼容组合与统一协议下的方法比较。


<details>
  <summary>Details</summary>
Motivation: 语言模型公平性研究涵盖偏见测量、缓解方法应用以及评估所依赖证据的检验。现有工具通过不同接口提供互补功能，但在组合使用时，需要先协调模型接口、证据格式、访问约束和结果类型，才能判断适用性或比较方法，导致整合成本高。

Method: 提出 FairLMs Python 库，以模型能力与输入需求的显式声明连接上述活动。库内提供 33 个内在与外在指标、14 个缓解组件（覆盖四类干预）、14 个数据集与评分工具诊断、适配三种 Transformer 架构及受支持的托管补全 API 的适配器，以及基准加载器。声明在执行前被检查，结果携带获得时的配置信息。

Result: 可实现兼容组件的组合、在统一协议下比较方法，并将工作流扩展到新模型和数据集。源码见 https://github.com/FairLMs/FairLMs。

Conclusion: FairLMs 通过声明式设计统一了公平性测量、缓解与诊断流程，降低了工具整合与协议不一致带来的障碍，提升了方法的可比性与工作流的可扩展性。
...
### [363] [Fast And Accurate Text Content File Type Identification](https://arxiv.org/abs/2609.21306)
*Manu Nandan,Michael Brautbar,Edward Raff*

Main category: cs.LG

TL;DR: 本文提出一种用于文本内容文件（尤其是源代码）类型识别的神经网络模型，在开源文件上实验表明，其识别准确率更高，速度约为Magika的4倍，模型体积小28%。


<details>
  <summary>Details</summary>
Motivation: 组织普遍需要基于内容而非扩展名或magic number识别文件类型，尤其在网络安全领域。现有工具存在不足：基于模型的方法（如Magika）在计算负载和检测时间上有改进空间，基于编程语言构造的文件解析工具则在准确率上存在不足。

Method: 提出一种神经网络模型，用于文本内容文件类型的识别，重点针对源代码文件。

Result: 在开源文件上的实验表明，该模型在文本内容文件类型识别上平均准确率更高，速度约为Magika的4倍，同时模型体积小28%。

Conclusion: 该神经网络模型在准确率、检测速度和模型体积方面均优于现有工具，是文本内容文件类型识别的一种更优方案。
...
### [364] [An Introduction to Compression-Based Machine Learning](https://arxiv.org/abs/2609.21309)
*John Hurwitz,Edward Raff,Charles K. Nicholas*

Main category: cs.LG

TL;DR: 该论文综述并形式化了利用压缩进行机器学习的多种策略，提出并实证验证了一个基于压缩的机器学习设计框架，发现该类方法可与传统基线竞争，在恶意软件任务上明显更强，设计选择变化可带来最高0.62的精度提升。


<details>
  <summary>Details</summary>
Motivation: 无损压缩算法可通过归一化压缩距离或最小描述长度原则转化为机器学习方法，自回归模型也可通过熵编码转化为无损压缩方法，这种看似循环的依赖关系在现代人工智能与机器学习中具有尚未实现的潜力，因此需要系统梳理并形式化相关策略。

Method: 论文对利用压缩服务机器学习（compression-based ML）的各种策略进行综述与形式化，提出一个用于压缩驱动机器学习的设计框架，并通过实证实验验证该框架，同时考察不同设计选择对精度的影响，并与常规基线进行对比。

Result: 实验表明，基于压缩的方法可与传统基线竞争，并在恶意软件任务上具有决定性优势；调整设计选择可带来最高0.62的精度增益。

Conclusion: 压缩与机器学习之间存在双向可转化的紧密联系，系统化的设计框架能够有效指导压缩驱动的机器学习方法，并在特定任务上展现显著优势，值得在人工智能与机器学习领域进一步挖掘其潜力。
...
### [365] [Routine Blood Tests Outperform CRP for Distinguishing Bacterial From Viral Infection in Children](https://arxiv.org/abs/2609.21332)
*Mihaela Demireva,Zhecho Mitev,Djuna Chinareva-Klimentova,Svetoslav Ivanov,Georgi Nalbantov,Dimitar Mitev*

Main category: cs.LG

TL;DR: 该回顾性研究基于906名2–14岁儿童的CBC、CRP及年龄数据，采用多种监督分类模型区分病毒与细菌感染，XGBoost表现最佳（AUC 81.7%、敏感度70.8%、特异度79.2%），所有模型均优于仅基于CRP的决策规则。


<details>
  <summary>Details</summary>
Motivation: 急性感染是儿童就诊和住院的主要原因，但病毒与细菌感染的临床鉴别困难，导致儿科医生倾向经验性使用抗生素，加剧抗菌药物耐药问题。研究旨在评估全血细胞计数（CBC）在判断当前感染类型方面的附加预测价值。

Method: 回顾性研究，纳入2022–2026年间906例2–14岁、经检测确诊为病毒或细菌感染的儿科患者，要求具备CBC结果和CRP测量值。以CBC参数、CRP水平及年龄作为输入特征，训练多种监督分类模型，采用AUC、敏感度和特异度评估模型性能。

Result: 表现最佳的模型为XGBoost，纳入全部特征，样本外性能为AUC 81.7%、敏感度70.8%、特异度79.2%。所有训练模型在AUC上均优于仅基于CRP的决策规则模型。
...
### [366] [IntBMoE: Integrating Block-Level Conditioning into Expert Composition for Full-Participation Mixture-of-Experts](https://arxiv.org/abs/2609.21346)
*Ran Cheng,Longfei Xu,Zheng Liu,Kaikui Liu,Xiangxiang Chu*

Main category: cs.LG

TL;DR: 提出 IntBMoE，一种块条件 MoE，通过"稠密专家组合 + 稀疏块执行"将参与度、执行量、物化量三者解耦；在图像分类、语言建模、序列推荐上均优于代表性 MoE 基线，并已在工业推荐系统上线。


<details>
  <summary>Details</summary>
Motivation: 现有 MoE 设计无法独立设定三个量：参与度（每个 token 有多少专家贡献知识）、执行量（实际计算多少专家，即计算成本）、物化量（需构建与存储多少专家规模参数集，即内存成本）。稀疏路由执行与物化低但参与度小；稠密输出混合恢复全参与度但执行量随专家数增长；参数合并执行量恒为一个专家但物化量随路由决策数增长。三者相互牵制，缺乏独立可调的机制。

Method: IntBMoE 采用块条件 MoE：块来自一个小型可学习码本（每个条目对应一个块），数量由码本而非输入决定。每一内部层中，轻量超网络将该层专家池中所有专家基合并为一个组合专家，因此组合专家可汲取整个专家池，参与度满额；路由器仅将每个 token 送到少量块，执行保持稀疏；物化量因块数由码本固定而有界。此外引入双路径残差门控（DPRG），通过乘性门控耦合两条独立组合路径。

Result: 图像分类实验相比代表性稀疏与稠密 MoE 基线取得一致提升；语言建模与序列推荐实验验证了其在视觉之外的泛化能力。IntBMoE 已完整部署于 AMap 生成式推荐系统，在 60ms 延迟预算下服务数亿用户，在线 A/B 测试取得 2.4% 的相对 UVCTR 提升。

Conclusion: IntBMoE 通过块条件组合与稀疏块执行，实现了参与度、执行量、物化量的解耦，兼顾表达能力与效率，并在大规模工业推荐场景中验证了有效性与可部署性。
...
### [367] [Probabilistic Forecasting of Business Process Executions with Neural Temporal Point Processes](https://arxiv.org/abs/2609.21382)
*Jiaxin Yuan,Daniela Grigori,Han van der Aa*

Main category: cs.LG

TL;DR: 该论文将预测性业务流程监控建模为标记时序点过程，用Transformer编码器加混合解码器输出联合预测分布，在十个公开日志上精度持平判别式基线、剩余时间分布校准与锐度更优，且推理最廉价。


<details>
  <summary>Details</summary>
Motivation: 服务型系统的运维者依赖对运行中执行将如何延续的预测，而这种预测只有在可靠性已知时才可操作。主流深度学习模型是判别式且确定性的：只输出单个下一活动与单个剩余时间估计，没有可供推理的分布。同时真实事件日志违反简单点过程假设（连续事件常携带相同时间戳），需要显式处理这种并列（ties）。

Method: 把问题转化为使用标记时序点过程的生成式序列建模，该过程对下一标记及其事件间时间定义联合密度，从而在结构上自然给出预测分布。针对相同时间戳的并列问题显式处理，并将Transformer编码器与事件间时间上的混合解码器相结合，用精确对数似然进行训练。

Result: 在十个公开日志上，所得模型在点精度上与判别式基线持平，在剩余时间分布的校准与锐度上超过它们，并且推理成本最低，因为完整预测分布可在一次前向传播中获得而无需采样。

Conclusion: 生成式标记时序点过程建模为预测性流程监控提供了可校准且锐利的预测分布，同时保持点精度并降低推理成本，是判别式确定性基线的有力替代方案。
...
### [368] [Tracing the Evidence Behind Zero-Shot Time-Series Forecasting: A Source-First Taxonomy and Audit Framework](https://arxiv.org/abs/2609.21425)
*Delun Kong,Wanyun Ling,Chenxi Liu,Ziyue Li*

Main category: cs.LG

TL;DR: 本文主张"零样本"时间序列预测的定义应转向"证据可及性"声明，并提出以证据来源为先的分类法，配合四项审计问题，使零样本预测榜单可被审计。


<details>
  <summary>Details</summary>
Motivation: 现有零样本TSF定义仅以"是否进行目标特定参数更新"为标准，属于训练状态条件，无法说明系统可以使用哪些证据。冻结LLM提示、大规模语料预训练的时间序列模型、检索增强预测器都可能满足"不更新"条件，却依赖不同的可迁移证据，导致基准比较含义不清、进展不可解释。

Method: 提出"证据来源优先"（source-first）的分类法：首先区分三类主要证据来源——冻结LLM先验复用、参数化时间序列预训练、检索增强的外部记忆——并与其实现架构分离；在确定来源后，再通过四个审计问题刻画系统：任务接口、预测对象与评分方式、预测时可用上下文、资源预算。

Result: 形成一套审计框架与议程：将"零样本"从训练状态标签重构为证据可及性声明，并明确来源识别在先、四项审计问题在后的核查流程，从而可以系统性地判别不同方法之间可比与不可比的边界。

Conclusion: 零样本预测榜单应在报告分数的同时报告证据边界与接口假设，使基准进展真正反映可迁移的预测能力，而非未披露的上下文、记忆或预算变化。
...
### [369] [Decision-Focused Learning for Mean-Variance Portfolio Optimization via KKT-Based Reformulation](https://arxiv.org/abs/2609.21427)
*Kensei Nosaka,Shunnosuke Ikeda,Yuichi Takano*

Main category: cs.LG

TL;DR: 该论文针对均值-方差投资组合优化（MVO）中"预测-优化"两阶段框架目标不一致的问题，提出将下层MVO的KKT最优性条件嵌入上层学习问题的单层优化公式，在保留预算约束与卖空约束的同时保持可解性，并在真实ETF数据上取得多项投资指标的最优表现。


<details>
  <summary>Details</summary>
Motivation: 传统两阶段方法先预测期望收益、再求解优化，预测模型以最小化预测误差为目标，但该目标与下游投资组合决策质量并不一致，存在目标错配。决策聚焦学习（DFL）虽直接最小化下游决策损失，但现有面向MVO的DFL方法为保证可解性依赖代理损失或约束松弛，导致训练阶段的预测模型与评估阶段求解的带约束MVO之间存在结构性不匹配。

Method: 提出单层优化（single-level optimization）公式：将下层MVO问题的KKT最优性条件作为约束纳入上层学习问题，从而在训练中显式保留预算约束与卖空约束，同时保持对标准非线性优化求解器的可解性；并引入正则化项以提升性能。

Result: 在两个具有不同相关结构资产池的真实ETF数据上进行滚动窗口实验，所提方法在多项投资指标上均取得最佳表现，并验证了所提正则化带来的性能提升。

Conclusion: 通过KKT条件将带约束的MVO嵌入决策聚焦学习框架，可在不依赖代理损失或约束松弛的前提下消除训练与评估之间的结构错配，为约束型投资组合优化提供了有效且可解的端到端学习方案。
...
### [370] [Optimal Randomized Proper Online Learning](https://arxiv.org/abs/2609.21445)
*Zachary Chase,Idan Mehalel*

Main category: cs.LG

TL;DR: 该论文证明了随机恰当在线学习算法的最优期望错误界为 O(L(H) log T)，改进了此前 O(L(H) log^6 T) 的结果，并在最坏情况下达到常数因子最优。


<details>
  <summary>Details</summary>
Motivation: 此前随机恰当在线学习的最优错误界仅已知为 O(L(H) log^6 T)（Daskalakis 与 Golowich, STOC 2022），与理论下界存在较大差距，因此需要缩小该差距并确定紧确的错误界。

Method: 针对函数类 H，研究随机恰当学习算法，结合 Littlestone 维数 L(H) 与时间视界 T 进行理论分析，构造并证明期望错误上界。

Result: 证明了随机恰当在线学习的最优期望错误界为 O(L(H) log T)，优于先前的 O(L(H) log^6 T)。

Conclusion: 该界在最坏情况函数类下达到仅差一个通用常数的意义下最优，从而确定了随机恰当在线学习的紧确期望错误界。
...
### [371] [Understanding LLM Quantization through Activation-Guided Compensation and Orthogonal Residuals](https://arxiv.org/abs/2609.21450)
*Yamato Narita,Issei Sato*

Main category: cs.LG

TL;DR: 该论文通过将局部权重-激活量化误差精确分解为"激活引导的权重补偿项"与"正交残差", 用持久通道离群量和规则激活量对残差给出上界, 并据此推导出Hadamard旋转、符号选择与通道缩放的设计准则, 在8个Llama/Mistral模型上以无反向传播配置取得与梯度训练的SpinQuant相当的性能。


<details>
  <summary>Details</summary>
Motivation: 训练后W4A4量化可显著降低大语言模型的显存与推理开销, 但激活离群值会破坏有效量化分辨率。已有的权重优化、通道缩放与正交旋转手段虽能缓解该问题, 但它们各自修正的误差成分及其相互关系尚不清晰, 缺乏统一的理论刻画, 导致变换设计缺乏原则性指导。

Method: 对局部权重-激活量化误差进行精确分解, 拆为激活引导的权重补偿项与正交残差; 利用持久通道离群量(persistent channel-wise outlier)和规则激活量对残差建立上界; 基于该上界推导随机Hadamard旋转、符号选择与通道缩放的实用准则, 包括随机符号抑制持久离群通道间构造性干涉、多符号模式采样改进变换选择、二阶矩平衡导出L2缩放规则, 并进一步松弛得到SmoothQuant式L∞缩放。方法为无需反向传播的配置化流程。

Result: 在八个Llama与Mistral模型上以无反向传播配置进行评测, 性能与需梯度训练的SpinQuant相当。

Conclusion: 该误差分解厘清了哪些误差成分可由权重补偿解决、哪些需依赖变换设计, 并为旋转、符号选择与缩放提供了统一的理论依据与实践准则; 表明在无需梯度训练的情况下亦可达到与梯度方法可比的W4A4量化性能。
...
### [372] [Efficient Architecture Search under Leave-One-Subject-Out Evaluation](https://arxiv.org/abs/2609.21457)
*Heinke Hihn,Friedhelm Schwenker*

Main category: cs.LG

TL;DR: 提出PainNAS：一种基于块的、防泄漏的神经架构搜索方法，通过受试者间共享NAS搜索将搜索次数从N降至B（B≪N），在BioVid热痛数据集上以更少参数和FLOPs取得相当的受试者级准确率。


<details>
  <summary>Details</summary>
Motivation: 深度神经架构广泛用于自动化疼痛评估的信号处理，但架构设计仍主要依赖人工，NAS的潜在效率收益未被充分利用。同时，将NAS嵌入留一受试者（LOSO）评估计算代价高昂：完全嵌套实现需N次独立架构搜索，按近似线性训练成本计复杂度为O(N²)。

Method: 提出一种基于块（block-based）、控制泄漏（leakage-controlled）的方法，在受试者之间共享NAS搜索过程，将搜索次数从N减少到B（B≪N），命名为PainNAS。

Result: 在BioVid Heat Pain数据集上，PainNAS取得了与基线相当的受试者级准确率，同时参数量和FLOPs大幅减少。

Conclusion: 通过受试者间共享搜索的块式防泄漏NAS设计，可有效缓解LOSO评估中NAS的O(N²)计算瓶颈，在保持受试者级性能的同时显著降低模型复杂度，为自动化疼痛评估提供了更高效的架构设计范式。
...
### [373] [What Must Survive? Exact Task-Information--State Frontiers for Resource-Sufficient Learning](https://arxiv.org/abs/2609.21523)
*Ronald Katende*

Main category: cs.LG

TL;DR: 研究在下游任务未完全已知时压缩系统所需保留状态的问题，刻画了任务信息的比特数与状态维度之间的精确前沿。


<details>
  <summary>Details</summary>
Motivation: 系统常在下游任务完全确定之前就被压缩，因此需要明确：此时保留多少状态是必要的，以及有限的“提前任务信息”能节省多少状态。论文为此建立理论框架并用实例说明。

Method: 对有限线性任务族，在状态形成前先揭示任务消息、之后才揭示确切任务。对大小为K的advice字母表，给出精确前沿 p*(K)=min_{|P|≤K} max_{C∈P} rank(T_C)，b比特前沿取K=min(2^b,|U|)。并给出近似奇异值前沿、公共核下界与精确直和律，以及最优advice划分的强NP难性（在每个固定正近似容差下均成立）。

Result: 三个例子：良态softmax注意力构造在9比特区分512种延续之一时给出精确的524,288→1,024坐标前沿；域分解数字孪生得到界面加局部状态律以及异构区域的加权划分问题；分层多任务模型给出两阶段前沿，3比特把所需状态从3,136降到448坐标，进一步任务信息逼近不可约的328坐标单任务下界。

Conclusion: 提前任务信息通过将任务划分为联合任务算子低秩的分区来减少必要状态；这一前沿可精确刻画，但寻找最优advice划分在计算上强NP难，且近似同样困难。
...
### [374] [IncentRL: The Trade-Off Between Preference Guidance and Task Performance](https://arxiv.org/abs/2609.21525)
*Xuening Wu,Yanlan Kang,Shenqin Yin*

Main category: cs.LG

TL;DR: 本文提出 IncentRL 框架，通过在结果分布与偏好分布间加入 KL 惩罚引入偏好引导，并显式刻画其对原始外部任务性能的影响。


<details>
  <summary>Details</summary>
Motivation: 基于偏好的奖励塑形能引导强化学习，但向奖励中加入偏好信号可能无意改变被优化的任务目标。

Method: IncentRL 在指定结果分布与偏好分布之间加入 KL 惩罚。针对有界塑形成本的有限折扣 MDP，推导外部价值扰动界，建立保持原最优策略的严格动作间隙充分条件，并用折扣累积偏好成本刻画大权重情形；实现上采用手工设计的距离型结果代理、固定偏好分布和分数加权系数搜索。

Result: 在 MiniGrid DoorKey-8x8 上，系数 0.01 时三随机种子两百万步平均成功率 98%，零系数基线为 90.5%，搜索逐渐偏向更小系数。

Conclusion: 结果给出了偏好型 RL 核心权衡的原则性视角：利用额外引导改善学习而不致过度扭曲原始任务目标；当前实验仍属描述性，尚未将 KL 塑形与更简单替代方案区分开。
...
### [375] [OpenMAS-GCom. A Diagnostic Benchmark for Graph-enhanced Multi-Agent Systems](https://arxiv.org/abs/2609.21527)
*Kairui Yang,Xunkai Li,Kaixiang Zhang,Minghao An,Zekai Chen,Yuxuan Ba,Rong-Hua Li*

Main category: cs.LG

TL;DR: 提出 OpenMAS-GCom 基准，通过受控干预诊断通信结构、角色分配与信息流对图增强多智能体系统（G-MAS）性能的具体影响。


<details>
  <summary>Details</summary>
Motivation: 最终分数比较混合了模型、通信模式、角色与计算成本等差异，导致性能差异难以归因到具体的通信结构、角色分配和信息流，即存在"评估归因问题"。

Method: 用协作单元、通信链路、共享中间信息和执行规则表示系统；在任务、模型、提示与预算固定下，每次只改变一个组件，对比原始系统与修改版本。干预方式包括：重连通信边、移除专家或批评者智能体、用错误内容替换中间消息、执行中禁用工作者。基准覆盖 17 种配置（单智能体、普通多智能体、图增强）与 6 个领域的 29 个数据集，并新增 400 个 G-MAS-Complex 任务（要求综合多文档信息、解决冲突记录并返回带来源标识的指定值）。

Result: 移除专家带来的平均性能损失大于移除批评者；在原始分数相近的情况下，错误消息与工作者故障导致不同程度的性能退化；在 G-MAS-Complex 上，不同配置分别在最高准确率和最高每 token 准确率上表现最佳。

Conclusion: OpenMAS-GCom 可实现对 G-MAS 各组件的受控诊断与归因，揭示角色、消息正确性和工作者可靠性对性能的差异化影响，并支持在准确率与计算效率间做配置取舍。
...
### [376] [MACE: Memory-Agent Co-Evolution with Adaptive Memory Graphs for Multi-Agent Systems](https://arxiv.org/abs/2609.21533)
*Kairui Yang,Minghao An,Xunkai Li,Ziheng Yi,Zekai Chen,Guangyuan He,Rong-Hua Li*

Main category: cs.LG

TL;DR: 该论文提出MACE框架，通过记忆-智能体协同演化，将协作痕迹组织为功能单元子图并自适应选择呈现格式，在八个基准上平均得分81.11%，优于最强基线SAGE的78.97%。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的多智能体系统产生的协作痕迹蕴含任务规划、结果验证与失败修复过程，复用这些过程需保留动作前置条件及后续智能体所需输出。然而现有方法未有效组织此类依赖关系，也未考虑呈现格式对记忆检索的影响。

Method: 通过实证研究发现：将依赖分组为功能记忆单元可提升保留率，连接单元能增加任务所需单元与链接的联合检索；指令与清单格式下最优单元组合不同。据此提出MACE：MemGoG将功能单元表示为条件-动作-输出子图，以支持、冲突、修复关系连接；MACE Loop在记忆预算内选择相关单元与关系，为智能体提供指令或清单，并记录选择、格式、输出与结果以更新单元分数与关系。

Result: 在八个基准测试中，MACE平均得分81.11%，超过十个基线方法，最强基线SAGE为78.97%。

Conclusion: 基于执行反馈协同演化记忆组织与智能体记忆使用，并动态选择呈现格式，能有效提升多智能体协作过程的复用效果。
...
### [377] [Trading Depth for Time in Recurrent Transformers](https://arxiv.org/abs/2609.21605)
*Zeyi Huang,Xuehai He,Yong Jae Lee,Yelong Shen*

Main category: cs.LG

TL;DR: 在循环Transformer中，将额外计算用于时间步（潜思考token）比单纯增加物理深度更参数高效：一个思考token使浅层模型以约少48%参数恢复深层模型67%–81%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 循环Transformer通过时间循环增加计算深度，但存在一个关键问题：额外计算应投入更多时间步还是更大物理深度？作者希望在同一受控条件下比较这两种增加计算量的方式。

Method: 提出Latent Recurrent Transformers（LRTs）。解码时每个词表token保留一次主干前向；在相邻词表token之间插入一个潜思考token，该token与词表token经过相同的L层、共享主干参数，从而在预测下一token前提供额外的隐藏状态精炼阶段。对比L层LRT（含思考token）与不含思考token的2L层LRT，两者每个词表token均执行2L个Transformer块，但思考token模型参数更少。

Result: 在16层和20层混合专家NanoChat主干上，一个思考token使浅层模型与双倍深度对应模型的差距分别缩小到0.006和0.004 bits per byte，恢复了67%和81%的改进，同时总参数减少约48%。

Conclusion: 在循环Transformer中，时间维度的"思考"是增加物理深度的一种参数高效替代方案。
...
### [378] [Riemannian Neural Hamiltonian Flows: Geodesic Symplectic Transport and Interpretability](https://arxiv.org/abs/2609.21647)
*Vincent Souveton*

Main category: cs.LG

TL;DR: 本文提出黎曼神经哈密顿流（Riemannian Neural Hamiltonian Flows），将流形上的固定动能、可学习标量势与显式测地线蛙跳积分器结合，构造可逆、保体积的生成模型，并给出该学习哈密顿量的可解释性理论与实验验证。


<details>
  <summary>Details</summary>
Motivation: 哈密顿归一化流因相空间映射可逆且保体积而成为有吸引力的生成模型，但现有的大多数神经构造局限于欧氏空间，难以直接应用于具有曲率的黎曼流形数据分布。因此作者希望把哈密顿流推广到一般黎曼流形，并解决其学习到的势函数缺乏可解释性的问题。

Method: 方法上将黎曼流形上的固定动能、一个可学习的标量势函数以及显式测地线蛙跳积分器组合成黎曼神经哈密顿流。理论分析方面，指出每个可归一化势函数定义了一个隐式轮廓（implicit profile），位置边缘分布在初始时沿该轮廓与基分布之间的相对得分加速；提出匹配势（matched potential）作为可解释的特例，使隐式轮廓即为目标分布。在等温高斯情形下，该机制对应于相空间旋转；并通过局部调和分析将结论推广到一般流形目标分布的每个众数附近。学习势与匹配势之间的差距被分解为基分布的残差记忆与模型偏差，当位置基分布已转移到动量时两者一致，而这在前者比目标更宽泛时可实现。

Result: 在欧氏、双曲和球面空间上的数值实验表明，该方法相比黎曼连续归一化流（Riemannian continuous normalizing flow）具有竞争力的采样质量和数值开销，并验证了所学势函数的可解释性。

Conclusion: 黎曼神经哈密顿流为流形上的生成建模提供了一种可逆、保体积且具有可解释哈密顿量的方案，理论与实验共同表明其势函数可被赋予明确的统计意义，并在多种几何空间上取得良好效果。
...
### [379] [Beyond Gaussian Worlds: Latent Geometry Matters for JEPAs](https://arxiv.org/abs/2609.21656)
*Léo Nicollier,Enric Meinhardt-Llopis,Marc Pic,Pablo Musé,Gabriele Facciolo*

Main category: cs.LG

TL;DR: 本文将分布匹配式JEPA的线性可恢复性理论从欧氏情形推广到嵌入黎曼流形的潜变量，证明球面均匀分布等非欧氏几何同样可实现线性恢复，并给出更紧的近似恢复界，实验验证几何匹配目标优于失配目标。


<details>
  <summary>Details</summary>
Motivation: Klindt等（2026）在欧氏假设下证明：匹配高斯目标可在线性变换意义下恢复高斯潜变量，且高斯是具此保证的唯一分布。本文质疑"高斯唯一性"是否为分布匹配式JEPA的普遍性质，探究非欧氏潜几何是否也能支持线性可恢复的分布。

Method: 将分析推广到支撑在嵌入黎曼流形上的潜变量，推导在何种潜几何与正对（positive-pair）动力学条件下，对齐与精确分布匹配可保证线性恢复；重点分析潜变量在球面上均匀分布、表征匹配同一球面分布的情形；并推导近似恢复界，比较球面世界与高斯世界的紧致程度。实验覆盖高斯、球面、环面潜空间以及高维Clifford环面世界。

Result: 当潜变量在球面上均匀分布且表征匹配同一球面分布时，每个最优表征均可恢复潜状态至正交变换；球面世界的近似恢复界严格紧于高斯世界；实验表明几何兼容的目标在优化成功时带来更好的线性恢复，而失配目标会扭曲潜结构，该优势在高维Clifford环面世界中依然保持。

Conclusion: 高斯唯一性并非分布匹配式JEPA的普遍性质：非欧氏潜几何可容纳其他线性可恢复分布（如球面均匀分布）。目标分布与潜几何的兼容性是线性恢复的关键，为设计几何匹配的JEPA目标提供了理论与实证依据。
...
### [380] [Optimization Geometry of Equivalent Brownian RKHS Representations](https://arxiv.org/abs/2609.21693)
*Mahdi Mohammadigohari,Gustau Camps-Valls*

Main category: cs.LG

TL;DR: 在受控的有限布朗RKHS中，节点、增量与谱坐标可表示同一函数与内蕴范数，却导致不同的优化几何与算法行为。理论证明映射轨迹等价性、增量GD的显式欧拉刻画、条件数上界，以及标准Adam下正交等变群恰为带符号置换，并用Float64实验验证。


<details>
  <summary>Details</summary>
Motivation: 有限参数化之间的可逆坐标变换不改变所表示的函数与内蕴范数，但会改变优化算法在参数空间中的更新路径与几何性质。作者希望在一个完全可解析的受控模型（有限布朗RKHS）中，分离并量化这种"坐标效应"，而不是混淆于假设类、正则项或近似空间的改变。

Method: 构造有限布朗RKHS，选择节点坐标、增量坐标和谱（DCT）坐标三种等价参数化，利用有限元、RKHS插值、布朗协方差和混合边界DCT恒等式，使共享假设类、布朗能量、近似算子与坐标映射显式化。分析映射初始化、相同标量步长和相同小批量下GD/SGD的映射轨迹；给出增量GD作为常布朗/Sobolev度量显式欧拉的$1/h$因子；推导正则最小二乘Hessian条件数上界；并按标准Adam约定确定正交等变群。用五组网格的Float64数值实验验证恒等式、轨迹、条件数预测与Adam分离。

Result: 节点与谱GD/SGD在映射初始化、相同标量步长和相同小批量下具有完全相同的映射轨迹；增量GD是常布朗/Sobolev度量的显式欧拉步，步长因子为$1/h$；对布朗正则最小二乘，$\kappa_2(\mathbf H_{\mathrm{inc}})\le 1+A/\rho$，在固定$A,\rho>0$及给定归一化下与网格分辨率$G$无关；标准Adam约定下普遍正交等变群恰为带符号置换，块DCT-VIII变换不属于该群。Float64五网格实验数值验证了上述结论。

Conclusion: 该受控框架在不改变所表示函数、内蕴正则化子与近似空间的前提下，成功隔离了坐标/参数化对优化几何与算法行为的影响：映射轨迹可因坐标选择而相同或不同，增量坐标带来分辨率无关的条件数上界，而Adam的等变性受坐标变换破坏。这为理解等价参数化下的优化差异提供了可解析的基准与判据。
...
### [381] [SpecQuant: Speculative Decoding with Multi-Parent Quantization for Adaptive LLM Inference](https://arxiv.org/abs/2609.21704)
*Harish KB,Jagadeeswaran M,Pradheep P,Yuvanesh S,Sivakumar T*

Main category: cs.LG

TL;DR: SpecQuant：一种免训练框架，结合投机解码与多父量化，实现LLM自适应高效推理，在MMLU等基准上取得35–43%加速且精度损失不超过2%。


<details>
  <summary>Details</summary>
Motivation: 在消费级硬件上本地运行大语言模型受限于算力与内存。现有加速技术（量化、投机解码、自适应推理）虽能显著提速，但通常需要重训练、针对特定架构调优或额外草稿模型。

Method: 提出免训练框架SpecQuant，将投机解码与多父量化结合。由同一基座模型派生出INT4、FP8、FP16多种量化变体，基于预测的查询复杂度动态路由：简单或事实性任务使用轻量变体，复杂推理或长上下文输入使用全精度模型。共享权重设计保证投机解码有足够的token接受率，且无需独立草稿父模型，避免兼容性问题。

Result: 在基于Qwen2.5的模型上，于MMLU、AlpacaEval、GSM8K数据集/基准进行评测，实现35–43%的加速，且精度下降不超过2%。

Conclusion: SpecQuant可无需特殊基础设施或专业知识，支持跨多种硬件的实用化端侧LLM部署。
...
### [382] [GEM-MPC: Balancing Exploration and Exploitation through Expert-Guided Planning](https://arxiv.org/abs/2609.21735)
*Alvaro Serra-Gomez,Thomas Moerland*

Main category: cs.LG

TL;DR: GEM-MPC是一种基于MPPI的强化学习框架，通过融合克隆规划器的策略与KL正则化探索策略，并提出门控先验蒸馏机制，在低计算预算下提升规划与学习的一致性及连续控制性能。


<details>
  <summary>Details</summary>
Motivation: 高维连续控制中的高效探索仍是强化学习核心难题。基于规划的方法结合在线规划与学习到的策略和价值函数，但训练中组件易失配：采样策略偏离规划器行为，回放中的规划分布随模型与价值函数演化而过时，而重分析虽可刷新目标却计算代价高昂。

Method: 提出GEM-MPC：利用MPPI在规划中融合两个策略——一个克隆规划器的策略与一个围绕其进行探索的KL正则化策略，从而同时提供利用与引导探索；并引入门控先验蒸馏（Gated Prior Distillation），仅在存储的规划分布优于当前先验时选择性地从中学习，无需完整重分析即可缓解过时规划数据的影响。

Result: 在多个连续控制基准上，GEM-MPC在更低计算预算下持续优于现有基于规划的基线方法。

Conclusion: 通过改进规划与学习之间的交互，GEM-MPC以较低计算开销实现了更优的规划一致性与探索效率。
...
### [383] [GraphSkillEvo: Evolutionary Optimization of Graph-Structured Agent Skills](https://arxiv.org/abs/2609.21749)
*Rui Sun,Zhi Zheng,Zhenkun Wang,Zhichao Lu*

Main category: cs.LG

TL;DR: 论文提出将技能表示为图结构的自然语言产物，并构建基于种群进化的优化框架 GraphSkillEvo，在五个智能体基准上显著优于现有技能优化基线。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 智能体技能优化方法将技能表示为非结构化自然语言指令，存在两大问题：一是缺乏显式的工作流级指导且冗余严重，LLM 难以执行；二是无约束的自然语言技能搜索空间过大，导致优化效率低下。

Method: 将技能建模为图结构的自然语言产物：节点表示执行步骤及其操作指导，有向边编码步骤间依赖上下文的转移关系。基于该结构化表示，提出 GraphSkillEvo——一种基于种群的进化优化框架，包含针对图结构技能的变异与交叉算子，通过维护多个候选技能并组合有效组件，实现比纯 LLM 迭代自精炼更广泛、更全面的结构化技能空间探索。

Result: 在五个智能体基准上的大量实验表明，GraphSkillEvo 持续优于强基线 SkillOpt，在 GPT-5.4-nano 上平均准确率提升 4.01%，在 GPT-5.4 上提升 1.76%。

Conclusion: 图结构的技能表示可提供清晰的工作流级指导并促进技能优化，结合种群进化框架能有效扩大并深化技能空间的探索，为 LLM 智能体的技能优化提供了新范式。
...
### [384] [Bilevel Optimization of Topology and Hyperparameters (BOTH)](https://arxiv.org/abs/2609.21758)
*Suryanarayanan Manoj Sanu,Miguel Anibal Bessa,Alejandro Marcos Aragón*

Main category: cs.LG

TL;DR: 该论文提出对拓扑优化过程本身进行自动微分以获得"超梯度"，从而与主优化同步调优众多超参数，实验证明一两步拓扑优化即可提供充分信息，方法可扩展到数千个超参数且代价仅相当于数次标准优化运行。


<details>
  <summary>Details</summary>
Motivation: 拓扑优化（TO）虽能自动化设计流程，但实际应用中高度依赖"魔法数字"——超参数，其调优需要深厚的领域知识和大量试错。替代方案（代理模型辅助的超参数优化）又要求通过精心的问题表述来严格限制超参数数量。

Method: 利用自动微分对拓扑优化本身求导，得到"超梯度"（hypergradients），使得超参数能够与主优化过程同步调优。仅评估一两步拓扑优化即可获得足够信息，并使方法能够扩展到数千个超参数。

Result: 在应力约束问题和柔度问题上验证了该方法，其中柔度问题采用了密度场的神经参数化。方法可扩展到数千个超参数，计算代价与仅运行几次标准拓扑优化相当。

Conclusion: 对拓扑优化本身进行自动微分可有效解决超参数调优难题，突破了传统替代方法的规模限制，为拓扑优化中超参数的自动调优提供了可行且高效的途径。
...
### [385] [RegKT: Interpretable and Robust Deep Knowledge Tracing With IRT-Regularizer](https://arxiv.org/abs/2609.21791)
*Samuel Girard,Juan D. Pinto,Jill-Jênn Vie,Amel Bouzeghoub*

Main category: cs.LG

TL;DR: 本文提出一种新型正则化技术，旨在同时提升深度学习知识追踪模型的鲁棒性与可解释性，缓解小数据集上的过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习知识追踪模型虽精度提升，但可解释性下降，且在小规模教育数据集上易过拟合，阻碍教育实践者采用。

Method: 提出一种新颖的正则化技术，用于增强深度学习知识追踪模型的鲁棒性，并同时改善其可解释性。

Result: 该技术同时应对可解释性与过拟合两大挑战（摘要未给出具体量化结果）。

Conclusion: 该方法使深度学习知识追踪更适用于真实教育场景。
...
### [386] [LLMs as Feature Engineers for Text-and-Tabular Prediction](https://arxiv.org/abs/2609.21894)
*Merwan Barlier,Blaz Skrlj*

Main category: cs.LG

TL;DR: 本文提出一种迭代框架，利用生成器LLM提出语义定义、抽取器LLM物化特征，并由下游表格模型评估性能，通过将模型误差（如AUC排序倒挂）转化为自然语言反馈来引导搜索，从而从非结构化文本中自动提取可解释、schema约束的分类特征。在三份公开数据集上，误差驱动循环将特征发现速度提升最高3倍，生成特征与TF-IDF和稠密嵌入结合时严格优于任一子集，并保证实例级可解释性，SHAP重要性排名中占据主导并提供透明的语义审计轨迹。


<details>
  <summary>Details</summary>
Motivation: 从非结构化文本中为表格预测模型提取可解释、schema约束的分类特征通常依赖人工设计，成本高且难以保证下游性能；同时LLM自动特征生成往往缺乏针对性的搜索引导，导致发现效率低。作者希望构建一种自动化、误差驱动且可解释的特征发现机制，以提升特征搜索效率并保持预测性能与透明度。

Method: 提出迭代式自动特征提取框架：生成器LLM负责提出候选特征的语义定义，抽取器LLM负责将定义物化为具体的实例级特征，下游表格模型评估这些特征的预测性能。搜索过程通过将显式模型误差（例如AUC排序倒挂）翻译为自然语言反馈，引导LLM针对特定预测失败进行修正与再生成，从而优化特征空间探索。

Result: 在三份公开数据集上评估：误差驱动循环将特征发现速度相较无引导搜索提升最高约3倍；生成的类别特征表现出较强的多视图互补性，与TF-IDF和稠密嵌入结合后严格优于任一单独特征子集；在实例级可解释性方面，所发现特征在SHAP重要性排名中占据主导，并为每次预测提供完全透明、语义化的审计轨迹。

Conclusion: 误差驱动的LLM迭代框架能够高效自动化地从非结构化文本中提取可解释、schema约束的分类特征，在提升搜索效率的同时增强多视图互补性与实例级可解释性，为表格预测模型提供了一条兼顾性能、效率与透明度的特征工程路径。
...
### [387] [ExpBoN: Exponential-Noise Best-of-$n$ for Efficient Test-Time LLM Alignment](https://arxiv.org/abs/2609.21899)
*Yanxiao Liu,Sicheng Wan,Deniz Gündüz*

Main category: cs.LG

TL;DR: 本文提出 ExpBoN，一种基于指数噪声 report-noisy-max 机制的软 Best-of-n 采样方法，并集成到 guided speculative inference 框架形成 ExpGSI，实现高效奖励引导的 LLM 对齐，在降低计算成本的同时保持精度。


<details>
  <summary>Details</summary>
Motivation: Best-of-n 采样是简单有效的推理时对齐方法，但硬最大化对奖励与分布偏移之间的权衡控制过于粗糙。Soft Best-of-n 提供更平滑的控制并收敛到 KL 正则化奖励最大化的最优分布，因此需要探索替代性的软 BoN 方法及其高效推理实现。

Method: 提出 ExpBoN，基于指数噪声的 report-noisy-max 机制实现软 BoN。该方法具有精确的有限 n 分解，可实现总变差、期望奖励及双向 KL 散度的指数级快速收敛。进一步将 ExpBoN 集成到 guided speculative inference（GSI）框架，得到 ExpGSI，用于高效奖励引导的 LLM 对齐。

Result: 提供了 ExpBoN 收敛性与遗憾行为的全面理论分析。在 MATH500、MMLU-STEM 和 Minerva Math 上，使用 Qwen2.5-Math 与 Qwen3 模型族实验表明：ExpGSI 在各候选预算下对 Qwen2.5-Math 减少约 14%-39% 的估算计算量，对 Qwen3 在 n=16 时减少最多 45%，同时保持可比精度。

Conclusion: 结果为指数噪声 BoN 及高效测试时 LLM 对齐提供了理论与算法基础。
...
### [388] [Intervention Granularity Matters: Coherent Treatment Bundles in Counterfactual Simulation with Clinical World Models](https://arxiv.org/abs/2609.21906)
*Fangzhou Wang,Yixuan Yang,Camilla Balzarotti,Rishikesan Kamaleswaran*

Main category: cs.LG

TL;DR: 在临床世界模型中，干预的编辑粒度会实质影响反事实预测结果：临床干预以"捆绑包"形式记录，单组件编辑会低估治疗敏感度，而捆绑包感知编辑更合理。


<details>
  <summary>Details</summary>
Motivation: 临床反事实模拟需要确定"何为一次干预"。但在真实临床中，干预以捆绑形式记录（如透析回路的每个参数从不单独出现），单独修改某一组件会描述数据中从未出现过的小时。作者假设：编辑干预的粒度会改变世界模型的响应，并对此进行验证。

Method: 对MIMIC-IV中945,707患者-小时进行共现审计，确认捆绑组件从不单独出现。构建Clin-JEPA——以每小时治疗文本为条件的患者轨迹潜在世界模型。在1,019个有创通气记录起点上，固定患者历史与其他治疗，比较"编辑单个呼吸机设置"与"编辑真实相似轨迹患者所记录的完整配置"两种方式。

Result: 完整捆绑包使预测的下一状态移动幅度大于任何单一设置，且在全部五个设置上一致；在控制编辑对模型输入改变量之后，这一差异依然存在。

Conclusion: 干预粒度对临床世界模型的响应有实质影响。单组件编辑可能低估治疗敏感度，捆绑包感知编辑为反事实治疗模拟提供了更有依据的基础。
...
### [389] [Beyond Kinematics: Benchmarking Simulation Fidelity for Muscle-Driven Imitation Learning](https://arxiv.org/abs/2609.21909)
*Ayah G. Ahmad,Claire E. Borden,Maegan Tucker*

Main category: cs.LG

TL;DR: 系统对比SCONE/HyFyDy与MuJoCo/MyoSim两条运动模仿强化学习流程：两者运动学精度相近，但HyFyDy的肌电激活与实验EMG更吻合，更适合肌骨建模。


<details>
  <summary>Details</summary>
Motivation: 现有MIRL流程虽能高保真复现人体运动学，但是否能捕捉产生该运动的神经肌肉行为尚不明确；该问题对以肌肉激活模式和代谢代价为优化目标的机器人辅助装置设计至关重要。

Method: 采用同一套人体运动捕捉与肌电（EMG）测量数据，系统对比两条最先进流程：基于SCONE/HyFyDy（强调肌腱精细建模的生理真实性）与基于MuJoCo/MyoSim（强调计算效率与可扩展策略学习）。

Result: 两者均能相对准确地产生相似的运动学；但肌肉激活与实验EMG的对齐程度，HyFyDy明显优于MuJoCo。平均合并（RMSE, r）指标：HyFyDy为(0.164, 0.4)，MuJoCo为(0.344, 0.11)。

Conclusion: 更先进的生理真实性使HyFyDy当前更适合肌骨建模；但两者均需进一步发展，以在GPU可并行仿真环境中实现生理真实性，从而推进机器人辅助装置设计。
...
### [390] [Kinks vs. Smoothness: Identifiability of Real Analytic nICA for Laplace-like Sources](https://arxiv.org/abs/2609.21926)
*Isaac Manring,Kejun Huang*

Main category: cs.LG

TL;DR: 该论文证明了在源分布概率密度函数一阶导数仅有有限个不连续点时，实解析生成函数下的非线性独立成分分析（nICA）可实现可辨识性（精确恢复至平凡歧义），并用正规化流和变分自编码器实验验证。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统常需用隐藏的独立因子解释图像或金融时间序列等复杂数据，核心挑战是从非线性混合中恢复真实因子而非打乱版本，即非线性独立成分分析（nICA）的可辨识性问题。已有工作对生成函数和源分布假设较强，限制了适用范围。

Method: 假设生成函数为实解析函数，源概率密度函数具有有限个一阶导数不连续点（如拉普拉斯分布）；利用源分布中的"拐点（kinks）"与实解析函数光滑性之间的对比来构造可辨识性证明。实解析函数类范围广泛，可用标准激活函数（tanh、softplus、GELU）的正规化流或变分自编码器近似，因此结论可最小改动地应用于现有训练流程。

Result: 在真实与合成数据上，使用正规化流和变分自编码器进行实验，验证了其可辨识性；在CelebA数据集上恢复出若干可解释的潜在因子，分别控制数据集中独特的属性。

Conclusion: 该工作将nICA可辨识性推广到实解析生成函数与一类含有限不连续点的源分布（如拉普拉斯分布），且与主流深度生成模型训练流程兼容，为可解释表示学习提供了理论与实证支持。
...
### [391] [Joint Remaining Useful Life Prediction and Capacity Estimation of Lithium-Ion Batteries Using Partial-Charging Data](https://arxiv.org/abs/2609.21932)
*Khoa Tran,Ho-Si-Hung Nguyen,Phone Wai Yan Moe,Hung-Cuong Trinh,Thi-Hoang-Giang Tran*

Main category: cs.LG

TL;DR: 提出"跨专家"框架，仅用部分充电测量、无需历史满充容量，通过RUL专家与容量专家特征调制融合，实现联合RUL预测与容量估计。


<details>
  <summary>Details</summary>
Motivation: 联合RUL预测与容量估计需同时表征电池的长期渐进退化与近期行为；且实际中常缺乏历史满充容量测量，故需在部分充电数据下实现双任务预测。

Method: RUL专家：对30循环历史内采样的10个循环、标称10分钟片段，用预训练GRU编码器+2D-CNN+时序GRU编码；容量专家：对连续10循环、标称40分钟片段的统计描述子，用2D-CNN+Transformer处理；用特征级线性调制（FiLM）以短期表征调制长期表征实现联合预测；训练分三阶段：监督自编码器预训练、各专家独立预训练、冻结专家后的融合训练。

Result: 在两个公开电池老化数据集上，参考配置取得平均RUL RMSE为143.69与161.10循环，容量误差为12.36与7.28 mAh；在数据集I上，融合相较任一单独专家均降低两类平均误差。

Conclusion: 结果揭示了RUL与容量精度之间的权衡：所提方法在两个数据集上取得对比方法中最低的RUL RMSE，但多个基线在容量误差上更优。
...
### [392] [Learning to Move Cities: Deep Meta-Models and Reinforcement Policies for Calibration and Control in Urban Networks](https://arxiv.org/abs/2609.21945)
*Adewumi Augustine Adepitan,Christopher J. Haruna,Oluwasegun Adegoke,Ayooluwatomiwa Ajiboye,Oluwatobi Oluwasakin*

Main category: cs.LG

TL;DR: 本文提出一个共享隐空间框架，通过统一的学习表征将交通仿真器校准与强化学习控制相连接，实现从校准到自适应控制的端到端优化。


<details>
  <summary>Details</summary>
Motivation: 城市交通网络优化面临高保真仿真器校准成本高、实时运行控制困难的双重挑战，传统方法在大规模网络上存在计算瓶颈，且校准与控制通常相互割裂。

Method: 1）设计组合式MLP自编码器，学习连接仿真器输入（OD需求、网络参数）与输出（行程时间、拥堵模式）的低维流形，用于高效贝叶斯优化校准；2）构建带经验回放和目标网络的深度Q学习智能体，通过调度与路径调整优化动态交通分配；3）将共享隐表征同时用于降低贝叶斯校准维度并纳入强化学习状态表示。

Result: 在基准网络上，相比传统降维方法，所提自编码器在固定计算预算下样本效率更高、对观测数据拟合更佳；控制策略使系统整体行程时间相比基线运营最多降低51%。

Conclusion: 共享隐空间表述为智能交通系统提供了从仿真器校准到自适应运行控制的统一路径，展示了深度学习方法在城市出行规划与管理中的变革潜力，尤其适用于传统优化方法面临计算瓶颈的大规模网络。
...
### [393] [RACER: Role-Aligned Competence Estimation for Human-AI Routing](https://arxiv.org/abs/2609.21953)
*Joshua Strong,Emma Sun,Alexander Capstick,Pramit Saha,Cheng Ouyang,J. Alison Noble*

Main category: cs.LG

TL;DR: 本文提出RACER——一种角色对齐的能力估计路由框架，通过角色相对估计未见专家在查询上的正确概率，并与模型后验结合，实现类别重标记不变且贝叶斯对齐的延迟决策。


<details>
  <summary>Details</summary>
Motivation: 群体自适应延迟需借助少量专家行为上下文集适配未见专家。现有神经上下文编码器（如L2D-Pop）可能依赖绝对类别坐标学习路由捷径；IFD虽通过角色索引的类别能力剖面消除捷径，但其估计在每个类别内为常数，无法刻画实例级专家专长。

Method: RACER在每个候选类别角色下估计专家在查询上正确的后验预测概率，再结合模型后验得到贝叶斯相关的专家正确概率。采用非参数与神经核池化估计器，利用候选—角色关系、共享聚合与对称摘要，排除绝对类别身份通道。理论层面证明类别重标记一致性不变性，推导贝叶斯对齐的延迟代理损失，并给出将路由遗憾关联于分类器与能力估计误差的插入式遗憾界。

Result: 在受控合成基准（含PathMNIST组织病理学上下文扩展实验与模拟专家）上，RACER在隐藏亚型依赖下能受益于更多上下文；在CIFAR-100合成实验的独立采样未见专家划分上取得最佳总体表现。在VinDr-CXR与CheXpert放射科及人机胸片基准上，RACER族在预算扫描延迟中具竞争力或最优，标定结果因指标与数据集而异。

Conclusion: RACER通过角色相对、类别身份无关的能力估计，兼顾贝叶斯对齐与类别重标记不变性，在合成与真实医学延迟基准上验证了有效性，为群体自适应延迟提供了一种可扩展且理论可解释的框架。
...
### [394] [Time series generation with spectrally aligned latent flow matching](https://arxiv.org/abs/2609.21989)
*Camilo Carvajal Reyes,Felipe Tobar*

Main category: cs.LG

TL;DR: 本文提出一种频谱对齐的潜变量流（latent-flow）时间序列生成器，通过在流匹配潜空间中引入基于傅里叶、小波和签名变换的微调损失，解决潜压缩导致的频谱失配问题，从而生成更真实、高效且与真实数据在局部结构上对齐的合成时间序列。


<details>
  <summary>Details</summary>
Motivation: 潜变量流模型是成本高效的时间序列生成方法，但潜压缩会引入不希望的伪影，尤其是与底层数据集之间的频谱失配，从而限制了其作为训练替代数据（training surrogate）的可用性。

Method: 提出频谱对齐的潜变量流时间序列生成器，将用于流匹配的潜空间训练为保留与合成样本适用性相关的动态特性。具体地，引入基于典型信号表示（傅里叶变换、小波变换和签名变换）的微调损失，利用这些变换的可解释性，确保合成信号在平滑度或目标频谱内容等相关特征上与真实信号对齐，而非仅依赖逐点重建损失。

Result: 在真实世界的长程单变量和多变量基准数据集上，将所提对齐模型与基础潜变量流模型及当前最优方法进行对比。定量结果显示，所提方法在反映信号真实性的指标及计算效率上均具优势，并在局部结构上与训练集保持一致。

Conclusion: 基于傅里叶、小波和签名变换的频谱对齐微调损失，可有效克服潜压缩带来的频谱失配问题，使潜变量流生成器在信号真实性、计算效率与局部结构对齐方面优于基线及现有最优方法。
...
### [395] [Assessment of Machine Learning-Based Critical Heat Flux Models in the CTF Subchannel Code for Square Rod Bundle Prediction](https://arxiv.org/abs/2609.21995)
*Aidan Furlong,Vinicius de Melo Monteiro,Robert Salko,Juliana Pacheco Duarte,Xu Wu*

Main category: cs.LG

TL;DR: 本研究在CTF子通道程序中，利用EPRI棒束CHF数据库评估了基于机器学习的临界热流密度（CHF）模型，涵盖纯模型与混合残差修正模型、局部与半局部形式。结果表明，管训练ML模型可较好地迁移到棒束工况，大多数情况下优于传统CHF方法，其中局部混合LUT模型整体表现最佳，半局部纯ML模型也极具竞争力，验证了ML方法在反应堆热工水力分析中的推广潜力。


<details>
  <summary>Details</summary>
Motivation: CHF预测是核热工水力中的关键安全量，直接关系燃料性能与反应堆安全。尽管机器学习方法相比传统经验关联式和查找表（LUT）显著提高了CHF预测精度，但现有ML模型多基于圆管数据库开发与评估，其在反应堆相关棒束几何中的适用性尚不明确，因此需要开展面向棒束的大规模评估。

Method: 研究将ML-based CHF模型部署于CTF子通道程序中，采用EPRI棒束CHF数据库进行评估。模型形式包括纯ML模型与混合残差修正模型，并分别考虑局部（local）与半局部（semilocal）两种建模框架。评估结果与Bowring关联式、W-3关联式以及2006 Groeneveld LUT进行对比。

Result: 基于圆管数据训练的ML CHF模型总体上能较好地迁移至棒束应用，并在大多数几何和运行工况下优于传统CHF方法。其中，局部混合LUT模型整体性能最强，半局部纯ML模型也保持高度竞争力。与Bowring、W-3关联式及2006 Groeneveld LUT的对比表明，即使仅使用管数据训练，也能在棒束CHF预测上取得显著提升。

Conclusion: 该研究首次在生产级子通道分析环境中对方形棒束内的ML-based CHF模型进行了大规模评估，证实了ML模型跨几何迁移的可行性，并支持其在反应堆热工水力分析中的更广泛应用。
...
### [396] [Abstention and Noise Filtering: Two Missing Primitives of Softmax Attention](https://arxiv.org/abs/2609.22005)
*Richard Zhe Wang*

Main category: cs.LG

TL;DR: 本文认为注意力中的门控提供了softmax注意力缺失的两种能力——放弃（abstention）与噪声过滤，并通过10M–350M参数的匹配实验验证：放弃的收益随规模下降，噪声过滤的收益随规模上升，同时具备两者时各规模均最优，且额外参数极少并兼容KV缓存。


<details>
  <summary>Details</summary>
Motivation: 已有研究报道对注意力值通路进行门控能改善语言模型预训练，但对其原因存在分歧。作者希望厘清门控究竟带来何种能力，从而解释并统一这些相互矛盾的观察。

Method: 在10M至350M参数的匹配模型上进行实验。通过可学习的逐头sink logit在softmax中实现放弃，通过对每个value施加门控实现噪声过滤；比较基线、仅门控、仅sink以及两者兼备的模型，并人为向value注入受控干扰以检验门控的过滤作用。

Result: 三点发现：其一，放弃的收益（相对基线的验证损失下降）随模型增大而减小，噪声过滤的收益则随规模增大而增加——10M时门控收益几乎全部来自放弃，350M时主要来自过滤；其二，各规模下同时内置两种原语的最优；其三，受控干扰实验证实门控能去除干扰，并揭示两类门控各有特征性盲区。

Conclusion: 门控的价值可分解为放弃与噪声过滤两种互补原语，二者随规模的贡献此消彼长，需同时具备才能在各规模取得最优。该方案额外参数可忽略，且与KV缓存兼容，具备实际部署可行性。
...
### [397] [COMPLEX: A Closed-Form Certified Embedding of Multiparameter Persistence Modules](https://arxiv.org/abs/2609.22012)
*Sushovan Majhi,Atish Mitra,Žiga Virk,Pramita Bagchi*

Main category: cs.LG

TL;DR: 提出COMPLEX：一种无需训练、闭式的多参数持久性向量化方法，首次给出双侧失真界，使特征保真度可度量，并在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有多参数持久性向量化仅具单侧Lipschitz上界，缺乏下界（下侧标尺），因此无法判断特征是否忠实，也无法给出逐预测保证。本文旨在补充缺失的下界。

Method: COMPLEX将多参数模沿固定近对角网切片，用经认证的PLACE/PALACE地标映射嵌入每个切片条形码，再拼接。在可检验的"见证切片一致性"条件下（在Orbit5k审计对上100%成立），单个切片即可提供闭式下侧标尺，结合标准上界得到首个双侧失真界。采用闭式选择地标半径、核函数（保证书）与双过滤集，仅用交叉验证的SVM头，无需学习嵌入与留出校准。

Result: 下界紧至与实现距离相差小因子，但操作上呈局部性：RBF-SVM达91%，1-NN仅78%。在Orbit基准上达91.95%（Orbit5k）与92.98%（Orbit100k），与欧拉特征曲面持平或更高，优于transformer与graphcode。在图上，以单一固定配置在四个共享分子基准上超越GRIL，且是唯一在COX2上超过多数类基线3分以上的多参数方法。

Conclusion: COMPLEX首次实现多参数特征图的双侧失真界，使保真度可度量。但逐预测局部认证因结构性原因失效——该原因普遍存在于下侧标尺由单一坐标见证的地标嵌入中。闭式选择带来额外精度提升，而梯度式自适应无增益。
...
### [398] [$λ$-Controlled GRPO: Turning Flow-Matching Ratio Instability into a Budgeted Resource](https://arxiv.org/abs/2609.22041)
*Yufeng Wang,Parivesh Priye,Meeshawn Marathe,Ramit Pahwa*

Main category: cs.LG

TL;DR: 论文提出λ-Controlled GRPO，将多步去噪强化学习中的不稳定现象统一归因于一个由高斯转移核精确决定的"路径方差"量，据此校准重要性比率并分配各去噪步的梯度预算，在文本到图像任务上优于最强经验稳定器。


<details>
  <summary>Details</summary>
Motivation: Flow-GRPO将去噪采样器视为可优化的随机策略，但其训练存在多步去噪特有的不稳定性：策略更新随去噪步系统变化，重要性比率持续低于1、离散度增大、裁剪率不一，训练后期可用样本减少。已有工作将这些视为彼此独立的失效模式，各自使用手工调节的稳定器修补。

Method: 作者证明上述现象源自同一个逐步量——路径方差（path variance），该量由采样器的高斯转移核精确确定，且可在训练中低成本估计。基于此提出λ-Controlled GRPO：依据预测的方差规律（而非噪声化的经验统计）校准重要性比率行为，并按各去噪步的预测成本分配梯度努力；控制更新的两个尺度由标准策略选择固定，而非作为自由调参引入。

Result: 在两个奖励设置下的文本到图像模型上——以OCR评分的困难目标文本渲染，以及以偏好模型评分的人类偏好匹配——λ-Controlled GRPO在文本准确率和偏好奖励两方面均优于最强的经验稳定器，并将后期步的路径方差保持在预期预算内，而这正是基线系统性超出的地方。

Conclusion: 该工作将多步去噪训练不稳定重构为可测量、可预算的资源，而非需要逐一修复的症状集合，得到由自身转移规律校准、而非在不稳定出现后再行修补的Flow-GRPO更新。
...
### [399] [Available Guardrails: Certifying Selective Prediction across ML Systems](https://arxiv.org/abs/2609.22048)
*Parivesh Priye,Yufeng Wang,Haibin Ling,Michael Chaykowsky*

Main category: cs.LG

TL;DR: 选择性预测器需在每个报告单元上以目标精度获得认证，其核心瓶颈是有限校准数据能否产生有效证书（即可用性）。本文用精确二项反演使可用性可计算，并将固定群序下的报告划分选择建模为动态规划，揭示安全性—粒度—覆盖流量三者的前沿权衡。


<details>
  <summary>Details</summary>
Motivation: 部署场景要求选择性预测器对每个报告单元（如工具、政策标签、患者亚组）在目标精度上获得可靠性认证。真正的困难往往不在于已授予证书是否有效，而在于有限校准数据能否产生证书：门槛越安全或划分越细，部分单元获得的证据越少而无法认证，这一"可用性"问题此前缺乏可计算的形式化刻画。

Method: 以经典精确二项反演（exact-binomial inversion）将可用性形式化为可计算量；在固定群序（group order）下把报告划分选择问题建模为动态规划，刻画安全性、粒度与服务流量之间的权衡前沿；提出在规划划分上构造候选划分、在另一划分上选择的两阶段方法，并引入在报告单元间重新分配族错误率预算这一保有效性调节手段。评估覆盖三个意图路由数据集、两种架构、共60个模型效应，并延伸至LLM工具调用、内容审核、病灶分类与推荐四类任务。

Result: 理论前沿显示存在被有限样本估计几乎抹去的大规模总体机会：真值知情规划器相较支持平衡平均覆盖提升0.157，而朴素估计器仅恢复0.005，说明从有限数据中恢复该机会是核心挑战。两阶段构造-选择方法可恢复部分差距，平均覆盖较支持平衡提升0.060，且该方向在三个意图路由数据集、两种架构的60个模型效应中有59个复现。重新分配族错误率预算在总体量与含噪估计下均能额外回收覆盖。相同前沿在四类任务中反复出现，仅预测器特定上限不同。

Conclusion: 认证可用性是一种可规划的部署资源，它决定安全门能否被认证、以何种粒度认证以及覆盖多少流量。该框架将可用性从被动观测转为主动规划对象，为在安全、粒度与流量间做部署决策提供了可计算依据。
...
### [400] [Particle Competition and Cooperation for Robust Graph Convolutional Network Learning Under Label Noise](https://arxiv.org/abs/2609.22053)
*Fabricio Breve*

Main category: cs.LG

TL;DR: 本文提出 PCC+GCN 框架，将粒子竞争与合作（PCC）作为图标签精炼预处理阶段，通过粒子支配动力学识别并处理可疑标签节点，从而提升 GCN 在标签噪声下的鲁棒性。在 NoisyGL 的十个图数据集上，该方法在常规噪声下取得最高平均准确率，平均提升 1.67 个百分点，且在实例依赖噪声下保持竞争力并显著降低执行时间。


<details>
  <summary>Details</summary>
Motivation: 图卷积网络对标签噪声高度敏感，错误的监督信号会沿图结构传播并损害节点表示学习。因此需要一种能有效识别并修正噪声标签的预处理策略，以增强 GCN 在噪声监督下的鲁棒性，同时兼顾计算效率。

Method: 提出 PCC+GCN 混合框架：先由粒子竞争与合作（PCC）机制基于粒子支配动力学识别可疑的已标注节点，决定其标签应当保留、移除还是重新分配；随后在精炼后的标签上训练 GCN。PCC 使用的图可加入基于特征的 k 近邻边进行增强，而 GCN 仍在原始图结构与节点特征上训练。此外进行了超参数分析。

Result: 在 NoisyGL 基准的十个图数据集上，针对 Uniform、Pair、Random 等常规标签噪声以及实例依赖标签噪声进行评估。常规噪声下，PCC+GCN 取得最高总体平均准确率与最佳平均排名，在干净设置及所有噪声场景下相较基线 GCN 平均提升 1.67 个百分点；实例依赖噪声下，与最佳鲁棒方法竞争力相当，且执行时间大幅降低，在十个数据集中的八个上为最快鲁棒方法。并在 Cora、CiteSeer、PubMed 上完成了详细的超参数分析。

Conclusion: 基于 PCC 的标签精炼是一种有效且计算高效的预处理策略，可显著提升 GCN 在噪声监督下的鲁棒性，兼具精度优势与执行效率。
...
### [401] [Benchmarking World Models for Continual Learning on Compositional Tasks](https://arxiv.org/abs/2609.22055)
*Haoyu Zhou,Joe Watson,Anson Lei,Ingmar Posner*

Main category: cs.LG

TL;DR: 该论文提出一个面向机器人操作世界模型的组合式持续学习基准，通过在动作与感知两个维度上分解任务组合，将"知识复用"与"快速学习新任务"两种能力解耦，并评估现有世界模型及模块化模型的表现。


<details>
  <summary>Details</summary>
Motivation: 世界模型理想上应能跨任务持续学习，在适应新环境的同时不遗忘已学知识。由于真实世界的动力学常以重复机制出现，复用先前经验是实现高效适应的关键。然而现有世界模型的适应度指标将"学习未见任务的速度与容量"和"已有知识的复用"混为一谈，因新任务同时包含新颖内容与重复成分，导致难以单独衡量知识复用能力。

Method: 作者构建了一个面向机器人操作世界模型的组合式持续学习基准。具体做法是为每个任务课程设计组合式任务，使其由序列中已出现任务的若干方面组合而成；并沿动作（action）与感知（perception）两个轴对这种组合进行因子化分解，以分析不同输入模态如何成为知识复用的瓶颈。评估对象包括当前最先进的世界模型在典型持续学习方法下的表现，以及一种动力学骨干网络包含显式可复用组件的模块化世界模型。

Result: 实验结果显示，模块化设计在"复用"与"遗忘"之间取得了比传统方法更好的平衡，但没有任何方法能完全解决该问题。

Conclusion: 现有世界模型与持续学习方法均无法同时实现无遗忘的知识复用，说明构建"复用而不遗忘"的持续世界模型仍存在明确的研究空间。更多细节见项目网站 https://object814.github.io/Compositional-Continual-Learning/。
...
### [402] [BrainWideBench: Benchmarking large-scale pretraining and across-animal transfer in multi-region neural recordings](https://arxiv.org/abs/2609.22064)
...
TL;DR: 该论文提出BrainWideBench，一个基于IBL Brainwide Map数据集（139只小鼠、276个脑区）的跨动物多脑区神经记录迁移评测基准，包含行为解码、神经活动预测和解剖结构恢复三套任务，系统评估预训练方法。


<details>
  <summary>Details</summary>
Motivation: 大规模神经记录技术已能跨多动物、多脑区采集数据，理论上可用于学习可迁移的通用神经表征；但现有研究受限于碎片化的评测协议和单一任务域的狭隘关注，缺乏统一、可复现的评测框架来验证这一目标。

Method: 构建BrainWideBench基准，基于IBL Brainwide Map数据集（139只小鼠、276个脑区，执行感觉引导决策任务）；设计三套互补任务：①下游行为解码；②掩码或未来神经活动预测；③生物学解剖组织的恢复；在微调与零样本跨动物泛化两种迁移设定下系统评测多种预训练方法。

Result: 预训练相比匹配的单会话基线确实提升性能；但当前方法迁移能力存在异质性——增益强烈依赖预训练目标与下游任务的对齐程度；没有任何单一方法在三套任务上均表现一致优异，多数方法仅针对其中一部分任务设计。

Conclusion: 同时跨行为、神经动力学和解剖结构泛化的表征学习仍是开放挑战；BrainWideBench提供了统一、可复现的评测套件，为衡量小鼠大脑通用模型的进展建立框架。
